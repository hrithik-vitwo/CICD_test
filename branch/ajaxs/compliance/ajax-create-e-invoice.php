...
class eInvoice
{
  ...
  function generatePayload($invoiceId = null)
  {
    ...
    if ($invoiceDetails["customer_gstin"] == "" && $invoiceDetails["compInvoiceType"]=="R") {
      ...
      if($invoiceDetails["compInvoiceType"]=="R"){
        $SupTyp = "B2B";
        $BuyerPos = $invoiceDetails["placeOfSupply"]."";
        $BuyerStcd = substr($invoiceDetails["customerDetails"]["customer_gstin"], 0, 2) . "";
        $BuyerGstin = $customerDetails["customer_gstin"];
        $BuyerPin = intval($billingAddress["customer_address_pin_code"]);
      }else{
        $SupTypArr=[
          "LUT" => "EXPWOP",
          "CBW" => "",
        ...
        $SupTyp = $SupTypArr[$invoiceDetails["compInvoiceType"]];
        $BuyerGstin = "URP";
      ...
      $payloadArr = [
        ...
        'DocDtls' => [
          'Typ' => 'INV',
          'No' => substr($invoiceDetails["invoice_no"], 0, 16),
      ...
      foreach ($invoiceItemList as $key => $row) {
        ...
        $totalDiscount = $row['totalDiscountAmt'] + $row['cashDiscountAmt'];
        ...
        $payloadArr["ItemList"][] = [
          ...
          'UnitPrice' => round(floatval($row["itemTargetPrice"]),2),
          'TotAmt' => round(floatval($row["qty"] * $row["itemTargetPrice"]),2),
          'Discount' => round(floatval($totalDiscount),2),
  ...
  function generateEinvoice($payloadArr = [], $invoiceId = 0)
  {
    ...
    $checkPrevEinvoiceObj = queryGet("SELECT * FROM `erp_e_invoices` WHERE `invoice_id`=$invoiceId AND `document_type` = 'INV' AND `location_id`=$this->location_id AND `branch_id`=$this->branch_id AND `company_id`=$this->company_id");
