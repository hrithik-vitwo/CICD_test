                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    <div class="label-select">
                      <h3 class="card-title mb-0">Books Vs Statutory Report</h3>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "(SELECT portal.invDate AS portal_inc_date,portal.vendorGstin AS portal_vendor_name,portal.invoiceNo AS portal_invoice_no,portal.invAmount AS portal_invoice_amount,portal.taxAmount AS portal_tax_amount,portal.cgstAmount AS portal_cgst_amount,portal.sgstAmount AS portal_sgst_amount,portal.igstAmount AS portal_igst_amount,grn.vendorDocumentNo AS books_vendor_invoice_num,grn.grnIvCode AS books_grn_iv_num,grn.vendorGstin AS books_vendor_gstin,grn.vendorName AS books_vendor_name,grn.vendorDocumentDate AS books_invoice_date,grn.grnTotalAmount AS books_invoice_amount,grn.grnTotalCgst AS books_cgst_amount,grn.grnTotalSgst AS books_sgst_amount,grn.grnTotalIgst AS books_igst_amount FROM erp_branch_gstr2b_portal_invoices AS portal LEFT OUTER JOIN erp_grninvoice AS grn ON portal.invoiceNo=grn.vendorDocumentNo WHERE portal.company_id=$company_id AND portal.branch_id=$branch_id AND portal.invDate BETWEEN '2022-04-01' AND '2023-05-31')
                    UNION
                    (SELECT portal.invDate AS portal_inc_date,portal.vendorGstin AS portal_vendor_name,portal.invoiceNo AS portal_invoice_no,portal.invAmount AS portal_invoice_amount,portal.taxAmount AS portal_tax_amount,portal.cgstAmount AS portal_cgst_amount,portal.sgstAmount AS portal_sgst_amount,portal.igstAmount AS portal_igst_amount,grn.vendorDocumentNo AS books_vendor_invoice_num,grn.grnIvCode AS books_grn_iv_num,grn.vendorGstin AS books_vendor_gstin,grn.vendorName AS books_vendor_name,grn.vendorDocumentDate AS books_invoice_date,grn.grnTotalAmount AS books_invoice_amount,grn.grnTotalCgst AS books_cgst_amount,grn.grnTotalSgst AS books_sgst_amount,grn.grnTotalIgst AS books_igst_amount FROM erp_branch_gstr2b_portal_invoices AS portal RIGHT OUTER JOIN erp_grninvoice AS grn ON portal.invoiceNo=grn.vendorDocumentNo WHERE grn.companyId=$company_id AND grn.branchId=$branch_id AND grn.vendorDocumentDate BETWEEN '2022-04-01' AND '2023-05-31');";
                        ...
                        <thead>
                          <tr>
                            ...
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Portal Invoice Date</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Portal Vendor Name</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                            ...
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books Vendor Invoice No.</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books GRN IV No.</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books Vendor GSTIN</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books Vendor Name</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books Invoice Date</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books Invoice Amount</th>
                            <?php }
                            $i++;
                            if (in_array($i, $settingsCheckbox)) { ?>
                              <th>Books CGST</th>
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo formatDateORDateTime($data['portal_inc_date']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo ($data['portal_vendor_name']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo ($data['portal_invoice_no']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo ($data['portal_invoice_amount']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['portal_tax_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['portal_cgst_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['portal_sgst_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['portal_igst_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_vendor_invoice_num']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_grn_iv_num']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_vendor_gstin']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_vendor_name']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo formatDateORDateTime($data['books_invoice_date']); ?></td>
                              <?php } 
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_invoice_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_cgst_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_sgst_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['books_igst_amount']; ?></td>
                              <?php } ?>
                            </tr>
                          <?php
                          }
                              ...
                              <table>
                                ...
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                    ...
                                    Portal Invoice Date</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Portal Vendor Name</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Portal Invoice No.</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Portal Invoice Amount</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                ...
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Books Vendor GSTIN</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Books Vendor Name</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Books Invoice Date</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
                                    Books Invoice Amount</td>
                                </tr>
                                <tr>
                                  <td valign="top" style="width: 165px"><input type="checkbox" <?php $p++;
                                                                                                echo (in_array($p, $settingsCheckbox) ? 'checked="checked"' : ''); ?> name="settingsCheckbox[]" id="settingsCheckbox1" value="<?php echo $p; ?>" />
