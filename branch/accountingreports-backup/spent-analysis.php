                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    <div class="label-select">
                      <h3 class="card-title mb-0">Spent Analysis</h3>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT branch.branch_name AS branch_name,loc.othersLocation_name AS loc_name,invoices.so_number AS so_number,invoices.invoice_date AS invoice_date,invoices.invoice_no AS invoice_num,customer.customer_code AS customer_code,customer.trade_name AS customer_name,items.itemCode AS item_code,groups.goodGroupName AS item_group,items.itemName AS item_name, items.qty AS total_qty,items.uom AS uom,items.unitPrice AS rate ,(items.basePrice - items.totalDiscountAmt) AS taxable_amount,items.totalPrice,invoices.created_at,invoices.created_by,invoices.updated_at,invoices.updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_branch_sales_order_invoice_items AS items ON invoices.so_invoice_id = items.so_invoice_id LEFT JOIN erp_inventory_items AS inventory ON items.itemCode=inventory.itemCode LEFT JOIN erp_inventory_mstr_good_groups AS groups ON inventory.goodsGroup=groups.goodGroupId LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id LEFT JOIN erp_branches AS branch ON invoices.branch_id=branch.branch_id WHERE invoices.company_id=$company_id AND groups.companyId = $company_id AND invoices.invoice_date BETWEEN '2022-04-01' AND '2023-03-31' AND invoices.so_number!='' AND invoices.status='active' ORDER BY so_number desc,invoice_date desc;";
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT location_name,gl_code,gl_label,SUM(amount) AS amount FROM ((/* Retrieve all credit transactions */SELECT loc.othersLocation_name AS location_name,credit_coa.gl_code AS gl_code,credit_coa.gl_label AS gl_label,credit.credit_amount * (-1) AS amount FROM erp_acc_journal AS journal INNER JOIN erp_acc_credit AS credit ON journal.id = credit.journal_id INNER JOIN ".ERP_ACC_CHART_OF_ACCOUNTS." AS credit_coa ON credit.glId = credit_coa.id LEFT JOIN erp_branch_otherslocation AS loc ON journal.location_id=loc.othersLocation_id WHERE credit_coa.typeAcc=4 AND journal.postingDate BETWEEN '2022-04-01' AND '2023-03-31' AND journal.company_id=$company_id AND journal.branch_id=$branch_id) UNION (/* Retrieve all debit transactions */SELECT loc.othersLocation_name AS location_name,debit_coa.gl_code AS gl_code,debit_coa.gl_label AS gl_label,debit.debit_amount AS amount FROM erp_acc_journal AS journal INNER JOIN erp_acc_debit AS debit ON journal.id = debit.journal_id INNER JOIN ".ERP_ACC_CHART_OF_ACCOUNTS." AS debit_coa ON debit.glId = debit_coa.id LEFT JOIN erp_branch_otherslocation AS loc ON journal.location_id=loc.othersLocation_id WHERE debit_coa.typeAcc=4 AND journal.postingDate BETWEEN '2022-04-01' AND '2023-03-31' AND journal.company_id=$company_id AND journal.branch_id=$branch_id)) AS transaction GROUP BY location_name,gl_code, gl_label ORDER BY gl_code;";
            ...
            <div class="card card-tabs reports-card">
              ...
              <div class="tab-content" id="custom-tabs-two-tabContent">
                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                  ...
                  $sql_list = "SELECT invoices.so_number AS so_number,invoices.invoice_no AS invoice_num,invoices.invoice_date AS invoice_date,customer.customer_code AS customer_code,customer.trade_name AS customer_name,(invoices.sub_total_amt - invoices.totalDiscount) AS base_amount,invoices.igst AS igst,invoices.sgst AS sgst,invoices.cgst AS cgst,invoices.all_total_amt AS total_amount,invoices.due_amount AS receivable_amount,DATE_ADD(invoices.invoice_date, INTERVAL invoices.credit_period DAY) AS due_date,status.label AS invoice_status,invoices.created_at AS created_at,invoices.created_by AS created_by,invoices.updated_at AS updated_at, invoices.updated_by AS updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_status_master AS status ON invoices.invoiceStatus=status.code WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND invoices.invoice_date BETWEEN '2022-04-01' AND '2023-03-31' AND invoices.so_number!='' AND invoices.status='active' ORDER BY so_number desc,invoice_date desc;";
