              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT loc.othersLocation_name AS loc_name, temp_table.* FROM (SELECT
                    summary1.*,
                ...
                FROM
                    ...
                    WHERE
                        journal.journal_status = 'active' AND journal.company_id = $company_id AND journal.branch_id = $branch_id
                    ) AS main_report
                        )
                    UNION
                        (
                ...
                ORDER BY postingDate desc) AS temp_table LEFT JOIN erp_branch_otherslocation AS loc ON temp_table.location_id = loc.othersLocation_id;";
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT log.gl_code,log.gl_label,SUM(log.debit_amount) AS debit_amount,SUM(log.credit_amount) AS credit_amount,opening.open_bal AS open_balance FROM
                    (SELECT
                        coa.gl_code,
                        coa.gl_label,
                        SUM(
                    ...
                    ORDER BY
                        coa.gl_code,
                        coa.gl_label) AS log LEFT JOIN (SELECT table1.subgl,SUM(table1.total_amount)AS open_bal FROM
                    ((SELECT subgl,SUM(closing_val) AS total_amount FROM erp_opening_closing_balance WHERE company_id=$company_id AND branch_id = $branch_id AND DATE_FORMAT(date,'%m')=DATE_FORMAT(DATE_SUB('" . $startDate . "',INTERVAL 1 MONTH),'%m') GROUP BY subgl)
                    UNION
                    (SELECT coa.gl_code AS subgl,SUM(table1.Amount) AS total_amount FROM
                    ((SELECT
                            journal.postingDate AS postingDate,
                            debit.glId AS glId,
                            debit.debit_amount AS Amount,
                        ...
                        WHERE
                            ...
                            journal.company_id=$company_id AND journal.branch_id = $branch_id)
                        UNION
                        (SELECT
                            journal.postingDate AS postingDate,
                            credit.glId AS glId,
                        ...
                        WHERE
                            journal.journal_status='active' AND
                            journal.company_id=$company_id AND journal.branch_id = $branch_id)) AS table1 INNER JOIN " . ERP_ACC_CHART_OF_ACCOUNTS . " AS coa ON table1.glId = coa.id
                    WHERE
                    ...
                    GROUP BY table1.subgl) AS opening ON log.gl_code = opening.subgl GROUP BY log.gl_code,log.gl_label,open_balance;";
                      ...
                      <table id="dataTable" class="table table-hover transactional-book-table" style="width: 100%; position: relative;">
                        ...
                        <tbody class="">
                          ...
                          foreach ($datas as $data) {
                            ...
                            // console($data);
                            $sl++;
                            $DebitAmountTotal = $DebitAmountTotal + $data['debit_amount'];
                            $CreditAmountTotal = $CreditAmountTotal + $data['credit_amount'];
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td><?php echo $data['open_balance']; ?></td>
                              <?php } ?>
                            </tr>
                          <?php
                          }
                          ...
                          <tr style="font-weight: bold;">
                            <?php if (in_array($k, $settingsCheckbox_concised_view)) { ?>
                              <td></td>
                            <?php }
                            $k++;
                            if (in_array($k, $settingsCheckbox_concised_view)) { ?>
                              <td></td>
                            <?php }
                            $k++;
                            if (in_array($k, $settingsCheckbox_concised_view)) { ?>
                            ...
                            $k++;
                            if (in_array($k, $settingsCheckbox_concised_view)) { ?>
                              <td><?php echo $DebitAmountTotal; ?></td>
                            <?php }
                            $k++;
                            if (in_array($k, $settingsCheckbox_concised_view)) { ?>
                              <td><?php echo $CreditAmountTotal; ?></td>
                            <?php }
                            $k++;
                            if (in_array($k, $settingsCheckbox_concised_view)) { ?>
                              <td></td>
