              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT location.othersLocation_name AS location_name,goods.goodCode AS item_code,items.itemName AS item_name,SUM(goods.goodQty) AS total_ordered_qty,SUM(goods.receivedQty) AS total_received_qty,goods.itemUOM AS uom,SUM(goods.totalAmount) AS total_amount FROM erp_grninvoice AS grn INNER JOIN erp_grninvoice_goods AS goods ON grn.grnIvId=goods.grnIvId LEFT JOIN erp_inventory_items AS items ON goods.goodCode=items.itemCode LEFT JOIN erp_branch_otherslocation AS location ON grn.locationId=location.othersLocation_id WHERE grn.companyId=$company_id AND grn.branchId=$branch_id AND items.company_id=1 AND grn.postingDate BETWEEN '2022-04-01' AND '2023-03-31' AND grn.grnStatus='active' GROUP BY location_name,item_code,item_name,uom;";
