              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT loc.othersLocation_name,items.itemCode AS item_code,items.itemName AS item_name,goods_type.goodTypeName AS goods_type,summary.itemPrice AS target_price,summary.movingWeightedPrice AS mwp,summary.priceType,summary.priceSetOn AS price_set_on,summary.itemMaxDiscount AS max_disc,summary.itemTotalQty AS total_qty,summary.rmWhOpen,summary.rmWhReserve,summary.sfgStockOpen,summary.sfgStockReserve,summary.fgWhOpen,summary.fgWhReserve,summary.fgMktOpen,summary.fgMktReserve,summary.min_stock,summary.max_stock FROM erp_inventory_items AS items INNER JOIN erp_inventory_stocks_summary AS summary ON items.itemId=summary.itemId LEFT JOIN erp_inventory_mstr_good_types AS goods_type ON items.goodsType=goods_type.goodTypeId LEFT JOIN erp_branch_otherslocation AS loc ON summary.location_id=loc.othersLocation_id WHERE items.company_id=$company_id AND summary.branch_id=$branch_id AND summary.status='active' ORDER BY summary.updatedAt desc;";
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT loc.othersLocation_name,warehouse.warehouse_code,warehouse.warehouse_name,storage_loc.storage_location_code,storage_loc.storage_location_name,storage_loc.storage_location_material_type,log.storageType,items.itemCode,items.itemName,log.itemQty,log.itemUom FROM erp_inventory_stocks_log AS log INNER JOIN erp_inventory_items AS items ON log.itemId=items.itemId LEFT JOIN erp_storage_location AS storage_loc ON log.storageLocationId=storage_loc.storage_location_id LEFT JOIN erp_storage_warehouse AS warehouse ON storage_loc.warehouse_id=warehouse.warehouse_id LEFT JOIN erp_branch_otherslocation AS loc ON log.locationId=loc.othersLocation_id WHERE log.companyId=$company_id AND log.branchId=$branch_id  AND items.status='active' ORDER BY log.createdAt desc;";
