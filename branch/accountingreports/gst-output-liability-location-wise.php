                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  ...
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="label-select">
                      <h3 class="card-title mb-0">GST Output Liability Location Wise</h3>
                    </div>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT loc.othersLocation_name AS loc_name,customer.trade_name AS customer_name,customer.customer_code AS customer_code,customer.customer_gstin AS gstin,invoices.invoice_no AS invoice_num,invoices.so_number AS so_num,invoices.invoice_date AS invoice_date,invoices.igst AS igst,invoices.sgst AS sgst,invoices.cgst AS cgst,invoices.total_tax_amt AS total_tax_amt,(invoices.sub_total_amt - invoices.totalDiscount) AS base_value ,invoices.all_total_amt AS total_price,invoices.created_at AS created_at,invoices.created_by AS created_by,invoices.updated_at AS updated_at,invoices.updated_by AS updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND (invoices.compFileDate IS NULL OR invoices.compFileDate ='') AND invoices.status='active' ORDER BY invoice_date desc;";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?= decimalValuePreview($data['total_tax_amt']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?= decimalValuePreview($data['base_value']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?= decimalValuePreview($data['total_price']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT customer.customer_code AS customer_code,customer.trade_name AS customer_name,SUM(invoices.igst) AS total_igst,SUM(invoices.sgst) AS total_sgst,SUM(invoices.cgst) AS total_cgst,SUM(invoices.total_tax_amt) AS total_tax_amt,SUM(invoices.sub_total_amt - invoices.totalDiscount) AS total_base_value ,SUM(invoices.all_total_amt) AS total_price FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND (invoices.compFileDate IS NULL OR invoices.compFileDate ='') AND invoices.status='active' GROUP BY customer_code,customer_name;";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td> <?php echo $data['customer_code']??'<p class="text-center"> - </p>'; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td> <?php echo $data['customer_name']??'<p class="text-center"> - </p>'; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"> <?= decimalValuePreview($data['total_cgst']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"> <?= decimalValuePreview($data['total_sgst']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"> <?= decimalValuePreview($data['total_igst']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"> <?= decimalValuePreview($data['total_tax_amt']); ?></td>
                              <?php }
                               $i++;
                               if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                 <td class="text-right"> <?= decimalValuePreview($data['total_base_value']); ?></td>
                               <?php }
                               $i++;
                               if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                 <td class="text-right"> <?= decimalValuePreview($data['total_price']); ?></td>
                      ...
                      <div class="row">
                        <div class="col-md-12 col-sm-12 d-flex">
                          <div class="card flex-fill reports-card">
                            ...
                            <div class="card-body">
                              <div id="chartDivGSTOutputLiability" class="chartContainer"></div>
                            </div>
                          </div>
                        </div>
                      </div>
...
<script>
  ...
  am4core.ready(function() {
    ...
    var chart = am4core.create("chartDivGSTOutputLiability", am4charts.PieChart3D);
    ...
    for (elem of chartData.data) {
      finalData[0].value += Number(elem.total_cgst);
      finalData[1].value += Number(elem.total_sgst);
      finalData[2].value += Number(elem.total_igst);
    };
