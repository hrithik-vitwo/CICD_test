              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT location.othersLocation_name AS loca_name,invoices.invoice_date AS document_date,invoices.invoice_no AS document_num,customer.trade_name AS customer_name,(invoices.sub_total_amt - invoices.totalDiscount) AS base_amount,invoices.igst AS igst,invoices.sgst AS sgst,invoices.cgst AS cgst,invoices.all_total_amt AS total_amount,invoices.due_amount AS receivable_amount,DATE_ADD(invoices.invoice_date, INTERVAL invoices.credit_period DAY) AS due_date , invoices.created_at,invoices.created_by,invoices.updated_at,invoices.updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_branch_otherslocation AS location ON invoices.location_id=location.othersLocation_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active'";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo ($data['loca_name']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo formatDateORDateTime($data['document_date']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo $data['document_num']; ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                              ...
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['base_amount']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['igst']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right">
                                  <?=  decimalValuePreview($data['sgst']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['cgst']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['total_amount']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['receivable_amount']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo formatDateORDateTime($data['due_date']); ?>
                                </td>
                              <?php }
                              ...
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo getCreatedByUser($data['created_by']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo formatDateORDateTime($data['created_at']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo getCreatedByUser($data['updated_by']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td>
                                  <?php echo formatDateORDateTime($data['updated_at']); ?>
                                </td>
                              <?php } ?>
                            </tr>
                          <?php
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT customer.customer_code AS customer_code,customer.trade_name AS customer_name,SUM(invoices.sub_total_amt - invoices.totalDiscount) AS total_base_amount,SUM(invoices.igst) AS total_igst,SUM(invoices.sgst) AS total_sgst,SUM(invoices.cgst) AS total_cgst,SUM(invoices.all_total_amt) AS total_amount,SUM(invoices.due_amount) AS total_receivable_amount FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_branches AS branch ON invoices.branch_id=branch.branch_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active' GROUP BY customer_code,customer_name";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right">
                                  <?=decimalValuePreview(['total_base_amount']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['total_igst']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['total_sgst']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['total_cgst']); ?>
                                </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                              ...
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right">
                                  <?= decimalValuePreview($data['total_receivable_amount']); ?>
                                </td>
                              <?php } ?>
                            </tr>
                          <?php
  ...
  am4core.ready(function() {
    ...
    for (elem of chartData.data) {
      finalData.push({
        ...
        "receivable_amount": elem.total_receivable_amount,
