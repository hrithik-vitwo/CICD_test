                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  ...
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="label-select">
                      <h3 class="card-title mb-0">Sales Register Vertical Wise</h3>
                    </div>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT loc.othersLocation_name AS loc_name,invoices.invoice_date AS invoice_date,invoices.invoice_no AS invoice_num,func.functionalities_name AS profit_center,customer.customer_code AS customer_code,customer.trade_name AS customer_name,items.itemCode AS item_code,groups.goodGroupName AS item_group,items.itemName AS item_name, items.qty AS total_qty,items.uom AS uom,items.unitPrice AS rate ,(items.basePrice - items.totalDiscountAmt) AS taxable_amount,items.totalPrice,invoices.created_at,invoices.created_by,invoices.updated_at,invoices.updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_branch_sales_order_invoice_items AS items ON invoices.so_invoice_id = items.so_invoice_id LEFT JOIN erp_inventory_items AS inventory ON items.itemCode=inventory.itemCode LEFT JOIN erp_inventory_mstr_good_groups AS groups ON inventory.goodsGroup=groups.goodGroupId LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id LEFT JOIN erp_company_functionalities AS func ON invoices.profit_center=func.functionalities_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND inventory.company_id=$company_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active'";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?= decimalQuantityPreview($data['total_qty']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?= decimalValuePreview($data['taxable_amount']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?= decimalValuePreview($data['totalPrice']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT DATE_FORMAT(invoices.invoice_date,'%M, %y') AS month,loc.othersLocation_name AS loc_name,f_area.functionalities_name AS functional_area_name,SUM(invoices.sub_total_amt - invoices.totalDiscount) AS total_base_amount,SUM(invoices.all_total_amt) AS total_amount,SUM(invoices.due_amount) AS due_amount FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_company_functionalities AS f_area ON invoices.profit_center=f_area.functionalities_id LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND  invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active' GROUP BY month,loc_name,functional_area_name;";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td><?= decimalValuePreview($data['total_base_amount']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?= decimalValuePreview($data['due_amount']);  ?></td>
                              <?php }
                              ?>
                            </tr>
                          <?php
