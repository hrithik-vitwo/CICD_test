              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT loc.othersLocation_name AS loc_name,invoices.invoice_no AS doc_num,invoices.invoice_date AS doc_date,customer.customer_code AS customer_code,customer.trade_name AS customer_name,items.itemCode AS item_code,items.itemName AS item_name,items.unitPrice AS so_unit_price,summary.itemPrice AS target_price,summary.priceType AS so_on FROM erp_branch_sales_order_invoices AS invoices INNER JOIN erp_branch_sales_order_invoice_items AS items ON invoices.so_invoice_id=items.so_invoice_id LEFT JOIN erp_inventory_stocks_summary AS summary ON items.inventory_item_id=summary.itemId LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id ORDER BY doc_date desc;";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="pre-normal"> <?php echo $data['item_name']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"> <?= decimalValuePreview($data['so_unit_price'])??'-'; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
  ...
  function compare_date() {
    ...
    if (fromDate && toDate) {
      if (diffDays > 366) {
        ...
        $("#to_date").parent().append(`<p class="text-danger text-xs prdatelabel">Date Range can not be greater than 1 year</p>`);
      } else {
        $(".prdatelabel").remove();
        document.getElementById("rangeid").disabled = false;

        if (toDate < fromDate) {
          $("#to_date").parent().append(`<p class="text-danger text-xs prdatelabel">From Date can not be greater than To Date</p>`);
          document.getElementById("rangeid").disabled = true;
