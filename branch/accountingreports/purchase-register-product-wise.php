              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT grn.vendorCode AS vendor_code,grn.vendorName AS vendor_name,goods.goodCode AS item_code,items.itemName AS item_name,SUM(goods.goodQty) AS total_ordered_qty,SUM(goods.receivedQty) AS total_received_qty,SUM(goods.totalAmount) AS total_amount FROM erp_grninvoice AS grn INNER JOIN erp_grninvoice_goods AS goods ON grn.grnIvId=goods.grnIvId LEFT JOIN erp_inventory_items AS items ON goods.goodCode=items.itemCode LEFT JOIN erp_branch_otherslocation AS location ON grn.locationId=location.othersLocation_id WHERE grn.companyId=$company_id AND grn.branchId=$branch_id AND grn.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND grn.grnStatus='active' AND items.company_id=$company_id GROUP BY vendor_code,vendor_name,item_code,item_name ORDER BY item_code;";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?= decimalValuePreview($data['total_received_qty']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT goods.goodCode AS item_code,items.itemName AS item_name,SUM(goods.goodQty) AS total_ordered_qty,SUM(goods.receivedQty) AS total_received_qty,goods.itemUOM AS uom,SUM(goods.totalAmount) AS total_amount FROM erp_grninvoice AS grn INNER JOIN erp_grninvoice_goods AS goods ON grn.grnIvId=goods.grnIvId LEFT JOIN erp_inventory_items AS items ON goods.goodCode=items.itemCode WHERE grn.companyId=$company_id AND grn.branchId=$branch_id AND grn.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND grn.grnStatus='active' AND items.company_id=$company_id GROUP BY item_code,item_name,uom;";
