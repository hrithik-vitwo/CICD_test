...
if (!isset($_COOKIE["cookiePostedGrn"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiePostedGrn", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                                        ...
                                                        <div class="filter-action filter-mobile-search mobile-page">
                                                            ...
                                                            <div class="exportgroup mobile-page mobile-export">
                                                                ...
                                                                <ul class="export-options">
                                                                    <li>
                                                                        <button>
                                                                            <ion-icon name="list-outline" class="ion-fulllistnew md hydrated" id="exportAllBtn" role="img" aria-label="list outline"></ion-icon>Export
                                                                        </button>
                                                                    </li>
                                                                    <li>
                                                                        <button>
                                                                            <ion-icon name="list-outline" class="ion-paginationlistnew md hydrated" role="img" aria-label="list outline"></ion-icon>Download
                                                                        </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookiePostedGrn');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-posted-grn.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            if (value.grnStatus == 'active') {
                                revBtn = `
                                        <li>
                                            <button class="reverseGRN" data-toggle="modal" data-id=${value.grnId}><ion-icon name="arrow-undo-outline"></ion-icon>Reverse</button>
                                        </li>`;
                            } else if (value.grnStatus == 'reversed' || value.grnStatus == 'reverse') {
                                revBtn = `
                                        <li>
                                        <p>Reversed</p>
                                        </li>`;
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal" data-id="${value.grnId}" data-code="${value.grnType}">${value.vendorDocumentNo}</a>`,
                                value.grnCode,
                                value.grnPoNumber,
                                value.vendorCode,
                                `<p class="pre-normal">${value.vendorName}</p>`,
                                value.vendorGstin,
                                `<p>${value.grnTotalAmount}</p>`,
                                value.grnCreatedBy,
                                `<div class="dropout">
                                    <button class="more">
                                         <span></span>
                                    ...
                                    <ul>
                                        <li>
                                            <button class="soModal" data-toggle="modal" data-id=${value.grnId} data-code="${value.grnType}"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                        </li>
                                        ${revBtn}   
                                    </ul>
                                </div>`
        ...
        $(document).on("click", ".ion-paginationlistnew", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiePostedGrn')
                },
                beforeSend:function(){
...
<script>
    $(document).on("click", ".soModal", function() {
        ...
        if (grnType == 'grn') {
            ...
            url = `<?= BRANCH_URL ?>location/grn-actions.php?posted=1&type=grn&view=${grnId}`;
        } else {

            url = `<?= BRANCH_URL ?>location/grn-actions.php?posted=1&type=srn&view=${grnId}`;
        }
    ...
    $(document).on("click", ".reverseGRN", function(e) {
