...
class GlSummary1
{
    ...
    private function sumGroupTotalAmount($children)
    {
        $total_opening = $total_debit = $total_credit = $total_closing = 0;

        foreach ($children as $item) {
            $total_opening += $item["opening_val"];
            $total_debit += $item["debit_amount"];
            $total_credit += $item["credit_amount"];
            $total_closing += $item["closing_val"];
            $sumObj = sumGroupTotalAmount($item["children"]);
            $total_opening += $sumObj["opening_val"];
            $total_debit += $sumObj["debit_amount"];
            $total_credit += $sumObj["credit_amount"];
            $total_closing += $sumObj["closing_val"];
        }
        return [
            "opening_val" => $total_opening,
            "debit_amount" => $total_debit,
            "credit_amount" => $total_credit,
            "closing_val" => $total_closing,
        ];
    }

    private function calculateSummary2($children)
    {
        $summary = [
            'opening_val' => 0,
            'debit_amount' => 0,
        ...
        foreach ($this->treeViewInputData["data"] as $element) {
            if ($element['p_id'] == $parentId) {
                ...
                if (!empty($children)) {
                    ...
                    // $element['children_summary'] = $this->sumGroupTotalAmount($children);
                }
                $tree[] = $element;
            }
        }
    ...
    public function generateSummary($start_date = null, $end_date = null)
    {
...
$glSummaryObj = new GlSummary1("company");
...
echo json_encode($glSummaryData, true);
