...
$searchValue = "yearlyProfitCenterWiseSales"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill yearlyProfitCenterWiseSales">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="yearlyProfitCenterWiseSalesChart"><?= chartExistorNot('yearlyProfitCenterWiseSalesChart.php') ? 'Pinned' : 'Pin'; ?></button>
                ...
                <div id="menu-wrap">
                    ...
                    <div class="menu ">
                        <div>
                            <ul>
                                <li>
                                    <select name="fYDropdown" id="fYDropdown" class="form-control fYDropdown yearlyProf">
                                        <?php
...
<script>
    ...
    function yearlyProfitCenterWiseSalesChart(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Profit Center Wise Sales ${$(".yearlyProf").find(":selected").text()}`);
    ...
    // ===============================================================================================
    var fYDropdown = $(".yearlyProfitCenterWiseSales").find("select.fYDropdown").val();

    function get_yearlyProfitCenterWiseSalesChart(fYDropdown, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=yearlyProfitCenterWiseSales&search=${search}&fYDropdown=${fYDropdown}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_yearlyProfitCenterWiseSalesChart(fYDropdown, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var fYDropdown = $(".yearlyProfitCenterWiseSales").find("select.fYDropdown").val();

        get_yearlyProfitCenterWiseSalesChart(fYDropdown, 'searching');
    });
</script>