...
$searchValue = "kamWisePriceProductGroupSales"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill kamWisePriceProductGroupSales">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="kamWisePricingProductGroupSales"><?= chartExistorNot('kamWisePricingProductGroupSales.php') ? 'Pinned' : 'Pin'; ?></button>
...
<script>
    ...
    // ===============================================================================================
    var quickDrop = $(".kamWisePriceProductGroupSales").find("select.quickDrop").val();

    function get_kamWisePricingProductGroupSales(quickDrop, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=kamWisePriceProductGroupSales&search=${search}&quickDrop=${quickDrop}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_kamWisePricingProductGroupSales(quickDrop, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var quickDrop = $(".kamWisePriceProductGroupSales").find("select.quickDrop").val();

        get_kamWisePricingProductGroupSales(quickDrop, 'searching');
    });
</script>