...
$searchValue = "yearlyProductPriceWiseSales"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill yearlyProductPriceWiseSales">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="yearlyProductPriceWiseSalesChart"><?= chartExistorNot('yearlyProductPriceWiseSalesChart.php') ? 'Pinned' : 'Pin'; ?></button>
                ...
                <div id="menu-wrap">
                    ...
                    <div class="menu ">
                        <div>
                            <ul>
                                <li>
                                    <select name="fYDropdown" id="fYDropdown" class="form-control fYDropdown yearlyProdP">
                                        <?php
...
<script>
    ...
    function yearlyProductPriceWiseSalesChart(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Product Price Wise Sales ${$(".yearlyProdP").find(":selected").text()}`);
    ...
    // ===============================================================================================
    var fYDropdown = $(".yearlyProductPriceWiseSales").find("select.fYDropdown").val();

    function get_yearlyProductPriceWiseSalesChart(fYDropdown, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=yearlyProductPriceWiseSales&search=${search}&fYDropdown=${fYDropdown}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_yearlyProductPriceWiseSalesChart(fYDropdown, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var fYDropdown = $(".yearlyProductPriceWiseSales").find("select.fYDropdown").val();

        get_yearlyProductPriceWiseSalesChart(fYDropdown, 'searching');
    });
</script>