...
$searchValue = "productQuantityWiseSalesProfitCenter"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill productQuantityWiseSalesProfitCenter">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="productQuantityWiseProfitCenterSales"><?= chartExistorNot('productQuantityWiseProfitCenterSales.php') ? 'Pinned' : 'Pin'; ?></button>
...
<script>
    ...
    function productQuantityWiseProfitCenterSales(chartData, chartTitle) {
        ...
        am4core.ready(function() {
            ...
            for (let obj of chartData) {
                ...
                if (outerIndex !== -1) {
                    finalData[outerIndex].value += Number(obj.total_qty);
    ...
    // ===============================================================================================
    var quickDrop = $(".productQuantityWiseSalesProfitCenter").find("select.quickDrop").val();

    function get_productQuantityWiseProfitCenterSales(quickDrop, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=productQuantityWiseSalesProfitCenter&search=${search}&quickDrop=${quickDrop}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_productQuantityWiseProfitCenterSales(quickDrop, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var quickDrop = $(".productQuantityWiseSalesProfitCenter").find("select.quickDrop").val();
        get_productQuantityWiseProfitCenterSales(quickDrop, 'searching');
    });
</script>