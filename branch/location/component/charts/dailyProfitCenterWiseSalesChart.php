...
<script>
    ...
    function get_dailyProfitCenterWiseSalesChart(quickDrop, search, profitCenter) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=dailyProfitCenterWiseSales&search=${search}&quickDrop=${quickDrop}&profitCenter=${profitCenter}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
