...
$searchValue = "dailyProductQuantityWiseSales"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill dailyProductQuantityWiseSales">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="dailyProductQuantityWiseSalesChart">
                    <?= chartExistorNot('dailyProductQuantityWiseSalesChart.php') ? 'Pinned' : 'Pin'; ?>
                </button>
                    ...
                    <div class="menu ">
                        <div>
                            <ul>
                                ...
                                <li>
                                    <? console($dashComponents) ?>
                                    <select name="itemCode" id="itemCode" class="form-control itemCode dailyProdQ">
...
<script>
    ...
    function dailyProductQuantityWiseSalesChart(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Daily Product Quantity Wise Sales for ${$(".dailyProdQ").find(":selected").text()}`);
    ...
    // ===============================================================================================
    var quickDrop = $(".dailyProductQuantityWiseSales").find("select.quickDrop").val();

    var itemCode = $(".dailyProductQuantityWiseSales").find("select.itemCode").val();

    function get_dailyProductQuantityWiseSales(quickDrop, search, itemCode) {
        // AJAX CALL 

        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=dailyProductQuantityWiseSales&search=${search}&quickDrop=${quickDrop}&itemCode=${itemCode}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_dailyProductQuantityWiseSales(quickDrop, null, itemCode);
    ...
    $('.applyFilter<?= $searchValue; ?>').click(function() {
        ...
        get_dailyProductQuantityWiseSales(quickDrop, 'searching', itemCode);
    });
</script>