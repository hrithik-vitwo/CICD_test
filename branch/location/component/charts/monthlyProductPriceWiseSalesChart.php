...
$searchValue = "monthlyProductPriceWiseSales"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill monthlyProductPriceWiseSales">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="monthlyProductPriceWiseSalesChart"><?= chartExistorNot('monthlyProductPriceWiseSalesChart.php') ? 'Pinned' : 'Pin'; ?></button>
                    ...
                    <div class="menu ">
                        <div>
                            <ul>
                                ...
                                <li>
                                    <select name="itemCode" id="itemCode" class="form-control itemCode monthlyProdP">
...
<script>
    ...
    function monthlyProductPriceWiseSalesChart(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Monthly Product Price Wise Sales for ${$(".monthlyProdP").find(":selected").text()}`);
    ...
    // ===============================================================================================
    var quickDrop = $(".monthlyProductPriceWiseSales").find("select.quickDrop").val();

    var itemCode = $(".monthlyProductPriceWiseSales").find("select.itemCode").val();

    function get_monthlyProductPriceWiseSalesChart(quickDrop, search, itemCode) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=monthlyProductPriceWiseSales&search=${search}&quickDrop=${quickDrop}&itemCode=${itemCode}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_monthlyProductPriceWiseSalesChart(quickDrop, null, itemCode);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var quickDrop = $(".monthlyProductPriceWiseSales").find("select.quickDrop").val();
        var itemCode = $(".monthlyProductPriceWiseSales").find("select.itemCode").val();

        get_monthlyProductPriceWiseSalesChart(quickDrop, 'searching', itemCode);
    });
</script>