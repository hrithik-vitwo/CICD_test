...
$searchValue = "vendorWisePayables"; // Add this from ajax "?chart=" value
...
<div class="col-md-12 col-sm-12 d-flex">
    <div class="card flex-fill vendorWisePayables">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="vendorWisePayables"><?= chartExistorNot('vendorWisePayables.php') ? 'Pinned' : 'Pin'; ?></button>
...
<script>
    ...
    // ===============================================================================================
    var quickDrop = $(".vendorWisePayables").find("select.quickDrop").val();

    function get_vendorWisePayables(quickDrop, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=vendorWisePayables&search=${search}&quickDrop=${quickDrop}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_vendorWisePayables(quickDrop, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var quickDrop = $(".vendorWisePayables").find("select.quickDrop").val();

        get_vendorWisePayables(quickDrop, 'searching');
    });
</script>