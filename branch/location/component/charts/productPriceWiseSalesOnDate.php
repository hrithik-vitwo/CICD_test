...
$searchValue = "productPriceWiseSalesOnDate"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill productPriceWiseSalesOnDate">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="productPriceWiseSalesOnDate"><?= chartExistorNot('productPriceWiseSalesOnDate.php') ? 'Pinned' : 'Pin'; ?></button>
                ...
                <div id="menu-wrap">
                    ...
                    <div class="menu ">
                        <div>
                            <ul>
                                <li>
                                    <input type="date" name="specificDay" id="specificDayP" class="form-control specificDay" style="max-width: 100%;" />
                                </li>
                                <li>
...
<script>
    ...
    document.getElementById("specificDayP").value = formattedDate;
    ...
    function productPriceWiseSalesOnDate(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Product Price Wise Sales On ${formatDate($("#specificDayP").val())}`);
    ...
    // ===============================================================================================
    var specificDay = $("#specificDayP").val();

    function get_productPriceWiseSalesOnDate(specificDay, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>branch/location/ajaxs/reports/ajax-charts.php?chart=productPriceWiseSalesOnDate&search=${search}&specificDay=${specificDay}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_productPriceWiseSalesOnDate(specificDay, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var specificDay = $("#specificDayP").val();

        get_productPriceWiseSalesOnDate(specificDay, 'searching');
    });
</script>