...
if (!isset($_COOKIE["cookieManageBoq"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieManageBoq", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Service Code',
        'slag' => 'itemDetails.itemCode',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Service Name',
        'slag' => 'itemDetails.itemName',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Prepared Date',
        'slag' => 'boqDetails.preparedDate',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'date'
    ],
    [
        'name' => 'COGM',
        'slag' => 'boqDetails.cogm',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => 'COSP-M',
        'slag' => 'boqDetails.cosp_m',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => 'COSP-A',
        'slag' => 'boqDetails.cosp_a',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => 'COSP-I',
        'slag' => 'boqDetails.cosp_i',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => 'COGS',
        'slag' => 'boqDetails.cogs',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage BOQ</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    function adjustTabs() {
        let navTabs = $("#nav-tab");
        let moreDropdown = $("#more-dropdown");
        ...
        let visibleTabs = 7; // Number of visible tabs
        ...
        let hiddenTabs = navTabs.children(":gt(" + (visibleTabs) + ")");
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            let checkboxSettings = Cookies.get('cookieManageBoq');
            let notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-boq.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            //         <li>
                            //             <button class="deleteSoBtn" data-toggle="modal"  data-id="${value.so_no}" data-target="#viewModal"><ion-icon name="trash-outline" class="ion-delete"></ion-icon>Delete</button>
                            //         </li>`;
                            // }

                            let status='';
                            if(value.boqStatus==="active"){
                                status=`<div class="status-bg status-open">Active</div>`;
                            }else if(value.boqStatus==="inactive"){
                                status=`<div class="status-bg status-pending">Inactive</div>`;
                            }else{
                                status=`<div class="status-bg status-closed">${capFirstLetter(value.boqStatus)}</div>`;
                            }


                            dataTable.row.add([
                                    value.sl_no,
                                    `<a href="#" class="soModal" data-id="${value.itemId}">${value.itemCode}</a>`,
                                    `<p class="pre-normal">${value.itemName}</p>`,
                                    formatDate(value.preparedDate),
                                    decimalAmount(value.cogm),
                                    decimalAmount(value.cosp_m),
                                    decimalAmount(value.cosp_a),
                                    decimalAmount(value.cosp_i),
                                    decimalAmount(value.cogs),
                                    decimalAmount(value.msp),
                                    status,
                                ` <div class="dropout">
                                    <button class="more">
                                         <span></span>
                                    ...
                                    <ul>
                                      ...
                                      <li>
                                         <button data-toggle="modal" class="editSobtn" data-id= data-code=""><ion-icon name="create-outline" class="ion-edit"></ion-icon>Edit</button>
                                     </li>
                                    <li>
                                        <button class="soModal"  data-id="${value.itemId}" ><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                    </li>
                                    </ul>
                                </div>`
                            ]).draw(false);
        ...
        $(document).ready(function() {
            $(document).on("click", "#serach_submit", function(event) {
                ...
                $(".selectOperator").each(function() {
                    ...
                    if (columnSlag === 'boqDetails.preparedDate') {
                        values = value3;
                    ...
                    if ((columnSlag === 'boqDetails.preparedDate' || columnSlag === 'so_date' || columnSlag === 'created_at') && operatorName == "BETWEEN") {
...
<script>
    $(document).ready(function() {
        $(document).on("change", ".selectOperator", function() {
            ...
            } else if (columnName === 'Prepared Date') {
                inputId = "value3_" + columnIndex;
            }

            if ((columnName === 'Delivery Date' || columnName === 'SO Date' || columnName === 'Prepared Date') && operatorName === 'BETWEEN') {
...
<script>
    $(document).on("click", ".soModal", function() {
        ...
        console.log(itemId);
        $('.auditTrail').attr("data-ccode", itemId);

        $.ajax({
            type: "GET",
            url: "ajaxs/modals/prp/ajax-boq-modal.php",
            dataType: 'json',
            data: {
                act:"modalData",
                itemId
            ...
            success: function(value) {
                ...
                if(value){
                    ...
                    let boqData=boqDetailObj.boq_data;
                    // data will be here

                    
                $(".left #amount").html(boqData.itemCode);
                // $("#amount-words").html("(" + responseObj.currecy_name_words + ")");
                $(".right #cus_name").html(boqData.itemName);
                $("#po-numbers").html(`Prepared By: ${boqData.preparedBy}`);
                $("#default_address").html(`Prepared At: ${boqData.preparedDate}`);
                }
                $("#globalModalLoader").remove();
            },
            complete: function() {
