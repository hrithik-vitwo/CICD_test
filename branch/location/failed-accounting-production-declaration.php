...
if (!isset($_COOKIE["cookiefailedproductiondeclaration"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiefailedproductiondeclaration", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Failed Production Declaration</h3>
                                            </div>
                                        </div>
                                        ...
                                        <div class="right-block">
                                            <div class="page-list-filer filter-list">
                                                ...
                                                <a class="filter-link active" id="active"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon> Production Declaration
                                                </a>
                                                <style>
                                                    @keyframes pulse {
                                                        0% {
                                                ...
                                                <a class="filter-link" id="reverse"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Reverse Production Declaration
                                                </a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '' , type = '') {
            ...
            var checkboxSettings = Cookies.get('cookiefailedproductiondeclaration');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-failed-acc-production-declaration.php",
                dataType: 'json',
                data: {
                    act: 'failedproductiondeclaration',
                    comid: comid,
                    locId: locId,
                    bId: bId,
                    formDatas: formDatas,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            // }
                            let post = '';
                            let actionType = `pay_id=${(value.id_action)}`;
                            if (response.type == 'active') {
                                post = ` <div class="view-accoiunt" data-id='${value.id}'>
                                    <a href="manage-failed-acc-production-declaration-view.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`
                                 ...
                                 post = ` <div class="view-accoiunt" data-id='${value.id}'>
                                    <a href="manage-failed-acc-production-declaration-reverse-view.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`
