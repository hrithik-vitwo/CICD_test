...
$columnMapping = [
    ...
    [
        ...
        'slag' => 'customer_group.customer_discount_group',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Item Discount Group',
        'slag' => 'item_group.item_discount_group',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
                                                ...
                                                <div class="modal-background">
                                                    <div class="modal">
                                                        ...
                                                        <div class="filter-action filter-mobile-search mobile-page">
                                                            ...
                                                            <a href="manage-discount-variation-actions.php?create"
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                `<p>${value.sl_no}</p>`,
                                `<p>${value['customer_group.customer_discount_group']}</p>`,
                                `<p>${value['item_group.item_discount_group']}</p>`,
                                `<p>${value.discount_percentage}</p>`,
                                `<p>${(value.discount_max_value)}</p>`,
                                `<p>${(value.discount_value)}</p>`,
                                `<p>${value.term_of_payment}</p>`,
                                `<p>${(value.valid_from)}</p>`,
                                `<p>${(value.valid_upto)}</p>`,
                                ...
                                ` <div class="dropout">
                                     ...
                                     <ul>
                                        <li>
                                        <a href="manage-discount-variation-actions.php?edit=${value.discount_variant_id}">
                                            <button>
        ...
        $(document).on("click", ".ion-paginationlistDisVariant", function(e) {
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    $('#loaderModal').show();
                    $('.ion-paginationlistDisVariant').prop('disabled', true)
                },
                ...
                complete: function() {
                    ...
                    $('#loaderModal').hide();
                    $('.ion-paginationlistDisVariant').prop('disabled', false)
                }
            })
        ...
        $(document).on("click", ".ion-fulllistitemDisVariant", function(e) {
            $.ajax({
                ...
                beforeSend: function() {
                    // console.log(sql_data_checkbox);
                    $('#loaderModal').show();
                    $('.ion-fulllistitemDisVariant').prop('disabled', true)
                },
                success: function(response) {
                ...
                complete: function() {
                    // Hide loader modal after request completes
                    $('#loaderModal').hide();
                    $('.ion-fulllistitemDisVariant').prop('disabled', false);
                }
            })
        ...
        $(document).ready(function() {
            $(document).on("click", "#serach_submit", function(event) {
                ...
                $(".selectOperator").each(function() {
                    ...
                    if (columnSlag.trim() === 'created_at') {
                        values = value4;
                    } else if (columnSlag.trim() === 'valid_from') {
                        values = value2;
                    } else if (columnSlag.trim() === 'valid_to') {
                        values = value3;
                    }

                    if ((columnSlag.trim() === 'created_at' || columnSlag.trim() === 'valid_from' || columnSlag.trim() === 'valid_from') && operatorName == "BETWEEN") {
...
<script>
    $(document).ready(function() {
        $(document).on("change", ".selectOperator", function() {
            ...
            } else if (columnName.trim() === 'Valid From') {
                inputId = "value2_" + columnIndex;
            } else if (columnName.trim() === 'Valid Upto') {
                inputId = "value3_" + columnIndex;
            }

            if ((columnName.trim() === 'Created At' || columnName.trim() === 'Valid From' || columnName.trim() === 'Valid Upto') && operatorName === 'BETWEEN') {
