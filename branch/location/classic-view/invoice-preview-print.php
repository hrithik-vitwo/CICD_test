...
<div class="container mt-5">
    ...
    if (isset($_GET['invoice_id'])) {
        ...
        if (isset($_GET['printChkbox'])) {
            ...
            if (isset($_GET['type']) && $_GET['type'] == 'company') {
                if (isset($_GET['template_id'])) {
                    ...
                    $templateInvoiceControllerObj->printInvoice($invoice_id, $_GET['template_id'], 'printChkbox');
                } else {
                    $templateInvoiceControllerObj->printInvoice($invoice_id, 'printChkbox');
                }
            ...
            if (isset($_GET['type']) && $_GET['type'] == 'company') {
                if (isset($_GET['template_id'])) {
                    $templateInvoiceControllerObj->printInvoice($invoice_id, $_GET['template_id']);
                } else {
        ...
        $templateSalesOrderControllerObj = new TemplateSalesOrderController();
        $templateSalesOrderControllerObj->printSalesOrderProforma($proformaInv);
        ...
        $tempdelvObj = new TemplateDeliveryController();
        ...
        $templateQuotationControllerObj->printQuotation($quotationId, $company_id, $branch_id, $location_id);
    } elseif (isset($_GET['quotation_id'])) { // quotation approval 
        $BranchSoObj = new BranchSo();
        $quotation_id = base64_decode($_GET['quotation_id']);
        $templateQuotationControllerObj = new TemplateQuotationController();
        $templateQuotationControllerObj->printQuotation($quotation_id, $company_id, $branch_id, $location_id, true);
