...
if (!isset($_COOKIE["cookieslocationItem"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieslocationItem", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Price',
        'slag' => 'stock.movingWeightedPrice',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => 'Valuation Class',
        'slag' => 'stock.priceType',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Target Price',
        'slag' => 'stock.itemPrice',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
    ...
    [
        'name' => 'Created By',
        'slag' => 'goods.createdBy',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'BOM Status',
        'slag' => 'goods.isBomRequired',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage Location Items</h3>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <div class="dropdown payment-collection-dropdown location-item-dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li><a id="addExistingItem" class="btn btn-transparent"><ion-icon name="add-outline"></ion-icon>Add From Existing Item</a></li>
                                                    <hr>
                                                    <li><a href="<?= LOCATION_URL ?>goods-actions.php?create" class="btn btn-transparent"><ion-icon name="add-outline"></ion-icon>Add New Items</a></li>
                                                </ul>
                                            </div>
...
<script>
    ...
    var checkboxSettings = Cookies.get('cookieslocationItem');
    $(document).ready(function() {

        let existUrl = `<?= LOCATION_URL ?>goods.php?locItemId=${btoa(1)}`;
        $("#addExistingItem").attr("href", existUrl);
        var indexValues = [];
        var dataTable;
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '') {
            ...
            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-location-item.php",
                dataType: 'json',
                data: {
                    act: 'items',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                ...
                                value.uomName,
                                value["groups.goodGroupName"],
                                value['stock.movingWeightedPrice'],
                                value["stock.priceType"],
                                value['stock.itemPrice'],
                                value["type.goodTypeName"],
                                value["goods.createdBy"],
                                value.isBomRequired,
                                value.status,
                                ` <div class="dropout">
                                    <button class="more">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                        <button class="soModal" data-toggle="modal" data-target="#viewGlobalModal" data-id="${value.itemId}" data-code="${value.itemCode}"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                        </li>
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieslocationItem')
                },
                beforeSend: function() {
                    $('#loaderModal').show();
...
<script>
    ...
    $(document).on("click", ".soModal", function() {
        ...
        let btnObj = ` <button class="editBtn btn btn-primary" data-toggle="modal" data-id="${itemId}" data-code="${code}" ><ion-icon name="create-outline" class="ion-edit"></ion-icon>Edit</button>`;
        $("#navBtn").html(btnObj);
        ...
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/mm/ajax-manage-location-items-modal.php",
            dataType: 'json',
            data: {
                act: 'modalData',
                itemId
            ...
            success: function(value) {
                ...
                if (responseObj.type == 'other') {
                    ...
                    $("#itemDesc").html(trimString(dataObj.itemDesc, 30));
                    ...
                    $('#defDiscount').html(`${decimalQuantity(responseObj.itemMaxDiscount)} %`);
                    ...
                    $("#serviceDesc").html(trimString(serviceDetails.itemDesc, 30));
                    $("#serviceHsn").html(serviceDetails.hsnCode);
                    $("#glDetails").html(serviceDetails.glName);
                    $("#glCode").html(serviceDetails.glCode);
                    $("#serviceTargetPrice").html(decimalAmount(summaryData.itemPrice));
