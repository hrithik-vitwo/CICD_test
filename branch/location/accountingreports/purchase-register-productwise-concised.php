...
$columnMapping = [
    ...
    [
        'name' => 'Total Recieved Qty',
        'slag' => 'goods.receivedQty',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Purchase Register Concised View </h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            $.ajax({
                type: "POST",
                url: "../ajaxs/newreportlist/ajax-purchase-register-concised.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                ...
                                decimalQuantity(value.total_ordered_qty),
                                decimalQuantity(value.total_received_qty),
                                value.uom,
                                value.total_amount
                            ]).draw(false);
