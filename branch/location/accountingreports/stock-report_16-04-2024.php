                                ...
                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                    <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                        ...
                                        SELECT
                                            ...
                                            MAX(LOG.logRef) AS logRef,
                                            
                                            ROUND(SUM(log_rep.total_opening_qty), 0) AS opening_quantity,
                                            SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('GRN', 'REV-GRN', 'MIGRATION', 'MAT-MAT-IN', 'PROD-IN') AND LOG.itemQty > 0 AND LOG.postingDate >= '" . $f_date . "' THEN LOG.itemQty
                                                    ELSE 0
                                                END
                                            ) AS in_quantity,
                                            SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('INVOICE', 'REV-INVOICE', 'PROD-ORDR', 'MAT-MAT-OUT', 'PROD-OUT', 'DN', 'COST CENTER', 'BOOK PHYSICAL') AND LOG.itemQty < 0 AND LOG.postingDate >= '" . $f_date . "' THEN ABS(LOG.itemQty)
                                                    ELSE 0
                                                END
                                            ) AS out_quantity,
                                            ROUND(SUM(log_rep.total_opening_qty), 0) +
                                            SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('GRN', 'REV-GRN', 'MIGRATION', 'MAT-MAT-IN', 'PROD-IN') AND LOG.itemQty > 0 AND LOG.postingDate >= '" . $f_date . "' THEN LOG.itemQty
                                                    ELSE 0
                                            ...
                                            MAX(UOM.uomName) AS uom,
                                            
                                            ROUND(ROUND(SUM(log_rep.total_opening_qty), 0) * summary.movingWeightedPrice,2) AS opening_balance,
                                            ROUND(SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('GRN', 'REV-GRN', 'MIGRATION', 'MAT-MAT-IN', 'PROD-IN') AND LOG.itemQty > 0 AND LOG.postingDate >= '" . $f_date . "' THEN LOG.itemQty
                                                    ELSE 0
                                                END
                                            ) * summary.movingWeightedPrice,2) AS in_balance,
                                            ROUND(SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('INVOICE', 'REV-INVOICE', 'PROD-ORDR', 'MAT-MAT-OUT', 'PROD-OUT', 'DN', 'COST CENTER', 'BOOK PHYSICAL') AND LOG.itemQty < 0 AND LOG.postingDate >= '" . $f_date . "' THEN ABS(LOG.itemQty)
                                                    ELSE 0
                                                END
                                            ) * summary.movingWeightedPrice,2) AS out_balance,
                                            ROUND(ROUND(SUM(log_rep.total_opening_qty), 0) * summary.movingWeightedPrice,2) +
                                            ROUND(SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('GRN', 'REV-GRN', 'MIGRATION', 'MAT-MAT-IN', 'PROD-IN') AND LOG.itemQty > 0 AND LOG.postingDate >= '" . $f_date . "' THEN LOG.itemQty
                                                    ELSE 0
                                                END
                                            ) * summary.movingWeightedPrice,2) -
                                            ROUND(SUM(
                                                CASE
                                                    WHEN LOG.refActivityName IN('INVOICE', 'REV-INVOICE', 'PROD-ORDR', 'MAT-MAT-OUT', 'PROD-OUT', 'DN', 'COST CENTER', 'BOOK PHYSICAL') AND LOG.itemQty < 0 AND LOG.postingDate >= '" . $f_date . "' THEN ABS(LOG.itemQty)
                                                    ELSE 0
                                                END
                                            ) * summary.movingWeightedPrice,2) AS closing_balance,
                                            
                                            MAX(curr.currency_name) AS currency
                                                  
                                        FROM
                                            erp_inventory_stocks_log AS LOG
                                        LEFT JOIN erp_inventory_items AS items ON LOG.itemId = items.itemId
                                        LEFT JOIN erp_inventory_stocks_log_report AS log_rep ON items.itemId = log_rep.item_id 
                                        ...
                                        WHERE
                                        LOG.companyId=$company_id AND LOG.branchId=$branch_id AND LOG.locationId=$location_id AND LOG.postingDate <= '" . $to_date . "'
                                        GROUP BY LOG.itemId, items.itemCode, items.itemName, summary.movingWeightedPrice;";
                                                ...
                                                <tbody class="">
                                                    ...
                                                    ?>
                                                        <tr>
                                                            ...
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo $data['itemCode']; ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-left">
                                                                    <p class="pre-normal"><?php echo $data['itemName']; ?>
                                                                        <p />
                                                                </td>
                                                            <?php }
                                                            ...
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo round($data['opening_quantity']); ?></td>
                                                            <?php }

                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo round($data['in_quantity']); ?></td>
                                                            <?php }

                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo round($data['out_quantity']); ?></td>
                                                            <?php }

                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo round($data['closing_quantity']); ?></td>
                                                            <?php }

                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo $data['currency']; ?></td>
                                                            <?php }

                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo number_format($data['opening_balance'], 2, '.', ''); ?></td>
                                                            <?php }


                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo number_format($data['in_balance'], 2, '.', ''); ?></td>
                                                            <?php }


                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo number_format($data['out_balance'], 2, '.', ''); ?></td>
                                                            <?php }


                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo number_format($data['closing_balance'], 2, '.', ''); ?></td>
                                                            <?php }
...
?>
    ...
    CASE
        ...
        WHEN LOG.refActivityName IN (
            'INVOICE', 'REV-INVOICE', 'DELIVERY', 'REV-DELIVERY', 'PGI'
        ) THEN customer.customer_code
        ELSE 'INTERNAL'
    END AS party_code,
    CASE
        ...
        WHEN LOG.refActivityName IN (
            'INVOICE', 'REV-INVOICE', 'DELIVERY', 'REV-DELIVERY', 'PGI'
        ) THEN customer.trade_name
        ELSE 'INTERNAL'
    END AS party_name,
    CASE
        WHEN LOG.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "'  THEN DATE_FORMAT(
            DATE(LOG.postingDate), '%d-%b'
        )
    END AS `date`,
    UOM.uomName AS uom,
...
WHERE   LOG.companyId=$company_id AND LOG.branchId=$branch_id AND LOG.locationId=$location_id
    AND DATE(LOG.postingDate) BETWEEN '" . $f_date . "' AND '" . $to_date . "'
GROUP BY
    items.itemId,
    LOG.logRef,
    LOG.refNumber,
...
HAVING
    date IS NOT NULL;";
                                        ...
                                        $queryset = queryGet($sql_list, true);
                                        // echo $company_id.' '.$branch_id.' '.$location_id.' '.$f_date.' '.$to_date;
                                                ...
                                                <tbody class="">
                                                    ...
                                                    ?>
                                                        <tr>
                                                            ...
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo (round($data['qty']));  ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo ($data['uom']);  ?></td>
                                                            ...
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                                <td class="text-right"><?php echo (round($data['value'], 2));  ?></td>
                                                            <?php }
