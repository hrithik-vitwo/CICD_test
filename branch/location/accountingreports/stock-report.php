...
if (!isset($_COOKIE["cookieStockReport"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieStockReport", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Inventory Movement Report</h3>
                                            </div>
                                        </div>
                                                                ...
                                                                <table>
                                                                    <tbody>
                                                                        ...
                                                                        foreach ($columnMapping as $columnIndex  => $column) {
                                                                            if ($columnIndex === 0 ||$columnIndex === 8||$columnIndex === 9||$columnIndex === 15) {
                                                                                continue;
                                                                            } ?>
                                                                            <tr>
                                                                                <td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookieStockReport');
            var notVisibleColArr = [];


            $.ajax({
                type: "POST",
                url: "../ajaxs/newreportlist/ajax-stock-report-h.php",
                dataType: 'json',
                data: {
                    act: 'cdata',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            let movement_type = (value.movement_type == null) ? '-' : `<p class='movement-type stockReportMovementType-${value.movement_type.toLowerCase()}'>${value.movement_type}</p>`;

                            dataTable.row.add([
                                value.sl_no,
                                value["loc.othersLocation_name"],
                                value["LOG.refNumber"],
                                value["items.itemCode"],
                                `<p class='item-name pre-normal'>${value["items.itemName"]}</p>`,
                                value["grp.goodGroupName"],
                                value["str_loc.storage_location_name"],
                                value["loLOG.logRefgRef"],
                                value.party_code,
                                value.party_name,
                                value["LOG.postingDate"],
                                value["UOM.uomName"],
                                movement_type,
                                value["LOG.itemQty"],
                                value["LOG.itemPrice"],
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieStockReport')
                },
                beforeSend:function(){
                    $('#loaderModal').show();
            ...
            $(document).on("click", "#serach_submit", function(event) {
                ...
                $(".selectOperator").each(function() {
                    ...
                    if (columnSlag === 'posting_date') {
                        ...
                        values = value2;
                    } else if (columnSlag === 'LOG.createdAt') {
                        values = value3;
                    } else if (columnSlag === 'LOG.postingDate') {
                        values = value5;
                    }

                    if ((columnSlag === 'posting_date' || columnSlag === 'so_date' || columnSlag === 'LOG.createdAt' || columnSlag == "LOG.postingDate") && operatorName == "BETWEEN") {
...
<script>
    $(document).ready(function() {
        $(document).on("change", ".selectOperator", function() {
            ...
            if ((columnName === 'Document Date' || columnName === 'SO Date' || columnName === 'Created Date' || columnName === 'Posting Date') && operatorName === 'BETWEEN') {
