            ...
            <div class="card card-tabs mb-0" style="border-radius: 20px;">
              ...
              <div class="tab-content" id="custom-tabs-two-tabContent">
                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                  ...
                  $sql_list = "SELECT gl_code,gl_label,SUM(amount) AS amount FROM ((/* Retrieve all credit transactions */SELECT credit_coa.gl_code AS gl_code,credit_coa.gl_label AS gl_label,credit.credit_amount * (-1) AS amount FROM erp_acc_journal AS journal INNER JOIN erp_acc_credit AS credit ON journal.id = credit.journal_id INNER JOIN " . ERP_ACC_CHART_OF_ACCOUNTS . " AS credit_coa ON credit.glId = credit_coa.id WHERE credit_coa.typeAcc=4 AND journal.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id) UNION (/* Retrieve all debit transactions */SELECT debit_coa.gl_code AS gl_code,debit_coa.gl_label AS gl_label,debit.debit_amount AS amount FROM erp_acc_journal AS journal INNER JOIN erp_acc_debit AS debit ON journal.id = debit.journal_id INNER JOIN " . ERP_ACC_CHART_OF_ACCOUNTS . " AS debit_coa ON debit.glId = debit_coa.id WHERE debit_coa.typeAcc=4 AND journal.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id)) AS transaction GROUP BY gl_code, gl_label ORDER BY gl_code;";
