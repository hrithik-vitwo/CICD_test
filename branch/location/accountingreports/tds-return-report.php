                            ...
                            <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                                <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                                    <div class="label-select">
                                        <h3 class="card-title mb-0">TDS Return Report</h3>
                                    </div>
                        ...
                        <div class="card card-tabs mb-0" style="border-radius: 20px;">
                            <div class="tab-content" id="custom-tabs-two-tabContent">
                                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                    ...
                                    $sql_list = "SELECT DATE_FORMAT(DATE(invoice.postingDate), '%b') AS `month`, invoice.postingDate AS `date`, company.company_code AS deductee_code, tds.section AS section, vendor.vendor_pan AS pan, vendor.trade_name AS party, SUM( goods.receivedQty * goods.unitPrice ) AS basic, COALESCE( (tds.TDSRate / 100) * SUM( goods.receivedQty * goods.unitPrice ), 0 ) AS amount_deducted, COALESCE(tds.TDSRate, 0) AS rate FROM erp_grninvoice AS invoice INNER JOIN erp_grninvoice_goods AS goods ON invoice.grnIvId = goods.grnIvId LEFT JOIN erp_vendor_details AS vendor ON invoice.vendorId = vendor.vendor_id LEFT JOIN erp_inventory_items AS item ON goods.goodId = item.itemId LEFT JOIN erp_tds_details AS tds ON item.tds = tds.id LEFT JOIN erp_companies AS company ON invoice.companyId = company.company_id WHERE invoice.companyId =$company_id AND invoice.branchId =$branch_id AND invoice.locationId = $location_id AND invoice.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' GROUP BY invoice.postingDate, vendor.vendor_pan, vendor.trade_name, tds.section, tds.TDSRate";
                                            ...
                                            <tbody class="">
                                                ...
                                                ?>
                                                    <tr>
                                                        ...
                                                        if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                            <td class="text-center"><?php echo $data['deductee_code']; ?></td>
                                                        <?php }
                                                        $i++;
                                                        ...
                                                        $i++;
                                                        if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                            <td class="text-center"><?php echo $data['pan']; ?></td>
                                                        <?php }

                                                        $i++;
                                                        if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                            <td>
                                                                <p class="pre-normal"><?php echo $data['party']; ?></p>
                                                            </td>
                                                        <?php }

                                                        $i++;
                                                        if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                            <td class="text-right"><?php echo number_format($data['basic'], 2, '.', ''); ?></td>
                                                        <?php }

                                                        $i++;
                                                        if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                                            <td class="text-right"><?php echo number_format($data['amount_deducted'], 2, '.', ''); ?></td>
                                                        <?php }
