              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT invoices.invoice_no AS doc_num,invoices.invoice_date AS doc_date,customer.customer_code AS customer_code,customer.trade_name AS customer_name,items.itemCode AS item_code,items.itemName AS item_name,items.unitPrice AS so_unit_price,summary.itemPrice AS target_price,summary.priceType AS so_on FROM erp_branch_sales_order_invoices AS invoices INNER JOIN erp_branch_sales_order_invoice_items AS items ON invoices.so_invoice_id=items.so_invoice_id LEFT JOIN erp_inventory_stocks_summary AS summary ON items.inventory_item_id=summary.itemId LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id ORDER BY doc_date desc;";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?php echo $data['avg_so_unit_price']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?php echo $data['target_price']; ?></td>
