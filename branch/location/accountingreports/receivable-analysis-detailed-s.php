...
if (!isset($_COOKIE["cookieReceivableAnalysisDetailed"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieReceivableAnalysisDetailed", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Receivable Analysis - Detailed View</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookieReceivableAnalysisDetailed');
            var notVisibleColArr = [];
            
            $.ajax({
                type: "POST",
                url: "../ajaxs/newreportlist/ajax-recievable-analysis-s.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                formatDate(value.document_date),
                                value.document_no,
                                value.reference_no,
                                value.transaction_type,
                                value.customer_code,
                                value.customer_name,
                                decimalAmount(value.base_amount),
                                decimalAmount(value.cust_igst),
                                decimalAmount(value.cust_sgst),
                                decimalAmount(value.cust_cgst),
                                decimalAmount(value.total_amount),
