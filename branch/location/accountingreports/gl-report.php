              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    ((SELECT subgl,closing_val AS total_amount FROM erp_opening_closing_balance WHERE company_id=$company_id AND branch_id = $branch_id AND location_id = $location_id AND DATE_FORMAT(date,'%m')=DATE_FORMAT(DATE_SUB('" . $f_date . "',INTERVAL 1 MONTH),'%m'))
                    UNION
                    ...
                    ((SELECT
                        ...
                        WHERE
                            ...
                            journal.company_id=$company_id AND journal.branch_id = $branch_id AND journal.location_id = $location_id)
                        UNION
                        (SELECT
                            journal.postingDate AS postingDate,
                            credit.glId AS glId,
                        ...
                        WHERE
                            journal.journal_status='active' AND
                            journal.company_id=$company_id AND journal.branch_id = $branch_id AND journal.location_id = $location_id)) AS table1 INNER JOIN " . ERP_ACC_CHART_OF_ACCOUNTS . " AS coa ON table1.glId = coa.id
                    WHERE
