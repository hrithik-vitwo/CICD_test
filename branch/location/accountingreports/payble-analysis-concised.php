...
$columnMapping = [
    ...
    [
        'name' => 'Total Unaccount',
        'slag' => '',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => '0-30 Days unaccount',
        'slag' => '',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => '31-60 Days unaccount',
        'slag' => '',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => '61-90 Days unaccount',
        'slag' => '',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => '91-180 Days unaccount',
        'slag' => '',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'number'
    ],
    [
        'name' => 'More Than 180 Days unaccount',
        'slag' => '',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'number'
    ],
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Payble Analysis Concised View</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            $.ajax({
                type: "POST",
                url: "../ajaxs/newreportlist/ajax-payble-analysis-concised.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                ...
                                value.vendor_code,
                                value.vendor_name,
                                decimalAmount(value.total_due),
                                `<p class="text-right">${decimalAmount(value.zero_thirty_days_due)}</p>`,
                                `<p class="text-right">${decimalAmount(value.thirtyone_sixty_days_due)}</p>`,
                                `<p class="text-right">${decimalAmount(value.sixtyone_ninety_days_due)}</p>`,
                                `<p class="text-right">${decimalAmount(value.ninetyone_oneeighty_days_due)}</p>`,
                                `<p class="text-right">${decimalAmount(value.more_than_oneeighty_days_due)}</p>`,
                                `<p class="text-right">${decimalAmount(value.total_unaccount)}</p>`,
                                `<p class="text-right">${decimalAmount(value.zero_thirty_days_unaccount)}</p>`,
                                `<p class="text-right">${decimalAmount(value.thirtyone_sixty_days_unaccount)}</p>`,                              
                                `<p class="text-right">${decimalAmount(value.sixtyone_ninety_days_unaccount)}</p>`,                              
                                `<p class="text-right">${decimalAmount(value.ninetyone_oneeighty_days_unaccount)}</p>`,                              
                                `<p class="text-right">${decimalAmount(value.more_than_oneeighty_days_unaccount)}</p>`,                              
                                // `<p>hs</p>`,
                            ]).draw(false);
