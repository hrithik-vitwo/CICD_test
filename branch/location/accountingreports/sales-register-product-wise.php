...
<style>
  ...
  @media only screen and (max-width: 1023px) {
    ...
    .content-wrapper>.content {
      padding: 30px 0.5rem 60px !important;
    }
            ...
            <div class="card card-tabs reports-card">
              <div class="p-0 pb-2" style="border-bottom: 1px solid #dbe5ee;">
                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 pt-md-0 px-3 px-md-3 d-flex justify-content-between align-items-center" style="width:100%">
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    // $sql_list = "SELECT invoices.invoice_date AS invoice_date,invoices.invoice_no AS invoice_num,customer.customer_code AS customer_code,customer.trade_name AS customer_name,items.itemCode AS item_code,groups.goodGroupName AS item_group,items.itemName AS item_name,items.hsnCode AS hsnCode, items.qty AS total_qty,items.uom AS uom,items.unitPrice AS rate ,(items.basePrice - items.totalDiscountAmt) AS taxable_amount,items.totalPrice,invoices.created_at,invoices.created_by,invoices.updated_at,invoices.updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_branch_sales_order_invoice_items AS items ON invoices.so_invoice_id = items.so_invoice_id LEFT JOIN erp_inventory_items AS inventory ON items.itemCode=inventory.itemCode LEFT JOIN erp_inventory_mstr_good_groups AS groups ON inventory.goodsGroup=groups.goodGroupId WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND groups.companyId = $company_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active';";
                    $sql_list = "SELECT
                    'IN' AS TYPE,
                    ...
                    items.qty AS total_qty,
                    UOM.uomName AS uom,
                    items.unitPrice AS rate,
                    (
                        items.basePrice - items.totalDiscountAmt
                    ) AS taxable_amount,
                    items.totalPrice AS totalPrice,
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-center"> <?php echo $data['item_group']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    SUM(items.totalPrice) AS total_price
                FROM
                    erp_branch_sales_order_invoice_items AS items
                ...
                ON
                    UOM.uomId = items.uom 
                WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active' GROUP BY item_code, item_name, uom;";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?php echo ($data['total_price']);  ?></td>
                              <?php } ?>
                            </tr>
                          <?php
                          }
              ...
              <div class="tab-content" id="custom-tabs-two-tabContent">
                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                  ...
                  ?>
                    ...
                    <div class="container-fluid mt-10 p-0 p-md-3">
                      <div class="row">
...
<script>
  ...
  am4core.ready(function() {
    ...
    chart.responsive.rules.push({
      relevant: am4core.ResponsiveBreakpoints.widthLargerThan(400), // Adjust the breakpoint as needed
      state: function(target, stateId) {
        // Handle state changes or adjustments when the chart goes to responsive state
        // For example, you can modify the axis labels, remove unnecessary elements, etc.
        if (stateId === "small") {
          // Modify settings for smaller screens
