...
if (!isset($_COOKIE["cookieManageNetSalesProductWise"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieManageNetSalesProductWise", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Created By',
        'slag' => 'items.createdBy',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Created At',
        'slag' => 'items.createdAt',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'date'
    ],
    [
        'name' => 'Updated By',
        'slag' => 'items.updatedBy',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Updated At',
        'slag' => 'items.updatedAt',
        'icon' => '<ion-icon name="document-outline"></ion-icon>',
        'dataType' => 'date'
    ]
];
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Net Sales Product Wise Report </h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookieManageNetSalesProductWise');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "../ajaxs/newreportlist/ajax-net-sales-product-wise-report.php",
                dataType: 'json',
                data: {
                    act: 'pdata',
                    comid: comid,
        ...
        $(document).on("click", "#check-box-submt", function(event) {
            ...
            if (settingsCheckbox.length < 5) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        act: 'manageNetSalesReportProductWise',
                        fromData: fromData
                    },
                    success: function(response) {
                        // console.log(response);
