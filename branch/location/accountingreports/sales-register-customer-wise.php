                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-3 pt-md-0 px-md-3 d-flex justify-content-between align-items-center" style="width:100%">
                    <div class="label-select">
                      <h3 class="card-title mb-0">Sales Register Customer Wise</h3>
                    </div>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    // $sql_list = "SELECT invoices.invoice_no AS invoice_num,invoices.invoice_date AS invoice_date,customer.customer_code AS customer_code,customer.trade_name AS customer_name,invoices.customer_gstin AS customer_gstin,(invoices.sub_total_amt - invoices.totalDiscount) AS base_amount,invoices.igst AS igst,invoices.sgst AS sgst,invoices.cgst AS cgst,invoices.all_total_amt AS total_amount,invoices.due_amount AS receivable_amount,DATE_ADD(invoices.invoice_date, INTERVAL invoices.credit_period DAY) AS due_date,status.label AS invoice_status,invoices.created_at AS created_at,invoices.created_by AS created_by,invoices.updated_at AS updated_at, invoices.updated_by AS updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_status_master AS status ON invoices.invoiceStatus=status.code WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active'";
                    $sql_list = "SELECT
                          invoices.invoice_no AS invoice_num,
                          'IN' AS type,
                          '-' AS reference_no,
                          invoices.invoice_date AS invoice_date,
                          customer.customer_code AS customer_code,
                          customer.trade_name AS customer_name,
                          invoices.customer_billing_address AS address,
                          invoices.remarks AS remarks,
                          func.functionalities_name AS variant_type,
                          kam.kamName AS sales_person_name,
                          invoices.customer_gstin AS party_gstin,
                          (
                              invoices.sub_total_amt - invoices.totalDiscount
                          ) AS base_amount,
                          invoices.igst AS igst,
                          invoices.sgst AS sgst,
                          invoices.cgst AS cgst,
                          invoices.all_total_amt AS total_amount,
                          invoices.due_amount AS receivable_amount,
                          DATE_ADD(
                              invoices.invoice_date,
                              INTERVAL invoices.credit_period DAY
                          ) AS due_date,
                      STATUS
                          .label AS invoice_status,
                          invoices.created_at AS created_at,
                          invoices.created_by AS created_by,
                          invoices.updated_at AS updated_at,
                          invoices.updated_by AS updated_by
                      FROM
                          erp_branch_sales_order_invoices AS invoices
                      RIGHT JOIN erp_customer AS customer
                      ON
                          invoices.customer_id = customer.customer_id
                      /*LEFT JOIN erp_credit_note AS cn
                      ON
                        customer.customer_id = cn.party_id AND cn.creditors_type = 'customer'*/
                      LEFT JOIN erp_kam AS kam 
                      ON
                        invoices.kamId = kam.kamId
                      LEFT JOIN erp_company_functionalities AS func
                      ON
                        invoices.profit_center = func.functionalities_id
                      LEFT JOIN erp_status_master AS STATUS
                      ON
                          invoices.invoiceStatus = STATUS.code
                      WHERE
                     invoices.company_id = $company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status = 'active';";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['party_gstin']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?php echo ($data['receivable_amount']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo ($data['variant_type']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT customer.customer_code AS customer_code,customer.trade_name AS customer_name,invoices.customer_gstin AS customer_gstin,SUM(invoices.sub_total_amt - invoices.totalDiscount) AS base_amount,SUM(invoices.igst) AS igst,SUM(invoices.sgst) AS sgst,SUM(invoices.cgst) AS cgst,SUM(invoices.all_total_amt) AS total_amount,SUM(invoices.due_amount) AS receivable_amount FROM erp_branch_sales_order_invoices AS invoices RIGHT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active' GROUP BY customer_code,customer_name,customer_gstin;";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td> <?php echo $data['customer_gstin']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?php echo $data['base_amount']; ?></td>
            ...
            <div class="card card-tabs reports-card">
              ...
              <div class="tab-content" id="custom-tabs-two-tabContent">
                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                  ...
                  $sql_list = "SELECT customer.trade_name AS customer_name,SUM(invoices.sub_total_amt - invoices.totalDiscount) AS taxable_amount,SUM(invoices.all_total_amt) AS total_amount,SUM(invoices.due_amount) AS due_amount FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id WHERE invoices.company_id=$company_id AND invoices.branch_id=$branch_id AND invoices.location_id=$location_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active' GROUP BY customer_name;";
                      ...
                      <div class="row">
                        <div class="col-md-12 col-sm-12 d-flex">
                          <div class="card flex-fill reports-card">
                            ...
                            <div class="card-body">
                              <div id="chartDivCustomerWiseSalesRegister" class="chartContainer"></div>
                            </div>
                          </div>
                        </div>
                      </div>
...
<script>
  ...
  am4core.ready(function() {
    ...
    var chart = am4core.create("chartDivCustomerWiseSalesRegister", am4charts.XYChart);
    ...
    for (elem of chartData.data) {
      finalData.push({
        "customer": elem.customer_name,
        "receivable_amount": Number(elem.due_amount),
    ...
    series.dataFields.valueY = "receivable_amount";
    series.dataFields.categoryX = "customer";
    series.clustered = false;
    series.tooltipText = "Receivable Amount of {categoryX} : [bold]{valueY}[/]";

    var series2 = chart.series.push(new am4charts.ColumnSeries());
    series2.dataFields.valueY = "total_amount";
    series2.dataFields.categoryX = "customer";
