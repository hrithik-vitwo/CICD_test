            ...
            <div class="card card-tabs reports-card">
              <div class="p-0 pb-2" style="border-bottom: 1px solid #dbe5ee;">
                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 pt-md-0 px-md-3 d-md-flex justify-content-between align-items-center" style="width:100%">
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['cogm'] * $data['itemQty']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td> <?php echo $data['cogm_m'] * $data['itemQty']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo ($data['cogm_a'] * $data['itemQty']);  ?> </td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td></td>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $queryset = queryGet("SELECT `itemName`, `itemCode`, `goodGroupName`, `uomName`, `itemPrice`, `bornDate`, `cogm`, `cogm_m`, `cogm_a`, SUM(`itemQty`) AS `itemQty` FROM `erp_inventory_stocks_log` AS stock LEFT JOIN `erp_inventory_items` AS items ON items.itemId = stock.itemId LEFT JOIN `erp_inventory_mstr_uom` AS uom ON uom.uomId = items.baseUnitMeasure LEFT JOIN `erp_bom` AS bom ON bom.itemId = stock.itemId LEFT JOIN `erp_inventory_mstr_good_groups` AS goodsgroup ON goodsgroup.goodGroupId = items.goodsGroup WHERE `refActivityName` = 'PROD-IN' AND stock.companyId = $company_id AND stock.branchId = $branch_id AND stock.locationId = $location_id AND items.goodsType IN (2,3) AND stock.bornDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' GROUP BY stock.itemId;", true);
                    // console($queryset);
                    $num_list = $queryset['numRows'];
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td><?php echo round($data['itemQty'], 2); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?php echo $data['cogm'] * $data['itemQty'];  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) {
                              ?>
                                <td class="text-right"><?php echo $data['cogm_m'] * $data['itemQty'];  ?></td>
                              <?php
                              }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) {
                              ?>
                                <td class="text-right"><?php echo $data['cogm_a'] * $data['itemQty'];  ?></td>
                              <?php
                              }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) {
              ...
              <div class="p-0 pb-2" style="border-bottom: 1px solid #dbe5ee;">
                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 pt-md-0 px-md-3 d-md-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="d-flex align-items-center">
                      <div class="daybook-filter-list filter-list">
                        <a href="<?= $_SERVER['PHP_SELF']; ?>?day-day" class="btn active waves-effect waves-light"><i class="fa fa-list mr-2"></i>Day-To-Day Tracking</a>
                      </div>
                      <div id="containerThreeDot">
                        <div id="menu-wrap">
                          <input type="checkbox" class="toggler bg-transparent" />
                            ...
                            ?> -->
                              ...
                              <div class="customrange-section">
                                ...
                                <form method="POST" action="" class="custom-Range" id="date_form" name="date_form">
                                  ...
                                  <div class="date-range-input d-flex">
                                    <div class="form-input">
                                      <input type="date" class="form-control" name="from_date" id="from_date_day" value="<?= $_POST['from_date']; ?>" required>
                                    </div>
                                    <div class="form-input">
                                      <label class="mb-0" for="">TO</label>
                                      <input type="date" class="form-control" name="to_date" id="to_date_day" value="<?= $_POST['to_date']; ?>" required>
                                    </div>
                                  </div>
                                  <button type="submit" class="btn btn-primary float-right" id="rangeid" name="add_date_form">Apply</button>
                                </form>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $queryset = queryGet("SELECT stock.`itemId`, `itemName`, `itemCode`, `goodGroupName`, `uomName`, `itemPrice`, `bornDate`, `cogm`, `cogm_m`, `cogm_a`, SUM(`itemQty`) AS `itemQty` FROM `erp_inventory_stocks_log` AS stock LEFT JOIN `erp_inventory_items` AS items ON items.itemId = stock.itemId LEFT JOIN `erp_inventory_mstr_uom` AS uom ON uom.uomId = items.baseUnitMeasure LEFT JOIN `erp_bom` AS bom ON bom.itemId = stock.itemId LEFT JOIN `erp_inventory_mstr_good_groups` AS goodsgroup ON goodsgroup.goodGroupId = items.goodsGroup WHERE `refActivityName` = 'PROD-IN' AND stock.companyId = $company_id AND stock.branchId = $branch_id AND stock.locationId = $location_id AND items.goodsType IN (2,3) AND stock.bornDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' GROUP BY stock.itemId;", true);
                    // console($queryset);
                    $num_list = $queryset['numRows'];
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              <td><?php echo $data['goodGroupName']; ?></td>
                              <td><?php echo $data['uomName']; ?></td>
                              <td><?php echo round($data['itemQty'], 2); ?>
                                <?php
                                ...
                                for ($date = clone $startDate; $date <= $endDate; $date->modify('+1 day')) {
                                  ...
                                  $each_date = $date->format('Y-m-d');
                                  $itemId = $data['itemId'];
                                  $query_qty = queryGet("SELECT SUM(`itemQty`) AS `itemQty` FROM `erp_inventory_stocks_log` AS stock LEFT JOIN `erp_inventory_items` AS items ON items.itemId = stock.itemId WHERE `refActivityName` = 'PROD-IN' AND items.itemId = '$itemId' AND items.goodsType IN(2, 3) AND stock.bornDate = '$each_date' GROUP BY stock.itemId;", false);

                                  echo "<td>" . $query_qty["data"]["itemQty"] . "</td>";
                                }
            ...
            <div class="card card-tabs reports-card">
              ...
              <div class="tab-content" id="custom-tabs-two-tabContent">
                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                  ...
                  // $sql_list = "SELECT grp.goodGroupName AS item_group, SUM(goods.receivedQty) AS total_received_qty, SUM(goods.totalAmount) AS total_amount FROM erp_grninvoice AS grn INNER JOIN erp_grninvoice_goods AS goods ON grn.grnIvId=goods.grnIvId LEFT JOIN erp_inventory_items AS items ON goods.goodCode=items.itemCode LEFT JOIN erp_inventory_mstr_good_groups AS grp ON items.goodsGroup=grp.goodGroupId WHERE grn.companyId=$company_id AND grn.branchId=$branch_id AND grn.locationId=$location_id AND items.company_id=$company_id AND grn.postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND grn.grnStatus='active' GROUP BY item_group";

                  // $queryset = queryGet($sql_list, true);

                  $config_query = queryGet("SET sql_mode =( SELECT REPLACE (@@sql_mode, 'ONLY_FULL_GROUP_BY', '') );");
                  $queryset = queryGet("SELECT `itemName`, `cogm`, SUM(`itemQty`) AS `itemQty` FROM `erp_inventory_stocks_log` AS stock LEFT JOIN `erp_inventory_items` AS items ON items.itemId = stock.itemId LEFT JOIN `erp_inventory_mstr_uom` AS uom ON uom.uomId = items.baseUnitMeasure LEFT JOIN `erp_bom` AS bom ON bom.itemId = stock.itemId LEFT JOIN `erp_inventory_mstr_good_groups` AS goodsgroup ON goodsgroup.goodGroupId = items.goodsGroup WHERE `refActivityName` = 'PROD-IN' AND stock.companyId = $company_id AND stock.branchId = $branch_id AND stock.locationId = $location_id AND items.goodsType IN (2,3) AND stock.bornDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' GROUP BY stock.itemId;", true);
                      ...
                      <div class="row">
                        <div class="col-md-12 col-sm-12 d-flex p-sm-0">
                          <div class="card flex-fill reports-card">
                            <div class="card-body">
                              <div id="chartDivPurchaseRegisterProductWise" class="chartContainer"></div>
                            </div>
                          </div>
                        </div>
                      </div>
...
<script>
  ...
  am4core.ready(function() {
    ...
    var chart = am4core.create("chartDivPurchaseRegisterProductWise", am4charts.XYChart);
    ...
    for (elem of chartData.data) {
      finalData.push({
        "label": elem.itemName,
        "total_received_qty": Number(elem.itemQty),
        "total_amount": Number(elem.cogm),
      });
    };
    ...
    valueAxis2.title.text = "Total Received Quantity";
    ...
    lineSeries.name = "Total Received Quantity";
    lineSeries.dataFields.valueY = "total_received_qty";
    ...
    // // Show category axis name on hover
    // categoryAxis.renderer.labels.template.adapter.add("tooltipHTML", function(text, target) {
    //   return '<span style="font-weight: bold;">X-Axis: </span>' + text;
    // });
  });
</script>