                    ...
                    <div class="col-md-6">
                      <div class="form-input">
                        ...
                        <?php
                            ...
                            ?>
                        <select id="" name="layer" class="form-control form-control-border borderColor">
          ...
          <div class="col-md-5">
            ...
            border-top-right-radius: 12px; color: white">Bin Current Changes</th>
                ...
                <tbody>
                  ...
                  $sqlList = "SELECT bin.* , erpBin.layer_name FROM `erp_storage_bin` as bin  LEFT JOIN erp_layer as erpBin ON erpBin.layer_id =bin.layer_id WHERE 1 AND  bin.`company_id`=$company_id AND bin.`branch_id`=$branch_id AND bin.`location_id`=$location_id " . $cond . " " . $sts . "  ORDER BY bin.bin_id desc";
...
<div class="content-wrapper report-wrapper is-stock-new is-sales-orders is-warehouse vitwo-alpha-global">
  ...
  <!-- /.content-header -->
  <?php
    $bin_id = base64_decode($_GET['edit']);
  $sql = "SELECT bin.*, erpLayer.layer_name FROM `erp_storage_bin` AS bin  LEFT JOIN `erp_layer` AS erpLayer ON erpLayer.layer_id = bin.layer_id WHERE bin.`company_id` = $company_id AND bin.`branch_id` = $branch_id AND bin.`location_id` = $location_id AND bin.bin_id = $bin_id";
  ...
  <section class="content">
    <div class="container-fluid">
      <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" id="Edit_data" name="Edit_data">
        <input type="hidden" name="editBin" id="editBin" value="<?=$bin_id?>">
        <input type="hidden" name="editBinCode" id="editBinCode" value="<?=$row['bin_code']?>">
        <div class="row">
          <div class="col-md-7">
            <button type="button" class="btn-position" data-toggle="modal" data-target="#myModal"><i class="fa fa-cog"
                aria-hidden="true"></i></button>
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-input">
                        ...
                        <select id="warehouseDropDown" name="layer"
                          class="form-control form-control-border borderColor">
                          <option value="" data-goodType="">Layer</option>
                          <?php
                              $sl = $warehouseController->getAllLayer()['data'];
                              foreach ($sl as $list) {
                              ?>

                          <option <?php if($list['layer_id']==$row['layer_id']){echo "selected";}?>
                            value="<?= $list['layer_id'] ?>" data-goodType=""><?= $list['layer_name'] ?>
                          </option>
                          <?php }
                              ?>
                        </select>
                    ...
                    <div class="col-md-6">
                      <div class="form-input">
                        ...
                        <input type="text" name="name" class="form-control" value="<?= $row['bin_name'] ?>">
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-input">
                        <label>Maximum Temperature</label>
                        <input type="text" name="max_temp" class="form-control" value="<?= $row['max_temperature'] ?>">
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-input">
                        <label>Minimum Temperature</label>
                        <input type="text" name="min_temp" class="form-control" value="<?= $row['min_temperature'] ?>">
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-input">
                        <label>Maximum weight capacity</label>
                        <input type="text" name="max_weight" class="form-control" value="<?= $row['max_weight'] ?>">
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-input">
                        <label>Maximun volume capacity</label>
                        <input type="text" name="max_vol" class="form-control" value="<?= $row['max_volume'] ?>">
                      </div>
                    </div>
                    <div class="col-md-4">
                      <div class="form-input">
                        <label>Spec 1</label>
                        <input type="text" name="spec1" class="form-control" value="<?= $row['spec_one'] ?>">
                      </div>
                    </div>
                    <div class="col-md-4">
                      <div class="form-input">
                        <label>Spec 2</label>
                        <input type="text" name="spec2" class="form-control" value="<?= $row['spec_two'] ?>">
                      </div>
                    </div>
                    <div class="col-md-4">
                      <div class="form-input">
                        <label>Spec 3</label>
                        <input type="text" name="spec3" class="form-control" value="<?= $row['spec_three'] ?>">
                      </div>
                    </div>
...
<?php
  ...
  require_once("components/goods/create-bom.php");
    ...
    $url = BRANCH_URL . 'location/manage-bin.php';
    ?>
<script>
