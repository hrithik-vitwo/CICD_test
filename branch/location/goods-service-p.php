...
// if (!isset($_COOKIE["cookiesGoodsService"])) {
...
//         setcookie("cookiesGoodsService", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
//     } else {
//         for ($i = 0; $i < 5; $i++) {
...
if (isset($_POST["creategoodsdata"])) {
    ...
    if ($addNewObj["status"] == "success") {
        ...
        swalAlert($addNewObj["status"], ucfirst($addNewObj["status"]), $addNewObj["message"], BASE_URL . "branch/location/goods-p.php");
    } else {
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Item Master</h3>
                                            </div>
                                        </div>
...
<script>
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '') {
            ...
            var checkboxSettings = Cookies.get('cookiesGoodsService');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-goods-service.php",
                dataType: 'json',
                data: {
                    act: 'goodsService',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                ...
                                `<p class='pre-normal'> ${value.goodGroupName}</p>`,
                                value.goodTypeName,
                                value.uomName,
                                value.hsnCode,
                                `<p class='pre-normal'> ${value.TDS}</p>`,
                                value.status,
                                ` <div class="dropout">
                                    <button class="more">
                                         <span></span>
