                    ...
                    <div class="col-12 mt-2 p-0">
                        ...
                        <ul class="nav nav-tabs mb-3 border-bottom-0" id="custom-tabs-two-tab" role="tablist">
                            <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                                <h3 class="card-title">Manage Customer Discount Group</h3>
                        ...
                        <div class="card card-tabs">
                                    ...
                                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" id="add_frm" name="add_frm">
                                        ...
                                        <div class="modal-content card">
                                            <div class="modal-header card-header pt-2 pb-2 px-3">
                                                <h4 class="text-xs text-white mb-0">Create Customer Discount Group</h4>
                            ...
                            <div class="tab-content" id="custom-tabs-two-tabContent">
                                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab">
                                    ...
                                    $sql_list = "SELECT * FROM `erp_customer_discount_group` WHERE 1 " . $cond . "  AND company_id='" . $company_id . "' AND location_id = '".$location_id."' ORDER BY customer_discount_group_id desc limit " . $GLOBALS['start'] . "," . $GLOBALS['show'] . " ";
                                    ...
                                    $countShow = "SELECT count(*) FROM `erp_customer_discount_group` WHERE 1 " . $cond . " AND company_id='" . $company_id . "'  AND location_id = '".$location_id."'";
                                    $countQry = mysqli_query($dbCon, $countShow);
                                    $rowCount = mysqli_fetch_array($countQry);
                                    $count = $rowCount[0];
                                    $cnt = $GLOBALS['start'] + 1;
                                    $settingsTable = getTableSettings(TBL_COMPANY_ADMIN_TABLESETTINGS, "erp_customer_discount_group", $company_id);
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <?php if (in_array(1, $settingsCheckbox)) { ?>
                                                        <th>Customer Discount Group Name</th>
                                                    <?php }
                                            ...
                                            <tbody>
                                                ...
                                                ?>
                                                    <tr>
                                                        ...
                                                        <?php }
                                                        if (in_array(4, $settingsCheckbox)) { ?>
                                                            <td><?php if($row['updated_by'] != NULL) { echo getCreatedByUser($row['updated_by']); } else { echo getCreatedByUser($row['created_by']); } ?></td>
                                                        <?php }
                                                        if (in_array(5, $settingsCheckbox)) { ?>
                                                        ...
                                                        <td>
                                                            ...
                                                            <!-- <a href="<?= basename($_SERVER['PHP_SELF']) . "?view=" . $row['customer_discount_group_id']; ?>" style="cursor: pointer;" class="btn btn-sm" title="View Branch"><i class="fa fa-eye po-list-icon"></i></a> -->
                                                            <a href="<?= basename($_SERVER['PHP_SELF']) . "?edit=" . $row['customer_discount_group_id']; ?>" style="cursor: pointer;" class="btn btn-sm" data-toggle="modal" data-target="#editFunctionality_<?= $row['customer_discount_group_id'] ?>" title="Edit Branch"><i class="fa fa-edit po-list-icon"></i></a>


                                                            <div class="modal fade add-modal func-add-modal" id="editFunctionality_<?= $row['customer_discount_group_id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog" role="document">
                                                                    <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" id="add_frm" name="add_frm">
                                                                        <input type="hidden" name="editdata" id="editdata" value="">
                                                                        <input type="hidden" name="id" id="" value="<?= $row['customer_discount_group_id'] ?>">
                                                                                ...
                                                                                <div class="row">
                                                                                    <div class="col-lg-12 col-md-12 col-sm-12">
                                                                                        <div class="form-input mb-3">
                                                                                            ...
                                                                                            <input type="text" class="form-control" id="name" name="name" value="<?= $row['customer_discount_group'] ?>" required>
                                                                                            <span class="error name"></span>
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                                                            ...
                                                            <!-- <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" class="btn btn-sm">
                                                                <input type="hidden" name="id" value="<?php echo $row['customer_discount_group_id'] ?>">
