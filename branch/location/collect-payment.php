                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li>
                                                        ...
                                                        <ul class="dropdown-menu dropdown-submenu" id="collectDropdown">
                                                            ...
                                                            <hr>
                                                            <li><a class="dropdown-item" href="collectpaymentsettelment.php?adjust-cn">Credit Note Settlement</a></li>
                                                        </ul>
                                                    </li>
                                                </ul>
                                            </div>
                ...
                <div class="modal-body">
                    <nav>
                        <div class="nav nav-tabs global-view-navTabs" id="nav-tab" role="tablist">
                            ...
                            <button class="nav-link classicview-btn classicview-link active ViewfirstTab" id="nav-classicview-tab" data-id="" data-bs-toggle="tab" data-bs-target="#nav-classicview" type="button" role="tab" aria-controls="nav-classicview" aria-selected="true"><ion-icon name="apps-outline"></ion-icon>Preview</button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-collect-payment.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            if (value.status === 'active') {
                                ...
                                revCollectPay = `
                                    <li>
                                        <button class="reverseCollection" data-id="${value.paymentId}"  data-no="${value.transactionId}"><i class="far fa-undo po-list-icon"></i>Reverse</button>
                                    </li>`;
                            ...
                            dataTable.row.add([
                                ...
                                `<p>${value.party_code ?? '--'}</p>`,
                                `<p class="pre-normal">${value.party_name ?? '--'}</p>`,
                                `<p>${formatDate(value.postingDate)}</p>`,
                                `<p>${value.paymentCollectType}</p`,
                                `<p class="text-right">${(value.collect_payment)}</p>`,
                                `<p>${value["sopayment.created_by"]}</p>`,
                                status,
                                ` <div class="dropout">
                                    <button class="more">
                                         <span></span>
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiescollectpayment')
                },
                beforeSend: function() {
                    $('#loaderModal').show();
            ...
            success: function(value) {
                ...
                if (value.status) {
                    ...
                    if(value.currencyName){
                        $('#amount').html(value.currencyName+" "+decimalAmount(responseObj.collect_payment));
                    }else{
                        $('#amount').html(decimalAmount(responseObj.collect_payment));
                    }
                   $('#cus_name').html(value.name);
                   $('#po-numbers').html(responseObj.transactionId);
                   $('#default_address').html(value.address);
                   $(".created_by_trail").html(value.created_by + "<span class='font-bold text-normal'> on </span>" + value.created_at);
                   $(".updated_by").html(value.updated_by + "<span class='font-bold text-normal'> on </span>" + value.updated_at);
                   $('#nav-trail-tab').attr('data-ccode', responseObj.collectionCode);
                }
            },
            complete : function() {
