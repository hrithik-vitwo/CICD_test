...
if (isset($_POST["createdata"])) {
  ...
  //  console($addNewObj);
  swalToast($addNewObj["status"], $addNewObj["message"], "manage-customers.php");
}
if (isset($_POST["editData"])) {
  // console($_SESSION);
  $editDataObj = updateDataCustomer($_POST);
  swalToast($editDataObj["status"], $editDataObj["message"], "manage-customers.php");
}
...
?>
                ...
                <div class="card vendor-details-card withOutGST-card mb-0">
                  ...
                  <div class="card-body">
                    <div class="multisteps-form__content">
                      ...
                      $sql = "SELECT " . ERP_CUSTOMER . ".*, " . ERP_CUSTOMER_ADDRESS . ".*, tbl_customer_admin_details.* FROM `" . ERP_CUSTOMER . "` LEFT JOIN `" . ERP_CUSTOMER_ADDRESS . "` ON `" . ERP_CUSTOMER . "`.customer_id = `" . ERP_CUSTOMER_ADDRESS . "`.customer_id AND `" . ERP_CUSTOMER_ADDRESS . "`.customer_address_primary_flag = 1 LEFT JOIN tbl_customer_admin_details ON tbl_customer_admin_details.customer_id = `" . ERP_CUSTOMER . "`.customer_id WHERE `" . ERP_CUSTOMER . "`.customer_id = $editCustomerId";
                      ...
                      $state_sql = queryGet("SELECT * FROM `erp_gst_state_code` WHERE `country_id` = $companyCountry ");
                      //console($row);
                        ...
                        ?>
                          ...
                          <div class="col-lg-6 col-md-6 col-sm-6">
                            <div class="form-input stateDivDropDown">
                              ...
                              ?>
                                <input class="form-control" type="text" name="state" id="state" value="<?= $row['customer_address_state'] ?>">
                              <?php
  ...
  <script>
    $(document).ready(function() {
      ...
      var dbPassword = "<?= $row['fldAdminPassword'] ?>";
      ...
      if (customerGstNo != "") {
        $.ajax({
          ...
          success: function(response) {
            ...
            if ($.trim(response) == "Something went wrong try again!" || $.trim(response) == "Please provide valid gstin number!") {
            ...
            } else if ($.trim(response) == "customer and company pan can not be same") {
              let Toast = Swal.mixin({
                toast: true,
                position: 'top-end',
                showConfirmButton: false,
              ...
              Toast.fire({
                icon: `error`,
                title: `&nbsp;customer and company pan will not be same!`
              });
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (response == 'alredy exits') {
          ...
          $('#pan_error').html('customer and company pan can not be same');
