...
$sql_list = "SELECT * FROM `erp_production_declarations` WHERE 1 " . $cond . "  AND`branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . " AND (`prod_declaration_journal_id`  IS NOT NULL AND `fgsfg_declaration_journal_id`  IS NOT NULL) AND (reverse_prod_declaration_journal_id IS NULL OR reverse_fgsfg_declaration_journal_id IS NULL) AND `status` IN ('reverse')  ORDER BY id desc";
...
$prod_declaration_journal_id = $MainData[0]['prod_declaration_journal_id'];
$fgsfg_declaration_journal_id = $MainData[0]['fgsfg_declaration_journal_id'];
...
if (isset($_POST['act'])) {
    ...
    if ($journalObj["status"] == 'success') {
        ...
        $check_Journal = queryGet("SELECT * FROM `erp_acc_journal` WHERE `documentNo`='" . $prodcode . "' AND `parent_slug`='ProductiondeclarationInventoryissuance' AND `branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . "", true);
        if ($check_Journal['numRows'] == 1) {
            ...
            if ($returnDataAcc['status'] == 'success') {
                ...
                $dbObj->queryUpdate("UPDATE `erp_production_declarations` SET `prod_declaration_journal_id` = 0, `reverse_prod_declaration_journal_id` = " . $newJournalId . " WHERE `location_id` = '$location_id' AND `id`='$pay_id'");
            }else{
                 $flug=1;
            }
        } else if ($check_Journal['numRows'] == 2) {
            $newid = $check_Journal['data'][1]['id'];
            $dbObj->queryUpdate("UPDATE `erp_production_declarations` SET `prod_declaration_journal_id` = 0, `reverse_prod_declaration_journal_id` = " . $newid . " WHERE `location_id` = '$location_id' AND `id`='$pay_id'");
        } else {
            $flug=1;
        }
    }
    $journalObjfgsfg = $dbObj->queryGet('SELECT `id`, `company_id`, `branch_id`, `location_id`, `jv_no`, `party_code`, `party_name`, `parent_id`, `parent_slug`, `refarenceCode`, `journalEntryReference`, `documentNo`, `documentDate`, `postingDate`, `remark`, `journal_created_at`, `journal_created_by`, `journal_updated_at`, `journal_updated_by`, `journal_status` FROM `erp_acc_journal` WHERE `id`=' . $fgsfg_declaration_journal_id . ' AND `branch_id`=' . $branch_id);
    ...
    if ($journalObjfgsfg["status"] == 'success') {
        ...
        $check_Journal = queryGet("SELECT * FROM `erp_acc_journal` WHERE `documentNo`='" . $prodcode . "' AND `parent_slug`='FGSFGDeclaration' AND `branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . "", true);
        if ($check_Journal['numRows'] == 1) {
            $accPostingObj = new AccountingPosting();
            $returnDataAcc = $accPostingObj->post($accounting);
            ...
            if ($returnDataAcc['status'] == 'success') {
                ...
                $dbObj->queryUpdate("UPDATE `erp_production_declarations` SET `fgsfg_declaration_journal_id` = 0, `reverse_fgsfg_declaration_journal_id` = " . $newJournalId . " WHERE `location_id` = '$location_id' AND `id`='$pay_id'");
            }else{
                $flug1=1;
            }
        }else if($check_Journal['numRows'] == 2){
             $newid = $check_Journal['data'][1]['id'];
          $dbObj->queryUpdate("UPDATE `erp_production_declarations` SET `fgsfg_declaration_journal_id` = 0, `reverse_fgsfg_declaration_journal_id` = " . $newid . " WHERE `location_id` = '$location_id' AND `id`='$pay_id'");
        }else{
            $flug1=1;
        }
    }
    if ($flug == 0 && $flug1 == 0) {
        $logAccFailedResponce = updatelogAccountingFailure($prodcode);
         swalAlert("success", 'Success', "Production Accounting Success !", 'failed-accounting-production-declaration.php?reverse');
    } else {
                ...
                <div class="card-border-area">
                    <table>
                        ...
                        <tbody>
                            <?php
                            $crtotal = 0;
                            $debitObj = $dbObj->queryGet('SELECT * FROM `erp_acc_debit` WHERE `journal_id` IN (' . $prod_declaration_journal_id . ',' . $fgsfg_declaration_journal_id . ')', true);
                            // console($debitObj);
                            foreach ($debitObj["data"] as $debitRow) {
                                $gl = $dbObj->queryGet("SELECT * FROM erp_acc_coa_" . $company_id . "_table WHERE `id`=" . $debitRow['glId'] . "")['data'];
                                // console($gl);
                                $crtotal += $debitRow['debit_amount'];
                            ?>
                                <tr>
                                    <td>
                                        <p class="pre-normal">
                            ...
                            <tr>
                                ...
                                <td class="text-right text-bold"><?php echo decimalValuePreview($crtotal) ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="account-list debit-acc-list">
                ...
                <div class="card-border-area">
                    <table>
                        ...
                        <tbody>
                            <?php
                            $creditObj = $dbObj->queryGet('SELECT * FROM `erp_acc_credit` WHERE `journal_id` IN (' . $prod_declaration_journal_id . ',' . $fgsfg_declaration_journal_id . ')', true);

                            foreach ($creditObj["data"] as $creditRow) {
                                $gl = $dbObj->queryGet("SELECT * FROM erp_acc_coa_" . $company_id . "_table WHERE `id`=" . $creditRow['glId'] . "")['data'];
                                $drtotal += $creditRow['credit_amount']; ?>
                            ...
                            <tr>
                                ...
                                <td class="text-right text-bold"><?php echo decimalValuePreview($drtotal) ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
