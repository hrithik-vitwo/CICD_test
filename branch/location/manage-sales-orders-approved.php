...
if (!isset($_COOKIE["cookieApproveSo"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieApproveSo", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage SO Open List</h3>
                                            </div>
                                        </div>
                                        ...
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <div class="filter-action">
                                                ...
                                                <div class="length-row">
                                                    <span>Show</span>
                                                    <select name="" id="itemsPerPage" class="custom-select" value="25">
                                                        <option value="10">10</option>
                                                        <option value="25" selected="selected">25</option>
                                                        <option value="50">50</option>
                                                        <option value="100">100</option>
                                                                ...
                                                                <table>
                                                                    <tbody>
                                                                        ...
                                                                        foreach ($columnMapping as $columnIndex  => $column) {
                                                                            if ($columnIndex === 0 || $column['slag']==='stat.label') {
                                                                                continue;
                                                                            } ?>
                                                                            <tr>
                                                                                <td>
                                        ...
                                        <div class="top-details">
                                            ...
                                            <div class="right">
                                                ...
                                                <p class="info-detail default-address"><ion-icon name="location-outline"></ion-icon><span id="default_address">
                                                        Lorem ipsum dolor sit amet consectetur, adipisicing elit. Eveniet, nostrum nesciunt! Cum.
                                                    </span></p>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '',columnMapping='') {
            ...
            var checkboxSettings = Cookies.get('cookieApproveSo');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-so-approve.php",
                dataType: 'json',
                data: {
                    act: 'soapprove',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal"  data-id="${value.so_number}" data-code="${value.so_number}" data-toggle="modal" data-target="#viewGlobalModal">${ value.so_number}</a>`,
                                ...
                                ` <div class="dropout">
                                    ...
                                    <li>
                                        <button class="soModal"  data-id="${value.so_number}" data-toggle="modal" data-target="#viewGlobalModal"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                    </li>
                                    </ul>
                                </div>`
                            ]).draw(false);
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieApproveSo')
                },
                beforeSend:function(){
                    $('#loaderModal').show();
...
<script>
    $(document).on("click", ".soModal", function() {
        ...
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/so/ajax-manage-so-approve-modal.php",
            dataType: 'json',
            data: {
                so_no: so_no
            },
            ...
            success: function(value) {
                ...
                if (disCount == 0 && totalTax != 0) {
                    subTotal = decimalAmount(totalAmt - totalTax);
                    taxableAmt = subTotal;
                } else {
                    subTotal = decimalAmount(totalAmt - disCount);
                    taxableAmt = decimalAmount(subTotal - disCount);
                }

                if (responseObj.dataObj.igst == 0) {
                    cgst = decimalAmount(totalTax / 2);
                    sgst = decimalAmount(totalTax / 2);
                } else {
                    igst = decimalAmount(responseObj.dataObj.igst);
                }
                ...
                if (responseObj.dataObj.igst == 0) {
                    ...
                    $("#igstP").hide();
                    $("#igst").hide();
                    $("#cgstVal").html(responseObj.dataObj.currency_name + " " + cgst);
                    $("#sgstVal").html(responseObj.dataObj.currency_name + " " + sgst);
                } else {
                    $("#igst").html(responseObj.dataObj.currency_name + " " + igst);
                }
