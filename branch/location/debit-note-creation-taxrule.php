...
if ($_POST['action'] == "dn") {
    ...
    if ($addDebit['status'] == "success") {
        ...
        swalToast($addDebit["status"], $addDebit["message"]);
    }
}
?>
<style>
                                                ...
                                                <div class="row credit-note-basic">
                                                    ...
                                                    <div class="col-lg-6 col-md-6 col-sm-12">
                                                        <div class="form-input">
                                                            ...
                                                            <input type="date" value="<?= $min ?>" min="<?= $min ?>" max="<?= $max ?>" name="posting_date" class="form-control" id='partyDebitDate'>
                                                        </div>
                                                    </div>
                                                    <div class="col-lg-6 col-md-6 col-sm-12">
                                                        <div class="form-input">
                                                    ...
                                                    <tr class="items_row" id="<?= $rand ?>">
                                                        ...
                                                        <td class="text-right">
                                                            <div>
                                                                ...
                                                                <input type="number" name="item[<?= $rand ?>][qty]" class="form-control itemQty" id="itemQty_<?= $rand ?>" value="<?= inputQuantity($data['qty']) ?>">
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td class="text-right  customertds"><input type="number" name="item[<?= $rand ?>][total_tds_per]" class="form-control itemTds inputAmountClass" id="total_tds_per_<?= $rand ?>" value="0.0">
                                                        </td>
                                                        ...
                                                        <td>
                                                            <div class="tax-amount">
                                                                <div class="d-flex gap-2">
                                                                    <input type="number" class="form-control tax inputAmountClass" name="item[<?= $rand ?>][tax]" id="tax_<?= $rand ?>" value="<?= $data['tax'] ?>">
...
<?php
    ...
    $url = BRANCH_URL . 'location/manage-debit-notes-tax.php';
?>
    <script>
...
<script>
    ...
    function datevariant(vid) {
        ...
        $.ajax({
            type: "GET",
            url: `ajaxs/debit-note/ajax-date-variant.php?vr=${vid}`,
            beforeSend: function() {
                // $("#itemsDropDown").html(`Loding...`);
            },
            success: function(response) {
                                                        ...
                                                        <td style="width:20%">
                                                            <select name="item[${rand}][item_id]" class="form-control item_select item_select_${rand}" data-attr="${rand}">
                                                                ...
                                                                ?>
                                                                    ...
                                                                    <option value="<?= $item['itemId'] . '_' . $item['goodsType'] ?>"><?= $item['itemName'] . '[' .  $item['itemCode']  . ']' ?></option>   <?php
                                                                                                                                                                                                        }
                                                                                                                                                                                                            ?>
                                                            </select>
                                                            <input type="hidden" value="" name="item[${rand}][item_code]">
...
<input type="hidden" name="item[${rand}][tdsval]" class="form-control itemTdsVal" id="itemTdsVal_${rand}" value="0">
                                                        ...
                                                        <td><p class="d-flex gap-2"><input type="number" class="form-control tax inputAmountClass" name="item[${rand}][tax]" id="tax_${rand}" value=""><span class="percent-position">%</span>
                                                        <input type="hidden" class="form-control tax_amount" name="item[${rand}][tax_amount]" id="tax_amount_${rand}">
                                                        </p>
                                                        </td>
...
<script>
    ...
    $(document).on("change", "#bill", function() {
        ...
        $.ajax({
            ...
            url: `ajaxs/debit-note/ajax-bill-details-rule-book.php`, // need to replace after testing
            data: {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#partyDebitDate').val(obj['posting_date']);
                $('#partyDebitDate').attr('min', obj['posting_date']); // Party debit date should not be earlier than the Reference Doc Date


                $("#partyDebitDate").on("input", function() {
                    let minDate = new Date($(this).attr("min")); // Get min date
                    let maxDate = new Date($(this).attr("max")); // Get max date
                    let enteredDate = new Date($(this).val()); // Get entered date
                    console.log(enteredDate);
                    console.log(minDate)
                    console.log(maxDate)
                    if (enteredDate >= minDate && enteredDate <= maxDate) {
                        $("#addNewCreditNoteFormSubmitBtn").prop("disabled", false);
                    } else {
...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (itemData.status === "success") {
                    ...
                    $(`#itemQty_${rowId}`).val(inputQuantity(0));
                    $(`#checkQty_${rowId}`).val(itemData['data']['sumofbatch']);
                    $(`#checkQtyVal_${rowId}`).val(itemData['data']['sumofbatch']);
                    $(`#checkQtySpan_${rowId}`).html(decimalQuantity(itemData['data']['sumofbatch']));
    ...
    function totalquentity(rdcodeSt) {
        ...
        $(".qty" + rdcode).each(function() {
            ...
            var value = helperAmount($(this).val()) || 0;
            sum += value;
        });
                    ...
                    if (data.status === "success") {
                        for (let key in itemData) {
                            if (itemData.hasOwnProperty(key)) {
                                $(`#itemQty_${key}`).val(inputQuantity(0));
