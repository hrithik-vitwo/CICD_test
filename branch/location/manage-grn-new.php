...
if (!isset($_COOKIE["cookiegrninvpending"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiegrninvpending", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
if (isset($_POST["ivPostingFormSubmitBtn"])) {
    ...
    // exit();
    $createInvObj = $grnObj->createInvoice2($_POST);
    ...
    if ($createInvObj["status"] == "success") {
        ...
        $check = "SELECT `use_type` 
                ...
                WHERE 
                    `po_number` = '" . $_POST['invoicePoNumber'] . "' AND 
                    `company_id` = '" . $company_id . "' AND 
                    `branch_id` = '" . $branch_id . "' AND 
                    `location_id` = '" . $location_id . "'";
        $res = queryGet($check);

        if ($res['data']['use_type'] == "asset" && $itc == 1) {
            $ivPostingObj = $accountingControllerObj->grnIvAccountingPostingiTC($ivPostingInputData, "grniv", $createInvObj['grnIVId']);
            $updatemultiple_grn = queryUpdate("UPDATE `erp_grn` SET `itc`= '1' WHERE `grnPoNumber`='" . $_POST['invoicePoNumber'] . "'");
        } else {
            $ivPostingObj = $accountingControllerObj->grnIvAccountingPosting2($ivPostingInputData, "grniv", $createInvObj['grnIVId']);
        }
    ...
    if ($createInvObj["status"] == "success") {
        //console($ivPostingInputData);
        // console($ivPostingInputData);
        $ivPostingObj = $accountingControllerObj->srnIvAccountingPosting2($ivPostingInputData, "srniv", $createInvObj['grnIVId']);
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Pending IV</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var columnMapping = <?php echo json_encode($columnMapping); ?>;
            var checkboxSettings = Cookies.get('cookiegrninvpending');
            // var checkboxSettings = Cookies.get('cookiesProdDeclare');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-grn-pending.php",
                dataType: 'json',
                data: {
                    act: 'grnPending',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal"  data-id="${value.grnId}" data-grnType="${value.grnType}" data-toggle="modal" data-target="#viewGlobalModal">${value.grnCode}</a>`,
                                value.grnPoNumber,
                                value.vendorCode,
                                ...
                                ` <div class="dropout">
                                    ...
                                    <ul>
                                        <li>
                                             <button class="soModal" data-id="${value.grnId}" data-grnType="${value.grnType}" data-toggle="modal" data-target="#viewGlobalModal"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                        </li>
                                    </ul>                                   
                                </div>`,
                            ]).draw(false);
        ...
        $(document).on("click", ".ion-paginationlistQuotationList", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiegrninvpending')
                },
                beforeSend: function() {
                    $('#loaderModal').show();
        ...
        $(document).ready(function() {
            $(document).on("click", "#serach_submit", function(event) {
                ...
                $(".selectOperator").each(function() {
                    ...
                    if (columnSlag === 'vendorDocumentDate') {
                        values = value2;
                    } else if (columnSlag === 'postingDate') {
                        values = value4;
                    }

                    if ((columnSlag === 'vendorDocumentDate' || columnSlag === 'postingDate') && operatorName == "BETWEEN") {
...
<script>
    $(document).ready(function() {
        $(document).on("change", ".selectOperator", function() {
            ...
            let inputId;
            if (columnName.trim() === 'Document Date') {
                inputId = "value2_" + columnIndex;
            } else if (columnName.trim() === 'Posting Date') {
                inputId = "value4_" + columnIndex;
            }

            if ((columnName.trim() === 'Document Date' || columnName.trim() === 'Posting Date') && operatorName === 'BETWEEN') {
