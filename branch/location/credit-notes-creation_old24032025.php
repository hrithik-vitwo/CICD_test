                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-right">
                                                            <div>
                                                                ...
                                                                <input type="number" step="any" name="item[<?= $rand ?>][qty]" class="form-control inputQuantityClass itemQty" id="itemQty_<?= $rand ?>" value="<?= inputQuantity($data['qty']) ?>">
                                                            </div>
                                                        </td>
                                                        <td class="text-right"><input type="number" step="any" name="item[<?= $rand ?>][rate]" class="form-control inputAmountClass price" id="price_<?= $rand ?>" value="<?= inputValue($data['unitPrice']) ?>"></td>
                                                        <td>
                                                            <div class="tax-amount d-flex gap-2">
                                                                <input type="number" step="any" class="form-control inputQuantityClass tax" name="item[<?= $rand ?>][tax]" id="tax_<?= $rand ?>" value="<?= inputQuantity($data['tax']) ?>">
...
<script>
    ...
    //     let num = helperAmount(112.52458)
    //     console.log(num)
    //     alert("called")
    // })
        ...
        var newRow = ` <tr>
                                                        ...
                                                        <td class="text-right">
                                                            <div >
                                                                ...
                                                                <input type="number" step="any" name="item[${rand}][qty]" class="form-control inputQuantityClass itemQty" id="itemQty_${rand}" value="">
                                                            </div>                                                        
                                                        </td>
                                                        <td class="text-right"><input type="number" step="any" name="item[${rand}][rate]" class="form-control inputAmountClass price" id="price_${rand}" value=""></td>
                                                        <td>
                                                            <div class="d-flex gap-2">
                                                                <input type="number" step="any" class="form-control inputQuantityClass tax" name="item[${rand}][tax]" id="tax_${rand}" value="">
                                                                <span class="percent-position">%</span>
                                                                <input type="hidden" class="form-control tax_amount" name="item[${rand}][tax_amount]" id="tax_amount_${rand}">
                                                            </div>
                                                        </td>
...
<script>
    ...
    function calculateAllItemTax() {
        ...
        if (source == dest) {
            // alert(1);
            tax_each = helperAmount(totalTax / 2);
            $('#sgst_span').html(decimalAmount(tax_each));
            $('#cgst_span').html(decimalAmount(tax_each));
            $('#igst_span').html(0);
            $('#sgst').val(decimalAmount(tax_each));
            $('#cgst').val(decimalAmount(tax_each));
            $('#igst').val(0);
            ...
            $('#sgst_span').html(0);
            $('#cgst_span').html(0);
            $('#igst_span').html(decimalAmount(totalTax));
            $('#sgst').val(0);
            $('#cgst').val(0);
            $('#igst').val(decimalAmount(totalTax));
    ...
    function calculateAllItemsGrandAmount() {
        ...
        $(".amount").each(function() {
            let itemTotalPrice = helperAmount($(this).html());
        ...
        if (round_val_operator == '+') {
            grandTotal = helperAmount(grandTotalBeforeDiscount + Number(round_value));
        } else {
            grandTotal = helperAmount(grandTotalBeforeDiscount - Number(round_value));
        }
        
        $("#grandTotal").html(decimalAmount(grandTotal));
        let las  = (decimalAmount(grandTotal));
        console.log("the value is " + las)
        //  alert();
        $("#grandTotalHidden").val(decimalAmount(grandTotal));
        // $("#discountAmount").html(discount_amount);
        // $("#discountAmountHidden").val(discount_amount);
        $("#subTotalHidden").val(decimalAmount(grandTotalBeforeDiscount));
        $("#subTotal").html(decimalAmount(grandTotalBeforeDiscount));
    ...
    function calculateOneItemRowAmount(rowNum) {
        ...
        let tax = helperQuantity($(`#tax_${rowNum}`).val());

        tax = tax > 0 ? tax : 0;
        //alert(tax);
        let tax_amount = helperAmount((tax / 100 * unitPrice) * qty);

        let totalPrice = helperAmount((unitPrice * qty) + tax_amount);

        // alert(totalPrice);

        $(`#tax_amount_${rowNum}`).val(decimalAmount(tax_amount));
        $(`#amount_${rowNum}`).html(decimalAmount(totalPrice));
        $(`#amountHidden_${rowNum}`).val(decimalAmount(totalPrice));
