...
require_once("../../app/v1/functions/branch/func-branch-failed-accounting-controller.php");
...
$faildacc=new FailedAccController();
...
if (isset($_POST["ivPostingFormSubmitBtn"])) {
    ...
    if ($createInvObj["status"] == "success") {
        ...
        if ($ivPostingObj["status"] == "success" && $ivPostingObj["journalId"] != "") {
            ...
            $logAccFailedResponce = $faildacc->logAccountingFailure($createInvObj['grnIVId'], "grniv");
        }
    ...
    if ($createInvObj["status"] == "success") {
        ...
        if ($ivPostingObj["status"] == "success" && $ivPostingObj["journalId"] != "") {
            ...
            $logAccFailedResponce = $faildacc->logAccountingFailure($createInvObj['grnIVId'], "srniv");
        }
        swalAlert($createInvObj["status"], ucfirst($createInvObj["status"]), $createInvObj["message"], BASE_URL . "branch/location/manage-vendor-invoice.php");
    } else {
        swalAlert($createInvObj["status"], ucfirst($createInvObj["status"]), $createInvObj["message"]);
                                ...
                                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        ...
                                        <div class="right-block">
                                            <?php require_once("components/mm/grnInvoice-tabs.php"); ?>
