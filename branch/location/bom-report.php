...
if (!isset($_COOKIE["cookieManageBomReport"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieManageBomReport", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Final Good Type',
        'slag' => 'i1.goodsType',
        'icon' => '<ion-icon name="cube-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Required Item Code',
        'slag' => 'i2.itemCode',
        'icon' => '<ion-icon name="barcode-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    ...
    [
        'name' => 'Required Good UOM',
        'slag' => 'uom2.uomName',
        'icon' => '<ion-icon name="pricetag-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Required Item Type',
        'slag' => 'i2.goodsType',
        'icon' => '<ion-icon name="layers-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Required Item Qty',
        'slag' => 'bi1.consumption',
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Bom Report</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookieManageBomReport');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-bom-report.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                `<p>${value['i1.itemCode']}</p>`,
                                `<p class="pre-normal">${value['i1.itemName']}</p>`,
                                `<p>${value['uom1.uomName']}</p>`,
                                `<p>${value['i1.goodsType']}</p>`,
                                `<p>${value['i2.itemCode']}</p>`,
                                `<p class="pre-normal">${value['i2.itemName']}</p>`,
                                `<p>${value['uom2.uomName']}</p>`,
                                `<p>${value['i2.goodsType']}</p>`,
                                `<p>${value['bi1.consumption']}</p>`,
                                `<p>${value['b.bomStatus']}</p>`
                            ]).draw(false);
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieManageBomReport')
                },
                beforeSend: function() {
                    $('#loaderModal').show();
