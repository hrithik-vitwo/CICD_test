...
if (!isset($_COOKIE["cookieDonejob"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieDonejob", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Done Job Order</h3>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body">
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <a href="invoice-creation.php?sales_order_creation" class="btn btn-create" type="button">
                                                <ion-icon name="add-outline"></ion-icon>
                                                Create
                                            </a>
...
<script>
    ...
    $(document).ready(function() {
        $('.filter-link[data-name="doneJobs"]').addClass('active');
        var indexValues = [];
        var dataTable;
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookieDonejob');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-job-order.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal" data-id="${value.soId}" data-code="${value.so_number}" >${value.so_number}</a>`,
                                ...
                                decimalAmount(value["so.totalAmount"]),
                                decimalQuantity(value["so.totalItems"]),
                                value.approvalStatus_page,
                                ` <div class="dropout">
                                    <button class="more">
                                         <span></span>
                                         <span></span>
                                    ...
                                    <li>
                                        <button class="soModal" data-id="${value.soId}" data-code="${value.so_number}"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                    </li>
                                    </ul>
                                </div>`
                            ]).draw(false);
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieDonejob')
                },
                beforeSend: function() {
                    $('#loaderModal').show();
...
<script>
    $(document).on("click", ".soModal", function() {
        ...
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/so/ajax-manage-job-order-modal.php",
            dataType: 'json',
            data: {
                act: "modalData",
                soId
                ...
                if (value.status) {
                    ...
                    if (dataObj.approvalStatus == 9 || dataObj.approvalStatus == 11) {
                        if (dataObj.goodsType == "material" || dataObj.goodsType == "both") {
                            ...
                            <a title="Create Invoice" href="invoice-creation.php?so_to_invoice=${btoa(soId)}" class="btn-primary text-xs text-light deliveryCreationBtn pl-2 pr-2 ml-2"><i class="fa fa-plus mr-2"></i>Create Invoice</a>`;
                            ...
                            navBtn = `<a title="Create Invoice" href="invoice-creation.php?joborder_to_invoice=${btoa(soId)}" class="btn-primary text-xs text-light deliveryCreationBtn pl-2 pr-2 ml-2"><i class="fa fa-plus mr-2"></i>Create Invoice</a>`;
                        } else {
                            navBtn = `<a title="Create Invoice" href="invoice-creation.php?so_to_invoice=${btoa(soId)}" class="btn-primary text-xs text-light deliveryCreationBtn pl-2 pr-2 ml-2"><i class="fa fa-plus mr-2"></i>Create Invoice</a>`;
                        }
                    }
