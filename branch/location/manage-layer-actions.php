...
if (isset($_POST["createLayer"])) {
  ...
  $addNewObj = $warehouseController->createLayer($_POST, $branch_id, $company_id, $location_id, $created_by);
  swalToast($addNewObj["status"], $addNewObj["message"]);
}
...
if (isset($_POST["editLayer"])) {
  ...
  // console($_POST);
  // exit();
  $addNewObj = $warehouseController->editLayer($_POST, $branch_id, $company_id, $location_id, $created_by);
  swalToast($addNewObj["status"], $addNewObj["message"]);
  // console($addNewObj);
  // exit();
      ...
      <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" id="SubmitForm" name="SubmitForm">
        ...
        <div class="row">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-12">
                      <div class="form-input">
                        ...
                        <textarea type="text" name="layer_desc" class="form-control" rows="6" cols="50"></textarea>
                      </div>
                    </div>
          ...
          <div class="col-md-5">
            ...
            border-top-right-radius: 12px; color: white">Layer Current Change</th>
                  </tr>
                  <tr>
                    <th>Layer Name</th>
                    <th>Rack Name</th>
...
<div class="content-wrapper report-wrapper is-stock-new is-sales-orders is-warehouse vitwo-alpha-global">
  ...
  <!-- /.content-header -->
  <?php
    $layer_id = base64_decode($_GET['edit']);
     $sql = "SELECT layer.*, erpRack.rack_name FROM `erp_layer` AS layer  LEFT JOIN `erp_rack` AS erpRack ON erpRack.rack_id = layer.rack_id WHERE layer.`company_id` = $company_id AND layer.`branch_id` = $branch_id AND layer.`location_id` = $location_id AND layer.layer_id = $layer_id";  
  ...
  <section class="content">
    <div class="container-fluid">
      <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" id="Edit_data" name="Edit_data">
        <input type="hidden" name="editLayer" id="editLayer" value="<?= $layer_id ?>">
        <input type="hidden" name="editLayerCode" id="editLayerCode" value="<?= $row['layer_code']?>">
        <div class="row">
          <div class="col-md-7">
            <button type="button" class="btn-position" data-toggle="modal" data-target="#myModal"><i class="fa fa-cog"
                aria-hidden="true"></i></button>
              ...
              <div id="collapseThree">
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-input">
                        <label for="date">Rack Details Name</label>
                        <select id="warehouseDropDown" name="rack"
                          class="form-control form-control-border borderColor">
                          <?php
                              $sl = $warehouseController->getAllRack()['data'];
                              foreach ($sl as $list) {
                              ?>
                              <option <?php if($list['rack_id']==$row['rack_id']){echo "selected";}?>
                            value="<?= $list['rack_id'] ?>" data-goodType=""><?= $list['rack_name'] ?>
                          </option>
                          <?php }
                              ?>
                        </select>
                    ...
                    <div class="col-md-6">
                      <div class="form-input">
                        ...
                        <input type="text" name="name" class="form-control" value="<?= $row['layer_name'] ?>">
                      </div>
                    </div>
                    <div class="col-md-12">
                      <div class="form-input">
                        <label>Layer Description</label>
                        <input type="text" name="layer_desc" class="form-control" value="<?= $row['layer_desc'] ?>">
                      </div>
                    </div>
            ...
            <div class="row">
              <table class="table table-hover table-nowrap stock-new-table transactional-book-table mt-n5">
                <thead>
                     <tr>
                    <th colspan="3" style="text-align: center; font-size: 14px; background-color: #003060; border-top-left-radius: 12px;
            border-top-right-radius: 12px; color: white">Layer Current Changes</th>
                  </tr>
                  <tr>
                    <th>Layer Name</th>
                    <th>Rack Name</th>
...
<?php
  ...
  require_once("components/goods/create-bom.php");
    ...
    $url = BRANCH_URL . 'location/manage-layer.php';
    ?>
<script>
