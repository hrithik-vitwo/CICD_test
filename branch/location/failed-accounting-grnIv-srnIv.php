...
if (!isset($_COOKIE["cookiefailedgrnsrnIv"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiefailedgrnsrnIv", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Accounting Failed  GRNIV/SRNIV</h3>
                                            </div>
                                        </div>
                                      ...
                                      <div class="right-block">
                                            <div class="page-list-filer filter-list">
                                                ...
                                                <a class="filter-link active" id="active"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>GRNIV/SRNIV
                                                </a>
                                                <style>
                                                    @keyframes pulse {
                                                        0% {
                                                ...
                                                <a class="filter-link" id="reverse"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Reverse GRNIV/SRNIV
                                                </a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '',type = '') {
            ...
            var columnMapping = <?php echo json_encode($columnMapping); ?>;
            var checkboxSettings = Cookies.get('cookiefailedgrnsrnIv');
            ...
            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-failed-acc-grnIv-srnIv.php",
                dataType: 'json',
                data: {
                    act: 'failedAcc',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            let actionType = (value.grnType=='grn')?`grn_id=${btoa(value.grnIvId+"_"+value.grnId)}`:`srn_id=${btoa(value.grnIvId)}`;
                            if (response.type == 'active') {
                                button= ` <div class="view-accoiunt" data-id='${value.grnId}'>
                                    <a href="manage-failed-acc-grnIv-srnIv-view.php?${actionType}" class="wallet-accountbtn">
                                ...
                                button= ` <div class="view-accoiunt" data-id='${value.grnId}'>
                                    <a href="manage-failed-acc-grnIv-srnIv-view_reverse.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`;
