...
if (isset($_POST['addNewinvoiveSaveFormSubmitBtn'])) {
  ...
  // console($_POST);
  //exit;
  $addBranchSoInvoice = $BranchSoObj->insertBranchInvoice($_POST, $body);
...
?>
    <script>
      //   window.location.href = "manage-invoices.php";
    </script>
<?php
...
$pgiID = base64_decode($_GET['pgi_no']);

$pgiDetails = $BranchSoObj->fetchBranchSoDeliveryPgiById($pgiID)['data'][0];
$SoDetails = $BranchSoObj->fetchSoDetailsById($pgiDetails['so_number'])['data'][0] ?? 0;
...
$pgiItemDetails = $BranchSoObj->fetchBranchSoDeliveryItemsPgi($pgiDetails['so_delivery_pgi_id'])['data'];
...
$invoiceDetails = $BranchSoObj->fetchBranchSoInvoiceById($pgiID)['data'][0];
$customerDetails = $BranchSoObj->fetchCustomerDetails($pgiDetails['customer_id'])['data'][0];
              ...
              <div class="col-lg-6 col-md-6 col-sm-6 col-6 col-parent">
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-sm-12 col-child border-bottom-0">
                    ...
                    <p>Company’s PAN: <?= $companyDetails['company_pan'] ?></p>
                ...
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-sm-12 col-child border-bottom-0">
                    ...
                    <p><?= $pgiDetails['customer_billing_address'] ?></p>
                ...
                <div class="row">
                  <div class="col-lg-12 col-md-12 col-sm-12 col-child border-bottom-0">
                    ...
                    <p class="font-bold"> <?= $customerDetails['trade_name'] ?></p>
                    <p><?= $pgiDetails['customer_shipping_address'] ?></p>
                    <p>State Name : <?= fetchStateNameByGstin($customerDetails['customer_gstin']) ?>, Code : <?= substr($customerDetails['customer_gstin'], 0, 2); ?></p>
                    <p>Place of Supply : <?= fetchStateNameByGstin($customerDetails['customer_gstin']) ?></p>
                  </div>
                </div>
              </div>
              <input type="hidden" name="invoiceDetails[pgiId]" value="<?= $pgiDetails['so_delivery_pgi_id'] ?>">
              <input type="hidden" name="invoiceDetails[pgiNo]" value="<?= $pgiDetails['pgi_no'] ?>">
              <input type="hidden" name="invoiceDetails[delivery_no]" value="<?= $pgiDetails['delivery_no'] ?>">
              <input type="hidden" name="invoiceDetails[so_number]" value="<?= $pgiDetails['so_number'] ?>">
              <input type="hidden" name="invoiceDetails[customer_id]" value="<?= $pgiDetails['customer_id'] ?>">
              <input type="hidden" name="invoiceDetails[totalItems]" value="<?= $pgiDetails['totalItems'] ?>">
              <input type="hidden" name="invoiceDetails[customer_billing_address]" value="<?= $pgiDetails['customer_billing_address'] ?>">
              <input type="hidden" name="invoiceDetails[customer_shipping_address]" value="<?= $pgiDetails['customer_shipping_address'] ?>">
              <input type="hidden" name="invoiceDetails[invNo]" value="<?= $invoiceGenCode ?>">
              <input type="hidden" name="invoiceDetails[poNumber]" value="<?= $pgiDetails['customer_po_no'] ?>">
              <input type="hidden" name="invoiceDetails[creditPeriod]" value="<?= $SoDetails['credit_period'] ?>">
              <input type="hidden" name="invoiceDetails[poDate]" value="<?= $SoDetails['delivery_date'] ?>">
            ...
            <div class="row">
              <table class="tax-invoice-table" style="overflow: auto;">
                <tbody>
                  ...
                  $allSubTotalAmt = 0;
                  $totalAmt = 0;
                  foreach ($pgiItemDetails as $key => $item) {
                  ...
                  ?>
                    ...
                    <input type="hidden" name="listItem[<?= $key ?>][so_delivery_pgi_id]" value="<?= $item['so_delivery_pgi_id'] ?>">
                    ...
                    <input type="hidden" name="listItem[<?= $key ?>][totalDiscountAmt]" value="<?= $item['totalDiscountAmt'] ?>">
                  ...
                  <tr class="border-top-none">
                    ...
                    <td class="border-top-0">
                      ...
                      <p class="text-right">
                        <input type="hidden" name="invoiceDetails[totalDiscount]" value="<?= $pgiDetails['totalDiscount'] ?>">
                        <?= number_format($pgiDetails['totalDiscount'], 2) ?>
                      </p>
                      <?php
                      if ($conditionGST) {
