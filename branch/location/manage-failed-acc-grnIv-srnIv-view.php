...
if (isset($_POST['act'])) {
    ...
    // exit();
    if ($grnType == 'grn') {
        $check_sqliv = queryGet("SELECT * FROM `erp_acc_journal` WHERE `parent_id` LIKE '" . $_POST['grnId'] . "' AND `parent_slug` LIKE 'grniv' AND `refarenceCode` LIKE '" . $_POST['grnCode'] . "'");
        if ($check_sqliv['status'] == 'success') {
            $grnJournalId = $check_sqliv['data']['id'];
            $queryObj = queryUpdate('UPDATE `erp_grninvoice` SET `ivPostingJournalId`=' . $grnJournalId . ' WHERE `grnIVId`=' . $_POST['grnIvId']);
            if ($queryObj['status'] == "success") {
                $update=updatelogAccountingFailure($grnIvCode);
            }
        } else {
            $grnPostingObj = $accountObj->grnIvAccountingPosting($ivPostingInputData, "grniv", $_POST['grnIvId']);
        ...
        if ($check_sqliv['status'] == 'success') {
            ...
            $grnPostingObj = $accountObj->srnIvAccountingPosting($ivPostingInputData, "srniv", $_POST['grnIvId']);
            if ($grnPostingObj['status'] == 'success') {
                $grnJournalId = $grnPostingObj['journalId'];
                $queryObj = queryUpdate('UPDATE `erp_grninvoice` SET `ivPostingJournalId`=' . $grnJournalId . ' WHERE `grnIVId`=' . $_POST['grnIvId']);
                if ($queryObj['status'] == "success") {
    ...
    if ($flug == 0) {
        swalAlert("success", 'Success', "GRNIv/SRNIv Accounting Posted Successfully", 'failed-accounting-grnIv-srnIv.php');
    } else {
...
$sql_Mainqry = "SELECT * FROM `erp_grninvoice` as erp_grn LEFT JOIN `erp_vendor_details`as ven ON erp_grn.vendorId = ven.vendor_id  WHERE 1 " . $cond . " AND (ivPostingJournalId = 0 OR ivPostingJournalId IS NULL)  AND companyId =" . $company_id . " AND branchId=" . $branch_id . " AND locationId=" . $location_id . "" . $sts . " ORDER BY grnIvId DESC";
...
<div class="content-wrapper is-failed-account-view vitwo-alpha-global" style="overflow: auto">
    ...
    <div class="container-fluid mt-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="failed-accounting-grnIv-srnIv.php" class="text-dark"><i class="fa fa-list po-list-icon"></i>Failed Accounting List</a></li>
    ...
    <form method="post" action="">
        ...
        <input type="hidden" name="grnPostingJournalId" value="<?= $grnMainData['grnPostingJournalId']; ?>">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-right text-bold"><?php echo decimalValuePreview($grnMainData['grnTotalTds'] + $itemTotalAmt); ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
