    ...
    if (!isset($_COOKIE["cookieRackLOg"])) {
        ...
        if ($settingsCheckbox_concised_view) {
            setcookie("cookieRackLOg", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
        }
    }
    ...
    $columnMapping = [
        ...
        [
            ...
            'slag' => 'erpStorage.storage_location_name',
            'icon' => '<ion-icon name="location-outline"></ion-icon>',
        ],
        [
            'name' => 'Description',
            'slag' => 'rack.rack_description',
            'icon' => '<ion-icon name="location-outline"></ion-icon>',
        ],
        [
            'name' => 'Created By',
            'slag' => 'rack.created_by',
            'icon' => '<ion-icon name="location-outline"></ion-icon>',
        ]
                                        ...
                                        <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                            <div class="left-block">
                                                <div class="label-select">
                                                    <h3 class="card-title mb-0">Manage Rack</h3>
                                                </div>
                                            </div>
                                            ...
                                            <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                                ...
                                                <div class="exportgroup">
                                                    ...
                                                    <ul class="export-options">
                                                        <li>
                                                            <button class="ion-paginationlistRack">
                                                                <ion-icon name="list-outline" class="ion-paginationlistRack md hydrated" role="img" aria-label="list outline"></ion-icon>Export
                                                            </button>
                                                        </li>
                                                        <li>

                                                            <button class="ion-fulllistRack">
                                                                <ion-icon name="list-outline" class="ion-fulllistRack md hydrated" role="img" aria-label="list outline"></ion-icon>Download
                                                            </button>
                                                        </li>
                                                    </ul>
                                                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
                    ...
                    success: function(response) {
                        ...
                        if (response.status) {
                            ...
                            $.each(responseObj, function(index, value) {
                                dataTable.row.add([
                                    value.sl_no,
                                    `<a href="#" class="soModal" data-id="${value['rack.rack_id']}">${value['rack.rack_name']}</a>`,
                                    value['erpStorage.storage_location_name'],
                                    value['rack.rack_description'],
                                    value['rack.created_by'],
                                    ` <div class="dropout">
                                        <button class="more">
                                            <span></span>
                                            <span></span>
                                        ...
                                        <ul>
                                        <li>
                                            <button id="editBtn" data-id="${btoa(value['rack.rack_id'])}" data-toggle="modal" data-target="#editModal"><ion-icon name="create-outline" class="ion-edit"></ion-icon>Edit</button>
                                        </li>
                            
                                        <li>
                                            <button class="soModal" data-toggle="modal" data-id="${value['rack.rack_id']}" data-target="#viewGlobalModal"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                        </li>
                                        <li>
                            ...
                            var checkboxSettings = Cookies.get('cookieRackLOg');
            ...
            $(document).on("click", ".ion-paginationlistRack", function(e) {
                $.ajax({
                    type: "POST",
                    ...
                    data: {
                        ...
                        sql_data_checkbox: Cookies.get('cookieRackLOg')
                    },
                    beforeSend: function() {
                        // console.log(sql_data_checkbox);
                        $('#loaderModal').show();
                        $('.ion-paginationlistRack').prop('disabled', true)
                    },
                    ...
                    complete: function() {
                        ...
                        $('#loaderModal').hide();
                        $('.ion-paginationlistRack').prop('disabled', false)
                    }
                })

            });
            $(document).on("click", ".ion-fulllistRack", function(e) {
                $.ajax({
                    type: "POST",
                    url: "ajaxs/ajax-mange-rack.php",
                    dataType: "json",
                    ...
                    beforeSend: function() {
                        // console.log(sql_data_checkbox);
                        $('#loaderModal').show();
                        $('.ion-fulllistRack').prop('disabled', true)
                    },
                    success: function(response) {
                    ...
                    complete: function() {
                        // Hide loader modal after request completes
                        $('#loaderModal').hide();
                        $('.ion-fulllistRack').prop('disabled', false);
                    }
                })
    ...
    <script>
        $(document).on("click", ".soModal", function() {
            ...
            // $('.auditTrail').attr("data-ccode", rack_id);

            $.ajax({
                type: "GET",
                url: "ajaxs/modals/wms/ajax-manage-rack-modal.php",
                dataType: 'json',
                data: {
                    rack_id: rack_id
                },
                ...
                success: function(value) {
                    ...
                    $('.auditTrail').attr("data-ccode", data.rack_code);
                    $('.rackName').html(data.rack_name)
                    $('.storagelocationName').html(data.storage_location_name)
                    $('#rackDescription').html(data.rack_description)
        ...
        $(document).on("click", "#editBtn", function() {
            ...
            let url = "<?php echo  BRANCH_URL . 'location/manage-rack-actions.php?edit=' ?>" + prId;
