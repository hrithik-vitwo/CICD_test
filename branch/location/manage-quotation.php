...
if (!isset($_COOKIE["cookiesquotationvm"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiesquotationvm", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Vendor-Response Ratio',
        'slag' => 'vendoracpratio1',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => ''
    ],
    [
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage Quotation</h3>
                                            </div>
                                        </div>
                                                                ...
                                                                <table>
                                                                    <tbody>
                                                                        ...
                                                                        foreach ($columnMapping as $columnIndex  => $column) {
                                                                            if ($columnIndex === 0 || $column['name'] == 'Vendor-Response Ratio') {
                                                                                continue;
                                                                            } ?>
                                                                            <tr>
                                                                                <td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var columnMapping = <?php echo json_encode($columnMapping); ?>;
            var checkboxSettings = Cookies.get('cookiesquotationvm');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-vm-quotation.php",
                dataType: 'json',
                data: {
                    act: 'quotation',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                `<a  href="#" class="soModal" data-id="${value.rfqId}">${value["rfq.rfqCode"]}</a>`,
                                value.vendoracpratio,
                                ...
                                ` <div class="dropout">
                                    ...
                                    <ul>                                    
                                        ...
                                        <li>
                                            <button class="deleteQuoBtn" data-toggle="modal"  data-id="${value.rfqId}" data-code="${value["rfq.rfqCode"]}"><ion-icon name="trash-outline" class="ion-delete"></ion-icon>Delete</button>
                                        </li>
        ...
        $(document).on("click", ".ion-paginationlistQuotationList", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiesquotationvm')
                },
                beforeSend: function() {
                    $('#loaderModal').show();
...
<script>
    ...
    $(document).on("click", ".deleteQuoBtn", function() {
        let id = $(this).data("id");
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    type: "GET",
                    url: "ajaxs/modals/vm/ajax-manage-vm-quotation-modal.php",
                    dataType: "json",
                    data: {
                        act: 'quotDel',
                        id
