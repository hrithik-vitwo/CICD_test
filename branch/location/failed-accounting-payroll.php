...
if (!isset($_COOKIE["cookiefailedpayroll"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiefailedpayroll", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Failed Payroll</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookiefailedpayroll');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-failed-acc-payroll.php",
                dataType: 'json',
                data: {
                    act: 'payroll',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            let actionType = `pay_id=${(value.payroll_main_id_action)}`;
                            ...
                            dataTable.row.add([
                                ...
                                value.payroll_code,
                                value.month_year,
                                decimalAmount(value.sum_gross),
                                decimalAmount(value.sum_pf_employee),
                                decimalAmount(value.sum_pf_employeer),
                                decimalAmount(value.sum_pf_admin),
                                decimalAmount(value.sum_esi_employee),
                                decimalAmount(value.sum_esi_employeer),
                                decimalAmount(value.sum_ptax),
                                decimalAmount(value.sum_tds),
                                status,
                                ` <div class="view-accoiunt" data-id='${value.dr_note_id}'>
                                    <a href="manage-failed-acc-payroll-view.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`,
