                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="filter-list vendor-invoice-tab">
                      ...
                      <a href="<?= LOCATION_URL; ?>manage-vendor-invoice.php?value=paid" class="btn active waves-effect waves-light"><i class="fa fa-clock mr-2 active"></i>Paid</a>
                    </div>
              ...
              <form name="search" id="search" action="<?php $_SERVER['PHP_SELF']; ?>" method="post" onsubmit="return srch_frm();">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  ...
                  <?php
                    ...
                    $sql_list = "SELECT grniv.*, grn.`grnCreatedAt` AS grnDate, grn.`po_date` AS poDate FROM `" . ERP_GRNINVOICE . "` AS grniv LEFT JOIN `erp_grn` AS grn ON grn.`grnId` = grniv.`grnId` WHERE 1 ".$cond." AND grniv.`companyId`='$company_id' AND grniv.`branchId`='$branch_id' AND grniv.`locationId`='$location_id' AND grniv.`paymentStatus`='4' ".$sts." ORDER BY grniv.`grnIvId` DESC limit " . $GLOBALS['start'] . "," . $GLOBALS['show'] . " ";
                    $qry_list = queryGet($sql_list, true);
                    $num_list = $qry_list['numRows'];
  
  
                    $countShow = "SELECT count(*) FROM `" . ERP_GRNINVOICE . "` AS grniv LEFT JOIN `erp_grn` AS grn ON grn.`grnId` = grniv.`grnId` WHERE 1 ".$cond." AND grniv.`companyId`='$company_id' AND grniv.`branchId`='$branch_id' AND grniv.`locationId`='$location_id' AND grniv.`paymentStatus`='4' ".$sts." ORDER BY grniv.`grnIvId` DESC";
