                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="filter-list vendor-invoice-tab">
                      ...
                      <a href="<?= LOCATION_URL; ?>manage-vendor-invoice.php?value=payable" class="btn active waves-effect waves-light"><i class="fa fa-clock mr-2 active"></i>Payable</a>
                      ...
                      <a href="<?= LOCATION_URL; ?>manage-vendor-invoice.php?value=overdue" class="btn waves-effect waves-light"><i class="fa fa-clock mr-2"></i>OverDue</a>
              ...
              <form name="search" id="search" action="<?php $_SERVER['PHP_SELF']; ?>" method="post" onsubmit="return srch_frm();">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  ...
                  <?php
                    ...
                    $sql_list = "SELECT grniv.*, grn.`grnCreatedAt` AS grnDate, grn.`po_date` AS poDate FROM `" . ERP_GRNINVOICE . "` AS grniv LEFT JOIN `erp_grn` AS grn ON grn.`grnId` = grniv.`grnId` WHERE 1 ".$cond." AND grniv.`companyId`='$company_id' AND grniv.`branchId`='$branch_id' AND grniv.`locationId`='$location_id' AND grniv.`paymentStatus`='15' ".$sts." ORDER BY grniv.`grnIvId` DESC limit " . $GLOBALS['start'] . "," . $GLOBALS['show'] . " ";
                    $qry_list = queryGet($sql_list, true);
                    $num_list = $qry_list['numRows'];
  
  
                    $countShow = "SELECT count(*) FROM `" . ERP_GRNINVOICE . "` AS grniv LEFT JOIN `erp_grn` AS grn ON grn.`grnId` = grniv.`grnId` WHERE grniv.`companyId`='$company_id' AND grniv.`branchId`='$branch_id' AND grniv.`locationId`='$location_id' AND grniv.`paymentStatus`='15' ".$sts." ORDER BY grniv.`grnIvId` DESC";
                        ...
                        ?>
                          <tr>
                            ...
                            <?php
                              ...
                              $duedate = strtotime(date("d-m-Y", strtotime($one['dueDate'])));

                              if($today < $duedate)
                              {
                                echo "<td><p class='status-info text-xs text-center'>Payable</p></td>";
                              }
                              elseif($today == $duedate)
                              {
                                echo "<td><p class='status-secondary text-xs text-center'>Due</p></td>";
                              }
                              elseif($today > $duedate)
                              {
                                echo "<td><p class='status-danger text-xs text-center'>Overdue</p></td>";
                              }
                            ?>
                            <td><?= $one['grnCode'] ?? "-" ?></td>
