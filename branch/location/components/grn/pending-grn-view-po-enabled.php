                    ...
                    <div class="row grn-vendor-details">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            ...
                            <div class="display-flex grn-form-input-text">
                                <i class="fa fa-check"></i>
                                &nbsp;
                                <p class="label-bold">Currency Conversion Rate :</p>
                                &nbsp;
                            ...
                            <div class="tab-pane fade quick-registration-vendor" id="vendor_list" role="tabpanel" aria-labelledby="invoice-po-list-tab">
                                <div class="container">
                                    <ul>
                                        ...
                                        <table class="table-sales-order grnPoEnableTable table defaultDataTable grn-table">
                                            <thead>
                                                <tr>
                                                    <th>Sl No.</th>
                                                    <th>Vendor Code</th>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                ...
                                                <tr>
                                                    ...
                                                    foreach ($poItemsList as $poItem) {
                                                        if ($poItem["itemName"] == $itemName) {
                                                            array_push($name_array_po, "1");
                                                            if ($oneItemObj["UnitPrice"] == $poItem["unitPrice"]) {
                                                                array_push($price_array_po, "1");
                                                                ...
                                                                // $match = "";
                                                                array_push($price_array_po, "0");
                                                                continue;
                                                            }
                                                        } else {
                                                            // $match = "";
                                                            array_push($name_array_po, "0");
                                                            continue;
                                                        }
                                                    }
                                                    ?>
                    ...
                    <tr id="grnItemRowTr_<?= $sl ?>">
                        ...
                        <td>
                            <div class="input-group input-group-sm m-0" style="flex-wrap: nowrap;">
                                ...
                                <input type="number" step="any" name="grnItemList[<?= $sl ?>][itemUnitPriceOtherCurrency]" value="<?= number_format($itemUnitPrice, 2, '.', '') ?>" id="grnItemUnitPriceTdInput_<?= $sl ?>" class="form-control text-xs itemUnitPrice w-auto" required>
                        ...
                        ?>
                            <td>
                                <div class="form-input d-flex" style="align-items: center; gap: 7px;">
                                    <input type="number" name="grnItemList[<?= $sl ?>][itemTds]" value="<?= $tds ?? 0 ?>" id="grnItemTdsTdInput_<?= $sl ?>" class="form-control text-center p-0 text-xs itemTds bg-transparent border-0" style="width: 30px !important;" required>
                                    <p class="text-xs">%</p>
                                </div>
                            </td>
                        <?php
                        ...
                        ?>
                            <td>
                                <div class="form-input d-flex" style="align-items: center; gap: 7px;">
                                    <input type="number" name="grnItemList[<?= $sl ?>][itemTds]" value="<?= $tds ?>" id="grnItemTdsTdInput_<?= $sl ?>" class="form-control text-center p-0 text-xs itemTds bg-transparent border-0" style="width: 30px !important;" required>
                                    <p class="text-xs">%</p>
                                </div>
                            </td>
                        <?php
        ...
        ?>
            ...
            ?>
                ...
                if ($no_po == false) {
                    if ($no_po_item == false) {
                        if ($customerPurchaseOrder != "") {
                            if (in_array("0", $name_array_po)) {
                ?>
                                <i class='fa fa-exclamation-triangle' aria-hidden='true'></i>PO Name not matched with Invoice name
                                ...
                                if (in_array("0", $price_array_po)) {
                                ?>
                                    <i class='fa fa-exclamation-triangle' aria-hidden='true'></i>PO Price not matched with Invoice price
                                    ...
                                    ?>
                                            <i class='fa fa-exclamation-triangle' aria-hidden='true'></i>Invoice Quantity is greater than PO Quantity
                                            <?php
                                        } else {
                                            if (in_array("0", $check_array_1)) {
                                                $item_id_po = 0;
                                                foreach ($po_id as $key => $po_item_id) {

                                            ?>
                                                    <input type="hidden" name="poItemId[<?= $po_item_id ?>]" value="<?= $po_qty[$key] ?>">
                            ...
                            ?>
                            <i class='fa fa-exclamation-triangle' aria-hidden='true'></i>No PO selected
                        ...
                        ?>
                        <i class='fa fa-exclamation-triangle' aria-hidden='true'></i>Selected PO has no Items
                    ...
                    ?>
                    <i class='fa fa-exclamation-triangle' aria-hidden='true'></i>Wrong PO selected
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <button type="button" id="allocate_id" name="" class="btn btn-primary btnstyle mt-2">Allocate Cost</button>
            </div>
        </div>
    </div>
</div>
                    ...
                    <select class="form-control" name="modalItemCodeChange" id="modalItemCodeDropDownChange" required>
                        ...
                        if ($rmGoodsObj["status"] == "success") {
                            ...
                            foreach ($rmGoodsObj["data"] as $oneRmGoods) {
                                ...
                                $oneRmGoods_summary = queryGet("SELECT * FROM `erp_inventory_stocks_summary` WHERE `itemId`='$oneRmGoods_item_id' AND `company_id`='$company_id' AND `branch_id`='$branch_id' AND `location_id`='$location_id'", false);
                        ?>
                                <option value="<?= $oneRmGoods["itemCode"]; ?>" data-qualityenable="<?= $oneRmGoods_summary["data"]["quality_enabled"] ?>" data-default="<?= $oneRmGoods_summary["data"]["default_storage_location"] ?>" data-qalocation="<?= $oneRmGoods_summary["data"]["qa_storage_location"] ?>" data-taxpercent="<?= $oneRmGoods["taxPercentage"]; ?>" data-name="<?= $oneRmGoods["itemName"]; ?>" data-uom="<?= $oneRmGoods["uomName"]; ?>" data-uomId="<?= $oneRmGoods["uomId"]; ?>" data-itemid="<?= $oneRmGoods["itemId"]; ?>" data-hsncode="<?= $oneRmGoods["hsnCode"]; ?>" data-hsnPercent="<?= $oneRmGoods["taxPercentage"]; ?>" data-goods="<?= $oneRmGoods["goodsType"]; ?>" data-tds="<?= $oneRmGoods["tds"]; ?>" data-itemtitle="<?= $oneRmGoods["itemName"]; ?>"><?= $oneRmGoods["itemCode"]; ?> | <?= $oneRmGoods["itemName"]; ?> | <?= $oneRmGoods["taxPercentage"] . " %"; ?></option>
                        <?php
                            }
                        }
                        ?>
    ...
    $(document).ready(function() {
        ...
        function calculateGrandTotalAmount() {
            ...
            if (totalAmount != totalInternalTaxValue) {
                $("#internalotaxwarning").html("<i class='fa fa-exclamation-triangle' aria-hidden='true'></i>OCR Tax Percentage not matched with internal tax percentage");
            } else {
