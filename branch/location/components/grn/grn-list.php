            ...
            <div class="row p-0 m-0">
                <div class="col-12 mt-2 p-0">
                    <div class="filter-list">
                        <a href="<?= LOCATION_URL; ?>manage-grn.php?pending" class="btn active"><i class="fa fa-stream mr-2 active"></i>Pending List</a>
                        <a href="<?= LOCATION_URL; ?>manage-grn.php?posting" class="btn"><i class="fa fa-list mr-2"></i>Posted List</a>
                    </div>
                    <?php
                    $keywd = '';
                    ...
                    <div class="card card-tabs grn-card-tabs" style="border-radius: 20px;">
                        ...
                        <div class="tab-content" id="custom-tabs-two-tabContent">
                            <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #dbe5ee; overflow: auto;">
                                ...
                                // $grnListObj = $grnObj->getGrnList();
                                $grnListObj = queryGet('SELECT * FROM `' . ERP_GRN . '` WHERE `companyId`=' . $company_id . ' AND `branchId`=' . $branch_id . ' AND `locationId`=' . $location_id . ' AND `iv_status`="0" '.$cond. $sts . ' ORDER BY grnId DESC limit ' . $GLOBALS['start'] . ',' . $GLOBALS['show'] . ' ', true);
                                //console($grnListObj);

                                $countShow = 'SELECT count(*) FROM `' . ERP_GRN . '` WHERE `companyId`=' . $company_id . ' AND `branchId`=' . $branch_id . ' AND `locationId`=' . $location_id . ' AND `iv_status`="0" '.$cond. $sts . ' ORDER BY grnId DESC';
                                // console($countShow);
                                ...
                                $settingsTable = getTableSettings(TBL_BRANCH_ADMIN_TABLESETTINGS, "ERP_PENDING_IV_DETAILS", $_SESSION["logedBranchAdminInfo"]["adminId"]);
                                        ...
                                        if ($grnListObj["status"] == "success") {
                                            foreach ($grnListObj["data"] as $oneGrnRow) { ?>
                                                <tr>
                                                    ...
                                                    <?php }
                                                    if (in_array(3, $settingsCheckbox)) { ?>
                                                        <td><p class="pre-normal"><?= $oneGrnRow["vendorName"] ?></p></td>
                                                    <?php }
                                                    if (in_array(4, $settingsCheckbox)) { ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form name="table-settings" method="post" action="<?php $_SERVER['PHP_SELF']; ?>" onsubmit="return table_settings();">
                ...
                <input type="hidden" name="pageTableName" value="ERP_PENDING_IV_DETAILS" />
                <div class="modal-body">
