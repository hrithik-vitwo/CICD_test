...
<form action="" method="POST" id="addNewGRNForm" enctype="multipart/form-data">
                    ...
                    <tr id="grnItemRowTr_<?= $sl ?>">
                        ...
                        <td>
                            <div class="form-input d-flex gap-2">
                                <input type="number" name="grnItemList[<?= $vendor_id ?>][<?= $sl ?>][itemReceivedQty]" value="<?= inputQuantity($itemQty) ?>" id="grnItemReceivedQtyTdInput_<?= $sl ?>" class="form-control text-xs received_quantity inputQuantityClass" required>
                        ...
                        <!-- <td class="text-right" id="grnItemInvoiceUnitPriceTdSpan_<?= $sl ?>"><?= inputValue($itemUnitPrice) ?></td> -->
                        <td>
                        ...
                        <td class="text-right" id="grnItemInvoiceBaseAmtTdSpan_<?= $sl ?>"><?= $curr_name . " : " . inputValue($itemUnitPrice * $itemQty) ?></td>
                        <td class="text-right" id="grnItemInvoiceCGSTTdSpan_<?= $sl ?>"><?= $curr_name . " : " . inputValue($cgst) ?></td>
                        <td class="text-right" id="grnItemInvoiceSGSTTdSpan_<?= $sl ?>"><?= $curr_name . " : " . inputValue($sgst) ?></td>
                        <td class="text-right" id="grnItemInvoiceIGSTTdSpan_<?= $sl ?>"><?= $curr_name . " : " . inputValue($igst) ?></td>
                        <td>
                            <div class="form-input d-flex" style="align-items: center; gap: 7px;">
                                <input type="number" name="grnItemList[<?= $vendor_id ?>][<?= $sl ?>][itemTds]" value="<?= $tds ?>" id="grnItemTdsTdInput_<?= $sl ?>" class="form-control text-xs itemTds inputAmountClass" required>
                                <p class="text-xs">%</p>
                            </div>
                        </td>
                ...
                <table>
                    <tbody>
                        <tr class="itemTotals">
                            ...
                            <td class="text-right" id="grandSubTotalTd" style="background: none;"><?= $curr_name . " : " . inputValue($totalSubtotal) ?></td>
                        </tr>
                        ...
                        ?>
                            <tr class="itemTotals">
                                ...
                                <td class="text-right" style="background: none;" id="grandCgstTd"><?= $curr_name . " : " . inputValue($totalCGST) ?></td>
                            </tr>
                            <tr class="itemTotals">
                                <td colspan="9" style="background: none;">Total SGST</td>
                                <td class="text-right" style="background: none;" id="grandSgstTd"><?= $curr_name . " : " . inputValue($totalSGST) ?></td>
                            </tr>
                        <?php
                        } else {
                        ?>
                            <tr class="itemTotals">
                                ...
                                <td class="text-right" style="background: none;" id="grandIgstTd"><?= $curr_name . " : " . inputValue($totalIGST) ?></td>
                            </tr>
                        <?php
                        }
                        ...
                        <tr class="itemTotals">
                            ...
                            <td class="text-right" id="grandTotalTd" style="background: none;"><?= $curr_name . " : " . inputValue($toalTotal) ?></td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <button type="submit" id="addNewGrnFormSubmitBtn" disabled value="Submit GRN" class="btn btn-primary float-right mt-3 mb-3">Submit SRN</button>
...
<script>
    $(document).ready(function() {
        ...
        function calculateOneItemAmounts(rowNo) {
            ...
            $(`#grnItemInvoiceTotalPriceTdSpan_${rowNo}`).html(`${curr_name}: ${inputValue(totalItemPrice)}`);
        ...
        function calculateGrandTotalAmount() {
            ...
            //     totalAmount += (helperAmount($(this).val()) > 0) ? helperAmount($(this).val()) : 0;
            // });
            ...
            $("#grandSgstTd").html(`${curr_name}: ${(ToTalsgst * currency_rate_value)}` + '<p class="text-small spanSgstGrandINR" id="spanSgstGrandINR"></p>');
