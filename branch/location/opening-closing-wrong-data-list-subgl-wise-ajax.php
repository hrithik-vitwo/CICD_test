    ...
    public function resolve($data)
    {
        ...
        foreach ($data as $subGlCode => $subGlData) {
            foreach ($subGlData as $yyyymmdd => $row) {
                ...
                $prevObj = $db->queryGet("SELECT * FROM `$this->erp_opening_closing_balance_tbl` WHERE `location_id`=$this->location_id AND `branch_id`=$this->branch_id AND `company_id`=$this->company_id AND `date`='$rowDateMonthFirstDay' AND `subgl`='$subGlCode' AND `gl`=$glId");
                if ($prevObj["numRows"] > 0) {
                    $db->queryUpdate("UPDATE `$this->erp_opening_closing_balance_tbl` SET $sqlUpdateColumns `updated_by`='$this->updated_by' WHERE `location_id`=$this->location_id AND `branch_id`=$this->branch_id AND `company_id`=$this->company_id AND `date`='$rowDateMonthFirstDay' AND `subgl`='$subGlCode' AND `gl`=$glId");
                } else {
                    $db->queryInsert("INSERT INTO `$this->erp_opening_closing_balance_tbl` SET $sqlInsertColumns `updated_by`='$this->updated_by', `created_by`='$this->created_by', `location_id`=$this->location_id, `branch_id`=$this->branch_id, `company_id`=$this->company_id, `date`='$rowDateMonthFirstDay', `subgl`='$subGlCode', `gl`=$glId");
                }
                // console([
                //     "yyyymmdd" => $yyyymmdd,
