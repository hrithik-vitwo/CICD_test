...
$show=0;
$dbObj= new Database();
$checksql=$dbObj->queryGet("SELECT isMailValid FROM ".ERP_CUSTOMER." WHERE customer_id=".$id."");
$status=$checksql["data"]["isMailValid"];
if($status=="no"){
    $show=1;
    $query=queryUpdate("UPDATE ".ERP_CUSTOMER." SET `isMailValid` = 'yes' WHERE `customer_id` = ".$id."");
}
?>
