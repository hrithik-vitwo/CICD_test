...
if (!isset($_COOKIE["cookieExceptionalSo"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieExceptionalSo", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage SO Exceptional</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    function adjustTabs() {
        ...
        var hiddenTabs = navTabs.children(":gt(" + (visibleTabs - 1) + ")");
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '',columnMapping='') {
            ...
            var checkboxSettings = Cookies.get('cookieExceptionalSo');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-so-exceptional.php",
                dataType: 'json',
                data: {
                    act: 'soapprove',
                    comid: comid,
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieExceptionalSo')
                },
                beforeSend:function(){
                    $('#loaderModal').show();
...
<script>
    $(document).on("click", ".soModal", function() {
        ...
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/so/ajax-manage-so-exceptional-modal.php",
            dataType: 'json',
            data: {
                so_no: so_no
            },
            ...
            success: function(value) {
                ...
                $(".left #amount").html(responseObj.dataObj.currency_name + " " + decimalAmount(responseObj.dataObj.totalAmount));
                ...
                if (responseObj.dataObj.igst == 0) {
                    ...
                    $("#sgstVal").html(responseObj.dataObj.currency_name + " " + parseFloat(sgst));
                } else {
