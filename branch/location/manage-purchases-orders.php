                                ...
                                <div class="card-body">
                                    ...
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active"
                                            ...
                                            <a href="purchase-to-pay-creations.php?po-creation"
...
<script>
    $(document).ready(function() {
        $(document).on("change", ".selectOperator", function() {
            ...
            let inputId;
            if (columnName.trim() === 'Delivery Date') {
                inputId = "value4_" + columnIndex;
            } else if (columnName.trim() === 'PO Date') {
                inputId = "value2_" + columnIndex;
            }
            // console.log(columnName);
            if ((columnName.trim() == 'Delivery Date' || columnName.trim() == 'PO Date') && operatorName == 'BETWEEN') {
                console.log(columnName);
...
<script>
    ...
    $(document).on("click", ".soModal", function() {
        ...
        document.getElementById('printChkbox').addEventListener('change', function() {
            if (this.checked) {
                $("#classicViewPrint").attr('href', `classic-view/invoice-preview-print.php?poId=${(poId)}&& printChkbox`);
            } else {
                $("#classicViewPrint").attr('href', `classic-view/invoice-preview-print.php?poId=${(poId)}`);
            }
        })
        $("#classicViewPrint").attr('href', `classic-view/invoice-preview-print.php?poId=${(poId)}`);
        ...
        $.ajax({
            ...
            success: function(value) {
                ...
                if (responseObj.companyCurrency != responseObj.vendorCur) {
                    ...
                    <ion-icon name="wallet-outline"></ion-icon>
                    <span class="amount-value" >${responseObj.vendorCur} ${parseFloat(responseObj.dataObj.totalAmount * responseObj.dataObj.conversion_rate).toFixed(2)}</span>
                    </p>
                ...
                $("#sub_total").html(responseObj.companyCurrency + " " + decimalAmount(responseObj.allSubTotal));
                ...
                $.each(itemsObj, function(index, val) {
                    ...
                    let td = ` <div class="row body-state-table">
                                                        ...
                                                        <div class="col-lg-2 col-md-2 col-sm-2 state-col-td">${decimalQuantity(val.remainingQty)}</div>
    ...
    $(document).on("click", ".editModal", function() {
        ...
        let url = `<?= LOCATION_URL ?>purchase-to-pay-creations.php?edit=${btoa(poId)}`;
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure?',
    ...
    $(document).on('click', '#closepo', function() {
        let poId = $(this).data('id');
        let poNo = $(this).data('po');
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure?',
            text: `You want to Close this Purchase Order ${poNo} ?`,
            showCancelButton: true,
            confirmButtonColor: '#4f32ec',
            cancelButtonColor: '#d33',
    ...
    $(document).on('click', '#rejectpo', function() {
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure?',
            text: `You want to Reject this Purchase Order ${poNo} ?`,
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    $(document).on('click', '#approvepo', function() {
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure?',
            text: `You want to Approve this Purchase Order ${poNo} ?`,
            showCancelButton: true,
