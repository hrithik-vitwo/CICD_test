...
if (isset($_GET['customer-so-creation'])) { ?>
    <div class="content-wrapper is-proforma-invoice">
        <!-- Modal -->
        ...
        <section class="content">
            <div class="container-fluid">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="manage-party-orders.php" class="text-dark"><i class="fas fa-home po-list-icon"></i> Home</a></li>
                        ...
                        <div class="p-0 pt-1 my-2">
                            <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                                <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                                    <h3 class="card-title">Manage Proforma Invoice</h3>
                                    <a href="direct-create-invoice.php?proforma_invoice" class="btn btn-sm btn-primary btnstyle m-2 float-add-btn" style="line-height: 32px;"><i class="fa fa-plus"></i></a>
                                </li>
                            </ul>
                        </div>
                        <div class="filter-list">
                            <!-- <a href="manage-party-orders.php" class="btn <?= $soActive ?>"><i class="fa fa-stream mr-2 active"></i>
                                All
                                <span class="<?= $soActive === 'active' ? 'activeNotification' : 'inactiveNotification' ?>"><?= $SOCountObj['rowCount'] ?></span>
                            </a>
                            <a href="#" class="btn <?= $soActive ?>"><i class="fa fa-stream mr-2 active"></i>
                                Orders
                                <span class="<?= $soActive === 'active' ? 'activeNotification' : 'inactiveNotification' ?>"><?= $SOCountObj['rowCount'] ?></span>
                            </a>
                            <a href="#" class="btn <?= $soActive ?>"><i class="fa fa-stream mr-2 active"></i>
                                        ...
                                        <div class="col-lg-11 col-md-11 col-sm-12">
                                            <div class="row table-header-item">
                                                ...
                                                <div class="col-lg-1 col-md-1 col-sm-1">
                                                    <a href="direct-create-invoice.php?proforma_invoice" class="btn btn-primary relative-add-btn"><i class="fa fa-plus"></i></a>
                                                </div>
                                            </div>
                            ...
                            <div class="tab-content p-0" id="custom-tabs-two-tabContent">
                                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab">
                                    ...
                                    if (isset($_REQUEST['keyword2']) && $_REQUEST['keyword2'] != '') {
                                        $cond .= " AND `invoice_no` like '%" . $_REQUEST['keyword2'] . "%' OR `invoice_date` like '%" . $_REQUEST['keyword2'] . "%'";
                                    } else {
                                        if (isset($_REQUEST['keyword']) && $_REQUEST['keyword'] != '') {
                                            $cond .= " AND `invoice_no` like '%" . $_REQUEST['keyword'] . "%'  OR `invoice_date` like '%" . $_REQUEST['keyword'] . "%'";
                                        }
                                    }

                                    $sql_list = "SELECT * FROM `" . ERP_PROFORMA_INVOICES . "` WHERE 1 " . $cond . "  AND company_id='" . $company_id . "'  AND branch_id='" . $branch_id . "'  AND location_id='" . $location_id . "' " . $sts . "  ORDER BY proforma_invoice_id desc limit " . $GLOBALS['start'] . "," . $GLOBALS['show'] . " ";
                                    $qry_list = mysqli_query($dbCon, $sql_list);
                                    $num_list = mysqli_num_rows($qry_list);

                                    $countShow = "SELECT count(*) FROM `" . ERP_PROFORMA_INVOICES . "` WHERE 1 " . $cond . "  AND company_id='" . $company_id . "'  AND branch_id='" . $branch_id . "'  AND location_id='" . $location_id . "' " . $sts . " ";
                                    $countQry = mysqli_query($dbCon, $countShow);
                                    $rowCount = mysqli_fetch_array($countQry);

                                    $ins = "SELECT * FROM `" . ERP_PROFORMA_INVOICES . "` WHERE status='active' ORDER BY so_id DESC";
                                    $soListing = queryGet($ins);

                                    $count = $rowCount[0];
                                    $cnt = $GLOBALS['start'] + 1;
                                    $settingsTable = getTableSettings(TBL_BRANCH_ADMIN_TABLESETTINGS, "ERP_PROFORMA_INVOICES", $_SESSION["logedBranchAdminInfo"]["adminId"]);
                                            ...
                                            <tbody class="tableBody">
                                                ...
                                                ?>
                                                    <tr class="tableOneRow">
                                                		...
                                                		?>
                                                        <td>
                                                            <a style="cursor:pointer" data-toggle="modal" data-target="#fluidModalRightSuccessDemo_<?= $oneSoList['proforma_invoice_id'] ?>" class="btn btn-sm"><i class="fa fa-eye po-list-icon"></i></a>

                                                            <div class="modal fade right customer-modal classic-view-modal" id="fluidModalRightSuccessDemo_<?= $oneSoList['proforma_invoice_id'] ?>" role="dialog" aria-labelledby="myModalLabel" data-backdrop="true" style="display: none;" aria-hidden="true">
                                                                    ...
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-header">
                                                                                ...
                                                                                <div class="icon-user-text">
                                                                                    ...
                                                                                    <div class="head-section">
                                                                                        ...
                                                                                        <div class="so_number-item">
                                                                                            ...
                                                                                            <p class="text-sm text-white"><?= $customerName ?></p>

                                                                                            <p class="heading lead text-xs"><?= $oneSoList['invoice_no'] ?></p>
                                                                                    ...
                                                                                    <div class="customer-head-info">
                                                                                        <div class="customer-name-code">
                                                                                            <!-- <h2 class="text-lg mb-0"><small><?= $companyCurrencyName ?></small><?= $oneSoList['all_total_amt'] ?> <span class="text-xs">(<?= number_to_words_indian_rupees($oneSoList['all_total_amt']) ?>)</span></h2> -->
                                                                                            <h2 class="text-light text-lg mb-3"><?= $companyCurrencyName ?><?= number_format($oneSoList['all_total_amt'], 2) ?>
                                                                                                <span class="text-xs"> (<?= number_to_words_indian_rupees($oneSoList['all_total_amt']) ?>)</span>
                                                                                            </h2>
                                                                                            <?php
                                                                                            if ($oneSoList['currency_name'] != $companyCurrencyName) {
                                                                                                $exchangeRate = $oneSoList['all_total_amt'] * $oneSoList['conversion_rate'];
                                                                                            ?>
                                                                            ...
                                                                            <div class="display-flex-space-between mt-2 mb-2">
                                                                                ...
                                                                                <ul class="nav nav-tabs" id="myTab" role="tablist">
                                                                                    <li class="nav-item">
                                                                                        <a class="nav-link active" id="home-tab" data-toggle="tab" href="#home<?= $oneSoList['proforma_invoice_id'] ?>" role="tab" aria-controls="home" aria-selected="true">Item Info</a>
                                                                                    </li>
                                                                                    <li class="nav-item">
                                                                                        <a class="nav-link" id="classic-view-tab" data-toggle="tab" href="#classic-view<?= $oneSoList['proforma_invoice_id'] ?>" role="tab" aria-controls="classic-view" aria-selected="false">Classic View</a>
                                                                                    </li>

                                                                                    <!-- -------------------Audit History Button Start------------------------- -->
                                                                                    <li class="nav-item">
                                                                                        <a class="nav-link auditTrail" id="history-tab<?= $oneSoList['so_id'] ?>" data-toggle="tab" data-ccode="<?= $oneSoList['proforma_invoice_id'] ?>" href="#history<?= $oneSoList['so_id'] ?>" role="tab" aria-controls="history<?= $oneSoList['so_id'] ?>" aria-selected="false"><i class="fa fa-history mr-2"></i>Trail</a>
                                                                                    </li>
                                                                                    <!-- -------------------Audit History Button End------------------------- -->
                                                                                    <li class="nav-item">
                                                                                        <a class="nav-link" href="direct-create-invoice.php?proforma_to_so=<?= base64_encode($oneSoList['proforma_invoice_id']) ?>">Create SO</a>
                                                                                    </li>
                                                                                    <li class="nav-item">
                                                                                        <a class="nav-link" href="direct-create-invoice.php?proforma_to_invoice=<?= base64_encode($oneSoList['proforma_invoice_id']) ?>">Create Invoice</a>
                                                                                    </li>
                                                                                </ul>
                                                                            </div>
                                                                        </div>
                                                                        <div class="modal-body">
                                                                            <div class="tab-content pt-0" id="myTabContent">
                                                                                <div class="tab-pane fade show active" id="home<?= $oneSoList['proforma_invoice_id'] ?>" role="tabpanel" aria-labelledby="home-tab">
                                                                                    <!-- action btn  -->
                                                                                ...
                                                                                <div class="tab-pane fade" id="profile<?= $oneSoList['proforma_invoice_id'] ?>" role="tabpanel" aria-labelledby="profile-tab">
                                                                                ...
                                                                                <div class="tab-pane fade" id="classic-view<?= $oneSoList['proforma_invoice_id'] ?>" role="tabpanel" aria-labelledby="profile-tab">
                                                                                ...
                                                                                <div class="tab-pane fade" id="history<?= $oneSoList['so_id'] ?>" role="tabpanel" aria-labelledby="history-tab">
                                                                                    ...
                                                                                    <hr>
                                                                                    <div class="audit-body-section mt-2 mb-3 auditTrailBodyContent<?= $oneSoList['proforma_invoice_id'] ?>">
