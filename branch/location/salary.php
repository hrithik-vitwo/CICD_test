...
if (!isset($_COOKIE["cookiesSalary"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiesSalary", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0"> Manage Salary </h3>
                                            </div>
                                        </div>
                                        ...
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <div class="dropdown payment-collection-dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li><a class="dropdown-item" href="payroll_actions.php?manual">Manual Payroll Form</a></li>
                                                </ul>
                                            </div>
                                                        ...
                                                        <div class="modal-header">
                                                            <div class="top-details">
                                                                <div class="left">
                                                                    ...
                                                                    <p class="info-detail doc-numbers"><ion-icon name="information-outline"></ion-icon><span id="doc-number"> </span></p>
                                                                </div>
                                                                <div class="right">
                                                                    <p class="info-detail name"><ion-icon name="business-outline"></ion-icon><span id="due_amt"></span></p>
...
<!---------------------------------Table Action Model End--------------------------------->

<div class="modal" id="actionModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-bold text-sm" id="exampleModalLongTitle">Post to Accounting</h5>
            </div>
            <div class="modal-body">
                <input type="hidden">
                <div class="row">
                    ...
                    <div class="col-6 col-lg-6 col-md-6 col-sm-12">
                        <div class="form-input">
                            <label>Amount</label>
                            <input type="number" id="dueAmt" class="form-control">
                            <span class="error" id="amountMsg"></span>
                        </div>
                    </div>
                    <div class="col-6 col-lg-6 col-md-6 col-sm-12">
                        <div class="form-input">
            ...
            <div class="modal-footer">
                <button class="btn btn-primary postAccountingBtn" type="button">Post</button>
            </div>
        </div>
    </div>
</div>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '') {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    $("#detailed_tbody").html(`<td colspan=16 class='else-td loading-td text-center'><img src="<?= BASE_URL ?>public/assets/gif/loading-data.gif" width="150" alt=""><p>Data Loading ....</p></td>`);
                },
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal"  data-id="${value.process_id}" data-code="${value.process_id}" data-toggle="modal" data-target="#viewGlobalModal">${ value.doc_no}</a>`,
                                value.month_year,
                                value.amount,
                                value.due_amount,
                                `<a href="#" class="soModal"  data-id="${value.process_id}" data-code="${value.process_id}" data-toggle="modal" data-target="#viewGlobalModal">${decimalAmount(value.proceed_amount)}</a>`,
                                value.remaining_days,
                                value.status,
                                ` <a href="#" data-toggle="modal" id="processId" data-id=${value.process_id} data-target="#actionModal">
                        ...
                        var checkboxSettings = Cookies.get('cookiesSalary');
        ...
        $(document).on("click", ".ion-paginationlistnew", function(e) {
            ...
            var filteredColumnMapping = columnMapping.filter(function(col) {
                return col.slag !== 'acconting_status';
            });
            ...
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiesSalary')
                },
                beforeSend: function() {
...
<script>
    $(document).on("click", ".postAccountingBtn", function() {
        let originalDueAmt = dueAmount;
        let amount = $('#dueAmt').val();
        let bank_id = $('#bankId').val();
        let posting_date = $('#postingDate').val();
        let process_id = $('#processId').data('id');

        if (amount > originalDueAmt || !amount) {
            let warningMsg = `<p class="error">Amount should be less than or equal to due amount </p>`;
            $('#amountMsg').html(warningMsg);
        }
        if (amount && bank_id && posting_date) {
            $.ajax({
                type: 'POST',
...
<script>
    $(document).on("click", ".soModal", function() {
        let processedId = $(this).data('id');
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/fa/ajax-payroll-processing-modal.php",
            dataType: 'json',
            data: {
                act: "salary",
                processedId
            ...
            success: function(res) {
                ...
                $('.left #amount').html("INR " + res.payrollsql_list.amount)
                $('.left #doc-number').html(formatDate(res.payrollsql_list.posting_date))
                $('.right #due_amt').html(res.payrollsql_list.doc_no)
                $.each(res.data, function(index, val) {
                    let td = ` <tr>
                                                                ...
                                                                <td>${val.status}</td>
                                                                <td><ion-icon name="checkmark-outline" class="status-checkmark"></ion-icon></td>
                                                              </tr>
                                                            `;
