<?php
$productionOrderId = isset($_GET["run-mrp"]) ? base64_decode($_GET["run-mrp"]) : 0;
$workCenterListObj = queryGet("SELECT * FROM `erp_work_center` WHERE `company_id` = $company_id", true);
include_once("../../common/footer2.php.php");
?>
                    ...
                    <div class="d-flex ml-auto">
                        <div class="btn-group btn-group-toggle col-2 pr-0" data-toggle="buttons">
                            <label class="btn btn-secondary active waves-effect waves-light">
                                <input type="radio" class="expand_collapse" id="collapser" name="expand_collapse" value="collapse" autocomplete="off" checked> Collapse
                            </label>
                            <label class="btn btn-secondary waves-effect waves-light">
                                <input type="radio" class="expand_collapse" id="expander" name="expand_collapse" value="expand" autocomplete="off"> Expand
                            </label>
...
<script>
    $(document).ready(function() {
        ...
        $('#expander').on('change', function() {
            if ($(this).prop('checked')) {
                console.log('Expanding Tree'); 
                $('#basic').simpleTreeTable('expandAll');
            }
        });

        $('#collapser').on('change', function() {
            if ($(this).prop('checked')) {
                console.log('Collapsing Tree');
                // Collapse all rows
                $('#basic').simpleTreeTable('collapseAll');
            }
        });
        ...
        $.ajax({
            url: '<?= BASE_URL ?>branch/location/bom/ajax/get-mrp-preview.php?production-order-id=<?= $productionOrderId ?>',
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                ...
                $("#treeTableBody").html(`
                    <tr data-node-id="0_0">
                        ...
                        <td class="text-right">-</td>
                        <td class="text-right">${decimalQuantity(productionOrderDetails.remainQty)}</td>
        ...
        function renderProduceableItemTableRow(item, index, isSplited = false) {
            ...
            return `
                    <tr>
                        ...
                        <td class="${cellColorClass} text-right">${decimalQuantity(item.availableQty)}</td>
                        <td class="${cellColorClass} text-right">${decimalQuantity(item.requiredQty)}</td>
                        <td class="${cellColorClass} text-right">${item.uom}</td>
                        <td class="${cellColorClass}">
                            <input type="number" step="any" name="consumeableItems[${index}][${subIndex}][prodQty]" class="form-control inputProdQty" data-item_index="${index}" data-item_id="${itemId}" data-parent_id="${item.parentId}" data-required_qty="${decimalQuantity(item.requiredQty)}" value="${decimalQuantity(prodQty)}" required>
        ...
        function renderPurchasableItemTableRow(item, index) {
            return `<tr>
                        ...
                        <td class="text-right">${decimalQuantity(item.availableQty)}</td>
                        <td class="text-right">${decimalQuantity(item.totalConsumptionQty)}</td>
                        <td class="text-right">${decimalQuantity(item.requiredQty)}
                        <input type="hidden" id="itemQty_${index}" value="${decimalQuantity(item.requiredQty)}">
                        </td>
                        <td>${item.uom}</td>
                                                    ...
                                                    <div class="row">
                                                        <div class=" col-lg-5 col-md-5 col-sm-5 col-12">
                                                            <div class="form-input">
                                                                ...
                                                                <input type="date"
                                                                    name="purchasableItems[${index}][deliverySchedule][${index}][multiDeliveryDate]"
                                                        ...
                                                        <div class="col-lg-5 col-md-5 col-sm-5 col-12">
                                                            <div class="form-input">
                                                                ...
                                                                <input type="text" name="purchasableItems[${index}][deliverySchedule][${index}][quantity]"
                                                                    class="form-control multiQuantity multiQty_${index}" data-attr="${index}"
                                                                    id="multiQuantity_${index}" placeholder="quantity" value="${item.requiredQty}">
                                                            </div>
                                                        </div>
            ...
            nodes.forEach(function(node, index) {
                ...
                html += `
                    <tr data-node-id="${nodeId}" ${ parentNodeId ? `data-node-pid="${parentNodeId}"` : ""}>
                        ...
                        <td><span class="pre-normal">-</span></td>
                        <td class="text-right">${node.consumptionRate}</td>
                        <td class="text-right">${decimalQuantity(node.consumptionRate*parentQty)}</td>
    ...
    function addDeliveryQty(randCode) {
        ...
        $(`.modal-add-row-delivery_${randCode}`).append(`
                                        ...
                                        <div class="form-input">
                                            ...
                                            <input type="date" name="purchasableItems[${randCode}][deliverySchedule][${addressRandNo}][multiDeliveryDate]" class="form-control" id="delivery-date" placeholder="delivery date" value="">
                                        </div>
                                    </div>
                                    <div class="col-lg-5 col-md-5 col-sm-5 col-12">
                                        <div class="form-input">
                                            <label>Quantity</label>
                                            <input type="text" data-attr="${randCode}" name="purchasableItems[${randCode}][deliverySchedule][${addressRandNo}][quantity]" class="form-control multiQuantity multiQty_${randCode}" id="multiQuantity_${addressRandNo}" placeholder="quantity" value="">
                                        </div>
                                    </div>
                                    <div class="col-lg-2 col-md-2 col-sm-2 col-12">
