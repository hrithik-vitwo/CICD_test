<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="<?= BASE_URL ?>branch/location/bom/bom.php" class="text-dark"><i class="fa fa-list po-list-icon"></i>BOM</a></li>
...
<div class="row m-0 p-0 messages justify-content-end">
    ...
    $bomDetailObj = $bomControllerObj->getBomDetails($copyFromItemId);
    ...
    if (isset($_POST["createBomSubmitBtn"])) {
        ...
        if ($createObj["status"] == "success") {
            swalAlert($createObj["status"], ucfirst($createObj["status"]), $createObj["message"], LOCATION_URL . "bom/bom.php");
        } else {
            ...
            <form action="" method="post">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <table>
                            <tbody>
                                <th>Cost Center<small class="bg-secondary px-1 rounded"><i class="fa fa-info" data-toggle="tooltip" data-placement="top" title="Cost Center Name"></i></small></th>
                                <th> Code <small class="bg-secondary px-1 rounded"><i class="fa fa-info" data-toggle="tooltip" data-placement="top" title="Cost center code"></i></small></th>
                                ...
                                ?>
                                    <tr id="bomHdDivRow_<?= $rowNo ?>">
                                            ...
                                            <div>
                                                <select name="bomHd[<?= $rowNo ?>][CostCenterId]" id="bomHdCostCenterId_<?= $rowNo ?>" class="form-control bomHdCostCenterDropDown" required>
                                                    ...
                                                    foreach (getGoodActivities()["data"] as $key => $itemObj) {
                                                        $isSelectedTxt = $hdrowData["cost_center_id"] == $itemObj["CostCenter_id"] ? "selected" : "";

                                                        echo '<option value="' . $itemObj["CostCenter_id"] . '" ' . $isSelectedTxt . ' data-row="' . base64_encode(json_encode($itemObj, true)) . '"   >' . $itemObj["CostCenter_code"] . ' - ' . $itemObj["CostCenter_desc"] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                                        ...
                                        <td class="p-1" style="width: 5%;">
                                            <input type="text" name="bomHd[<?= $rowNo ?>][CostCenterCode]" id="bomHdCostCenterCode_<?= $rowNo ?>" placeholder="Item Code" class="form-control m-0 bomHdCostCenterCode" value="<?= $hdrowData['head_code'] ?>" readonly>
                                        </td>
                                    ...
                                    <tr id="bomOtherHeadDivRow_<?= $rowNo ?>">
                                        <td class="p-1" style="width: 15%;">
                                            <select name="bomOtherHead[<?= $rowNo ?>][CostCenterId]" id="bomOtherHeadCostCenterId_<?= $rowNo ?>" class="form-control bomOtherHeadCostCenterDropDown" required>
                                                ...
                                                <?php
                                                foreach (getGoodActivities()["data"] as $key => $itemObj) {
                                                    $isSelectedTxt = $hdOtherRowData["cost_center_id"] == $itemObj["CostCenter_id"] ? "selected" : "";
                                                    echo '<option value="' . $itemObj["CostCenter_id"] . '" ' . $isSelectedTxt . '  data-row="' . base64_encode(json_encode($itemObj, true)) . '"  >' . $itemObj["CostCenter_code"] . ' - ' . $itemObj["CostCenter_desc"] . '</option>';
                                                }
                                                ?>
                                            </select>
                                        ...
                                        <td class="p-1" style="width: 5%;">
                                            <input type="text" name="bomOtherHead[<?= $rowNo ?>][CostCenterCode]" id="bomOtherHeadCostCenterCode_<?= $rowNo ?>" placeholder="Item Code" class="form-control m-0 bomOtherHeadCostCenterCode" value="<?= $hdOtherRowData['CostCenter_code'] ?>" readonly>
                                        </td>
