...
class BomController
{
    ...
    public function createBom($data)
    {
        ...
        $bomObj = $dbObj->queryInsert('INSERT INTO `erp_bom` SET `companyId`=' . $this->company_id . ',`branchId`=' . $this->branch_id . ',`locationId`=' . $this->location_id . ',`itemId`=' . $data["itemId"] . ',`itemType`="goods",`preparedBy`="' . $data["preparedBy"] . '",`preparedDate`="' . $data["preparedDate"] . '",`cogm`=' . $cogm . ', `cogm_m`='.$cogm_m.', `cogm_a`='.$cogm_a.', `cogs`=' . $cogs . ',`msp`=' . $msp . ', `bomProgressStatus`="' . $bomProgressStatus . '", `createdBy`="' . $this->created_by . '", `updatedBy`="' . $this->updated_by . '"');
