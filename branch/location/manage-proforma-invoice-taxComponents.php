...
if (!isset($_COOKIE["cookiesProformaInvoice"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiesProformaInvoice", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage Proforma Invoice</h3>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <a href="proformaInvoice.php" class="btn btn-create waves-effect waves-light" type="button">
                ...
                <div class="modal-body">
                    <nav>
                        <div class="nav nav-tabs global-view-navTabs" id="nav-tab" role="tablist">
                            ...
                            <button class="nav-link classicview-btn classicview-link classicView" id="nav-classicview-tab" data-id="" data-bs-toggle="tab" data-bs-target="#nav-classicview" type="button" role="tab" aria-controls="nav-classicview" aria-selected="true"><ion-icon name="apps-outline"></ion-icon>Print Preview</button>
                            <button class="nav-link auditTrail" id="nav-trail-tab" data-bs-toggle="tab" data-bs-target="#nav-trail" type="button" role="tab" aria-controls="nav-trail" aria-selected="false"><ion-icon name="time-outline"></ion-icon>Trail</button>
                        </div>
                    </nav>
                    <div class="tab-content global-tab-content" id="nav-tabContent">
                        ...
                        <div class="tab-pane fade transactional-data-tabpane show active" id="nav-overview" role="tabpanel" aria-labelledby="nav-overview-tab">
                            <div class="d-flex nav-overview-tabs">
                                <a href="#" class="btn btn-primary" id="soLink"><ion-icon name="add-outline"></ion-icon>Create SO</a>
                                <a href="#" class="btn btn-primary" id="inVoiceLink"><ion-icon name="add-outline"></ion-icon>Create Invoice</a>
                                            ...
                                            <div class="address-contact">
                                                <div class="address-customer">
                                                    <div class="details">
                                                        ...
                                                        <p id="custBillAdd" class="pre-normal"></p>
                                                    </div>
                                                    <div class="details">
                                    ...
                                    <div class="items-table">
                                        ...
                                        <div class="multiple-item-table">
                                            <div class="row head-state-table">
                                                ...
                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-th state-col-td text-right" id="taxName2" style="display: none;"><span id="GstName2"></span></div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        const fill_datatable = (formDatas = '', pageNo = '', limit = '', columnMapping = '') => {
            var fdate = "<?php echo $f_date; ?>";
            var to_date = "<?php echo $to_date; ?>";
            var comid = <?php echo $company_id; ?>;
            var locId = <?php echo $location_id; ?>;
            var bId = <?php echo $branch_id; ?>;
            var checkboxSettings = Cookies.get('cookiesProformaInvoice');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-proforma-invoice.php",
                dataType: 'json',
                data: {
                    act: 'proformaIv',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            if (value.status === 'active') {
                                ...
                                status = `<p class="status-bg status-closed">${value.status}</p>`
                            }
                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal"  data-id="${value.proforma_invoice_id}" data-toggle="modal" data-target="#viewGlobalModal">${value.invoice_no}</a>`,
                                value.trade_name ,
                                value.all_total_amt,
                                value.created_at,
                                value.created_by,
                                ...
                                ` <div class="dropout">
                                    ...
                                    <li>
                                        <button class="deleteProformaBtn" data-invno=${value.invoice_no} data-toggle="modal" data-target="#viewModal"><ion-icon name="trash-outline" class="ion-delete"></ion-icon>Delete</button>
                                    </li>
                                    <li>
                                    <button class="soModal" data-toggle="modal" data-target="#viewGlobalModal" data-id=${value.proforma_invoice_id}><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiesProformaInvoice')
                },
                beforeSend:function(){
                    $('#loaderModal').show();
        ...
        $(document).ready(function() {
            ...
            $(document).on("click", "#serach_submit", function(event) {
                ...
                $(".selectOperator").each(function() {
                    ...
                    if ((columnSlag === 'created_at') && operatorName == "BETWEEN") {
    ...
    $(document).ready(function() {
        $(document).on("change", ".selectOperator", function() {
            ...
            if ((columnName === 'Created At') && operatorName === 'BETWEEN') {
    ...
    $(document).on("click", ".soModal", function() {
        ...
        let proforma_invoice_id = $(this).data('id');
        console.log(proforma_invoice_id);
        $("#classicViewPrint").attr('href', `classic-view/invoice-preview-print-taxcomponents.php?proformaInv=${btoa(proforma_invoice_id)}`);


        $.ajax({
            type: "GET",
            url: "ajaxs/modals/so/ajax-manage-proforma-invoice-modal-taxComponents.php",
            dataType: 'json',
            data: {
                act: "modalData",
                proforma_invoice_id
            ...
            success: function(value) {
                ...
                if(components.compliance_invoice==true){
                    ...
                    $("#comInvType").html(` : ` + dataObj.compInvoiceType);
                }



                $(".left #amount").html(responseObj.companyCurrencyName + " " + decimalAmount(dataObj.all_total_amt));
                ...
                $("#postingDate").html(` : ` + formatDate(dataObj.invoice_date));
                $("#postingTime").html(` : ` + dataObj.invoice_time);
                $("#deliveryDate").html(` : ` + dataObj.delivery_date);
                ...
                // $("#comInvType").html(` : ` + dataObj.compInvoiceType);
                $(".auditTrail").attr('data-ccode', responseObj.ccode);
                ...
                if (dataObj.status == "active") {
                    $("#inVoiceLink").show();
                    $("#soLink").show();
                    $("#inVoiceLink").attr('href', `invoice-creation.php?proforma_to_invoice=${responseObj.bashProformaId}`);
                    $("#soLink").attr('href', `sales_order_creation.php?proforma_to_so=${responseObj.bashProformaId}`);
                ...
                if (responseObj.exchange.currencyName) {
                    let obj = ` <p class="info-detail amount"><ion-icon name="wallet-outline"></ion-icon><span>${responseObj.exchange.currencyName}</span>${responseObj.exchange.exchangeNum}</p>
                            <p class="info-detail po-number">(${responseObj.exchange.exchangeWord})</p>`;
                ...
                let totalTax = responseObj.dataObj.total_tax_amt;
                ...
                if (countrycode == 103) {
                    if (responseObj.dataObj.igst == 0) {
                        ...
                        $("#igstP").hide();
                        $("#igst").hide();
                        $("#cgstVal").html(responseObj.companyCurrencyName + " " + decimalAmount(cgst));
                        $("#sgstVal").html(responseObj.companyCurrencyName + " " + decimalAmount(sgst));
                    } else {
                        $("#csgst").hide();
                        $("#csgstVal").hide();
                        $("#igstP").show();
                        ...
                        $("#igst").html(responseObj.companyCurrencyName + " " + decimalAmount(igst));
                    }
                } else {
                    $.each(taxComponents, function(index, component) {
                        ...
                        $("#compval").html(responseObj.companyCurrencyName + " " + decimalAmount(component.taxAmount));
                    });
                }

                // card details section

                $("#cardSoNo").html(responseObj.dataObj.invoice_no);
                $("#cardCustPo").html(responseObj.dataObj.customer_po_no);
                $("#totalItem").html(responseObj.dataObj.totalItems + " " + "Items");
                $("#sub_total").html(responseObj.companyCurrencyName + " " + decimalAmount(subTotal));
                $("#totalDis").html(responseObj.companyCurrencyName + " " + decimalAmount(disCount));
                $("#taxableAmt").html(responseObj.companyCurrencyName + " " + decimalAmount(taxableAmt));
                $("#total_amount").html(responseObj.companyCurrencyName + " " + decimalAmount(responseObj.dataObj.all_total_amt));
                $("#remark").html(responseObj.dataObj.declaration_note);
                ...
                $.each(itemsObj, function(index, val) {
                    if (countrycode != 103) {
                        ...
                        $("#GstName2").html(responseObj.gstName + " Amount (" + responseObj.companyCurrencyName + ")");

                    }

                    let tableData = ` <div class="row body-state-table">
                                                                ...
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td">${responseObj.companyCurrencyName}</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${responseObj.companyCurrencyName} ${decimalAmount(val.unitPrice)}</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${responseObj.companyCurrencyName} ${decimalAmount(val.subTotal)}</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${responseObj.companyCurrencyName} ${decimalAmount(val.total_discount)}</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${responseObj.companyCurrencyName} ${decimalAmount(val.taxAbleAmount)}</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${decimalQuantity(val.tax)}%</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${responseObj.companyCurrencyName} ${decimalAmount(val.gstAmount)}</div>
                                                                <div class="col-lg-2 col-md-2 col-sm-2 state-col-td text-right">${responseObj.companyCurrencyName} ${decimalAmount(val.itemTotalAmount)}</div>
                                                            </div>
                                                            `;

                    $("#currencyHead").html(responseObj.companyCurrencyName)
    ...
    // <!-- // so delete script -->
    $(document).on('click', '.deleteProformaBtn', function() {
        var proformaInvNo = $(this).data('invno');
        if (!confirm(`Are you sure to close Invoice No #${proformaInvNo}?`)) {
            return false;
        }
        $.ajax({
            type: "GET",
            ...
            success: function(response) {
                ...
                Swal.fire({
                    icon: data.status,
                    title: ` #${proformaInvNo} deleted successfully!`,
