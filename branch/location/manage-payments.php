...
if (!isset($_COOKIE["cookieManagePayments"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieManagePayments", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Advance Payment</h3>
                                            </div>
                                        </div>
                                                        ...
                                                        <div class="filter-action filter-mobile-search mobile-page">
                                                            ...
                                                            <div class="exportgroup mobile-page mobile-export">
                                                                ...
                                                                <ul class="export-options">
                                                                    <li>
                                                                        <button>
                                                                            <ion-icon name="list-outline" class="ion-fulllistPayments md hydrated" id="exportAllBtn" role="img" aria-label="list outline"></ion-icon>Export
                                                                        </button>
                                                                    </li>
                                                                    <li>
                                                                        <button>
                                                                            <ion-icon name="list-outline" class="ion-paginationlistPayments md hydrated" role="img" aria-label="list outline"></ion-icon>Download
                                                                        </button>
                                                                    </li>
                                                                </ul>
                                                            </div>
                                    ...
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <div class="exportgroup">
                                                ...
                                                <ul class="export-options">
                                                    <li>
                                                        <button class="ion-paginationlistPayments">
                                                            <ion-icon name="list-outline" class="ion-paginationlistPayments md hydrated" role="img" aria-label="list outline"></ion-icon>Export
                                                        </button>
                                                    </li>
                                                    <li>

                                                        <button class="ion-fulllistPayments">
                                                            <ion-icon name="list-outline" class="ion-fulllistPayments md hydrated" role="img" aria-label="list outline"></ion-icon>Download
                                                        </button>
                                                    </li>
                                                </ul>
                                            </div>
                                            <div class="dropdown payment-collection-dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li><a href="advancePaymnetSettlement.php?vendor-payments" class="btn btn-transparent"><ion-icon name="paper-plane-outline"></ion-icon>Make Payment</a></li>
                                                    <hr>
                                                    <li><a href="advancePaymnetSettlement.php?adjust-payment" class="btn btn-transparent"><ion-icon name="shield-checkmark-outline"></ion-icon>Adjust Payment</a></li>
                                                </ul>
                                            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookieManagePayments');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-payment.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    formDatas: formDatas,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            let status;
                            let revCollectPay = '';
                            if (value['pay.status'] === 'active') {
                                status = '<p class="status-bg status-open">Paid</p>';
                                revCollectPay = `
                                    <li>
                                        <button class="reversePayment" data-id="${value.payment_id}" data-no="${value['pay.transactionId']}" ><ion-icon name="repeat-outline"></ion-icon>Reverse</button>
                                    </li>`;
                            } else if (value['pay.status'] === "reverse") {
                                status = '<p class="status-bg status-pending">Reverse</p>'
                            }

                            dataTable.row.add([
                                value.sl_no,
                                `<a href="#" class="soModal"  data-id="${value.payment_id}" data-toggle="modal" data-target="#viewGlobalModal">${ value['pay.transactionId']}</a>`,
                                value.party_code,
                                `<p class="pre-normal">${value.party_name}</p>`,
                                (value['pay.postingDate']),
                                value['bank.bank_name'],
                                (value['pay.collect_payment']),
                                status,
                                ` <div class="dropout">
                                    <button class="more">
                                         <span></span>
                                    ...
                                    <ul>
                                    <li>
                                        <button class="soModal" data-toggle="modal" data-id=${value.payment_id}  data-target="#viewGlobalModal"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                    </li>
                                    ${revCollectPay}
                                    </ul>
                                </div>`
        ...
        $(document).on("click", ".ion-fulllistPayments", function(e) {
            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-payment.php",
                dataType: "json",
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieManagePayments')
                },
                beforeSend:function(){
                    // console.log(sql_data_checkbox);
                    $('#loaderModal').show();
                    $('.ion-fulllistPayments').prop('disabled', true)
                },
                ...
                complete: function() {
                    ...
                    $('#loaderModal').hide();
                    $('.ion-fulllistPayments').prop('disabled', false)
                }
            })

        });

        $(document).on("click", ".ion-paginationlistPayments", function(e) {
            $.ajax({
                type: "POST",
                ...
                beforeSend:function(){
                    // console.log(sql_data_checkbox);
                    $('#loaderModal').show();
                    $('.ion-paginationlistPayments').prop('disabled', true)
                },
                ...
                complete: function() {
                    // Hide loader modal after request completes
                    $('#loaderModal').hide();
                    $('.ion-paginationlistPayments').prop('disabled', false)
                }
            })
...
<script>
    $(document).on('click', '.soModal', function() {
        ...
        $("#classicViewPrint").attr('href', `classic-view/invoice-preview-print.php?pay_id=${pay_id}`);
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/mm/ajax-manage-payments-classicview.php",
            data: {
                pay_id,
            },
            beforeSend: function() {
...
<script>
    $(document).on('click', '.reversePayment', function(e) {

        e.preventDefault(); // Prevent default click behavior
        var dep_keys =atob ($(this).data('id'));
        let no =$(this).data('no');
        ...
        Swal.fire({
            icon: 'warning',
            title: 'Are you sure?',
            text: `You want to reverse this ${no}?`,
