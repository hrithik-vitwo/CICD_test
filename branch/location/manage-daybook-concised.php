...
if (!isset($_COOKIE["cookieDaybookTransacConcised"])) {
  ...
  if ($settingsCheckbox_concised_view) {
    setcookie("cookieDaybookTransacConcised", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
  } else {
    for ($i = 0; $i < 5; $i++) {
                  ...
                  <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space"
                    ...
                    <div class="right-block">
                      ...
                      <div class="page-list-filer filter-list">
                        <a href="manage-daybook-concised.php" class="filter-link active"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Concised View
                        </a>
                        <a href="manage-daybook.php" class=""><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Detailed View
                        </a>
                      </div>
                                ...
                                <div class="modal-body">
                                  ...
                                  <div id="main2">
                                    ...
                                    <table class="colomnTable">
                                      <?php
                                      $cookieTableStockReport = json_decode($_COOKIE["cookieDaybookTransacConcised"], true) ?? [];
                                ...
                                <table>
                                  <tbody>
                                    ...
                                    foreach ($columnMapping as $columnIndex => $column) {
                                      if ($columnIndex == 0 || $columnIndex == 1 || $columnIndex == 2 || $columnIndex === 6 || $columnIndex === 19 || $columnIndex === 20 || $columnIndex === 21 || $columnIndex === 22 || $columnIndex === 23) {
                                        continue;
                                      } ?>
                                      <tr>
                                        <td>
...
<script>
  $(document).ready(function() {
    ...
    function fill_datatable(formDatas = '', pageNo = '', limit = '') {
      ...
      var checkboxSettings = Cookies.get('cookieDaybookTransacConcised');
      ...
      $.ajax({
        type: "POST",
        url: "ajaxs/daybook/ajax-manage-daybook-transactional-concised.php",
        dataType: 'json',
        data: {
          act: 'detailedTransactionalConcised',
          comid: comid,
          locId: locId,
          bId: bId,
          formDatas: formDatas,
          ...
          if (response.status) {
            ...
            $.each(responseObj, function(index, value) {
              ...
              dataTable.row.add([
                ...
                value['parent_slug'], // journal.journal_entry_ref
    ...
    $(document).on("click", ".ion-paginationlistnew", function(e) {
      $.ajax({
        ...
        data: {
          ...
          sql_data_checkbox: Cookies.get('cookieDaybookTransacConcised'),
          from_date : currentStartDate,
          to_date : currentEndDate,
        },
        beforeSend: function() {
    ...
    $(document).on("click", "#check-box-submt", function(event) {
      ...
      if (settingsCheckbox.length < 5) {
        ...
        $.ajax({
          ...
          dataType: 'json',
          data: {
            act: 'transactionalDaybookConcised',
            fromData: fromData
          },
          success: function(response) {
            console.log(response);
