...
if (!isset($_COOKIE["cookieItemOrder"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieItemOrder", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
...
<div class="content-wrapper report-wrapper is-stock-new is-sales-order vitwo-alpha-global">
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage Item Order List</h3>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <div class="modal manage-column-setting-modal" data-backdrop="static" id="myModal1">
...
<script>
    $(document).ready(function() {
        $('.filter-link[data-name="itemOrderList"]').addClass('active');
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '',columnMapping='') {
            ...
            var checkboxSettings = Cookies.get('cookieItemOrder');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-manage-so-item-wise.php",
                dataType: 'json',
                data: {
                    act: 'soapprove',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            dataTable.row.add([
                                value.sl_no,
                                value["so.so_number"],
                                formatDate(value["so.so_date"]),
                                formatDate(value["so.delivery_date"]),
                                value["cust.customer_code"],
                                value["cust.trade_name"],
                                value["items.itemCode"],
                               `<p class='pre-normal'>${value["items.itemName"]}</p>`,
                                (value["items.qty"]),
                                value["uoms.uomName"],
                                value["items.tax"],
                                (value["items.totalDiscount"]),
        ...
        $(document).on("click", ".ion-paginationliststock", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookieItemOrder')
                },
                beforeSend:function(){
                    $('#loaderModal').show();
        ...
        $(document).ready(function() {
            $(document).on("click", "#serach_submit", function(event) {
                ...
                $(".selectOperator").each(function() {
                    ...
                    if (columnSlag === 'so.so_date') {
                        values = value2;
                    } else if (columnSlag === 'so.delivery_date') {
                        values = value3;
                    }

                    if ((columnSlag === 'so.so_date' || columnSlag === 'so.delivery_date') && operatorName == "BETWEEN") {
