...
if ($_POST['act'] == 'failedAcc') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($type == 'active') {
            $sql_list = "SELECT * FROM erp_grn WHERE 1 " . $cond . " AND (grnPostingJournalId = 0 OR grnPostingJournalId IS NULL)   AND companyId =" . $company_id . " AND branchId=" . $branch_id . " AND locationId=" . $location_id . " AND `grnStatus` IN('active') ORDER BY grnId DESC";
        }else{
            $sql_list = "SELECT * FROM erp_grn WHERE 1 " . $cond . " AND (grnPostingJournalId != 0 AND grnPostingJournalId IS  NOT NULL) AND (reverse_grnPostingJournalId = 0 OR reverse_grnPostingJournalId IS NULL)   AND companyId =" . $company_id . " AND branchId=" . $branch_id . " AND locationId=" . $location_id . "   AND `grnStatus` IN('reverse') AND grnCreatedAt > '2025-05-01' ORDER BY grnId DESC";
        }
