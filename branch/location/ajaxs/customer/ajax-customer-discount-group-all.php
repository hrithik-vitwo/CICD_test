...
if ($_POST['act'] == 'discountGroupTable') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_list = "SELECT * FROM `erp_customer_discount_group` WHERE 1 " . $cond . "  AND company_id='" . $company_id . "' AND location_id = '" . $location_id . "' ORDER BY customer_discount_group_id desc";
        ...
        if ($num_list > 0) {
            foreach ($sql_data as $data) {
                $dynamic_data[] = [
                    ...
                    "customer_discount_group_id" => $data['customer_discount_group_id'] ?? "-"
                ];
                $sl++;
            }
            $output .= "</table>";
