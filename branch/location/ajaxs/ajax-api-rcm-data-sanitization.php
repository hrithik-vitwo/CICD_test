...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ($_GET['act'] == "rcm") {
        $returnObj = [];
        $sql = "SELECT * FROM erp_grninvoice AS grniv WHERE grniv.companyId=$company_id AND grniv.rcm_enabled=1 AND grniv.grnStatus='active'  ORDER BY grniv.grnIvId DESC;";
    ...
    if ($_POST['act'] == "updateRoundOff") {
        $returnObj = [];
        $id = $_POST['id'];
        $sql = "SELECT * FROM erp_grninvoice as inv WHERE inv.companyId=$company_id AND inv.grnIvId=$id AND inv.rcm_enabled=1";
        $res = $dbObj->queryGet($sql, true);
        if ($res['numRows'] == 1) {
            $data = $res['data'][0];
            ...
            if ($leftSide+$roundoff!=$total) {
                $newTotalAmount = $grnTotalAmount + $roundoff;
                ...
                $returnObj = ["status" => "error", "msg" => "can not update!", "sql" => $sql];
            }
        } else if ($res['numRows'] == 0) {
            $returnObj = ["status" => "warning", "msg" => "No data found!", "sql" => $sql];
        } else {
