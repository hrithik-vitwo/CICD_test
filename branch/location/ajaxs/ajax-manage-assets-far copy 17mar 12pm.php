...
(
    ...
    WHERE
        LOG.companyId = $company_id 
        AND items.goodsType = 9
        AND LOG.refActivityName <> 'Put to Use'  -- Moved filter inside this subquery
...
ORDER BY sort_order DESC, equip_sort_order DESC";
        ...
        if ($num_list > 0) {
            foreach ($sql_data as $data) {
                ...
                $accumulated=queryGet("SELECT 
    COALESCE(SUM(depreciated_value), 0) AS total_depreciated_value
FROM erp_asset_depreciation
WHERE asset_id = $itemId AND asset_use_id = $assetused_id AND `company_id`=$company_id AND `branch_id`=$branch_id AND location_id=$location_id");
                $total_accu=$accumulated['data']['total_depreciated_value'];
