...
if ($_POST['act'] == 'failedCreditNote') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($type == 'active') {
            $sql_list = "SELECT * FROM `erp_credit_note` WHERE 1 " . $cond . "  AND`branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . " AND `journal_id`=0  AND `status` IN ('active') ORDER BY cr_note_id desc";
        } else {
            $sql_list = "SELECT * FROM `erp_credit_note` WHERE 1 " . $cond . "  AND`branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . " AND `journal_id`!=0 AND `reverse_journal_id`IS NULL AND `status` IN ('reverse') AND  `created_at` > '2025-05-27' ORDER BY cr_note_id desc";
        }
        ...
        if ($num_list > 0) {
            foreach ($sql_data as $data) {
                ...
                $dynamic_data[] = [
                    ...
                    "cr_note_id_action" => base64_encode($data['cr_note_id']),
                    "credit_note_no" => $data['credit_note_no'],
                    "debtor_type" => $data['creditors_type'],
                    "ref" => $ref,
