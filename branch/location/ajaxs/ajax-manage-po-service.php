...
if ($_POST['act'] == 'poService') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sts = " AND `so`.status !='deleted'";
             ...
             $sql_list = "SELECT po.*, vendor.trade_name FROM erp_branch_purchase_order AS po LEFT JOIN erp_vendor_details AS vendor ON po.vendor_id = vendor.vendor_id WHERE 1 AND  po.service_po='yes'  AND po.branch_id=$branch_id AND po.location_id=$location_id AND po.company_id=$company_id ORDER BY po_id desc";
        ...
        if ($num_list > 0) {
            foreach ($sql_data as $data) {
                ...
                $dynamic_data[] = [
                    ...
                    "service_type"=>$data['service_type']??"Direct Service PO",
                    "service_gst"=>$data['service_gst'],
