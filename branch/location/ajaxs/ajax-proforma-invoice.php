...
if ($_POST['act'] == 'proformaIv') {
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_list = "SELECT so.*, cust.trade_name AS customer_name FROM `erp_proforma_invoices`AS so LEFT JOIN erp_customer AS cust ON so.customer_id =cust.customer_id  WHERE 1   " . $cond . "  AND so.company_id='" . $company_id . "'   AND so.branch_id='" . $branch_id . "'  AND so.location_id='" . $location_id . "' " . $sts . "   ORDER BY so.proforma_invoice_id desc ";
    ...
    if ($num_list > 0) {
      foreach ($sql_data as $data) {
        ...
        $dynamic_data[] = [
           ...
           "all_total_amt"=>decimalValuePreview($data['all_total_amt']),
