...
if ($_POST['act'] == 'goodsService') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_list = "SELECT items.itemCode, items.itemName, items.status, UOM.uomName, groups.goodGroupName, types.goodTypeName, summary.movingWeightedPrice, summary.priceType, summary.itemPrice, CONCAT(tds.section, tds.natureOfTransaction) AS TDS, items.hsnCode FROM erp_inventory_items AS items LEFT JOIN erp_inventory_mstr_uom AS UOM ON items.baseUnitMeasure = UOM.uomId LEFT JOIN erp_inventory_mstr_good_groups AS groups ON items.goodsGroup = groups.goodGroupId LEFT JOIN erp_inventory_mstr_good_types AS types ON items.goodsType = types.goodTypeId LEFT JOIN erp_inventory_stocks_summary AS summary ON items.itemId = summary.itemId LEFT JOIN erp_tds_details AS tds ON items.tds = tds.id WHERE (items.goodsType=5 OR items.goodsType=7 OR items.goodsType=10) AND summary.company_id=$company_id AND summary.branch_id=$branch_id AND summary.location_id=$location_id ORDER BY items.itemId DESC";
