...
if ($_POST['act'] == 'failedproductiondeclaration') {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($type == 'active') {
            $sql_list = "SELECT * FROM `erp_production_declarations` WHERE 1 " . $cond . " AND `branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=$company_id AND (`prod_declaration_journal_id` IS NULL OR `fgsfg_declaration_journal_id` IS NULL) AND `status` IN ('active') ORDER BY id DESC";
        } else {
             $sql_list = "SELECT * FROM `erp_production_declarations` WHERE 1 " . $cond . "  AND`branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . " AND (`prod_declaration_journal_id`  IS NOT NULL AND `fgsfg_declaration_journal_id`  IS NOT NULL) AND (reverse_prod_declaration_journal_id IS NULL OR reverse_fgsfg_declaration_journal_id IS NULL) AND `status` IN ('reverse')  AND `created_at` >='2025-05-22'  ORDER BY id desc";
        }
        ...
        if ($num_list > 0) {
            foreach ($sql_data as $data) {
                ...
                // $debitor_type = $data['creditors_type'];
                ...
                $dynamic_data[] = [
                    "sl_no" => $sl,
                    "id" => $data['id'],
                    "id_action" => base64_encode($data['id']),
                    "code" => $data['code'],
                    "prod_id" => $data['prod_id'],
                    "prod_code" => $data['prod_code'],
                    "sub_prod_id" => $data['sub_prod_id'],
                    "sub_prod_code" => $data['sub_prod_code'],
