...
if (isset($_GET['auditTrailBodyContent'])) {
    ...
    if (isset($_POST['ccode']) && !empty($_POST['ccode'])) {
        ...
        $auditQuery = "SELECT * FROM `" . ERP_AUDIT_TRAIL . "` WHERE `party_type`='vendor' AND `document_number` = '" . $ccode . "' OR party_id=".$_POST['id']." AND `company_id`=$company_id AND `branch_id`=$branch_id AND `location_id`=$location_id AND `status`='active' GROUP BY `action_code`  ORDER BY `id` DESC";
...
?>
            <ol class="timeline">
                ...
                ?>
                    <li class="timeline-item mb-0 bg-transparent auditTrailBodyContentLineVendor" type="button" data-toggle="modal" data-id="<?= $trailData['id']; ?>" data-ccode="<?= $trailData['document_number']; ?>" data-target="#innerModal">
