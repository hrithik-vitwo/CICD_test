...
if (isset($_GET['auditTrailBodyContent'])) {
    ...
    if (isset($_POST['ccode']) && !empty($_POST['ccode'])) {
        ...
        $cond = '';
        if ($cid != NULL) {
            $cond = 'AND `document_id` = ' . $cid;
        }
        $auditQuery = "SET sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY',''))";
        $resultaudit = queryGet($auditQuery);
        $auditQuery = "SELECT * FROM `" . ERP_AUDIT_TRAIL . "` WHERE `document_number` = '" . $ccode . "' " . $cond . " AND `company_id`=$company_id AND `branch_id`=$branch_id AND `location_id`=$location_id AND `status`='active' GROUP BY `action_code`  ORDER BY `id` DESC";
                ...
                ?>
                    <li class="timeline-item mb-0 bg-transparent auditTrailBodyContentLine" type="button" data-toggle="modal" data-id="<?= $trailData['id']; ?>" data-ccode="<?= $ccode; ?>" data-target="#innerModal">
                        ...
                        <div class="new-comment font-bold">
                            <?php
                            $created_by = getCreatedByUser($trailData['created_by']);
                            if ($created_by == 'NULL' || $created_by == '-') {
                                $created_by = $trailData['created_by'] ?? '-';
                            } else {
                                $created_by = getCreatedByUser($trailData['created_by']);
                            }
                            ?>
        ...
        if ($currentResultaudit['status'] == 'success') {
            ...
            $html .= '<div class="modal-body p-0"> 
                        <div class="free-space-bg">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" id="concise-tab" data-toggle="tab" href="#consize" role="tab" aria-controls="concise" aria-selected="true"><i class="fa fa-th-large mr-2" aria-hidden="true"></i>Edit History</a>
                            </li>
            ...
            if (!$changes) {
                $html .= '<div class="dotted-box">No history found</div>';
            } else {
