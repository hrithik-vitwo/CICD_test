...
if (isset($_GET['auditTrailBodyContent'])) {
    ...
    if (isset($_POST['ccode']) && !empty($_POST['ccode'])) {
        ...
        $auditQuery = "SELECT * FROM `" . ERP_AUDIT_TRAIL . "` WHERE `party_type`='customer' AND `document_number` = '" . $ccode . "' OR party_id=".$_POST['id']." AND `company_id`=$company_id AND `branch_id`=$branch_id AND `location_id`=$location_id AND `status`='active' GROUP BY `action_code`  ORDER BY `id` DESC";
...
?>
            <ol class="timeline">
                ...
                ?>
                    <li class="timeline-item mb-0 bg-transparent auditTrailBodyContentLineCustomer" type="button" data-toggle="modal" data-id="<?= $trailData['id']; ?>" data-ccode="<?= $trailData['document_number']; ?>" data-target="#innerModal">
    ...
    if (isset($_POST['ccode']) && !empty($_POST['ccode']) && isset($_POST['id']) && !empty($_POST['id'])) {
        $ccode=str_replace('-','/',$_POST['ccode']);
        $currentAuditQuery = "SELECT * FROM `" . ERP_AUDIT_TRAIL . "` WHERE `id`=" . $_POST['id'] . " AND `company_id`=$company_id AND `branch_id`=$branch_id AND `location_id`=$location_id AND `status`='active'";
        $currentResultaudit = queryGet($currentAuditQuery);
        $previousAuditQuery = "SELECT * FROM `" . ERP_AUDIT_TRAIL . "` WHERE id < (SELECT `id` FROM `" . ERP_AUDIT_TRAIL . "` WHERE `id`=" . $_POST['id'] . " AND `document_number` = '" . $ccode . "' AND `company_id`=$company_id AND `branch_id`=$branch_id AND `location_id`=$location_id  AND `status`='active' LIMIT 1) AND `document_number` = '" . $ccode . "' AND (`trail_type`='ADD' OR `trail_type`='EDIT') AND `company_id`=$company_id AND `branch_id`=$branch_id AND `location_id`=$location_id AND `status`='active' ORDER BY id DESC LIMIT 1";
