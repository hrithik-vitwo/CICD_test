            ...
            $prev_day = date($from_date, strtotime("-1 day"));
               ...
               // console($opening_query);
                  ...
                  //   ORDER BY temp_table.postingDate asc");
                   ...
                   $transaction_first_sql = queryGet("SELECT * FROM (SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,debit.debit_amount AS amount,0 AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_debit AS debit ON journal.id=debit.journal_id WHERE journal.parent_slug='Payment' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND journal.party_code=$party_code 
                   UNION ALL
                   SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,0 AS amount,credit.credit_amount AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='grniv' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND journal.party_code=$party_code 
                   UNION ALL
                   SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,0 AS amount,credit.credit_amount AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='srniv' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND journal.party_code=$party_code 
                   UNION ALL
                   SELECT journal.journalEntryReference AS type,journal.remark AS remark,journal.postingDate,debit.debit_amount AS amount,0 AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_debit AS debit ON journal.id=debit.journal_id WHERE journal.parent_slug='journal' AND journal.journalEntryReference='Payment' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND debit.subGlCode=$party_code 
                   UNION ALL
                   SELECT 'grniv' AS type,journal.remark AS remark,journal.postingDate,0 AS amount,credit.credit_amount AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='journal' AND journal.journalEntryReference='Purchase' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND credit.subGlCode=$party_code
                   UNION ALL
                   SELECT 'srniv' AS type,journal.remark AS remark,journal.postingDate,0 AS amount,credit.credit_amount AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='journal' AND journal.journalEntryReference='Purchase' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND credit.subGlCode=$party_code 
                   UNION ALL
                     SELECT parent_slug AS type, journal.remark AS remark, cn.postingDate, cn.total AS debit, 0 AS credit FROM erp_credit_note AS cn LEFT JOIN erp_acc_journal AS journal ON cn.journal_id = journal.id WHERE journal.parent_slug='VendorCN' AND cn.company_id = 1 AND cn.branch_id = 1 AND cn.location_id = 1 AND cn.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND cn.status = 'active' AND journal.party_code = $party_code 
                   UNION ALL
                   SELECT parent_slug AS type, journal.remark AS remark, dn.postingDate, 0 AS debit, dn.total AS credit FROM erp_debit_note AS dn LEFT JOIN erp_acc_journal AS journal ON dn.journal_id = journal.id WHERE journal.parent_slug='VendorCN' AND dn.company_id = 1 AND dn.branch_id = 1 AND dn.location_id = 1 AND dn.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND dn.status = 'active' AND journal.party_code = $party_code) AS subquery ORDER BY postingDate;");
                   //console($transaction_first_sql);
...
//  $balance = 0.00;
$sum_sql = queryGet("SELECT (SUM(amount) - SUM(payment)) AS transactions FROM (SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,debit.debit_amount AS amount,0 AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_debit AS debit ON journal.id=debit.journal_id WHERE journal.parent_slug='Payment' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND journal.party_code=$party_code
UNION ALL
SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,0 AS amount,credit.credit_amount AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='grniv' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND journal.party_code=$party_code
UNION ALL
SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,0 AS amount,credit.credit_amount AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='srniv' AND journal.company_id=1 AND journal.branch_id=1 AND journal.location_id=1 AND journal.postingDate BETWEEN '".$from_date."' AND '".$to_date."' AND journal.journal_status='active' AND journal.party_code=$party_code 
 ...
 $html .= '<div class="row state-table">
  <div class="col-12 mobile-flex">
    ...
    <div class="row body-state-table">
      <div class="col-lg-2 col-md-2 col-sm-2 state-col-td">'.$to_date.'</div>
