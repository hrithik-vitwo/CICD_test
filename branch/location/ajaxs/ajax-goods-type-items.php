            ...
            foreach ($sql_data as $data) {
                ...
                if ($data['bomStatus'] == 1) {
                    ...
                    if ($goodsBomController->isBomCreated($data['itemId'])) {
                        ...
                        $bom_status .= '<span class="bom-status bom-required">Created</span>';
                    } else {

                        $bom_status .=  '<span class="bom-status bom-notrequired">Not Created</span>';
                    }
                } else {
                ...
                if ($data['status'] == "active") {
                    $formObj .= '<p class="status-bg status-new-active ">' . ucfirst($data['status']) . '</p>';
                } elseif ($data['status'] == "inactive" || $data['status'] == "guest") {
                    $formObj .= '<p class="status-bg status-new-danger ">' . ucfirst($data['status']) . '</p>';
                } elseif ($data['status'] == "draft") {
                    $formObj .= '<p class="status-bg status-new-warning ">' . ucfirst($data['status']) . '</p>';
                }
