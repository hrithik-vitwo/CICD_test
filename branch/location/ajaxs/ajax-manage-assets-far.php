...
if ($_POST['act'] == 'tdata') {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        foreach ($formObj as $slag => $data) {
            ...
            if (in_array($slag, ['grn_postingDate', 'po_date', 'use_date', 'posting_date'])) {
                if ($data['operatorName'] === 'BETWEEN') {
                    $condition = "DATE($slag) BETWEEN '" . $data['value']['fromDate'] . "' AND '" . $data['value']['toDate'] . "'";
                } else {
                    $condition = "DATE($slag) " . $data['operatorName'] . " '" . $data['value'] . "'";
                }
            } elseif (in_array($slag, ['total_value', 'dep_rate', 'asset_life', 'grnSubTotal', 'depreciation_value', 'depreciation_on_value', 'total_accu'])) {
                $condition = "$slag " . $data['operatorName'] . " " . $data['value'];
            } else {
                $condition = "$slag " . $data['operatorName'] . " '%" . $data['value'] . "%'";
            }
        ...
        if (!empty($havingConditions)) {
            $cond = "HAVING " . implode(" AND ", $havingConditions);
        }
...
LEFT JOIN (
    SELECT logRef, SUM(itemQty) AS total_qty
    ...
    GROUP BY logRef
) AS total_log ON LOG.logRef = total_log.logRef
LEFT JOIN (
    SELECT d1.asset_id, 
           d1.asset_use_id, 
           d1.method, 
