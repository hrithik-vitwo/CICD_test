...
if ($_POST['act'] == 'soapprove') {
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_list = "SELECT so.*, cust.customer_code,cust.trade_name AS customer_name,  stat.label  FROM `" . ERP_BRANCH_SALES_ORDER . "` AS so LEFT JOIN erp_customer AS cust ON so.customer_id = cust.customer_id  LEFT JOIN `erp_status_master` AS stat
                  ON so.approvalStatus = stat.status_id WHERE 1 " . $cond . "  AND approvalStatus=12 AND so.company_id='" . $company_id . "'  AND so.branch_id='" . $branch_id . "'   AND so.location_id='" . $location_id . "' " . $sts . "  
    ...
    if ($num_list > 0) {
      foreach ($sql_data as $data) {
        ...
        if (fetchStatusMasterByCode($data['approvalStatus'])['data']['label'] == "exceptional") {
