...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    LEFT JOIN erp_company_functionalities as func ON so.profit_center = func.functionalities_id WHERE 1 " . $cond . " AND approvalStatus=14  AND so.company_id='" . $company_id . "'  AND so.branch_id='" . $branch_id . "'   AND so.location_id='" . $location_id . "'  AND so.status !='deleted'  ORDER BY so.so_id DESC;";
    ...
    if ($num_list > 0) {
        ...
        foreach ($itemDetails[$data['so_id']] as $oneItem) {
            ...
            foreach ($deliverySchedule as $dSchedule) {
                ...
                $deliveryOBj[] = [
                    'delqnty' => decimalQuantityPreview($dSchedule['qty']),
        ...
        // $classic_view = '<div class="classic-inner">'. $templateSalesOrderControllerObj->printSalesOrder($data['so_id']) .' </div>';
