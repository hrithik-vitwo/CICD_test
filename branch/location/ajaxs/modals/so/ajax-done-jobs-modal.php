...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  $sql_list = "SELECT so.*, cust.trade_name, cust.customer_code, cust.customer_gstin,cust.customer_pan,cust.customer_currency, cust.customer_status, custInvoiceLog.placeOfSupply, custAddress.customer_address_building_no, custAddress.customer_address_flat_no, custAddress.customer_address_street_name, custAddress.customer_address_pin_code, custAddress.customer_address_location, custAddress.customer_address_district, custAddress.customer_address_state, kam.kamName,func.functionalities_name , custInvoiceLog.complianceInvoiceType,cust.customer_authorised_person_email,
                       ...
                       AND  goodsType='project' AND approvalStatus='9' AND jobOrderApprovalStatus=14 AND so.status !='deleted'  ORDER BY so.so_id DESC;";
  ...
  if ($num_list > 0) {
    ...
    foreach ($itemDetails[$data['so_id']] as $oneItem) {
      ...
      $boqDetailObj = $boqControllerObj->getBoqDetails($oneItem['inventory_item_id'])["data"];
      $deliveryOBj = [];
      ...
      if ($oneItem['invStatus'] == "done") { 
        $text .= "<span class=\"font-weight-bold my-1 shadow-sm text-uppercase border border-success text-success px-2\"><i class=\"fas fa-check-circle\"></i> " . $oneItem['invStatus'] . "</span>";
    } elseif ($oneItem['invStatus'] == "pending") {
        $text .= " <span class=\"font-weight-bold my-1 shadow-sm text-uppercase border border-warning text-muted px-2\"><i class=\"far fa-clock\"></i> " . $oneItem['invStatus'] . "</span>";
    }
    ...
    if ($data['approvalStatus'] == 9 || $data['approvalStatus'] == 11) {
     if($data['goodsType'] == 'project') {
        $navBtn .= '<a title="Create Invoice" href="direct-create-invoice.php?joborder_to_invoice=' . base64_encode($data['so_id']) . '" class="btn btn-primary  disabled-link"><i class="fa fa-plus mr-2"></i>Create Invoice</a>';
      } else {
