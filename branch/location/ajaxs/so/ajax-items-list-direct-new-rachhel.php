...
if ($_GET['act'] === "listItem") {
    ...
    if ($type === "?quotation" || $type === "?proforma_to_invoice" || $type === '?quotation_to_so') {
        $itemMRP = $items["unitPrice"];
    }
    ...
    $vClass = $getItemSummaryObj["priceType"];
    
    if(strtolower($vClass)=='v'){
        $itemTargetPrice = $getItemSummaryObj["movingWeightedPrice"];
    }else{
        $itemTargetPrice = $getItemSummaryObj["itemPrice"];
    }

    // console(["itemTargetPrice"=> $itemTargetPrice]);
    // console($BranchSoObj->fetchItemSummaryDetails($itemId));
    // console($getItemObj);
...
?>
    ...
    <tr class="rowDel itemRow" id="delItemRowBtn_<?= $getItemObj['data']['itemId'] ?>_<?= $randCode ?>" data-id="<?= $getItemObj['data']['goodsType'] ?>">
        ...
        <?php if ($type === "?pos-invoice") { ?>
            ...
            <td class="inp-td" style="min-width: 40px !important;">
                <div class="d-flex">
                    ...
                    <input type="hidden" name="listItem[<?= $randCode ?>][unitPrice]" class="inp-design full-width-center originalItemUnitPriceInp" id="originalItemUnitPriceInp_<?= $randCode ?>" value="<?php echo  inputValue($itemMRP); ?>">
                    <!-- this value should be inserted in DB 👇🏾 -->
                    <input type="text" name="listItem[<?= $randCode ?>][unitPrice]" class="inp-design text-right full-width-center originalChangeItemUnitPriceInp inputAmountClass" id="originalChangeItemUnitPriceInp_<?= $randCode ?>" value="<?php echo inputValue($itemMRP); ?>">
                </div>
                <div style="font-size: 0.8em;" class="float-right font-weight-bold text-primary convertedDiv" id="convertedDiv_<?= $randCode ?>">
                    <strong>(<span class="rupee-symbol currency-symbol-dynamic"><?= $currencyName ?> </span> <span class="convertedItemUnitPriceSpan" id="convertedItemUnitPriceSpan_<?= $randCode ?>"><?= decimalValuePreview($itemMRP) ?></span>)</strong>
                </div>
            </td>
        <?php } ?>
        <!-- ************************* end pos invoice -->
        ...
        <?php if ($type === "?sales_order_creation" || $type === "?quotation_creation" || $type === "?quotation_to_so"  || $type === "?party_order_to_so" || $type === "?party_order_to_quotation" || $type === "?proforma_invoice" || $type === "?edit_so" || $type === "?proforma_to_so") { ?>
            <td class="text-center">
                ...
                <?php } else {
                    ...
                    // echo $sumOfBatches = inputQuantity($qtyObj['sumOfBatches']);
                    $qtyObj = $BranchSoObj->itemQtyStockCheck($getItemObj['data']['itemId'], "'rmWhOpen', 'fgWhOpen'", "DESC");
                    echo $sumOfBatches = inputQuantity($qtyObj['sumOfBatches']);
                }
                ?>
        ...
        <?php } else if ($type == '?pgi_to_invoice') { ?>
            <td class="text-center">
                ...
                $qtyObj = $BranchSoObj->itemQtyStockCheckWithAcc($getItemObj['data']['itemId'], "'fgMktOpen'", "DESC", "", $invoicedate);
                echo $sumOfBatches = $qtyObj['sumOfBatches'] . ' ';
                // console($qtyObj);
            ...
            <td>
                <div class="d-flex">
                    <?php if ($type === '?pgi_to_invoice') {
                        ...
                        $qtyObj = $BranchSoObj->itemQtyStockCheckWithAcc($getItemObj['data']['itemId'], "'fgMktOpen'", "DESC", $pgiCode, $invoicedate);
                    ...
                    <?php } else { ?>
                        ...
                        $qtyObj1 = $BranchSoObj->itemQtyStockCheckWithAcc($getItemObj['data']['itemId'], "'rmWhOpen', 'fgWhOpen'", "DESC", '', $invoicedate);

                        $sumOfBatches = $qtyObj['sumOfBatches'];
                        $sumOfBatches1 = $qtyObj1['sumOfBatches'];
                        $batchesDetails = $BranchSoObj->convertToWHSLBatchArray($qtyObj['data']);
                        // console($qtyObj);
                        ?>
                        <input type="hidden" name="listItem[<?= $randCode ?>][stockQty]" class="form-control checkQty" id="checkQty_<?= $randCode ?>" value="<?= $sumOfBatches1; ?>">

                        <!-- Button to Open the Modal -->
                        <div class="qty-modal py-2">
                            <p class="font-bold text-center checkQtySpan" id="checkQtySpan_<?= $randCode ?>"><?= inputQuantity($sumOfBatches1); ?></p>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-header" style="background: #003060; color: #fff;">
                                        ...
                                        <p class="text-xs my-2 ml-5">Total Picked Qty :
                                            <span class="font-bold itemSelectTotalQty" id="itemSelectTotalQty_<?= $randCode ?>"><?= decimalQuantityPreview(0) ?></span>
                                        </p>
                                                                                            ...
                                                                                            ?>
                                                                                                <div class="storage-location mb-2">
                                                                                                    ...
                                                                                                    <div class="input">
                                                                                                        ...
                                                                                                        <?php } else { ?>
                                                                                                            <input step="any" <?= $disbaledstatus ?> type="number" name="listItem[<?= $randCode ?>][batchselection][<?= $batch['logRef'] . '_' . $locationKey . '_' . $whKey . $batchKey; ?>]" data-maxval="<?= $batch['itemQty'] ?>" data-rdcode="<?= $randCode . '|' . $batch['logRef']; ?>" class="<?= $placeholderClass ?> form-control ml-auto enterQty inputQuantityClass batchqty<?= $batch['logRef']; ?> qty<?= $randCode; ?>" id="enterQty_<?= $batch['logRef']; ?>" placeholder="<?= $placeholderText ?>" disabled>
                                                                                                        <?php } ?>
                                                                                                    </div>
                                                                                                </div>
                                                                                                <hr>
                        ...
                        <input class="form-control" type="hidden" name="listItem[<?= $randCode ?>][sumOfBatches]" value="<?= $sumOfBatches1 ?>">
                    <?php } ?>
                </div>
            </td>
        <?php } ?>
        <?php if ($type === "?so_to_invoice" || $type === "?quotation" || $type === "?quotation_to_so" || $type === "?party_order_to_so" || $type === "?party_order_to_quotation" || $type === "?pgi_to_invoice" || $type === "?edit_so" || $type === "?proforma_to_so" || $type === "?proforma_to_invoice") { ?>
            <td class="inp-td">
                <input step="any" type="number" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity($itemQty) ?>" class="inp-design full-width inputQuantityClass itemInvQty" id="itemInvQty_<?= $randCode ?>" readonly>
            ...
            <div class="d-flex">
                <?php if ($goodsType == 5) { ?>
                    <?php if ($type == "?so_to_invoice" || $type == "?quotation_to_so" || $type == "?quotation" || $type == "?party_order_to_so" || $type == "?party_order_to_quotation" || $type == "?pgi_to_invoice"  || $type === "?edit_so" || $type === "?proforma_to_so" || $type === "?proforma_to_invoice") { ?>
                        <input type="number" step="any" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity(0) ?>" class="inp-design full-width itemQty inputQuantityClass" id="itemQty_<?= $randCode ?>">
                        <?php
                    ...
                    <?php } else { ?>
                        <?php if ($type == "?joborder_to_invoice") { ?>
                            <input type="number" step="any" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity($invoiceQty) ?>" class="inp-design full-width itemQty inputQuantityClass" id="itemQty_<?= $randCode ?>" readonly>
                        <?php } else { ?>
                            <input type="number" step="any" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity($itemQty) ?>" class="inp-design full-width itemQty inputQuantityClass" id="itemQty_<?= $randCode ?>" data-itemtype="<?php echo  $goodsType ?>">
                        <?php } ?>
                        <?php
                        echo $uomName = getUomDetail($getItemObj['data']['service_unit'])['data']['uomName'];
                        //  $ItemsObj->getBaseUnitMeasureById($getItemObj['data']['service_unit'])['data']['uomName'];
        ...
        <td class="inp-td">
            <div class="d-flex">
                <input type="number" step="any" name="listItem[<?= $randCode ?>][itemTargetPrice]" value="<?= inputValue($itemTargetPrice) ?>" class="inp-design full-width itemTargetPrice" id="itemTargetPrice_<?= $randCode ?>" readonly>
            </div>
        </td>
        <!-- end mrp -->
        <!-- rate -->
        <td class="inp-td" style="min-width: 40px !important;">
            ...
            <span style="display:none; font-size: .8em!important" class="text-danger rateMsg" id="rateMsg_<?= $randCode ?>">Rate cannot be greater than MRP</span>
        </td>
        ...
        <td class="text-right">
            ...
            <div style="border-bottom: 1px solid #ccc; font-size: 0.8em;">
                <span class="itemTradeDiscountPercentageSpan" id="itemTradeDiscountPercentageSpan_<?= $randCode ?>"><?= inputQuantity($tradeDiscountPercentage) ?></span> %
            </div>
            <div>
        ...
        <td class="text-right">
            ...
            <div style="border-bottom: 1px solid #ccc; font-size: 0.8em;">
                <span class="itemCashDiscountPercentageSpan" id="itemCashDiscountPercentageSpan_<?= $randCode ?>"><?=inputQuantity(0)?></span> %
            </div>
            ...
            ?>
                <div style="border-top: 1px solid #ccc; font-size: 0.8em;" class="float-right font-weight-bold text-primary" id="cashDiscountWarning_<?= $randCode ?>">
                    <strong>Previously, a cash discount of <?= decimalValuePreview($cashDiscountPercentage) ?>% was applied and amount was <?= decimalValuePreview($cashDiscountAmount) ?></strong>
                </div>
            <?php
            }
            ?>
        ...
        <td>
            <input class="form-control itemTaxBkup" id="itemTaxBkup_<?= $randCode ?>" type="hidden" name="listItem[<?= $randCode ?>][taxBkup]" value="<?= inputQuantity($itemTaxPercentageBkup) ?>">
                            ...
                            <div class="tab-content delivery-discount-tabContent" id="nav-tabContent">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <input type="number" step="any" name="listItem[<?= $randCode ?>][totalDiscount]" value="<?= $totalDiscount ?>" class="form-control itemDiscount inputQuantityClass" id="itemDiscount_<?= $randCode ?>" readonly>
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <input type="number" step="any" value="<?= $itemTotalDiscount ?>" name="listItem[<?= $randCode ?>][itemTotalDiscount1]" class="form-control itemTotalDiscount1 inputAmountClass" id="itemTotalDiscount1_<?= $randCode ?>" readonly>
                                ...
                                <div class="tab-pane fade" id="nav-cash-discount-<?= $randCode ?>" role="tabpanel" aria-labelledby="nav-cash-discount-tab">
                                    <div class="row mb-3 border-bottom">
                                        ...
                                        <?php
                                        if ($cashDiscountType != "") { ?>
                                            <p class="pre-normal text-muted mt-1">In previous document the Cash discount was selected based on <b><i><?= ucfirst($cashDiscountType) ?></i></b> and the percentage was <b><i><?= $cashDiscountPercentage ?>%</i></b> and amount was <b><i> <?= decimalValuePreview($cashDiscountAmount) ?></i></b></p>
                                        <?php }
                                        ?>
                                    </div>
                                    <div class="row">
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                <span class="rupee-symbol symbol pr-1"><?= '%' ?> </span>
                                                <input type="number" step="any" name="listItem[<?= $randCode ?>][totalCashDiscount]" value="<?= $totalCashDiscount ?>" class="form-control itemCashDiscount inputQuantityClass" id="itemCashDiscount_<?= $randCode ?>">
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                <span class="rupee-symbol currency-symbol pr-1"><?= $currencyName ?> </span>
                                                <input type="hidden" value="<?= inputValue($itemTotalCashDiscount) ?>" name="listItem[<?= $randCode ?>][itemTotalCashDiscount]" class="form-control itemTotalCashDiscountHidden" id="itemTotalCashDiscountHidden_<?= $randCode ?>">
                                                <input type="number" step="any" value="<?= inputValue($itemTotalCashDiscount) ?>" name="listItem[<?= $randCode ?>][itemTotalCashDiscount1]" class="form-control itemTotalCashDiscount1 inputAmountClass" id="itemTotalCashDiscount1_<?= $randCode ?>">
...
$(document).ready(function () {
    $('.itemCashDiscount').on('keydown', function (e) {
