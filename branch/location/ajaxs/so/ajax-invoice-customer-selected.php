...
$joinInvoice = array();
foreach ($invoiceArray as $line) {
    $parts = explode('||', $line);
    $invoiceNo = $parts[2];
    $joinInvoice[] = '"' . $invoiceNo . '"';
}

$joinedInvoices = implode(',', $joinInvoice);

$fetchInvoiceByCustomer = $BranchSoObj->fetchBranchSoInvoiceBycustomerIdAndInvoiceNo($customerId, $joinedInvoices)['data'];
$fetchAdvanceAmt = $BranchSoObj->fetchAdvanceAmt($customerId)['data']['totalAdvanceAmt'];
if(isset($customerId)){
    $fetchInvoiceAmtDetails = $BranchSoObj->totalInvoiceAmountDetailsByCustomerSelectedInvoice($customerId, $joinedInvoices)['data'];
}else{
    $fetchInvoiceAmtDetails = $BranchSoObj->totalInvoiceAmountDetailsByCustomer($customerId)['data'];
}
