...
if ($_POST['act'] == 'getVerientExamplecopy') {
    ...
    if(isset($_POST['vid']) && !empty($_POST['vid'])){
        ...
        $iv_varient_map = queryGet("SELECT * FROM `erp_branch_func_varient_map` WHERE company_id=$company_id AND branch_id=$branch_id AND location_id=$location_id AND functional_area_id=".$_POST['functionalArea']." AND status=1");
    ...
    $responseData=getInvoiceNumberByVerientView($iv_varient_id);

}elseif($_POST['act'] == 'getInvoiceNumberByVerient') {   

    
// $responseData=getInvoiceNumberByVerient($_POST['vid']);
