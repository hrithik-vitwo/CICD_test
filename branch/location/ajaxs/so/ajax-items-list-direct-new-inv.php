...
if ($_GET['act'] === "listItem") {
    ...
    if ($type === '?so_to_invoice' || $type === "?quotation") {
        $tradeDiscountPercentage = $items['totalDiscount'] ?? 0;
        $cashDiscountPercentage = $items['cashDiscountPercentage'] ?? 0;
        $cashDiscountAmount = $items['cashDiscountAmount'] ?? 0;
        if ($type === "?quotation") {
            $cashDiscountPercentage = $items['cashDiscount'] ?? 0;
        }
    }else if($type === "?pgi_to_invoice"){
        $so_delivery_pgi_id=$items['so_delivery_pgi_id'];
        $inventory_item_id=$items['inventory_item_id'];
        $so_number_qry=queryGet("SELECT so_number FROM `erp_branch_sales_order_delivery_pgi` WHERE `so_delivery_pgi_id` = ".$so_delivery_pgi_id." AND `company_id` = ".$company_id." AND `branch_id` = ".$branch_id." AND `location_id` = ".$location_id." ")['data'];
        $so_number=$so_number_qry['so_number'];
        ...
        $discountqry=queryget("SELECT * FROM `erp_branch_sales_order_items` WHERE `so_id` = ".$so_id." AND `inventory_item_id` = ".$inventory_item_id."")['data'];
        $cashDiscountPercentage=$discountqry['cashDiscountPercentage'];
        $cashDiscountAmount=$discountqry['cashDiscountAmount'];
        $cashDiscountType=$discountqry['cashDiscountType'];
        // console($discountqry);
        if($goodsType==5){
            $discountVariantId = $items['discountVariantId'] ?? 0;
            $cashDiscountType = $items['cashDiscountType'] ?? "";
    ...
    $itemUnitPrice = 0;
    if ($type === "?quotation" || $type === "?joborder_to_invoice" || $type === "?so_to_invoice" || $type === "?pgi_to_invoice"  ||  $type === "?proforma_to_invoice") {
...
?>
    <tr class="rowDel itemRow" id="delItemRowBtn_<?= $getItemObj['data']['itemId'] ?>_<?= $randCode ?>" data-id="<?= $getItemObj['data']['goodsType'] ?>">
        ...
        <?php if ($type === "?pos-invoice") { ?>
            ...
            <td class="inp-td" style="min-width: 40px !important;">
                <div class="d-flex">
                    ...
                    <input type="text" name="listItem[<?= $randCode ?>][unitPrice]" class="inp-design text-right full-width-center inputAmountClass originalChangeItemUnitPriceInp" id="originalChangeItemUnitPriceInp_<?= $randCode ?>" value="<?php echo  inputValue($itemMRP); ?>">
                </div>
        ...
        <?php if ($type === "?quotation" || $type === "?proforma_to_invoice") { ?>
                                                                                        ...
                                                                                        <div class="card-body bg-light mx-3">
                                                                                            ...
                                                                                            ?>
                                                                                                <div class="storage-location mb-2">
                                                                                                    ...
                                                                                                    <div class="input">
                                                                                                        <?php if ($batch['itemQty'] > 0) { ?>
                                                                                                            <input type="number" step="any" name="listItem[<?= $randCode ?>][batchselection][<?= $batch['logRef'] . '_' . $locationKey . '_' . $whKey . $batchKey; ?>]" data-maxval="<?= $batch['itemQty'] ?>" data-rdcode="<?= $randCode . '|' . $batch['logRef']; ?>" class="form-control inputQuantityClass ml-auto enterQty batchqty<?= $batch['logRef']; ?> qty<?= $randCode; ?>" id="enterQty_<?= $batch['logRef']; ?>">
                                                                                                        <?php } else { ?>
                                                                                                            <input type="number" step="any" name="listItem[<?= $randCode ?>][batchselection][<?= $batch['logRef'] . '_' . $locationKey . '_' . $whKey . $batchKey; ?>]" data-maxval="<?= $batch['itemQty'] ?>" data-rdcode="<?= $randCode . '|' . $batch['logRef']; ?>" class="form-control inputQuantityClass ml-auto enterQty batchqty<?= $batch['logRef']; ?> qty<?= $randCode; ?>" id="enterQty_<?= $batch['logRef']; ?>" disabled>
                                                                                                        <?php } ?>
                                                                                                    </div>
                                                                                                </div>
                                                                                                <hr>
        ...
        <?php } ?>
        <?php if ($type === "?so_to_invoice" || $type === "?quotation"  || $type === "?pgi_to_invoice" || $type === "?proforma_to_invoice") { ?>
            <td class="inp-td">
                <input step="any" type="number" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity($itemQty) ?>" class="inp-design full-width itemInvQty inputQuantityClass" id="itemInvQty_<?= $randCode ?>" readonly>
        ...
        <td class="inp-td">
            ...
            <div class="d-flex">
                <?php if ($goodsType == 5) { ?>
                    <?php if ($type == "?so_to_invoice" || $type == "?quotation" || $type == "?pgi_to_invoice" || $type === "?proforma_to_invoice") { ?>
                        <input step="any" type="number" name="listItem[<?= $randCode ?>][qty]" value="0" class="inp-design full-width itemQty inputQuantityClass" id="itemQty_<?= $randCode ?>">
                        <?php
                    ...
                    <?php } else { ?>
                        <?php if ($type == "?joborder_to_invoice") { ?>
                            <input step="any" type="number" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity($invoiceQty) ?>" class="inp-design full-width itemQty inputQuantityClass" id="itemQty_<?= $randCode ?>" readonly>
                        <?php } else { ?>
                            <input step="any" type="number" name="listItem[<?= $randCode ?>][qty]" value="<?= inputQuantity($itemQty) ?>" class="inp-design full-width itemQty inputQuantityClass" id="itemQty_<?= $randCode ?>">
                        <?php } ?>
                        <?php
                        echo $uomName = getUomDetail($getItemObj['data']['service_unit'])['data']['uomName'];
                        //  $ItemsObj->getBaseUnitMeasureById($getItemObj['data']['service_unit'])['data']['uomName'];
            ...
            <span style="display:none; font-size: .8em" class="text-danger qtyMsg1" id="qtyMsg1_<?= $randCode ?>"></span>
        ...
        <td class="inp-td">
            <div class="d-flex">
                <input step="any" type="number" name="listItem[<?= $randCode ?>][itemTargetPrice]" value="<?= inputValue($itemTargetPrice) ?>" class="inp-design full-width itemTargetPrice inputQuantityClass" id="itemTargetPrice_<?= $randCode ?>" readonly>
            </div>
        </td>
        <!-- end mrp -->
        <!-- rate -->
        <td class="inp-td" style="min-width: 40px !important;">
            <div class="d-flex">
                <span class="rupee-symbol currency-symbol pr-1"><?= $currencyName ?> </span>
                <input type="hidden" name="listItem[<?= $randCode ?>][unitPrice]" class="inp-design full-width-center originalItemUnitPriceInp" id="originalItemUnitPriceInp_<?= $randCode ?>" value="<?php echo  decimalValuePreview($itemMRP); ?>">
                <!-- this value should be inserted in DB 👇🏾 -->
                <input type="text" name="listItem[<?= $randCode ?>][unitPrice]" class="inp-design text-right full-width-center inputAmountClass originalChangeItemUnitPriceInp" id="originalChangeItemUnitPriceInp_<?= $randCode ?>" value="<?php echo  inputValue($itemMRP); ?>">
            </div>
            <div style="font-size: 0.8em;" class="float-right font-weight-bold text-primary convertedDiv" id="convertedDiv_<?= $randCode ?>">
                <strong>(<span class="rupee-symbol currency-symbol-dynamic"><?= $currencyName ?> </span> <span class="convertedItemUnitPriceSpan" id="convertedItemUnitPriceSpan_<?= $randCode ?>"><?php echo  decimalValuePreview($itemMRP); ?></span>)</strong>
            </div>
        </td>
        <!-- end rate -->
        ...
        <td class="text-right">
            ...
            <div style="border-bottom: 1px solid #ccc; font-size: 0.8em;">
                <span class="itemTradeDiscountPercentageSpan" id="itemTradeDiscountPercentageSpan_<?= $randCode ?>"><?= decimalQuantityPreview($tradeDiscountPercentage) ?></span> %
            </div>
            <div>
        ...
        <td class="text-right">
            ...
            <div style="border-bottom: 1px solid #ccc; font-size: 0.8em;">
                <span class="itemCashDiscountPercentageSpan" id="itemCashDiscountPercentageSpan_<?= $randCode ?>">0.00</span> %
            </div>
            ...
            if($cashDiscountPercentage > 0){
                ?>
                <div style="border-top: 1px solid #ccc; font-size: 0.8em;" class="float-right font-weight-bold text-primary" id="cashDiscountWarning_<?= $randCode ?>">
                <strong>Previously, a cash discount of <?=decimalQuantityPreview($cashDiscountPercentage)?>% was applied, amounting to <?=$cashDiscountAmount?></strong>
                </div>
                <?php
            }            
            ?>
        ...
        <td>
            ...
            <span class="itemTaxPercentage" id="itemTaxPercentage_<?= $randCode ?>"><?= decimalQuantityPreview($itemTaxPercentage) ?></span>%
        </td>
        <td class="text-right">
                            ...
                            <div class="tab-content delivery-discount-tabContent" id="nav-tabContent">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <input type="number" step="any" name="listItem[<?= $randCode ?>][totalDiscount]" value="<?= inputValue($totalDiscount) ?>" class="form-control itemDiscount" id="itemDiscount_<?= $randCode ?>" readonly>
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <input type="number" step="any" value="<?= inputValue($itemTotalDiscount) ?>" name="listItem[<?= $randCode ?>][itemTotalDiscount1]" class="form-control itemTotalDiscount1" id="itemTotalDiscount1_<?= $randCode ?>" readonly>
                                                <span class="itemTotalDiscount" style="display: none;" id="itemTotalDiscount_<?= $randCode ?>"><?= decimalValuePreview($itemTotalDiscount) ?></span>
                                            </div>
                                            <div style="font-size: 0.8em;" class="font-weight-bold text-primary convertedDiv">
                                ...
                                <div class="tab-pane fade" id="nav-cash-discount-<?= $randCode ?>" role="tabpanel" aria-labelledby="nav-cash-discount-tab">
                                    <div class="row mb-3 border-bottom">
                                        ...
                                        <?php
                                        if ($cashDiscountType != "") { ?>
                                            <p class="pre-normal text-muted mt-1">In previous document the Cash discount was selected based on <b><i><?= ucfirst($cashDiscountType) ?></i></b> and the percentage was <b><i><?= decimalQuantityPreview($cashDiscountPercentage) ?>%</i></b></p>
                                        <?php }
                                        ?>
                                    </div>
                                    <div class="row">
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                <span class="rupee-symbol symbol pr-1"><?= '%' ?> </span>
                                                <input type="number" step="any" name="listItem[<?= $randCode ?>][totalCashDiscount]" value="<?= inputValue($totalCashDiscount) ?>" class="form-control itemCashDiscount" id="itemCashDiscount_<?= $randCode ?>">
                                                <small class="maxLimitStyle" style="display: none;">Max <br><?php if ($itemMaxCashDiscount == 0) { ?> <span class="text-danger">0</span> <?php } else { ?> <strong class="itemMaxCashDiscount" id="itemMaxCashDiscount_<?= $randCode ?>"><?= decimalValuePreview($itemMaxCashDiscount) ?></strong> <?php } ?>%</small>
                                            </div>
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            ...
                                            <div class="d-flex">
                                                <span class="rupee-symbol currency-symbol pr-1"><?= $currencyName ?> </span>
                                                <input type="hidden" value="<?= decimalValuePreview($itemTotalCashDiscount) ?>" name="listItem[<?= $randCode ?>][itemTotalCashDiscount]" class="form-control itemTotalCashDiscountHidden" id="itemTotalCashDiscountHidden_<?= $randCode ?>">
                                                <input type="number" step="any" value="<?= inputValue($itemTotalCashDiscount) ?>" name="listItem[<?= $randCode ?>][itemTotalCashDiscount1]" class="form-control itemTotalCashDiscount1" id="itemTotalCashDiscount1_<?= $randCode ?>">
                                                <span class="itemTotalCashDiscount" style="display: none;" id="itemTotalCashDiscount_<?= $randCode ?>"><?= $itemTotalCashDiscount ?></span>
                                            </div>
                                            <div style="font-size: 0.8em;" class="font-weight-bold text-primary convertedDiv">
                                                <strong>(<span class="rupee-symbol currency-symbol-dynamic"><?= $currencyName ?> </span> <span class="convertedItemCashDiscountAmountSpan" id="convertedItemCashDiscountAmountSpan_<?= $randCode ?>"><?= decimalValuePreview($itemTotalCashDiscount) ?></span>)</strong>
                                            </div>
                                        </div>
                                    </div>
                                </div>
