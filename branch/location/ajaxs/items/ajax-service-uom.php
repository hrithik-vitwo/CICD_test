...
//   console($createNewGoodGroupObj);
   ...
   // $responseData = $createNewGoodGroupObj;
    ...
    if($createNewGoodGroupObj["status"]=="success") {
        $getAllGoodGroupObj = queryGet("SELECT * FROM `erp_inventory_mstr_uom` WHERE `companyId`=$company_id AND `uomType`='service'",true);
        ...
        if($getAllGoodGroupObj["status"]=="success"){
          ...
          //  echo '<option value="">Select UOM</option>';
            for($i=0; $i<$numGoodTypes; $i++){
                ...
                if($i == $numGoodTypes-1){
                    echo '<option selected value="'.$oneGoodGroup["uomId"].'">'.$oneGoodGroup["uomName"].'</option>';
                }else{
                    echo '<option value="'.$oneGoodGroup["uomId"].'">'.$oneGoodGroup["uomName"].'</option>';
                }
            }
        }else{
            //echo 1;
...
//             ?>
//                 <option value="<?= $onePurchaseGroup["uomId"] ?>"><?= $onePurchaseGroup["uomName"] ?></option>
//             <?php
//         }
//     }else{
//         echo '<option value="">Select Purchase Group</option>';
