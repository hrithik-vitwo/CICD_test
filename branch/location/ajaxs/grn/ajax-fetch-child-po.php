...
if(isset($_GET["vendor_id"]) && $_GET["vendor_id"] != "" && isset($_GET["currency"]) && $_GET["currency"] != "" && isset($_GET["po"]) && $_GET["po"] != "")
{
    global $company_id;
    global $branch_id;
    global $location_id;
    ...
    $poDetailsObj = queryGet('SELECT * FROM `erp_branch_purchase_order` WHERE `company_id`=' . $company_id . ' AND `branch_id`=' . $branch_id . ' AND `location_id`=' . $location_id . ' AND `po_status`="9"  AND `use_type` IN ("servicep","material","asset")  AND `currency`="'.$currency.'" AND `parent_id` = "'.$po.'" ORDER BY `po_id` DESC', true);
    ...
    $result .= "
       ...
       $result .= "</td>
        ...
        <td>".$curr_name." ".$poDetail["totalAmount"]."</td>
        </tr>";
        $poItemSl++;
    }
...
{
    $result = "<tr><td colspan=8>Data Not Found</tr></td>";
    echo json_encode($result);
}
