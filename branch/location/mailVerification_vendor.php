...
$show=0;
$checksql=queryGet("SELECT isMailValid FROM ".ERP_VENDOR_DETAILS." WHERE vendor_id=$id AND `company_id`=$company_id");
// console($checksql);
// exit();
$status=$checksql["data"]["isMailValid"];
if($status=="no"){
    $show=1;
    $query=queryUpdate("UPDATE ".ERP_VENDOR_DETAILS." SET `isMailValid` = 'yes' WHERE `vendor_id` = ".$id." AND `company_id` = ".$company_id."");
}
?>
<!DOCTYPE html>
<html lang="en">
