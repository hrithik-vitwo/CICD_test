                                ...
                                <div class="card-body">
                                    ...
                                    <div class="tab-content" id="custom-tabs-two-tabContent">
                                        <div class="tab-pane dataTableTemplate dataTable_stock fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                            ...
                                            <a href="credit-notes-creation-rule-book.php?create" class="btn btn-create" type="button">
                                                                ...
                                                                <table>
                                                                    <tbody>
                                                                        ...
                                                                        foreach ($columnMapping as $columnIndex  => $column) {
                                                                            if ($columnIndex === 0 || $columnIndex === 3 || $columnIndex === 4 || $columnIndex === 6 || $columnIndex === 7 || $columnIndex === 8 || $columnIndex === 9 || $columnIndex === 10 || $columnIndex === 11 || $columnIndex === 12) {
                                                                                continue;
                                                                            } ?>
                                                                            <tr>
                                                                                <td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '') {
            ...
            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-credit-notes-taxComponents.php",
                dataType: 'json',
                data: {
                    act: 'creditnote',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            let taxComponents = JSON.parse(value.taxComponents);
                            ...
                            row.push(decimalAmount(value.taxableAmount));
                            if (countrycode == 103) {
                                row.push(
                            ...
                            row.push(
                                ...
                                <ul>
                                    <li>
                                        <button data-toggle="modal" class="soModal" data-id="${value.cr_note_id}" data-target="#viewGlobalModal">
        ...
        $(document).on("click", ".ion-paginationlistnew", function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    sql_data_checkbox: Cookies.get('cookiecreditNotes')
                },
                beforeSend: function() {
...
<script>
    ...
    $(document).on("click", ".soModal", function() {
        ...
        $.ajax({
            type: "GET",
            url: "ajaxs/modals/fa/ajax-credit-note-modal-taxComponents.php",
            dataType: 'json',
            data: {
                act: 'modaldata',
                cr_id
            ...
            success: function(value) {
                ...
                let currency = responseObj.companyCurrencyName;
                let taxComponents;
                let creditors = (responseObj.partydetails.creditors_type);
                if (dataObj.taxComponents) {
                    taxComponents = JSON.parse(dataObj.taxComponents);
                }
                $('.auditTrail').attr("data-ccode", dataObj.credit_note_no);
                if (components.fields['businessTaxID'] != null) {
                    $("#businessTaxIDdiv").show();
                    $("#businessTaxID").html(components.fields['businessTaxID']);
                    if (creditors == 'customer') {
                        $("#custgst").html(partyDetails.customer_gstin ? partyDetails.customer_gstin : "--");
                    } else {
                        $("#custgst").html(partyDetails.vendor_gstin ? partyDetails.vendor_gstin : "--");
                    }
                }
                if (components.fields['taxNumber'] != null) {
                    ...
                    if (creditors == 'customer') {
                        $("#custpan").html(partyDetails.customer_pan ? partyDetails.customer_pan : "--");
                    } else {
                        $("#custpan").html(partyDetails.vendor_pan ? partyDetails.vendor_pan : "--");
                    }
                }
                ...
                // let dataObj = responseObj.crNoteobj;
                // let taxComponents = JSON.parse(dataObj.taxComponents);
                ...
                if (countrycode == 103) {
                    if (igst != 0 || cgst != 0 || sgst != 0) {
                        if (igst == 0) {
                            ...
                            $("#igstP").hide();
                            $("#igst").hide();
                            $("#cgstVal").show().html(currency + " " + decimalAmount(cgst));
                            $("#sgstVal").show().html(currency + " " + decimalAmount(sgst));
                        } else {
                            //console.log(1);
                            $("#igstP").show();
                            $("#igst").show();
                    ...
                    $.each(taxComponents, function(index, component) {
                        ...
                        $("#igst").hide();
                        $('#ccompval').show();
                        $('#tcompname').show().html(component.gstType);
                        $("#compval").show().html(currency + " " + decimalAmount(component.taxAmount));
                    })
                }
                ...
                $.each(itemsObj, function(index, val) {
                    ...
                    let td = ` <tr>
                                                                ...
                                                                <td class="text-right"> ${decimalAmount(val.item_discount)}</td>
        ...
        // classic view ajax
        $("#classicViewPrint").attr('href', `classic-view/invoice-preview-print-taxcomponents.php?cr_note_id=${btoa(cr_id)}`);
        // $.ajax({
        //     type: "GET",
...
<script>
    ...
    $(document).on("click", ".repostCreditNote", function() {
        ...
        let url = `credit-notes-repost_tax.php?cnId=${btoa(cnId)}`;
