...
if (!isset($_COOKIE["cookiefailedpgi"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiefailedpgi", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Status',
        'slag' => 'salesInvoice.invoiceStatus',
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Failed Delivery to PGI</h3>
                                            </div>
                                        </div>
                                        ...
                                        <div class="right-block">
                                            <div class="page-list-filer filter-list">
                                                ...
                                                <a href="failed-accounting-pgi.php" class="filter-link active "><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Delivery to Pgi
                                                </a>
                                                <style>
                                                    @keyframes pulse {
                                                        0% {
                                                ...
                                                <a href="failed-accounting-pgi-invoice.php" class="filter-link "><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Invoice Pgi
                                                </a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookiefailedpgi');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-failed-acc-pgi.php",
                dataType: 'json',
                data: {
                    act: 'failedpgi',
                    comid: comid,
                ...
                success: function(response) {
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            let invStatus
                            if (value.invoiceStatus === 'open') {
                                invStatus = `<p class="status-bg status-open">open</p>`;
                            } else if (value.invoiceStatus === 'invoice') {
                                invStatus = `<p class="status-bg status-closed">invoice</p>`;
                            }
                            let actionType = `pay_id=${(value.soInvoiceIdaction)}`;
                            let actionbtn = ` <div class="view-accoiunt" data-id='${value.soInvoiceId}'>
                                    <a href="manage-failed-acc-pgi-view.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`;
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                value.invNo,
                                `<p class="pre-normal">${value.tradeName}</p>`,
                                decimalAmount(value.totalItems),
                                decimalAmount(value.totalAmt),
                                formatDate(value.invDate),
                                invStatus,
                                value.createdBy,
                                actionbtn
                            ]).draw(false);
