                    ...
                    <div class="print-padding-none row p-0 m-0">
                        <div class="action-btns-sheet d-flex flex-wrap p-0 m-0 mb-4">
                            ...
                            <div class="balancesheet_inner d-flex flex-wrap">
                                ...
                                <button class="btn btn-success mr-3" id="printExcel">Excel</button>
                            </div>
                        ...
                        <div class="printable-blncsheet">
                            ...
                            // $glSummaryData = $glSummaryObj->getSummaryTree($balanceSheetReportDate);
                            ...
                            // $glSummaryForProfitAndLoss = $glSummaryObj1->getSummaryTree(date("Y-m-01", strtotime($balanceSheetReportDate)), $balanceSheetReportDate);
                            $glSummaryForProfitAndLossObj = $glSummaryObj1->getSummaryTree(null, $balanceSheetReportDate);
                            ...
                            $glSummaryForOpeningBalanceObj = $glSummaryForOpeningBalObj->getSummaryTree(null, $balanceSheetReportDate);
                                    ...
                                    ?>
                                        <div class="card mb-2 bg-light text-dark">
                                            <div class="card-header p-0 d-flex bg-light text-dark accordionGlRowHover">
                                                ...
                                                <?php if ($itemIsGroup) { ?>
                                                    <p class="mr-2 text-sm font-weight-bold <?= $itemIsGroup ? "text-primary" : "text-dark" ?>"><?= ($closingBalance < 0) ? "(" . decimalValuePreview(abs($closingBalance)) . ")" : decimalValuePreview($closingBalance) ?></p>
                                                <?php } else if ($oneData["id"] == "") { ?>
                                                    <p class="mr-2 text-sm font-weight-bold <?= $itemIsGroup ? "text-primary" : "text-dark" ?>"><a><?= ($closingBalance < 0) ? "(" . decimalValuePreview(abs($closingBalance)) . ")" : decimalValuePreview($closingBalance) ?></a></p>
                                                <?php } else {
                                                ...
                                                ?>
                                                    <p class="mr-2 text-sm font-weight-bold <?= $itemIsGroup ? "text-primary" : "text-dark" ?>"><a <?= $fullLink ?>><?= ($closingBalance < 0) ? "(" . decimalValuePreview(abs($closingBalance)) . ")" : decimalValuePreview($closingBalance) ?></a></p>
                                                <?php } ?>
                                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#printExcel').on('click', function(e) {
            ...
            let $accordion = $('.printable-blncsheet').find('div[id^="accordion_"]');

            $accordion.find('.card-header').each(function() {
                let account = $(this).find('p.text-sm.font-weight-bold').first().text().trim();

                let total = $(this).find('p.mr-2.text-sm.font-weight-bold').text().trim();
                if ($(this).find('p.mr-2.text-sm.font-weight-bold a').length) {
                    total = $(this).find('p.mr-2.text-sm.font-weight-bold a').text().trim();
                }

                if (account && total) {
                    let totalValue = total;
                    if (total.startsWith('(') && total.endsWith(')')) {
                        totalValue = '-' + total.slice(1, -1).replace(/,/g, '');
                        totalValue = parseFloat(totalValue) || total; // Convert to number or keep as string
                    } else {
                        totalValue = parseFloat(total.replace(/,/g, '')) || total; // Convert to number or keep as string
                    }
