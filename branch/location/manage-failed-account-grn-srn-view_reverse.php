...
if (isset($_POST['act'])) {
    ...
    // console($_POST);
    // exit();
    $grnPostingJournalId = $_POST['grnPostingJournalId'];
    $grn_id = $_POST['grnId'];
    ...
    if ($journalObj["status"] == 'success') {
        ...
        $check_Journal = queryGet("SELECT * FROM `erp_acc_journal` WHERE (`parent_id`='" . $grn_id . "' OR `refarenceCode`='" . $grncode . "') AND `parent_slug`='" . $grnType . "' AND `branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . "", true);
        if ($check_Journal['numRows'] == 1) {
            ...
            if ($returnDataAcc['status'] == 'success') {
                ...
                $dbObj->queryUpdate('UPDATE `erp_grn` SET `grnPostingJournalId` = 0,`reverse_grnPostingJournalId` = ' . $newJournalId . ' WHERE `grnId`=' . $grn_id);
            } else {
                $flug == 1;
            }
        }else if($check_Journal['numRows'] == 2){
            $newid = $check_Journal['data'][1]['id'];
            $dbObj->queryUpdate('UPDATE `erp_grn` SET `grnPostingJournalId` = 0,`reverse_grnPostingJournalId` = ' . $newid . ' WHERE `grnId`=' . $grn_id);
    ...
    if ($flug == 0) {
         $logAccFailedResponce = updatelogAccountingFailure($grncode);
        swalAlert("success", 'Success', "GRN/SRN Accounting Posted Successfully", 'failed-accounting-grn-srn.php?reverse');
    } else {
...
$sql_Mainqry = "SELECT * FROM erp_grn LEFT JOIN `erp_vendor_details`as ven ON erp_grn.vendorId = ven.vendor_id  WHERE 1 " . $cond . " AND (grnPostingJournalId != 0 AND grnPostingJournalId IS  NOT NULL) AND (reverse_grnPostingJournalId = 0 OR reverse_grnPostingJournalId IS NULL)  AND companyId =" . $company_id . " AND branchId=" . $branch_id . " AND locationId=" . $location_id . " AND `grnStatus` !='deleted' " . $sts . " ORDER BY grnId DESC";
...
// console($grnMainData);
$grnPostingJournalId = $grnMainData['grnPostingJournalId'];
...
<div class="content-wrapper is-failed-account-view vitwo-alpha-global" style="overflow: auto">
    ...
    <div class="container-fluid mt-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="failed-accounting-grn-srn.php?reverse" class="text-dark"><i class="fa fa-list po-list-icon"></i>Failed Reverse GRN/ List</a></li>
            <li class="breadcrumb-item active"><a class="text-dark"><i class="fa fa-plus po-list-icon"></i>Reverse GRN/SRN Account Posting View</a></li>
            <li class="back-button">
                <a href="failed-accounting-grn-srn.php?reverse">
    ...
    <form method="post" action="">
        ...
        <input type="hidden" name="grnPostingJournalId" value="<?= $grnPostingJournalId; ?>">
                ...
                <div class="card-border-area">
                    <table>
                        ...
                        <tbody>
                            ...
                            $totaldr = 0;
                            $impact = false;
                            $debitObj = $dbObj->queryGet('SELECT * FROM `erp_acc_debit` WHERE `journal_id`=' . $grnPostingJournalId, true);
                            if ($debitObj['numRows'] == 0) {
                                ...
                                foreach ($debitObj["data"] as $debitRow) {
                                    ...
                                    <tr>
                                        <td>
                                            <p class="pre-normal">
                                                <?= $grnDebitAccList[0]['gl_code'] ?> ||<?= $grnDebitAccList[0]['gl_label']; ?>
                                            </p>
                                        </td>
                                        <td>
                                            <p class="pre-normal">
            ...
            <div class="account-list debit-acc-list">
                ...
                <div class="card-border-area">
                    <table>
                        ...
                        <tbody>
                            <?php
                            $creditObj = $dbObj->queryGet('SELECT * FROM `erp_acc_credit` WHERE `journal_id`=' . $grnPostingJournalId, true);
                            // console($creditObj);
