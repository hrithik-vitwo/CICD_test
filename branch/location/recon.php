...
<div class="content-wrapper recon-wrapper">
    ...
    if (isset($_GET['party_id']) && isset($_GET['type']) && isset($_GET['from_date']) && isset($_GET['to_date'])) {
        ...
        if ($_GET['type'] == 'vendor') {
            ...
            if ($dayOfMonth === '01') {
                ...
                $transaction_first_sql = queryGet("SELECT SUM(temp_table.amount) - SUM(temp_table.payment) AS transaction_open FROM (
                                SELECT journal.parent_slug AS type,journal.remark,journal.postingDate,0 AS amount,SUM(credit.credit_amount) AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.parent_slug='Payment' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id AND journal.postingDate BETWEEN '" . $firstDayOfMonth . "' AND '" . $prev_day . "' AND journal.journal_status='active' AND journal.party_code=$party_code GROUP BY journal.parent_slug,journal.remark,journal.postingDate
                                UNION
        ...
        //customer 
            ...
            //end customer opening

            $statement_sql = queryGet("SELECT * FROM (
            SELECT journal.id AS id,journal.parent_slug AS type,journal.documentNo AS document_no,journal.remark,journal.postingDate,0 AS amount,SUM(credit.credit_amount) AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.reconcile_status = 0 AND journal.parent_slug='Collection' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id AND journal.postingDate BETWEEN '" . $from_date . "' AND '" . $to_date . "' AND journal.journal_status='active' AND journal.party_code=$party_code GROUP BY journal.parent_slug,journal.remark,journal.postingDate,journal.id
            UNION
            SELECT journal.id AS id,journal.parent_slug AS type,journal.documentNo AS document_no,journal.remark,journal.postingDate,SUM(debit.debit_amount) AS amount,0 AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_debit AS debit ON journal.id=debit.journal_id WHERE journal.reconcile_status = 0 AND journal.parent_slug='SOInvoicing' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id AND journal.postingDate BETWEEN '" . $from_date . "' AND '" . $to_date . "' AND journal.journal_status='active' AND journal.party_code=$party_code GROUP BY journal.parent_slug,journal.remark,journal.postingDate,journal.id
            UNION
            SELECT journal.id AS id,journal.journalEntryReference AS type,journal.documentNo AS document_no,journal.remark AS remark,journal.postingDate,0 AS amount,SUM(credit.credit_amount) AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_credit AS credit ON journal.id=credit.journal_id WHERE journal.reconcile_status = 0 AND journal.parent_slug='journal' AND journal.journalEntryReference='Collection' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id AND journal.postingDate BETWEEN '" . $from_date . "' AND '" . $to_date . "' AND journal.journal_status='active' AND credit.subGlCode=$party_code GROUP BY journal.parent_slug,journal.remark,journal.postingDate,journal.id
            UNION
            SELECT journal.id AS id,'SOInvoicing' AS type,journal.remark AS remark,journal.documentNo AS document_no,journal.postingDate,SUM(debit.debit_amount) AS amount,0 AS payment FROM erp_acc_journal AS journal LEFT JOIN erp_acc_debit AS debit ON journal.id=debit.journal_id WHERE journal.reconcile_status = 0 AND journal.parent_slug='journal' AND journal.journalEntryReference='Sales' AND journal.company_id=$company_id AND journal.branch_id=$branch_id AND journal.location_id=$location_id AND journal.postingDate BETWEEN '" . $from_date . "' AND '" . $to_date . "' AND journal.journal_status='active' AND debit.subGlCode=$party_code GROUP BY journal.parent_slug,journal.remark,journal.postingDate,journal.id) AS temp_table
        ...
        // $locStatement_sql = queryGet("SELECT recon.* FROM `erp_vendor_customer_reconciliation` as recon LEFT JOIN `erp_reconciliation` as files ON files.id=recon.recon_file_id WHERE files.type='" . $party_type . "' AND files.code=$party_id AND recon.recon_journal_id = 0 AND files.location_id=$location_id AND recon.`transaction` NOT LIKE 'opening' OR recon.`transaction` NOT LIKE 'opening balance'", true);
         $locStatement_sql = queryGet("SELECT recon.* FROM `erp_vendor_customer_reconciliation` as recon LEFT JOIN `erp_reconciliation` as files ON files.id=recon.recon_file_id WHERE files.type='" . $party_type . "' AND files.code=$party_id AND recon.recon_journal_id = 0 AND files.location_id=$location_id ", true);


        // console($locStatement_sql);

        $opening_loc_sql =  queryGet("SELECT * FROM `erp_vendor_customer_reconciliation` as recon LEFT JOIN `erp_reconciliation` as files ON files.id=recon.recon_file_id WHERE files.type='" . $party_type . "' AND files.code=$party_id AND recon.recon_journal_id = 0 AND location_id=$location_id AND `opening_amount` IS NOT NULL AND `transaction` LIKE 'opening' OR `transaction` LIKE 'opening balance'");
        $opening_loc = $opening_loc_sql['data']['opening_amount'] ?? 0;
        $opening_transaction = $opening_loc_sql['data']['transaction'] ?? 0;
                            ...
                            <div class="card-body">
                                <div class="recon-table-head">
                                    <div class="amount-section">
                                        ...
                                        <p class="text-sm">Opening Balance <span class="font-bold text-lg"><?= $currency ?> <?= round($opening_balance, 2) ?></span> </p>
                                                                    ...
                                                                    <div class="modal-dialog modal-dialog-centered">
                                                                        <div class="modal-content">
                                                                            <div class="modal-header">
                                                                                ...
                                                                                <button type="button" class="close recon-close text-white" data-dismiss="modal" aria-label="Close">x</button>
                                                                            </div>
                                                                                ...
                                                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                                                    <div class="modal-content">
                                                                                        <div class="modal-header">
                                                                                            ...
                                                                                            <button type="button" class="close recon-excel-close recon-excel-close_<?= $datas['id'] ?> text-white" data-attr = "<?= $datas['id'] ?>"  data-bs-dismiss="modal" aria-label="Close">x</button>
                                                                                            <!-- <button type="button" id="recon-excel-close" class="close recon-close text-white" aria-label="Close">x</button> -->
                                                                                        </div>
                                                                                        <div class="modal-body">
...
<script>
    $(document).ready(function() {
            ...
            $('#portalGstr2bTableBody > tr').each(function(leftTrIndex, leftTr) {
                ...
                $('#localGstr2bTableBody > tr').each(function(rightTrIndex, rightTr) {
                    ...
                    if (statementDocument == localStatementDocument) {
                        ...
                        matchedConditions += 34;
                        color += 1;
                    }
                    if (statementDate == localStatementDate) {

                        matchedConditions += 33;
                        color += 1;
                    }
                    ...
                    } else if (statementCredit == localStatementDebit) {
                        // alert(4);
                        matchedConditions += 33;
                        color += 1;
                    }
        ...
        function calculateMatchedConditionsRows() {
            $(`#localGstr2bTableBody tr`).removeClass(`matchedRowColor-3 matchedRowColor-2 matchedRowColor-1`);
            $(`#portalGstr2bTableBody tr`).removeClass(`matchedRowColor-3 matchedRowColor-2 matchedRowColor-1`);
            ...
            $('#portalGstr2bTableBody > tr').each(function(leftTrIndex, leftTr) {
                ...
                $(leftThis).find('.reconPercentageColumn').html(`${matchedConditions}%`);
                $(`#localGstr2bTableBody tr:eq(${leftTrIndex})`).addClass(`matchedRowColor-${color}`);
                $(leftTr).addClass(`matchedRowColor-${color}`);
...
<script>
    ...
    function downloadCSV() {
        ...
        // Path to your demo CSV file
        var csvFilePath = '<?php echo BASE_URL; ?>/public/assets/templates/recon11.csv';
        ...
        link.download = 'reconciliation_template'+timestamp+'.csv';
    ...
    // Attach the downloadCSV function to the button's click event
...
<script>
    $(document).ready(function() {
        ...
        $("#portalGstr2bTableBody > tr").each((index, elem) => {
            //console.log(elem);
            let statementDate = $(elem).find('.statementDate').text();
            let statementDocument = $(elem).find('.statementDocument').text();
            let statementDebit = $(elem).find('.statementDebit').text();
            let statementCredit = $(elem).find('.statementCredit').text();
            let statementId = $(elem).find('.statementId').text();

            leftTable.push({
                statementDate: statementDate,
                statementDocument: statementDocument,
                statementDebit: statementDebit,
        ...
        $("#localGstr2bTableBody > tr").each((index, elem) => {
            //console.log(elem);
            let localStatementDate = $(elem).find('.localStatementDate').text();
            let localStatementDebit = $(elem).find('.localStatementDebit').text();
            let localStatementDocument = $(elem).find('.localStatementDocument').text();
            let localStatementCredit = $(elem).find('.localStatementCredit').text();
            let partyStatementId = $(elem).find('.localStatementId').text();


            rightTable.push({
                statementDate: localStatementDate,
                statementDocument: localStatementDocument,
                statementDebit: localStatementDebit,
                statementCredit: localStatementCredit,
                statementId: partyStatementId
        ...
        let ans = findBestPair(leftTable, rightTable);
        let calWeight = calculateWeight(leftTable, rightTable);
        console.log(ans);
...
<script>
                                                                                $(document).ready(function() {
                                                                                    ...
                                                                                    $(".recon-excel-close").on("click",function(){
                                                                                        //alert(1);
                                                                                        var attr = $(this).data('attr');
                                                                                        // alert(attr);
                                                                                        var modalClass = '.recon-excel-close_' + attr;
                                                                                        // alert(modalClass);
