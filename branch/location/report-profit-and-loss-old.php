            ...
            <div class="row p-0 m-0">
                <div class="col-12 mt-2 p-0">
                    <div class="print-padding-none row p-0 m-0">
                        <div class="action-btns-sheet d-flex flex-wrap justify-content-between p-0 m-0 mb-4">
                            <?php
                            ...
                            <div class="text-center">
                                <div class="d-flex">
                                    <input type="date" value="<?= $reportStartDate ?>" max="<?= date("Y-m-01") ?>" id="reportStartDate" class="form-control">
                                    <span class="px-1">To</span>
                                    <input type="date" value="<?= $reportEndDate ?>" max="<?= date("Y-m-d") ?>" id="reportEndDate" class="form-control">
                                </div>
                                <button class="btn btn-sm btn-primary ml-1 mt-3" style="height:30px;" id="applyDateFilter">Apply</button>
                            </div>
                            <div class="d-flex">
                        ...
                        <div class="printable-blncsheet">
                                    ...
                                    <?php foreach ($data as $key => $oneData) {
                                        ...
                                        <div class="card mb-2 bg-light text-dark">
                                            <div class="card-header p-0 d-flex bg-light text-dark accordionGlRowHover">
                                                ...
                                                <?php } else { ?>
                                                    <p class="mr-2 text-sm font-weight-bold <?= $itemIsGroup ? "text-primary" : "text-dark" ?>"><a target="_blank" href="<?= LOCATION_URL ?>manage-daybook.php?mode=sablager&glId=<?= $oneData["id"]; ?>&from_date=<?= $reportStartDate ?>&to_date=<?= $reportEndDate ?>"><?= ($closingBalance < 0) ? "(" . number_format(abs($closingBalance), 2) . ")" : number_format($closingBalance, 2) ?></a></p>
                                                <?php } ?>
                                                <!-- <p class="mr-2 text-sm font-weight-bold <?= $itemIsGroup ? "text-primary" : "text-dark" ?>"><?= ($closingBalance < 0) ? "(" . number_format(abs($closingBalance), 2) . ")" : number_format($closingBalance, 2) ?></p> -->
                                            </div>
                            ...
                            <p class="d-flex text-muted text-sm"><span class="mr-auto">(Profit) / Loss</span> <span class="ml-auto"><?= ($profitBeforeTax < 0) ? "(" . number_format(abs($profitBeforeTax), 2) . ")" : number_format($profitBeforeTax, 2) ?></span></p>
                            <hr class="p-0 mt-0">
                        </div>
                    </div>
                </div>
