...
if (!isset($_COOKIE["cookiefailedpaymentAcc"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiefailedpaymentAcc", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Failed Payments</h3>
                                            </div>
                                        </div>
                                      ...
                                      <div class="right-block">
                                            <div class="page-list-filer filter-list">
                                                ...
                                                <a class="filter-link active" id="active"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Payments
                                                </a>
                                                <style>
                                                    @keyframes pulse {
                                                        0% {
                                                ...
                                                <a class="filter-link" id="reverse"><ion-icon name="list-outline" role="img" class="md hydrated" aria-label="list outline"></ion-icon>Reverse Payments
                                                </a>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '',type = '') {
            ...
            var checkboxSettings = Cookies.get('cookiefailedpaymentAcc');
            var notVisibleColArr = [];
             var invoicetype = type;

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-failed-acc-payment.php",
                dataType: 'json',
                data: {
                    act: 'tdata',
                    formDatas: formDatas,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            if (value.status === 'active') {
                                ...
                                revCollectPay = `
                                    <li>
                                        <button class="reversePayment" data-id="${value.payment_id}" data-no="${value.transactionId}" ><ion-icon name="repeat-outline"></ion-icon>Reverse</button>
                                    </li>`;
                            } else if (value.status === "reverse") {
                                status = '<p class="status-bg status-pending">Reverse</p>'
                            }
                            let actionType = `pay_id=${(value.payment_id)}`;
                            let btn='';
                            if(response.type=='active'){
                                btn=` <div class="view-accoiunt" data-id='${value.payment_id}'>
                                    <a href="manage-failed-acc-payment-view.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`;
                                 ...
                                 btn=` <div class="view-accoiunt" data-id='${value.payment_id}'>
                                    <a href="manage-failed-acc-payment-reverse-view.php?${actionType}" class="wallet-accountbtn">
                                      <ion-icon name="wallet-outline"></ion-icon>
                                      <span>Post</span>
                                    </a>
                                  </div>`;
