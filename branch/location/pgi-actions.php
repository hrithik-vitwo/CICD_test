...
if (isset($_POST['addNewPgiFormSubmitBtn'])) {
  ...
  if ($addBranchSoDeliveryPgi['status'] == "success") {
    swalAlert($addBranchSoDeliveryPgi["status"], $addBranchSoDeliveryPgi['pgiNo'], $addBranchSoDeliveryPgi["message"], 'manage-pgi.php');
  } else {
              ...
              <div class="card pgi-creation">
                ...
                <div class="card-body">
                  <div class="row" id="customerInfo">
                    ...
                    if ($customerDetails['customer_status'] != 'active') {
                      swalAlert("warning", "failed", "customer should be active", "manage-sales-orders-delivery.php");
                    }
                    ?>
                  ...
                  <div class="row">
                    <div class="col-lg-4 col-md-4 col-sm-4">
                      <div class="form-input">
                        ...
                        <?php if ($singleSoDetails['so_delivery_posting_date'] > $min) {
                          $min = $singleSoDetails['so_delivery_posting_date'];
                        } ?>
                  ...
                  <table class="table-sales-order">
                    ...
                    <tbody id="itemsTable">
                      ...
                      foreach ($itemDetails as $key => $item) {
                        $getItemObj = $ItemsObj->getItemById($item['inventory_item_id']);
                        // console($item);
                      ...
                      ?>
                        <tr class="rowDel itemRow" id="delItemRowBtn_<?= $item['inventory_item_id'] ?>_<?= $key ?>">
                          ...
                          <input class="form-control full-width" type="hidden" name="listItem[<?= $key ?>][parentGlId]" value="<?= $getItemObj['data']['parentGlId'] ?>">
                          ...
                          <td>
                            ...
                            <div class="qty-modal py-2">
                              <p class="font-bold checkQtySpan_<?= $key ?>"><?= inputQuantity($sumOfBatches); ?></p>
                          ...
                          <td>
                            ...
                            // console($schedule);
                            $dDate='';
                            if($schedule['delivery_date']==null){
                              $sql=queryGet("SELECT so.delivery_date FROM erp_branch_sales_order as so JOIN erp_branch_sales_order_delivery as del WHERE so.so_id=del.so_id AND del.so_delivery_id=".$singleSoDetails['so_delivery_id']."")['data'];
                              $delDate=$sql['delivery_date'];
                              $dDate=$delDate;
                            }else{
                              $dDate=$schedule['delivery_date'];
                            }
                            ?>
                            <span><?= formatDateWeb($dDate) ?></span>
                          ...
                          <td>
                            <input step="any" type="number" name="listItem[<?= $key ?>][enterQty]" value="<?= helperQuantity($item['qty']) ?>" class="form-control full-width itemQty" id="itemQty_<?= $key ?>" readonly>
