<style>
    ...
    .action-btns-production label.active {
        background-color: #545b62 !important;
        border-color: #545b62 !important;
    }

    table.simple-tree-table tbody tr:nth-child(odd) td {
        background-color: #fff;
    }

    table.simple-tree-table tbody tr:nth-child(even) td {
                                                            ...
                                                            <div class="modal-header">
                                                                ...
                                                                <div class="display-flex-space-between mt-4 mb-3">
                                                                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                                                                        ...
                                                                        <li class="nav-item">
                                                                            <a class="nav-link treeTable" id="treeTable-tab<?= str_replace('/', '-', $listRow["so_por_id"]) ?>" data-toggle="tab" data-ccode="<?= str_replace('/', '-', $listRow["so_por_id"]) ?>" href="#treeTable<?= str_replace('/', '-', $listRow["so_por_id"]) ?>" role="tab" aria-controls="treeTable<?= str_replace('/', '-', $listRow["so_por_id"]) ?>" aria-selected="false"><i class="fa fa-history mr-2"></i> Tree Table </a>
                                                                        </li>
...
<script>
    // Move the TreeTable initialization outside the click event
    // $(document).ready(function() {
    ...
    //     $.ajax({
    //         url: `https://www.devalpha.vitwo.ai/branch/location/bom/ajax/get-mrp-preview.php?production-order-id=${ccode}`,
    //         type: 'GET',
    //         dataType: 'json',
    ...
    //             $(`${modalId} #treeTableBody`).html(`
    //             <tr data-node-id="0_0">
    //                 <td>
    //                     <div class="circlePercent">
    //                         <div class="counter" data-percent="40">10/100</div>
    ...
    //             // Initialize TreeTable for the specific modal
    ...
    $('.treeTable').on('click', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                // initializeProgressBar($(`${modalId} .circlePercent`), parseFloat(data["productionOrder"]["remainQty"]));
            },
            error: function(error) {
                console.error('Error fetching data:', error);
            }
    ...
    // Reusable function to initialize progress bar
    //  function initializeProgressBar(elem, initialPercent) {
    //     let percent = initialPercent || 0;
    //     const stopPercent = parseInt(elem.data('stop-percent')) || 0;
    //     let stopped = false;

    //     function animate() {
    //         setProgress(elem[0], (percent += 0.25));
    ...
    function setProgress(elem, qty) {
        const totalQty = 100;
        const percent = (qty / totalQty) * 100;
        ...
        const prevPercent = parseInt(counter.getAttribute('data-percent'), 10);

        elem.querySelector('.counter').setAttribute('data-percent', Math.round(percent));

        // Animate progress bar with a smooth transition
        elem.querySelector('.progressEnd').style.transition = 'transform 0.5s ease-out';
        elem.querySelector('.progress').style.transition = 'transform 0.5s ease-out';
        ...
        counter.textContent = `${Math.round(qty)}/${totalQty}`;
        ...
        if (qty > 0 && prevPercent === 0) {
            // Trigger animation when data quantity increases from 0
            elem.classList.remove('fiftyPlus'); // Reset class for 50% condition
            requestAnimationFrame(() => {
                elem.classList.add('fiftyPlus');
            });
        }

        // Update remaining quantity in the tree table cell
        const treeTableCell = elem.closest('tr').querySelector('.text-right');
        treeTableCell.textContent = `${Math.round(qty)}`;
    }
