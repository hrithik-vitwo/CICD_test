...
$sql_list = "SELECT * FROM `erp_credit_note` WHERE 1 " . $cond . "  AND`branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . " AND `journal_id`!=0 AND `reverse_journal_id`IS NULL AND `status` IN ('reverse')  ORDER BY cr_note_id desc";
...
if (isset($_POST['act'])) {
    ...
    if ($journalObj["status"] == 'success') {
        ...
        $check_Journal = queryGet("SELECT * FROM `erp_acc_journal` WHERE `documentNo`='" . $credit_note_no . "' AND `parent_slug`='".$accslug."' AND `branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . "", true);
    ...
    if ($goods_journal_id > 0) {
        ...
        if ($journalObj["status"] == 'success') {
            ...
            $slug=$accslug . 'Goods';
            $check_Journal = queryGet("SELECT * FROM `erp_acc_journal` WHERE `documentNo`='" . $credit_note_no . "' AND `parent_slug`='$slug' AND `branch_id`=$branch_id AND `location_id`=$location_id AND `company_id`=" . $company_id . "", true);
    ...
    if ($flug == 0 && $flug1 == 0) {
        $dbObj->queryUpdate('UPDATE `erp_credit_note` SET `journal_id` = 0, `goods_journal_id` = 0, `reverse_journal_id` = ' . $newJournalIdAcc . ', `reverse_goods_journal_id` = ' . $newGoodsJournalId . ' WHERE `cr_note_id`=' . $credit_note_id);
         $logAccFailedResponce = updatelogAccountingFailure($credit_note_no);
         swalAlert("success", 'Success', "Credit Note  Reverse Accounting Success !", 'failed-accounting-credit-note.php?reverse');
    }else{
        swalAlert("warning", 'Failed', "Credit Note  Reverse Accounting Failed !");
    }
...
<div class="content-wrapper is-failed-account-view vitwo-alpha-global" style="overflow: auto;">
    ...
    <div class="container-fluid mt-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="failed-accounting-credit-note.php?reverse" class="text-dark"><i class="fa fa-list po-list-icon"></i>Failed Credit Note List</a></li>
    ...
    <form method="post" action="">
        ...
        <div class="wrapper-account">
            ...
            <?php $diffAmount = $drtotal - $crtotal ?>
            <div class="account-amount deffrence-amount">
