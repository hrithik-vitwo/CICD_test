...
require_once("common/soDeletefunction.php");
$pageName =  basename($_SERVER['PHP_SELF'], '.php');

if (!isset($_COOKIE["cookiePendingSo"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiePendingSo", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
<div class="content-wrapper report-wrapper is-stock-new is-sales-orders vitwo-alpha-global">
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">Manage SO Pending List</h3>
                                            </div>
                                        </div>
                                                ...
                                                <div class="modal-background">
                                                    <div class="modal">
                                                        ...
                                                        <div class="filter-action filter-mobile-search mobile-page">
                                                            ...
                                                            <a href="direct-create-invoice.php?sales_order_creation" class="btn btn-create mobile-page mobile-create" type="button">
    ...
    <script>
        ...
        function adjustTabs() {
            ...
            var hiddenTabs = navTabs.children(":gt(" + (visibleTab) + ")");
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "ajaxs/ajax-manage-so-pending.php",
                    dataType: 'json',
                    data: {
                        act: 'detailed_view',
                        comid: comid,
                    ...
                    success: function(response) {
                        ...
                        if (response.status) {
                            ...
                            $.each(responseObj, function(index, value) {
                                dataTable.row.add([
                                    value.sl_no,
                                    `<a href="#" class="soModal"  data-id="${value.so_id}" data-code="${value.so_number}" data-toggle="modal" data-target="#viewGlobalModal">${ value.so_number}</a>`,
                                    ...
                                    value.goodType,
                                    value.customer_code,
                                    `<p class='item-name pre-normal'>${value.trade_name}</p>`,
                                    value.totalAmount,
                                    value.created_by,
                                    ...
                                    <ul>
                                         <li>
                                             <button data-toggle="modal" class="editSobtn" data-id=${value.so_id} data-code="${value.so_number}"><ion-icon name="create-outline" class="ion-edit"></ion-icon>Edit</button>
                                        </li>
                                        <li>
                                            <button class="deleteSoBtn" data-toggle="modal" data-target="#" data-id="${value.so_id}"><ion-icon name="trash-outline" class="ion-delete"></ion-icon>Delete</button> 
                                        </li>
                                        <li>
                                            <button class="soModal"  data-id="${value.so_id}" data-code="${value.so_number}" data-toggle="modal" data-target="#viewGlobalModal"><ion-icon name="create-outline" class="ion-view"></ion-icon>View</button>
                                        </li>
                                    </ul>
                                </div>`
                                ]).draw(false);
                            ...
                            var checkboxSettings = Cookies.get('cookiePendingSo');
            ...
            $(document).on("click", ".ion-paginationliststock", function(e) {
                $.ajax({
                    ...
                    data: {
                        ...
                        sql_data_checkbox: Cookies.get('cookiePendingSo')
                    },
                    beforeSend: function() {
                        $('#loaderModal').show();
            ...
            $(document).ready(function() {
                ...
                $(document).on("click", "#serach_submit", function(event) {
                    ...
                    $(".selectOperator").each(function() {
                        ...
                        if ((columnSlag == "so_date" || columnSlag == "created_at" || columnSlag == "delivery_date") && operatorName == "BETWEEN") {
        ...
        $(document).ready(function() {
            $(document).on("change", ".selectOperator", function() {
                ...
                if ((columnName === 'Delivery Date' || columnName === 'SO Date' || columnName === 'Created Date') && operatorName === 'BETWEEN') {
                    inputContainer.append(`<input type="date" name="value[]" class="fld form-control m-input m-input2" id="value2_${(columnIndex)}" placeholder="Enter Keyword" value="">`);
                } else {
    ...
    <script>
        $(document).on("click", ".soModal", function() {
            ...
            $.ajax({
                type: "GET",
                url: "ajaxs/modals/so/ajax-manage-so-pending-modal.php",
                dataType: 'json',
                data: {
                    soId
                },
                ...
                success: function(value) {
                    ...
                    let subTotal = decimalAmount(responseObj.allSubTotal);

                    let totalTax = decimalAmount(responseObj.dataObj.totalTax);
                    let disCount = decimalAmount(responseObj.dataObj.totalDiscount);
                    let totalAmt = decimalAmount(responseObj.dataObj.totalAmount);


                    if (disCount == 0) {
                        taxableAmt = subTotal;
                    } else {
                        taxableAmt = decimalAmount(subTotal - disCount);
                    }
                    ...
                    $("#sub_total").html(responseObj.dataObj.currency_name + " " + subTotal);
                    $("#totalDis").html(responseObj.dataObj.currency_name + " " + disCount);
                    $("#taxableAmt").html(responseObj.dataObj.currency_name + " " + taxableAmt);
                    $("#total_amount").html(responseObj.dataObj.currency_name + " " + totalAmt);
