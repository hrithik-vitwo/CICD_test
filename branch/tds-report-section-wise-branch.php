...
if (!isset($_COOKIE["cookieTdsSectionBranch"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookieTdsSectionBranch", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
   ...
   let csvContent;
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '',columnMapping = '') {
            ...
            var columnMapping = <?php echo json_encode($columnMapping); ?>;
            var checkboxSettings = Cookies.get('cookieTdsSectionBranch');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "location/ajaxs/ajax-tds-report-section-wise-branch.php",
                dataType: 'json',
                data: {
                    act: 'tData',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                value.sl_no,
                                value.section,
                                decimalAmount(value.totalAmount),
                                // ` <div class="dropout">
                                //     <button class="more">
                                //          <span></span>
                                //          <span></span>
