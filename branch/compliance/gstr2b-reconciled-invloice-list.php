...
if (!isset($_COOKIE["cookiereconInv"])) {
    ...
    if ($settingsCheckbox_concised_view) {
        setcookie("cookiereconInv", json_encode($settingsCheckbox_concised_view), time() + 86400 * 30, '/');
    }
}
...
$columnMapping = [
    ...
    [
        'name' => 'Recon Percentage',
        'slag' => 'recper',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
    [
        'name' => 'Recon total Amount',
        'slag' => 'amt',
        'icon' => '<ion-icon name="location-outline"></ion-icon>',
        'dataType' => 'string'
    ],
                                    ...
                                    <li class="pt-2 pt-md-0 px-3 d-flex justify-content-between align-items-center header-space" style="width:100%">
                                        <div class="left-block">
                                            <div class="label-select">
                                                <h3 class="card-title mb-0">GSTR2B Reconciled Documents</h3>
                                            </div>
                                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function fill_datatable(formDatas = '', pageNo = '', limit = '', columnMapping = '') {
            ...
            var checkboxSettings = Cookies.get('cookiereconInv');
            var notVisibleColArr = [];

            $.ajax({
                type: "POST",
                url: "ajaxs/ajax-gstr2b-recon-invoice.php",
                dataType: 'json',
                data: {
                    act: 'tData',
                    comid: comid,
                    ...
                    if (response.status) {
                        ...
                        $.each(responseObj, function(index, value) {
                            ...
                            dataTable.row.add([
                                ...
                                value.igst_amount,
                                value.cess_amount,
                                value.recon_document,
                                value.recon_percentage,
                                value.recon_total_amount,
                                value.status
                            ]).draw(false);
                        });
    ...
    $(document).ready(function() {
        $(document).on("click", "#check-box-submt", function(event) {
            ...
            if (settingsCheckbox.length < 5) {
                ...
                $.ajax({
                    type: "POST",
                    url: `<?= BRANCH_URL?>location/ajaxs/ajax-save-cookies.php`,
                    dataType: 'json',
                    data: {
                        act: 'cookiereconInv',
                        fromData: fromData
