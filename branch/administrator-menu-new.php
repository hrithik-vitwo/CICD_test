...
require_once("../app/v1/functions/branch/func-branch-administrator-new.php");
...
if (isset($_GET["menu-edit"]) && $_GET["menu-edit"] > 0) {
    $menuDetailsObj = new_getAdministratorMenuDetails($_GET["menu-edit"]);
?>
                                        ...
                                        <div class="row m-0 p-0">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <div class="form-group">
                                                    <select name="fldGrandParentMenu" class="form-control" required>
                                                        <option value="">Select One Menu</option>
                                                        <?php
                                                        $sql = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`='-0'";
                                                        if ($res = mysqli_query($dbCon, $sql)) {
                                                            while ($row = mysqli_fetch_assoc($res)) {
                                                                if ($menuDetails["fldGrandParentMenu"] == $row["fldMenuKey"]) {
                                            ...
                                            <div class="col-12" style="display:flex;">
                                                <a href="<?= basename($_SERVER['PHP_SELF']); ?>" class="btn btn-outline-secondary mr-2">Cancel</a>
                                                <button type="submit" name="updateMenuFormBtn" class="btn btn-primary">Modify Menu Details</button>
                                            </div>
                                        </div>
                                    </form>
                                <?php
...
<?php
    ...
    $menuDetailsObj = new_getAdministratorMenuDetails($_GET["submenu-edit"]);
?>
    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
        <!-- Content Header (Page header) -->
                                    ...
                                    <form action="" method="POST">
                                        <div class="row m-0 p-0">
                                            ...
                                            <div class="col-3">
                                                ...
                                                <span class="btn btn-success btn-xs" id="add_new_bullet_point"><i class="fa fa-plus-square" aria-hidden="true"></i></span>
                                                <div class="row" id="bullet_point_table">
                                                    <?php
                                                     if (!empty($menuDetails["menu_accesses"])) {
                                                         $sql_access =  explode(',',$menuDetails["menu_accesses"]);
                                                        foreach ($sql_access as $row_access) { ?>
                                                            <div class="col-lg-11">
                                                                <div class="form-group">
                                                                    <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="<?= $row_access?>" readonly>
                                                                </div>
                                                            </div>
                                                    <?php }
                                                    } ?>
                                            ...
                                            <div class="col-12" style="display: flex;">
                                                <a href="<?= basename($_SERVER['PHP_SELF']); ?>" class="btn btn-outline-secondary mr-2">Cancel</a>
                                                <button type="submit" name="updateSubMenuFormBtn" class="btn btn-primary">Modify Sub Menu Details</button>
                                            </div>
                                        </div>
                                    </form>
                                <?php
                                ...
                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                    <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab">
                                        <table class="table table-bordered">
                                            ...
                                            <?php foreach (new_getAdministratorMenuList()['data'] as $menuDetails) { ?>
                                                <tr>
                                                    <td> <a class="btn btn-outline-primary btnstyle menu-btn" href="?menu-edit=<?= $menuDetails["fldMenuKey"] ?>"><label class="rounded"><?= $menuDetails["fldMenuIcon"] ?><?= $menuDetails['fldMenuLabel'] ?></label><i class="nav-icon fas fa-edit" style="cursor: pointer;"></i></a></td>
                                                    <td>
                                                        <?php foreach (new_getAdministratorMenuList($menuDetails['fldMenuKey'])['data'] as $subMenuDetails) { ?>
                                                            <span class="form-group mr-2">
                                                                <a class="btn btn-outline-primary btnstyle menu-btn" href="?submenu-edit=<?= $subMenuDetails["fldMenuKey"] ?>"><label class="rounded"><?= $subMenuDetails["fldMenuIcon"] ?> <?= $subMenuDetails['fldMenuLabel'] ?></label><i class="nav-icon fas fa-edit" style="cursor: pointer;"></i></a>
                                                            </span>
                                                        <?php } ?>
                                                    </td>
                                                </tr>
                                    ...
                                    <div class="tab-pane fade" id="addNewTabPan" role="tabpanel" aria-labelledby="addNewTab">
                                        <form action="" method="POST">
                                            <div class="row m-0 p-0">
                                                <div class="col-md-6">
                                                    <span class="text-muted">Select Parent Menu:</span>
                                                    <div class="form-group">
                                                        <select name="fldGrandParentMenu" class="form-control" required>
                                                            <option value="">Select One Parent Menu</option>
                                                            <?php
                                                            $sql = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`='-0' AND `fldMenuStatus`='active'";
                                                            if ($res = mysqli_query($dbCon, $sql)) {
                                                                while ($row = mysqli_fetch_assoc($res)) {
                                                                    echo '<option value="' . $row['fldMenuKey'] . '">' . $row['menuFor'] . ' > '.$row['fldMenuLabel'] . '</option>';
                                                                }
                                                            }
                                                            ?>
                                                        </select>
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    <button type="submit" name="addMenuFormBtn" class="btn btn-primary btnstyle">Add New Menu</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                            ...
                                            <div class="row m-0 p-0">
                                                ...
                                                <div class="col-3">
                                                    ...
                                                    <div class="row" id="bullet_point_table">
                                                        <div class="col-lg-11">
                                                            <div class="form-group">
                                                                <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="List" readonly>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-11">
                                                            <div class="form-group">
                                                                <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="Add" readonly>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-11">
                                                            <div class="form-group">
                                                                <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="Edit" readonly>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-11">
                                                            <div class="form-group">
                                                                <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="Delete" readonly>
                                                            </div>
                                                        </div>
                                                        <div class="col-lg-11">
                                                            <div class="form-group">
                                                                <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="Status Change" readonly>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-12" style="display: flex;">
                                                    <a href="<?= basename($_SERVER['PHP_SELF']); ?>" class="btn btnstyle btn-outline-secondary mr-2">Cancel</a>
                                                    <button type="submit" name="addSubMenuFormBtn" class="btn btn-primary btnstyle">Add New Sub Menu</button>
                                                </div>
                                            </div>
                                        </form>
...
<script>
    ...
    $("#add_new_bullet_point").click(function() {
      ...
      let addressRandNo = Math.ceil(Math.random() * 100000);
        var bullet_point_html = `<div class="col-lg-11">
                                    <div class="form-group" style="display: flex;">
                                        <input type="text" class="form-control" id="access" name="access[]" placeholder="Enter Access title" autocomplete="off" value="" >
                                        <span class="btn btn-danger btn-xs delete_new_bullet_point"><i class="fa fa-times" aria-hidden="true"></i></span>
                                    </div>
                                </div>`;
