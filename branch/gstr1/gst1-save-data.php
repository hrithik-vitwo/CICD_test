...
if ($authObj["status"] == "success") {
    ...
    if (isset($_POST["saveGstr1PortalData"])) {
        ...
        // console($actionObj);
        swalToast($actionObj["status"], $actionObj["message"]);
    }
...
<div class="content-wrapper">
    <section class="gstr-1">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="" class="text-dark"><i class="fa fa-list po-list-icon"></i>GSTR1 Save Data to Portal <?= $branch_gstin_file_frequency != "" ? "(" . date("F, Y", strtotime($gstr1ReturnPeriod)) . " - " . strtoupper($branch_gstin_file_frequency) . ")" : "" ?></a></li>
        ...
        <div class="head-btn-section mb-3">
            <div class="filter-list gst-one-filter">
                <a href="<?= BRANCH_URL ?>gstr1/gst1-preview.php<?= isset($_GET["period"]) ? '?period=' . $_GET["period"] : '' ?>" class="btn"><i class="fas fa-chart-bar mr-2"></i>Preview</a>
                <a href="" class="btn active"><i class="fa fa-list mr-2"></i>Save Data to Portal</a>
            </div>
        </div>
                ...
                <div class="row">
                    <div class="col-lg-8 col-sm-8 col-sm-8">
                        <h4 class="text-sm font-bold m-4">
                            Save File For The Month Of <?= date("F, Y", strtotime($gstr1ReturnPeriod)) ?>
                        </h4>
                        <div class="proceedToFile">
                            <div class="box mb-4">
                                <p class="text-sm mb-2"><b>Total Of GST</b> - <?= number_format($gstr1SummaryData["gstr1_return_total_cgst"] + $gstr1SummaryData["gstr1_return_total_sgst"] + $gstr1SummaryData["gstr1_return_total_igst"] + $gstr1SummaryData["gstr1_return_total_cess"], 2) ?></p>
                                <p class="text-sm mb-2"><b>CGST</b> - <?= number_format($gstr1SummaryData["gstr1_return_total_cgst"], 2) ?></p>
                                <p class="text-sm mb-2"><b>SGST</b> - <?= number_format($gstr1SummaryData["gstr1_return_total_sgst"], 2) ?></p>
                                <p class="text-sm mb-2"><b>IGST</b> - <?= number_format($gstr1SummaryData["gstr1_return_total_igst"], 2) ?></p>
                                <p class="text-sm mb-2"><b>CESS</b> - <?= number_format($gstr1SummaryData["gstr1_return_total_cess"], 2) ?></p>
                            ...
                            <div class="text">
                                ...
                                <div class="btns-group my-3">
                                    <form action="" method="post">
                                        ...
                                        <button type="submit" class="btn btn-primary border mr-2" name="saveGstr1PortalData" onclick="return confirm('Are you sure to save data?')">Yes</button>
                                        <a href="<?= BRANCH_URL ?>gstr1/gst1-preview.php<?= isset($_GET["period"]) ? '?period=' . $_GET["period"] : '' ?>" class="btn btn-primary reset-btn">No</a>
                                    </form>
                                </div>
