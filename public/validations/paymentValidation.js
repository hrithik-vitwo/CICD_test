$(document).ready(function () {
    ...
    $("#submitCollectPaymentBtn").on("click", function(e){
        ...
        if ($("#customerSelect").val() == "") {
            ...
            $("#customerSelect").parent().append('<span class="error collection_customerSelect">Vendor name is required</span>');
            $(".collection_customerSelect").show();

            $(".notescustomerSelect").remove();
            $("#notesModalBody").append('<p class="notescustomerSelect font-monospace text-danger">Vendor name is required</p>');
        } else {
            $(".collection_customerSelect").remove();
        ...
        // BANK VALIDATION
        if ($("[name='paymentDetails[bankId]']").val() == "" || $("[name='paymentDetails[bankId]']").val() == "0") {
