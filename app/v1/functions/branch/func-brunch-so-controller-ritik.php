...
class BranchSo extends Accounting
{
	...
	// fetch fetchAllItemsByGroupWise with search
    function fetchAllItemsByGroupWiseSearch($gid,$searchText)
    {
        global $company_id;
    ...
    function fetchCustomerPaymentLogDetails($paymentId)
    {
     ...
     function fetchAdvanceAmt($id)
     {
         ...
         $sql = "SELECT
             SUM(LOG.payment_amt) AS totalAdvanceAmt
         FROM
             erp_branch_sales_order_payments_log AS LOG
         JOIN erp_branch_sales_order_payments AS payment
         ON
             LOG.payment_id = payment.payment_id
         WHERE
             LOG.customer_id = $id AND LOG.payment_type = 'advanced' AND payment.status = 'active'";
         if ($res = $dbCon->query($sql)) {
             $row = $res->fetch_assoc();
             $returnData['success'] = "true";
             $returnData['message'] = "Data found!";
     ...
     function fetchOneAdvanceAmt($paymentId)
     {
         ...
         ON
             LOG.payment_id = payment.payment_id
         WHERE
         LOG.`payment_id`='$paymentId' AND LOG.payment_type = 'advanced' AND payment.status = 'active'";
         if ($res = $dbCon->query($sql)) {
             $row = $res->fetch_assoc();
             $returnData['success'] = "true";
             $returnData['message'] = "Data found!";
