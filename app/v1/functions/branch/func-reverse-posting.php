...
class ReversePosting extends AccountingPosting
{
    ...
    function reversePayment($value)
    {
        ...
        //Reverse the journal
        $updatesql = $dbObj->queryUpdate("UPDATE `" . ERP_GRN_PAYMENTS . "` SET `status`='reverse' WHERE `payment_id`=$payment_id");
        if ($updatesql['affectedRows'] > 0) {
    ...
    function reverseCollection($value)
    {
        ...
        //Reverse the journal
        $updatesql = $dbObj->queryUpdate('UPDATE `' . ERP_BRANCH_SALES_ORDER_PAYMENTS . '` SET `status`="reverse" WHERE `payment_id`=' . $payment_id);
        if ($updatesql['affectedRows'] > 0) {
    ...
    private function checkGRN($value)
    {
        ...
        if ($obj["status"] == "success" && $obj['numRows'] > 0) {
            $grnCode = $obj["data"]["grnCode"];
            $prodCheckSql = "SELECT lg.refNumber as prodCode FROM `erp_inventory_stocks_log` as lg WHERE lg.companyId=$this->company_id AND lg.logRef='" . $grnCode . "' AND lg.refActivityName='PROD-OUT'";
            $prodCheckRes = $dbObj->queryGet($prodCheckSql, true);
            $prodCheckNumRows = $prodCheckRes['numRows'] ?? 0;
            if ($prodCheckNumRows > 0) {
                // logic for prod active check
                // foreach ($prodCheckRes['data'] as $data) {
                ...
                return [
                    "status" => "warning",
                    "message" => "Production is Already done for this GRN, Could not Reversed",
                ];
                exit();
            }
        }
    ...
    function reverseGRN($value)
    {
        ...
        //GRN Status change
        $updatesql = $dbObj->queryUpdate('UPDATE `erp_grn` SET `grnStatus`="reverse" WHERE `grnId`=' . $grnId);
        if ($updatesql['affectedRows'] > 0) {
            if ($pending_grn_id != 0) {
            ...
            if ($grnType == 'grn') {
                ...
                foreach ($stock_batch_items as $oneItem) {
                    ...
                    calculateReverseMwp($itemId, $itemqty, $itemPrice, "grnrev");
            ...
            if ($journalObj["status"] == 'success') {
                ...
                if ($returnDataAcc['status'] == 'success') {
                    ...
                    //Journal Status change
                    $dbObj->queryUpdate('UPDATE `' . ERP_ACC_JOURNAL . '` SET `reverse_jid`=' . $newJournalId . ' WHERE `id`=' . $journal_id);
                    $dbObj->queryUpdate('UPDATE `erp_grn` SET `grnPostingJournalId` = 0,`reverse_grnPostingJournalId` = ' . $newJournalId . ' WHERE `grnId`=' . $grnId);
                } else {
    ...
    function reverseGRNIV($value)
    {
        ...
        $journal_id = $checkObj["journal_id"];
        $type = strtolower(substr($grnIvCode, 0, 5));
        $reverseRefCode = "REV" . $grnIvCode;

        //Reverse the journal

        $updatesql = $dbObj->queryUpdate('UPDATE `erp_grninvoice` SET `grnStatus`="reverse" WHERE `grnIvId`=' . $grnIvId);
        ...
        if ($updatesql['affectedRows'] > 0) {
            ...
            if ($journalObj["status"] == 'success') {
                ...
                if ($returnDataAcc['status'] == 'success') {
                    ...
                    $logAccFailedResponce = $this->failedAccController->logAccountingFailure($grnId, $type);
                }
            }
            //GRN Status change
    ...
    function reverseDelivery($value)
    {
        ...
        $stocks_log = $dbObj->queryGet("SELECT `storageLocationId`,`parentId`,`storageType`, `itemId`, -1 * `itemQty` AS `itemQty`, `itemUom`, `itemPrice`, `refActivityName`, `logRef`, `refNumber`, `bornDate`, `postingDate` FROM `erp_inventory_stocks_log` WHERE `refActivityName`='DELIVERY' AND `refNumber`='$delCode' AND status='active'", true);
    ...
    function reversePGI($value)
    {
        ...
        $stocks_log = $dbObj->queryGet("SELECT `storageLocationId`,`parentId`,`storageType`, `itemId`, -1 * `itemQty` AS `itemQty`, `itemUom`, `itemPrice`, `refActivityName`, `logRef`, `refNumber`, `bornDate`, `postingDate` FROM `erp_inventory_stocks_log` WHERE `refActivityName`='PGI' AND `refNumber`='$pgiCode' AND status='active'", true);
    ...
    function checkInvoice($value)
    {
        ...
        $invId = $obj["data"]["so_invoice_id"];
        $sqlEinv = "SELECT inv.id,inv.ack_no,inv.company_id FROM `erp_e_invoices` AS inv WHERE inv.company_id=$this->company_id AND inv.invoice_id=$invId ORDER BY inv.id DESC";
        $resEinv = $dbObj->queryGet($sqlEinv);
        if ($resEinv["status"] == "success" && $resEinv['numRows'] > 0) {
            return [
                "status" => "warning",
                "message" => "E-Invoice is already generated. You can't reverse it.",
            ];
            exit();
        }
    ...
    function reverseInvoice($value)
    {
        ...
        $stocks_log = $dbObj->queryGet("SELECT `storageLocationId`,`parentId`,`storageType`, `itemId`, -1 * `itemQty` AS `itemQty`, `itemUom`, `itemPrice`, `refActivityName`, `logRef`, `refNumber`, `bornDate`, `postingDate` FROM `erp_inventory_stocks_log` WHERE `refActivityName`='INVOICE' AND `refNumber`='$invoiceCode' AND `parentId`=$invoiceId AND status='active'", true);

        $check = 0;
        $checkMsg = '';
        $messages = [];
        foreach ($stocks_log["data"] as $row) {
            $itemId = $row["itemId"];
        ...
        foreach ($stocks_log['data'] as $oneItem) {
            ...
            $vClass = fetchValuationByItemId($goodId);
            if ($vClass == 'v') {
                calculateNewMwp($goodId, $receivedQty, $unitPrice, "invrev");
            }
        ...
        $updateqry = $dbObj->queryUpdate('UPDATE `' . ERP_BRANCH_SALES_ORDER_INVOICES . '` SET `status`="reverse",`updated_at` = "'.$currentDateTime.'", `updated_by` ="'. $this->updated_by .'" WHERE `so_invoice_id`=' . $invoiceId);
        if ($updateqry['affectedRows'] > 0) {
            ...
            if ($journalObj["status"] == 'success') {
                ...
                if ($returnDataAcc['status'] == 'success') {
                    ...
                    //Journal Status change
                    $dbObj->queryUpdate('UPDATE `' . ERP_ACC_JOURNAL . '` SET `reverse_jid`=' . $newJournalId . ' WHERE `id`=' . $journal_id);
                    $dbObj->queryUpdate('UPDATE `' . ERP_BRANCH_SALES_ORDER_INVOICES . '` SET `rev_inv_journal_id`=' . $newJournalId . ' ,`journal_id`=0 WHERE `so_invoice_id`=' . $invoiceId);
                } else {
                    $isFailedInv = true;
                }
            }
            ...
            if (!empty($pgi_journal_id)) {
                ...
                if ($journalObj["status"] == 'success') {
                    ...
                    if ($returnDataAcc['status'] == 'success') {
                        ...
                        //Journal Status change
                        $dbObj->queryUpdate('UPDATE `' . ERP_ACC_JOURNAL . '` SET `reverse_jid`=' . $newJournalId . ' WHERE `id`=' . $journal_id);
                        $dbObj->queryUpdate('UPDATE `' . ERP_BRANCH_SALES_ORDER_INVOICES . '` SET `rev_pgi_journal_id`=' . $newJournalId . ',`pgi_journal_id`=0  WHERE `so_invoice_id`=' . $invoiceId);
                        if ($pgi_id) {
                            $dbObj->queryUpdate("UPDATE " . ERP_BRANCH_SALES_ORDER_DELIVERY_PGI . " SET `status` = 'reverse' WHERE `so_delivery_pgi_id` = " . $pgi_id);
                        }
                    } else {
                        $isFailedPgi = true;
                    }
            ...
            if (!empty($pgi_id) || $pgi_id != 0) {
                $sql = "UPDATE `erp_branch_sales_order_delivery_pgi` SET `invoiceStatus` = 9, `pgiStatus` = 'open', `journal_id` = {$newpgiJournalId}, `updated_by` = '{$this->updated_by}' WHERE `so_delivery_pgi_id` = {$pgi_id}";

                $dbObj->queryUpdate($sql);
            }
            $invdetails = queryGet("SELECT * FROM `erp_branch_sales_order_invoices` WHERE `so_invoice_id`=$invoiceId AND`company_id`=$company_id")['data'];
            $customerId = $invdetails['customer_id'];
            $invNo = $invdetails['invoice_no'];
            $creditPeriod = $invdetails['credit_period'];
            $invoice_date = $invdetails['invoice_date'];
            $totalItems = $invdetails['totalItems'];
            $subTotal = $invdetails['sub_total_amt'];
            $totalDiscount = $invdetails['totalDiscount'];
            $totalTaxAmt = $invdetails['total_tax_amt'];
            ...
            $auditTrail['basicDetail']['document_id'] = $invoiceId;  // primary key
            ...
            $auditTrail['basicDetail']['action_title'] = 'Invoice Reversed ';  //Action comment
            ...
            if (($isFailedInv || $isFailedPgi)) {
                $logAccFailedResponce = $this->failedAccController->logAccountingFailure($invoiceId, "inv");
            }
    ...
    private function checkProdDeclaration($value)
    {
        // $dbObj = new Database();
        // $objLast = $dbObj->queryGet("SELECT * FROM `erp_production_declarations` WHERE `location_id`=" . $this->location_id . " AND `id`=$value AND `status`!='reverse'");
        // return $objLast;
    ...
    function reverseDebitNote($value)
    {
        ...
        foreach ($stocks_log['data'] as $oneItem) {
            ...
            $vClass = fetchValuationByItemId($goodId);
            if ($vClass == 'v') {
                calculateNewMwp($goodId, $receivedQty, $unitPrice, "dnrev");
            }else{
                summeryDirectStockUpdateByItemId($goodId,$receivedQty,"+");
            }
        ...
        $updatesql =  $dbObj->queryUpdate('UPDATE `erp_debit_note` SET `status`="reverse" WHERE `dr_note_id`=' . $dr_note_id);
        if ($updatesql['affectedRows'] > 0) {
            //Reverse the journal
            $revAcc = [];
            //Account reverse for PGI insert with REVERSEINV000001 reference---------------------------------------------------------
            if ($journal_id > 0) {
                ...
                if ($journalObj["status"] == 'success') {
                    ...
                    if ($returnDataAcc['status'] == 'success') {
                        ...
                        //Journal Status change
                        $dbObj->queryUpdate('UPDATE `' . ERP_ACC_JOURNAL . '` SET `reverse_jid`=' . $newJournalId . ' WHERE `id`=' . $journal_id);
                        $dbObj->queryUpdate('UPDATE `erp_debit_note` SET  `journal_id`=0,`reverse_journal_id`=' . $newJournalId . ' WHERE `dr_note_id`=' . $dr_note_id);
                    }
                }



                // $dbObj->queryUpdate('UPDATE `erp_debit_note` SET `status`="reverse", `journal_id`=' . $newJournalId . ' WHERE `dr_note_id`=' . $dr_note_id);
            }
            ...
            if ($revAcc['status'] == 'success' && $revGoodsAcc['status'] == 'success') {
                // $dbObj->queryUpdate('UPDATE `erp_debit_note` SET `status`="reverse" WHERE `dr_note_id`=' . $dr_note_id);
                ...
                $logAccFailedResponce = $this->failedAccController->logAccountingFailure($debitNoteId, "debitNote");
            }
    ...
    function reverseCreditNote($value)
    {
        ...
        $stocks_log = $dbObj->queryGet("SELECT `storageLocationId`,`parentId`, `storageType`, `itemId`, -1 * `itemQty` AS `itemQty`, `itemUom`, `itemPrice`, `refActivityName`, `logRef`, `refNumber`, `bornDate`, `postingDate` FROM `erp_inventory_stocks_log` WHERE `refActivityName`='CN' AND `refNumber`='$credit_note_no' AND status='active'", true);
        ...
        foreach ($stocks_log['data'] as $oneItem) {
            ...
            $vClass = fetchValuationByItemId($goodId);
            if ($vClass == 'v') {
            calculateReverseMwp($goodId,abs($receivedQty),$unitPrice,"cnrev");
            }else{
                summeryDirectStockUpdateByItemId($goodId,abs($receivedQty),"-");
            }
        ...
        $updatesql = $dbObj->queryUpdate('UPDATE `erp_credit_note` SET `status`="reverse" WHERE `cr_note_id`=' . $cr_note_id);
        if ($updatesql['affectedRows'] > 0) {
            // cn accounting
            $newJournalIdAcc = 0;
            $revAcc = [];
            ...
            if ($revAcc['status'] == 'success' && $revGoodsAcc['status'] == 'success') {
                $dbObj->queryUpdate('UPDATE `erp_credit_note` SET `status`="reverse", `journal_id` = 0, `goods_journal_id` = 0, `reverse_journal_id` = ' . $newJournalIdAcc . ', `reverse_goods_journal_id` = ' . $newGoodsJournalId . ' WHERE `cr_note_id`=' . $cr_note_id);
                $creditsql = $dbObj->queryGet("SELECT * FROM `erp_credit_note` WHERE cr_note_id=$cr_note_id AND company_id=$company_id")['data'];
                $creditor_type = $creditsql['creditors_type'];
                $party_id = $creditsql['party_id'];
                $remark = $creditsql['remark'];
                $party_name = $creditsql['party_name'];
                $party_code = $creditsql['party_code'];
                $posting_date = $creditsql['postingDate'];
                $subtotal = $creditsql['total'];
                $createdby = $creditsql['created_by'];
                $taxComponents = $creditsql['taxComponents'];
                $taxComponent_tax = json_decode($taxComponents, true);
                $currentTime = date("Y-m-d H:i:s");
                $action_code = time() . rand(11, 99) . rand(11, 99) . rand(11, 9999);
                $auditTrail = array();
                ...
                $auditTrail['basicDetail']['document_id'] = $cr_note_id;  // primary key
                $auditTrail['basicDetail']['party_type'] = $creditor_type;
                $auditTrail['basicDetail']['party_id'] = $party_id;
                $auditTrail['basicDetail']['action_code'] = $action_code;
                $auditTrail['basicDetail']['document_number'] = $credit_note_no;
                $auditTrail['basicDetail']['action_referance'] = '';
                $auditTrail['basicDetail']['action_title'] = 'Credit Note Reversed';  //Action comment
                $auditTrail['basicDetail']['action_name'] = 'Add';     //	Add/Update/Deleted
                $auditTrail['basicDetail']['action_type'] = 'Non-Monitory'; //Monitory/Non-Monitory
                $auditTrail['basicDetail']['action_url'] = BASE_URL . $_SERVER['REQUEST_URI'];
                $auditTrail['basicDetail']['action_previous_url'] = $_SERVER['HTTP_REFERER'];
                ...
                $auditTrail['action_data']['Credit Note Detail']['created_by'] = getCreatedByUser($createdby);
                $auditTrail['action_data']['Credit Note Detail']['created_by'] = getCreatedByUser($createdby);
                $auditTrail['action_data']['Credit Note Detail']['updated_by'] = getCreatedByUser($createdby);
                $auditTrail['action_data']['Credit Note Detail']['updated_by'] = getCreatedByUser($createdby);

                $auditTrail['action_data']['Reverse Details']['Reversed By'] = getCreatedByUser($created_by);
                $auditTrail['action_data']['Reverse Details']['Reversed By'] = formatDateTime($currentTime);

                $auditTrailreturn = generateAuditTrail($auditTrail);
            } else {
                $logAccFailedResponce = $this->failedAccController->logAccountingFailure($creditNoteId, "debitNote");
            }
