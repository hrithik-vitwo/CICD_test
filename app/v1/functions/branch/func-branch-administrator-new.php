...
function new_administratorAuth()
{
    global $dbCon;
        ...
        if ($adminType == "branch") {
            if ($adminRole == 1) {
                if (!in_array($currentPage, $commonAccessPages)) {
                    $sqlCheckAccessableFile = "SELECT `fldMenuFile` FROM `tbl_branch_admin_menu_new` WHERE  `menuFor` ='" . $adminType . "' AND `fldMenuStatus`='active' AND `fldMenuFile`='" . $currentPage . "'";
                ...
                if (!in_array($currentPage, $commonAccessPages)) {
                    ...
                    $sqlCheckAccessableFile = "SELECT `fldMenuFile` FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' AND `fldMenuFile`='" . $currentPage . "'";
...
function new_administratorLocationAuth()
{
    global $dbCon;
    $commonAccessPages = ["index.php", "empty-page.php", "empty-form.php"];
    if (!isset($_SESSION["logedBranchAdminInfo"]["adminId"]) || !isset($_SESSION["logedBranchAdminInfo"]["adminRole"])) {
...
function new_getAdministratorMenuSubMenu()
{
    global $dbCon;
    $returnData = [];
    $accesses = "";
    ...
    if ($adminRole == 1 && $adminType == 'branch') {
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`!='-0' AND `menuFor`='branch' AND `fldMenuStatus`='active'";
        ...
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`!='-0' AND `menuFor`='location' AND `fldMenuStatus`='active'";
        if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
            $accessesArr = mysqli_fetch_all($qryAccesses, MYSQLI_ASSOC);
            $accesses = implode(",", array_column($accessesArr, 'fldMenuKey'));
        }
    ...
    $sqlgrandParentMenuKeys = "SELECT `fldGrandParentMenu` FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' GROUP BY `fldGrandParentMenu`;";
    ...
    $parentMenuKeysArr = [];
    $sqlParentMenuKeys = "SELECT `fldParentMenuKey` FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' GROUP BY `fldParentMenuKey`;";
    ...
    $sqlGrandMenuList = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $grandParentMenuKeys . ") AND `fldMenuStatus`='active' ORDER BY `fldMenuOrderBy` ASC";
    if ($qryGrandMenuList = mysqli_query($dbCon, $sqlGrandMenuList)) {
        if (mysqli_num_rows($qryGrandMenuList) > 0) {
            ...
            while ($rowGrandMenuList = mysqli_fetch_assoc($qryGrandMenuList)) {
                ...
                $sqlMenuList = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $parentMenuKeys . ") AND `fldMenuStatus`='active' ORDER BY `fldMenuOrderBy` ASC";
                if ($qryMenuList = mysqli_query($dbCon, $sqlMenuList)) {
                    if (mysqli_num_rows($qryMenuList) > 0) {
                        ...
                        while ($rowMenuList = mysqli_fetch_assoc($qryMenuList)) {
                            ...
                            $sqlSubMenuList = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' AND `fldParentMenuKey`=" . $parentMenuKey . " AND `fldGrandParentMenu`=" . $grandParentMenuKey . " ORDER BY `fldMenuOrderBy` ASC";
...
function new_checkAccess($BtnName)
{
    global $dbCon;
    ...
    if ($adminRole != '1' && $adminRole != '2') {
        $sql = "SELECT `fldMenuKey` FROM `tbl_branch_admin_menu_new` WHERE 1 AND `fldGrandParentMenu`!='' AND `fldParentMenuKey`!='0' AND `fldParentMenuKey`!='-0' AND `fldMenuFile`='" . $pageName . "' AND `menuFor`='" . $adminType . "'";
...
function new_ForgotAdministratorUser($POST)
{
    global $dbCon;
    $returnData = [];
...
function new_VisitBranchesUserLogout(){
    global $dbCon;
    $returnData=[];
...
function new_VisitLocationsUserLogout(){
    global $dbCon;
    $returnData=[];
...
function new_loginAdministratorUser($POST)
{
    global $dbCon;
    $returnData = [];
    $username = $POST["email"];
...
function new_saveAdministratorSettings($POST)
{
    global $dbCon;
    $returnData = [];
...
function new_getAdministratorSettings($settingName = "")
{
    global $dbCon;
...
function new_addAdministratorMenu($POST)
{
    global $dbCon;
    ...
    $ins = "INSERT INTO `tbl_branch_admin_menu_new`
                SET
...
function new_updateAdministratorMenu($POST)
{
    global $dbCon;
    ...
    if ($res = mysqli_query($dbCon, "SELECT COUNT(`fldMenuLabel`) AS 'noOfRecords' FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey`=" . $POST["menuKey"])) {
        if (mysqli_fetch_assoc($res)["noOfRecords"] == 1) {
            $ins = "UPDATE `tbl_branch_admin_menu_new`
                SET
                    `fldMenuLabel` = '" . $menuLabel . "',
                    `fldMenuIcon` = '" . $menuIcon . "',
                    `fldMenuFile` = '" . $menuFile . "',
                ...
                WHERE 
                    `fldMenuKey`=" . $menuKey;
            if (mysqli_query($dbCon, $ins)) {
                $ins2 = "UPDATE `tbl_branch_admin_menu_new`
                    SET
                        `fldGrandParentMenu` = '" . $fldGrandParentMenu . "'
                    WHERE 
...
function new_updateAdministratorSubMenu($POST)
{
    global $dbCon;
    $menuKey = $POST["menuKey"];
...
function new_addAdministratorSubMenuAccess($accessaArr, $parentMenuKey, $menuFile)
{
    global $dbCon;
    foreach ($accessaArr as $key => $value) {
        if (!empty($value[0])) {
            $inss = "UPDATE `tbl_branch_admin_menu_new_access`
                SET
            ...
            $inss = "INSERT INTO `tbl_branch_admin_menu_new_access`
                    SET
                        `fldMenuLabel` = '" . $value[1] . "',
                        `fldMenuFile` = '" . $menuFile . "',
                        `fldParentMenuKey` = '" . $parentMenuKey . "'";
...
function new_addAdministratorSubMenu($POST)
{
    global $dbCon;
    $menuKey = $POST['menuKey'];
...
function new_getAdministratorMenuList($parentMenuKey = '0')
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`='" . $parentMenuKey . "' AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
...
function new_getAdministratorMenuListNew($parentMenuKey = '0', $menufor=null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`='" . $parentMenuKey . "' AND `menuFor`='" . $menufor . "' AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function new_getAdministratorGrandMenuList($parentMenuKey, $grandparentMenuKey)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldParentMenuKey`='" . $parentMenuKey . "' AND `fldGrandParentMenu`='" . $grandparentMenuKey . "' AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function new_getAdministratorSubMenuListAccess($parentMenuKey = null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu_new_access` WHERE `fldParentMenuKey`=" . $parentMenuKey . " AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function new_getAdministratorMenuDetails($menuKey = '0')
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey`=" . $menuKey;
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
// Administrator Role
function new_addNewAdministratorRole($POST = [])
{
    global $dbCon;
    $returnData = [];
    $isValidate = validate($POST, [
...
function new_updateAdministratorRole($POST = [])
{
    global $dbCon;
    $returnData = [];
    $isValidate = validate($POST, [
...
function new_getAdministratorRoleDetails($key = null)
{
    global $dbCon;
    $returnData = [];
...
function new_getAllAdministratorRoles()
{
    global $dbCon;
    $returnData = [];
    $fldRoleCompanyId = $_SESSION["logedBranchAdminInfo"]["fldAdminCompanyId"];
...
// Administrator User
function new_getAllAdministratorUsers()
{
    global $dbCon;
...
function new_getAdministratorUserDetails($key = null)
{
    global $dbCon;
...
function new_updateAdministratorUserDetails($POST)
{
    global $dbCon;
    $returnData = [];
    $isValidate = validate($POST, [
...
function new_addNewAdministratorUser($POST = [])
{
    global $dbCon;
    $returnData = [];
    $isValidate = validate($POST, [
...
function new_getAdministratorAccessesNames($roleAccesses = "0")
{
    global $dbCon;
    $returnData = [];
    if ($roleAccesses == "0") {
        ...
        $sql = "SELECT `fldMenuLabel` FROM `tbl_branch_admin_menu_new` WHERE `fldMenuKey` IN (" . $roleAccesses . ") AND `fldMenuStatus`='active'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $returnData["status"] = "success";
                $returnData["message"] = "success";
...
function new_administratorFuncUpdateQry($table = "", $fieldsAndData = [], $conditions = "")
{
    global $dbCon;
...
function new_administratorFuncChangeStatus($data = [], $tableName = "", $tableKeyField = "", $tableStatusField = "status")
{
    global $dbCon;
