...
class GoodsController extends Accounting
{
    ...
    function createGoods($POST = [])
    {
       ...
       // console(json_encode($discountGroupArray));
        ...
        //    $admin['discountGroup'] = $discountGroup;
            ...
            $default_storage = $POST['default_storage'] ?? '';
    ...
    function createGoodsLocation($POST)
    {
     ...
     //   $bom_req = $select_summary['data']['bomStatus'] ?? 0;
        ...
        if ($insertStorage["status"] == "success") {
            ...
            $auditTrail['action_data']['Basic Detail']['parentGlId'] = $resItems['parentGlId'];
            $auditTrail['action_data']['Basic Detail']['itemCode'] =  $resItems['itemCode'];
            $auditTrail['action_data']['Basic Detail']['itemName'] = $resItems['itemName'];
            $auditTrail['action_data']['Basic Detail']['itemDesc'] = $resItems['itemDesc'];
            $auditTrail['action_data']['Basic Detail']['netWeight'] = $resItems["netWeight"];
            $auditTrail['action_data']['Basic Detail']['grossWeight'] = $resItems["grossWeight"];
            $auditTrail['action_data']['Basic Detail']['volume'] =  $resItems["volume"];
            $auditTrail['action_data']['Basic Detail']['height'] = $resItems["height"];
            $auditTrail['action_data']['Basic Detail']['width'] = $resItems["width"];
            $auditTrail['action_data']['Basic Detail']['length'] = $resItems["length"];
            $auditTrail['action_data']['Basic Detail']['goodsType'] = $resItems["goodsType"];
            $auditTrail['action_data']['Basic Detail']['goodsGroup'] = $resItems["goodsGroup"];
            $auditTrail['action_data']['Basic Detail']['purchaseGroup'] = $resItems["purchaseGroup"];
            $auditTrail['action_data']['Basic Detail']['availabilityCheck'] = $resItems["availabilityCheck"];
            $auditTrail['action_data']['Basic Detail']['baseUnitMeasure'] = $resItems["baseUnitMeasure"];
            $auditTrail['action_data']['Basic Detail']['issueUnitMeasure'] = $resItems["issueUnitMeasure"];
            $auditTrail['action_data']['Basic Detail']['tds'] = $resItems['tds'];
            $auditTrail['action_data']['Basic Detail']['cost_center'] = $resItems['costCenter'];
            $auditTrail['action_data']['Basic Detail']['bomStatus'] = $resItems['bomStatus'];
            $auditTrail['action_data']['Basic Detail']['purchasingValueKey'] = $resItems["purchasingValueKey"];
            $auditTrail['action_data']['Basic Detail']['uomRel'] = $resItems['uomRel'];
            $auditTrail['action_data']['Basic Detail']['status'] = $resItems['status'];
            $auditTrail['action_data']['Basic Detail']['hsnCode'] = $resItems['hsn'];
            $auditTrail['action_data']['Basic Detail']['volumeCubeCm'] = $resItems['volumeCubeCm'];
            $auditTrail['action_data']['Basic Detail']['weight_unit'] = $POST['net_unit'];
            $auditTrail['action_data']['Basic Detail']['measuring_unit'] = $POST['measure_unit'];
            $auditTrail['action_data']['Basic Detail']['service_unit'] = $resItems["serviceUnit"];
            ...
            $auditTrail['action_data']['Storage']['storageControl'] = $invItemsStorage["storageControl"];
            $auditTrail['action_data']['Storage']['maxStoragePeriod'] = $invItemsStorage["maxStoragePeriod"];
            $auditTrail['action_data']['Storage']['maxStoragePeriodTimeUnit'] = $invItemsStorage["maxStoragePeriodTimeUnit"];
            $auditTrail['action_data']['Storage']['minRemainSelfLife'] = $invItemsStorage["minRemainSelfLife"];
            $auditTrail['action_data']['Storage']['minRemainSelfLifeTimeUnit'] = $invItemsStorage["minRemainSelfLifeTimeUnit"];


            $auditTrail['action_data']['Summary']['itemPrice'] = $price["price"];
            $auditTrail['action_data']['Summary']['priceType'] = 'target';
            $auditTrail['action_data']['Summary']['itemMaxDiscount'] = $price["discount"];
    ...
    function getAllHsnPegination($POST)
    {
        ...
        $selectSql = "SELECT * FROM " . ERP_HSN_CODE . " WHERE 1 " . $cond . " AND (isPublic=0 OR isPublic='" . $company_id . "') ORDER BY hsnCode  ASC LIMIT " . $start . "," . $end . " ";
                        ...
                        if ($itemOpenStocks >= $qty) {
                            ...
                            GROUP BY
                                ...
                                else
                                {
                                    ...
                                    //price Difference
                                    $price_diff = ($from_quantity * $from_price) - ($to_quantity * $to_price);
                                ...
                                {
                                    $goodStockInserSql = 'UPDATE `erp_inventory_stocks_summary` SET `itemTotalQty`=' . $destination_qty . ', `movingWeightedPrice`=' . $movingWeightedPrice . ' ,`updatedBy`="' . $updated_by . '" WHERE `company_id`=' . $company_id . ' AND `branch_id`=' . $branch_id . ' AND `location_id`=' . $location_id . ' AND `itemId`=' . $destination_item_id;
    ...
    function asset_use($POST)
    {
        ...
        if($POST['stock_log_id'] == ' ' || $POST['stock_log_id'] == null){
            $stock_log_id = 0;
        }
        else{
            $stock_log_id = $POST['stock_log_id'];
        }
        ...
        $log = queryGet("SELECT * FROM `erp_inventory_stocks_log` WHERE `stockLogId`=$stock_log_id");
        $rem = $log['data']['remainingQty'] - $qty;
        //  exit();
        $update_log = queryUpdate("UPDATE `erp_inventory_stocks_log` SET `remainingQty`=$rem WHERE `stockLogId`=$stock_log_id");


        $sql = "INSERT INTO `erp_asset_use` SET `lot_no`='" . $lot . "',`rcv_date`='" . $rcvDate . "',`asset_id`=$asset_id,`asset_code`= '" . $assetCode . "',`assetName`= '" . $assetName . "',`qty`=$qty,`uom`=$uom,`rate`=$rate,`total_value`=$total,`use_date`='" . $use_date . "',`stock_log_id`='".$stock_log_id."',`scrap_rate`='" . $scrap . "',`scrap_value`='" . $scrap_val . "',`depreciation_amount`=0,`depreciated_asset_value`='" . $total . "',`dep_percentage`='" . $dep_percentage . "',`company_id`=$company_id,`location_id`=$location_id,`branch_id`=$branch_id,`depreciation_schedule`='" . $dep_schedule . "',`created_by`='" . $created_by . "',`updated_by`='" . $updated_by . "'";
        $returnData = queryInsert($sql);
        // console($returnData);
        // exit();
        return $returnData;
    ...
    function createManualDepreciation($POST)
    {
        ...
        foreach ($POST['asset'] as $asset) {
            // console($asset);
            $buying_val = $asset['buying_val'];
            $asset_use_id = $asset['asset_use_id'];
            $dep_on_val = $asset['dep_on_val'];
            $asset_id = $asset['asset_id'];
            $dep_rate = $asset['dep_rate'];
            $dep_val = $asset['dep_val'];
            $asset_new_val = $asset['asset_new_val'];
            $scrap = $asset['scrap'];
            $tody = date('Y-m-d');
            $posting_date = $asset['posting_date'] ?? $tody;
            ...
            $depInputData = [
                "BasicDetails" => [
                    ...
                    "remarks" => "Asset depreciation for - " . $assUsesql['assetName'] . " (" . $assUsesql['assetCode'] . ")",
