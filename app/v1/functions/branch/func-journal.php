...
class Accounting
{
  ...
  //Modifed By Nitesh 
  function grnIvAccountingPosting2($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
    ...
    if ($isValidate["status"] == "success") {
      ...
      } else if (!empty($imputgst['sgst']) && ($imputgst['sgst'] > 0) || !empty($imputgst['cgst']) && ($imputgst['cgst'] > 0)) {
        //-------------------------CGST------------------------
        $totalamount += $imputgst['cgst'];
      ...
      if (!empty($imputgst['gst']) && ($imputgst['gst'] > 0)) {
        $totalamount += $imputgst['gst'];
        $accounting['debit'][] = [
          'glId' => $debitAccList[1]['id'],
          'subGlCode' => '',
          'subGlName' => '',
          'debit_amount' => $imputgst['gst'],
          'debit_remark' => 'GST Amount'
        ];
      }
  ...
  function srnIvAccountingPosting2($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
    ...
    if ($isValidate["status"] == "success") {
      ...
      } else if (!empty($imputgst['cgst'] && !empty($imputgst['sgst']))) {
        //-------------------------CGST------------------------
        $totalamount += $imputgst['cgst'];
        $totalgst += $imputgst['cgst'];
  ...
  //PGI AccountingPosting
  function pgiAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
    ...
    if ($isValidate["status"] == "success") {
      ...
      foreach ($pgiItems  as $crkey => $pgivalue) {
        ...
        $withouttaxamount = $pgivalue['unitPrice'] * $pgivalue['itemQty'];
  ...
  function collectionAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    ...
    if ($isValidate["status"] == "success") {
      ...
      if ($inputes['type'] == 'vendor') {
        $party_code = $customerDetails['vendor_code'] ?? "";
  ...
  function multicollectionAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    ...
    if ($isValidate["status"] == "success") {
      ...
      foreach ($paymentDetails as $vendKey => $paymentDetails) {
        ...
        $tdsGlId=queryGet("SELECT * FROM `" . ERP_ACC_CHART_OF_ACCOUNTS . "` WHERE company_id=$company_id AND gl_label='TDS Receivable'")['data']['id'];
        ...
        foreach ($crItems as $crkey => $crvalue) {
          ...
          $tds = $crvalue["tds"] ?? 0;
          $totaltds += $tds;
          $onlytds=$crvalue['onlytds'];
          $amt=0;
          // First Entry: Credit for payment items
          if($onlytds==0)
          {
            $amt=$tds+ $crvalue['recAmt'];
          }else{
            $amt= $crvalue['recAmt'];
          }
          $accounting['credit'][] = [
        ...
        $totaldebitamount = $dramount + $roundOffValue + $writeoff + $financialCharge + $forex;
        if($onlytds==0){
        $accounting['debit'][] = [
          'glId' => $queryBankCash['parent_gl'],
          'subGlCode' => $queryBankCash['acc_code'],
        ...
        if (!empty($tdsGlId) && $totaltds != 0) {
          
            $accounting['debit'][] = [
              'glId' => $tdsGlId,
              'subGlCode' => '',
              'subGlName' => '',
              'debit_amount' => abs($totaltds),
              'debit_remark' => 'TDS'
            ];
  ...
  function paymentAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    ...
    if ($isValidate["status"] == "success") {
      ...
      if ($inputes['type'] == 'vendor') {
        ...
        $party_gl = $vendorDetails['parentGlId'];
      } else {
        $party_name = $vendorDetails['trade_name'] ?? "";
        $party_code = $vendorDetails['customer_code'] ?? "";
        $party_gl = $vendorDetails['parentGlId'];
      }
  ...
  function depreciationAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    ...
    if ($isValidate["status"] == "success") {
      ...
      $accounting['credit'][] = [
        'glId' => $creditAccList[0]['id'],
        'subGlCode' => $assetDetail['itemCode'],
        'subGlName' => $assetDetail['itemName'],
        'credit_amount' => $assetDetail['amount'],
        'credit_remark' => ''
      ];
      $accounting['debit'][] = [
        'glId' => $debitAccList[0]['id'],
  ...
  function stockDifferenceBookToPhysicalAccountingPosting($inputes, $functionSlug, $parent_id, $type=null)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
    ...
    if ($isValidate["status"] == "success") {
      ...
      foreach ($consumpProductData  as $crkey => $value) {
        ...
        if($price_difference>0)
        {
          ...
          $totalamount = $price + abs($price_difference);
        }
            //100-500=
  ...
  function dNoteForCustomerAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    ...
    if ($isValidate["status"] == "success") {
      ...
      if (isset($inputes['tcsDetails']) && !empty($inputes['tcsDetails'])) {
        $tcstotal = ($inputes['tcsDetails']);

        $accounting['credit'][] = [
          'glId' => $tcsgl,
          'subGlCode' => '',
          'subGlName' => '',
          'credit_amount' => $tcstotal,
          'credit_remark' => 'TCS Amount'
        ];
        $totalamount += $tcstotal;
      }
  ...
  function dNoteForCustomerAccountingPosting_by_rule_book($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
    ...
    if ($isValidate["status"] == "success") {
      ...
      $gstgl = $creditAccList[1]['id'];
      $tcsgl = $creditAccList[2]['id'];
  ...
  function dNoteForVendorAccountingPosting($inputes, $functionSlug, $parent_id)
  {
    ...
    if ($isValidate["status"] == "success") {
      ...
      $accounting2 = array();
      $accounting2['journal']['parent_id'] = $parent_id;
      $accounting2['journal']['parent_slug'] = $goodSlags;
      $accounting2['journal']['refarenceCode'] = $accountingRefNo;
      $accounting2['journal']['remark'] = $accountingRemarks;
      $accounting2['journal']['party_code'] = $party_code;
      $accounting2['journal']['party_name'] = $party_name;
      ...
      if (isset($inputes['tdsDetails']) && !empty($inputes['tdsDetails'])) {
        ...
        $accounting['debit'][] = [
          ...
          'subGlCode' => '',
          'subGlName' => '',
          'debit_amount' => $tdstotal,
          'debit_remark' => 'TDS Amount'
        ];
        $totalamount -= $tdstotal;
      }
  ...
  function dNoteForVendorAccountingPosting_by_rule_book($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
 ...
 $goodSlags = $functionSlug . 'Goods';
    if ($isValidate["status"] == "success") {
      ...
      $gstgl = $grndebitAccList[1]['id'];
      $tcsGlId = $grndebitAccList[2]['id'];
      $tdsGlId = $grncreditAccList[1]['id'];
 ...
 $goodSlags = $functionSlug . 'Goods';
  ...
  function cNoteForCustomerAccountingPosting_by_rule_book($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
  ...
  function cNoteForVendorAccountingPosting_by_rule_book($inputes, $functionSlug, $parent_id)
  {
    global $company_id;
    global $branch_id;
    global $location_id;
