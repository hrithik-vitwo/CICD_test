...
class AccountingPosting
{
  ...
  function getGLById($id){
    global $company_id;
    $sql = "SELECT * FROM `" . ERP_ACC_CHART_OF_ACCOUNTS . "` WHERE company_id=$company_id AND id=$id";
  ...
  function post($data = [])
  {
    ...
    foreach ($data["credit"] as $creditData) {
      ...
      $glcName=$this->getGLById( $creditData['glId']);
      $auditTrail['action_data']['Credit Details'][$glcName['gl_label']]['Ledger Name'] = $glcName['gl_label'];
      $auditTrail['action_data']['Credit Details'][$glcName['gl_label']]['Sub Ledger'] = addslashes($creditData['subGlName']);
      $auditTrail['action_data']['Credit Details'][$glcName['gl_label']]['Credit Amount'] = $creditData['credit_amount']; 
    }
    ...
    foreach ($data["debit"] as $debitData) {
      ...
      $glName=$this->getGLById($debitData['glId']);
      $auditTrail['action_data']['Debit Details'][ $glName['gl_label']]['Ledger'] = $glName['gl_label'];
      $auditTrail['action_data']['Debit Details'][ $glName['gl_label']]['Sub Ledger'] = addslashes($debitData['subGlName']);
      $auditTrail['action_data']['Debit Details'][ $glName['gl_label']]['Debit Amount'] = $debitData['debit_amount']; 
    }
    ...
    if ($accPostingObj["status"] == "success") {
      ...
      $accPostingObj["accPostingObj"]=$glName;
    }
...
// try {
//   $accObj = new AccountingPosting();
//   echo "<pre>";
//   print_r($accObj->post(["dcuhh"]));
//   echo "</pre>";
// } catch (Exception $e) {
//   echo "Error";
// }