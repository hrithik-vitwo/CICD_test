...
function debit_note_add($POST)
{
  ...
  $adjustment = $POST['round_sign'] . '' . ($POST['round_value'] ?? 0);
  ...
  $insert_debit_sql = "INSERT INTO  `erp_debit_note`
                      SET
  ...
  $insert_debit = queryInsert($insert_debit_sql);
  if ($insert_debit['status'] == "success") {
    // $invoice_current_dueObj = queryGet("SELECT * FROM `erp_branch_sales_order_invoices` WHERE so_invoice_id=$parent_id")['data'];
    // $invoice_current_due = $invoice_current_dueObj['due_amount'];
    // if ($subtotal+$invoice_current_due < $invoice_current_dueObj['all_total_amt'] ) {
    //   $update_due = queryUpdate("UPDATE `erp_branch_sales_order_invoices` SET `due_amount`=$invoice_current_due+$subtotal,`invoiceStatus` = 3 WHERE  `so_invoice_id` =$parent_id");
    // }
    // exit();
    ...
    $auditTrail['basicDetail']['document_id'] = $insert_debit['insertedId'];  // primary key
    ...
    $auditTrail['basicDetail']['action_title'] = 'Debit Note Add';  //Action comment
    ...
    $auditTrail['basicDetail']['action_sqlQuery'] = base64_encode($insert_debit_sql);
    ...
    $auditTrail['action_data']['Debit Note Detail']['postingDate'] = $posting_date;
    $auditTrail['action_data']['Debit Note Detail']['source_address'] = getStateDetail($source_address)["data"]['GstStateName'];
    $auditTrail['action_data']['Debit Note Detail']['destination_address'] = getStateDetail($destination_address)['data']['GstStateName'];
    $auditTrail['action_data']['Debit Note Detail']['billing_address'] = getStateDetail($bill_address)['data']['GstStateName'];
    $auditTrail['action_data']['Debit Note Detail']['shipping_address'] = getStateDetail($ship_address)['data']['GstStateName'];
    // $auditTrail['action_data']['Debit Note Detail']['contact_details'] = $contactDetails;
    $auditTrail['action_data']['Debit Note Detail']['total'] = decimalValuePreview($subtotal);
    // $auditTrail['action_data']['Debit Note Detail']['adjustment'] = $adjustment;
    // $auditTrail['action_data']['Debit Note Detail']['attachment'] = $attachment_name;
    $auditTrail['action_data']['Debit Note Detail']['cgst'] = $cgst;
    $auditTrail['action_data']['Debit Note Detail']['igst'] = $igst;
    $auditTrail['action_data']['Debit Note Detail']['sgst'] = $sgst;
    $auditTrail['action_data']['Debit Note Detail']['created_by'] = getCreatedByUser($created_by);
    $auditTrail['action_data']['Debit Note Detail']['updated_by'] = getCreatedByUser($created_by);

    $debit_note_id = $insert_debit['insertedId'] ?? 0;
    if (isset($POST['repost']) && $POST['repost'] == 1) {
      $reverse_dn_id = $POST['reverse_dn_id'];
      $variant_id = $POST['last_iv_varient'];

      $update_reverse_dn = queryUpdate("UPDATE `erp_debit_note` SET `status`='reposted' WHERE `dr_note_id` = $reverse_dn_id");

      $debit_note_no = $POST['iv_varient'];
    ...
    $debit_note_no_unserialized = unserialize($invoice_no_serialized);
    $auditTrail['basicDetail']['document_number'] = $debit_note_no;
    $auditTrail['action_data']['Debit Note Detail']['prefix'] = $debit_note_no_unserialized['prefix'];
    $auditTrail['action_data']['Debit Note Detail']['fy'] = $debit_note_no_unserialized['fy'];
    $auditTrail['action_data']['Debit Note Detail']['serial'] = $debit_note_no_unserialized['serial'];
    ...
    foreach ($debit_items as $key => $item) {
      ...
      if ($insert_debit_items['status'] == "success") {
        ...
        $auditTrail['action_data']['Debit Note Item Detail']['item'] = $itemName['data']['itemName'];
        $auditTrail['action_data']['Debit Note Item Detail']['account'] = $accountGl['data']['gl_label'];
        $auditTrail['action_data']['Debit Note Item Detail']['item_qty'] = $qty;
        $auditTrail['action_data']['Debit Note Item Detail']['item_rate'] = $rate;
        $auditTrail['action_data']['Debit Note Item Detail']['item_tax'] = $tax;
        $auditTrail['action_data']['Debit Note Item Detail']['cgst'] = $itemcgst;
        $auditTrail['action_data']['Debit Note Item Detail']['sgst'] = $itemsgst;
        $auditTrail['action_data']['Debit Note Item Detail']['igst'] = $itemigst;
        $auditTrail['action_data']['Debit Note Item Detail']['item_amount'] = $amount;
        ...
        if ($manualbatchselectionQty > 0 && !empty($manualbatchselectionSL)) {
          ...
          $refNumber = $manualBatchNumber ?? $debit_note_no;
        ...
        $returnData['insert_debit'] = $insert_debit;
      }
    }
    ...
    if ($accPostingObj["status"] == "success" && $accPostingObj["journalId"] != "") {
      $queryObj = queryUpdate('UPDATE `erp_debit_note` SET `journal_id`=' . $accPostingObj["journalId"] . ', `goods_journal_id` = ' . $accPostingObj['goodsJournalId'] . ' WHERE `dr_note_id`=' . $debit_note_id);
    ...
    if ($mailValid == 'yes') {
      ...
      $to = $partyMail;
      $sub = 'Debit Note ' . $debit_note_no . ' for Your Recent Purchase';
      $msg = ' <div>
                 <div><strong>Dear ' . $party_name . ',</strong></div>
                  <p>
                    I hope this email finds you well. I am writing to inform you that an debit note for your recent purchase with <b>' . $company_name . '</b> has been generated and is now ready for payment.
                </p>
                 <strong>
                    Debit Note details:
                </strong>
                <div style="display:grid">
                    <span>
                        Debit Note Number: ' . $debit_note_no . '
                    </span>
                    <span>
                        Amount Due: ' . $total . '
                    </span>
                ...
                <p>
                    To make a payment, please follow the instructions included in the attached debit note. If you have any questions or need assistance with the payment process, please do not hesitate to contact our finance team.
                </p>
                <p>
                ...
                <p>
                <a href="' . BASE_URL . 'branch/location/classic-view/invoice-preview-print.php?dr_note_id=' . base64_encode($debit_note_id) . '" style="background: #174ea6;padding: 8px;color: white;text-decoration: none;border-radius: 5px;"><img width="15" src="' . BASE_URL . 'public/storage/invoice-icon/invoice.png" /> View Debit Note</a>
                
                </p>
                </div>';
      SendMailByMySMTPmailTemplate($to, $sub, $msg, null, $party_code, 'debitNote', $debit_note_id, $debit_note_no);
    }
...
function date_select($variant_id, $dr_id)
{
  global $dbCon;
  global $company_id;
  ...
  //  //return $company_id;
  //   $sql = queryGet("SELECT (SELECT MAX(postingDate) FROM erp_debit_note WHERE postingDate < (SELECT postingDate FROM erp_debit_note WHERE credit_note_no = '$credit_note_no' AND `variant_id` = $variant_id AND company_id = $company_id LIMIT 1)) AS previous_date, (SELECT MIN(postingDate) FROM erp_debit_note WHERE postingDate > (SELECT postingDate FROM erp_debit_note WHERE credit_note_no = '$credit_note_no' AND `variant_id` = $variant_id AND company_id = $company_id LIMIT 1)) AS next_date FROM erp_debit_note WHERE credit_note_no = '$credit_note_no' AND `variant_id` = $variant_id AND company_id = $company_id");

  $sql = queryGet("SELECT
        (SELECT t1.postingDate
        FROM erp_debit_note t1
        WHERE t1.company_id = $company_id AND t1.dr_note_id < $dr_id AND t1.variant_id = $variant_id
        ORDER BY t1.postingDate DESC
        LIMIT 1) AS prev_date,
        (SELECT t2.postingDate
        FROM erp_debit_note t2
        WHERE t2.company_id = $company_id AND t2.dr_note_id > $dr_id AND t2.variant_id = $variant_id
...
// tax rule function 
function debit_note_add_taxrule($POST)
{
  ...
  //     $returnData['status'] = "Warning";
  //     $returnData['message'] = "Debit note amount can not greter than invoice due amount";
  //     return $returnData;
  //     exit();
  //   }
  // }
  // if ($debitor_type == 'vendor') {
  ...
  if (
    ...
    $invoice_current_dueObj = queryGet("SELECT * FROM `erp_branch_sales_order_invoices` WHERE so_invoice_id=$parent_id");
    $cnSql = "SELECT *  FROM erp_debit_note WHERE debitNoteReference='" . $parent_id . "' AND company_id=$company_id AND branch_id=$branch_id AND location_id=$location_id AND `debitor_type`='customer' AND status='active'";
    ...
    if ($cnRes['numRows'] > 0) {
      ...
      $totalDebitNoteAmount = queryGet("SELECT SUM(total) AS totalDebitNoteAmount FROM erp_credit_note WHERE creditNoteReference='" . $parent_id . "' AND company_id=$company_id AND `debitor_type`='customer' AND branch_id=$branch_id AND location_id=$location_id GROUP BY creditNoteReference;")['data']['totalCreditNoteAmount'] ?? 0;
    }
    $invoice_current_due = $invoice_current_dueObj['data']['due_amount'];
    $invoice_current_due = $invoice_current_due - $totalDebitNoteAmount;
  ...
  if (
    ...
    $invoice_current_dueObj = queryGet("SELECT * FROM `erp_grninvoice` WHERE grnIvId=$parent_id");
    $invoice_current_due = $invoice_current_dueObj['data']['dueAmt'];
    $cnSql = "SELECT *  FROM erp_debit_note WHERE debitNoteReference='" . $parent_id . "' AND company_id=$company_id AND branch_id=$branch_id AND location_id=$location_id AND `debitor_type`='vendor' AND status='active'";
    ...
    if ($cnRes['numRows'] > 0) {
      ...
      $totalDebitNoteAmount = queryGet("SELECT SUM(total) AS totalDebitNoteAmount FROM erp_credit_note WHERE creditNoteReference='" . $parent_id . "' AND `debitor_type`='vendor' AND company_id=$company_id AND branch_id=$branch_id AND location_id=$location_id GROUP BY creditNoteReference;")['data']['totalCreditNoteAmount'] ?? 0;
    }
    $invoice_current_due = $invoice_current_due - $totalDebitNoteAmount;
    if ($invoice_current_dueObj['numRows'] > 0 && $subtotal > $invoice_current_due) {
      $returnData['status'] = "Warning";
  ...
  if ($insert_debit['status'] == "success") {
    ...
    foreach ($debit_items as $key => $item) {
      ...
      foreach ($taxComponent_tax['tax'] as $taxDetail) {
        $gstDetails = [
      ...
      if ($insert_debit_items['status'] == "success") {
        ...
        $auditTrail['action_data']['Debit Note Item Detail']['item'] = $itemName['data']['itemName'];
        $auditTrail['action_data']['Debit Note Item Detail']['account'] = $accountGl['data']['gl_label'];
        $auditTrail['action_data']['Debit Note Item Detail']['item_qty'] = decimalQuantityPreview($qty);
        $auditTrail['action_data']['Debit Note Item Detail']['item_rate'] = decimalValuePreview($rate);
        $auditTrail['action_data']['Debit Note Item Detail']['item_tax'] = decimalValuePreview($tax);
        // $auditTrail['action_data']['Debit Note Item Detail']['cgst'] = $itemcgst;
        // $auditTrail['action_data']['Debit Note Item Detail']['sgst'] = $itemsgst;
        // $auditTrail['action_data']['Debit Note Item Detail']['igst'] = $itemigst;
        $auditTrail['action_data']['Debit Note Item Detail']['item_amount'] = decimalValuePreview($amount);
        ...
        if (count($item['batchselection']) > 0) {
          ...
          foreach ($selStockLog['data'] as $stockLogKey => $logdata) {
            ...
            $auditTrail['action_data']['Stock Log Detail']['Refarance Id'] = $logRef;
            $auditTrail['action_data']['Stock Log Detail']['bornDate'] = formatDateWeb($bornDate);
            $auditTrail['action_data']['Stock Log Detail']['postingDate'] = $posting_date;
    ...
    if ($POST['select_customer_vendor'] == 'Vendor') {
      if ($companyCountry == '103') {
        ...
        $accPostingObj = $accountingObj->dNoteForVendorAccountingPosting_by_rule_book($postingAccountingData, $accslug, $debit_note_id);
      }
    } else {
      if ($companyCountry == '103') {
        $accPostingObj = $accountingObj->dNoteForCustomerAccountingPosting($postingAccountingData, $accslug, $debit_note_id);
      } else {
        $accPostingObj = $accountingObj->dNoteForCustomerAccountingPosting_by_rule_book($postingAccountingData, $accslug, $debit_note_id);
      }
    }
    ...
    if ($accPostingObj["status"] == "success" && $accPostingObj["journalId"] != "") {
      ...
      // $auditTrail['action_data']['Account Detail']['compInvoiceType'] = $postingAccountingData['compInvoiceType'];
      // $auditTrail['action_data']['Account Detail']['roundOffValue'] = $postingAccountingData['roundOffValue'];
      // $auditTrail['action_data']['Account Detail']['partyDetails']['partyName'] = $$vendorDetailsObj["data"]["trade_name"] ;
      // $auditTrail['action_data']['Account Detail']['partyDetails']['partyCode'] = $vendorDetailsObj["data"]["customer_code"];
      // $auditTrail['action_data']['Account Detail']['taxDetails'] = $postingAccountingData['taxDetails'];
      ...
      $logAccFailedResponce=$failedAccController->logAccountingFailure($debit_note_no,"debitNote");
    ...
    if ($mailValid == 'yes') {
      ...
      $msg = ' <div>
                ...
                <p>
                <a href="' . BASE_URL . 'branch/location/classic-view/invoice-preview-print-taxcomponents.php?dr_note_id=' . base64_encode($debit_note_id) . '" style="background: #174ea6;padding: 8px;color: white;text-decoration: none;border-radius: 5px;"><img width="15" src="' . BASE_URL . 'public/storage/invoice-icon/invoice.png" /> View Debit Note</a>
