...
function administratorAuth()
{
    ...
    if (!isset($_SESSION["logedCustomerAdminInfo"]["adminId"]) || !isset($_SESSION["logedCustomerAdminInfo"]["adminRole"])) {
        redirect(CUSTOMER_URL . "login.php");
    } else {
        $adminRole = $_SESSION["logedCustomerAdminInfo"]["adminRole"];
        if ($adminRole != 1) {
            $currentPage = basename($_SERVER['PHP_SELF']);
            if (!in_array($currentPage, $commonAccessPages)) {
                $sqlAccesses = "SELECT `fldRoleAccesses` FROM `tbl_customer_admin_roles` WHERE `fldRoleStatus`='active' AND `fldRoleKey`=" . $adminRole;
                $accesses = "";
                ...
                $sqlCheckAccessableFile = "SELECT `fldMenuFile` FROM `tbl_customer_admin_menu` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' AND `fldMenuFile`='".$currentPage."'";
...
function getAdministratorMenuSubMenu()
{
    ...
    $adminRole = $_SESSION["logedCustomerAdminInfo"]["adminRole"];
    if($adminRole==1){
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_customer_admin_menu` WHERE `fldParentMenuKey`!=0 AND `fldMenuStatus`='active'";
        if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
    ...
    $parentMenuKeysArr=[];
    $sqlParentMenuKeys = "SELECT `fldParentMenuKey` FROM `tbl_customer_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' GROUP BY `fldParentMenuKey`;";
    ...
    $sqlMenuList="SELECT * FROM `tbl_customer_admin_menu` WHERE `fldMenuKey` IN (".$parentMenuKeys.") AND `fldMenuStatus`='active' ORDER BY `fldMenuOrderBy` ASC";
    if($qryMenuList=mysqli_query($dbCon,$sqlMenuList)){
        if (mysqli_num_rows($qryMenuList)>0) {
            ...
            while($rowMenuList=mysqli_fetch_assoc($qryMenuList)){
                ...
                $sqlSubMenuList="SELECT * FROM `tbl_customer_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' AND `fldParentMenuKey`=".$parentMenuKey." ORDER BY `fldMenuOrderBy` ASC";
...
function loginAdministratorUser($POST){
    global $dbCon;
    $returnData=[];
   // $sql="SELECT * FROM `tbl_customer_admin_details` WHERE `fldAdminEmail`='".$POST["email"]."' AND `fldAdminStatus`='active'";
    $sql="SELECT `tbl_customer_admin_details`.*,".ERP_CUSTOMER.".customer_code FROM `tbl_customer_admin_details`,".ERP_CUSTOMER." WHERE ".ERP_CUSTOMER.".`customer_id`= `tbl_customer_admin_details`.`customer_id` AND ".ERP_CUSTOMER.".`customer_code`='".$POST["customer_code"]."' AND `fldAdminStatus`='active'";
    if($result=mysqli_query($dbCon,$sql)){
        if(mysqli_num_rows($result)>0){
            $row=mysqli_fetch_assoc($result);
            if($POST["pass"]==$row["fldAdminPassword"]){
                $_SESSION["logedCustomerAdminInfo"]["adminId"]=$row["fldAdminKey"];
                $_SESSION["logedCustomerAdminInfo"]["adminName"]=$row["fldAdminName"];
                $_SESSION["logedCustomerAdminInfo"]["adminEmail"]=$row["fldAdminEmail"];
                $_SESSION["logedCustomerAdminInfo"]["adminPhone"]=$row["fldAdminPhone"];
                $_SESSION["logedCustomerAdminInfo"]["adminRole"]=$row["fldAdminRole"];
                $_SESSION["logedCustomerAdminInfo"]["customer_id"]=$row["customer_id"];
                $_SESSION["logedCustomerAdminInfo"]["company_id"]=$row["company_id"];
                $_SESSION["logedCustomerAdminInfo"]["adminType"]='customer';
                $returnData["status"]="success";
...
function addAdministratorMenu($POST)
{
    ...
    $ins = "INSERT INTO `tbl_customer_admin_menu`
                SET
...
function updateAdministratorMenu($POST)
{
    ...
    if ($res = mysqli_query($dbCon, "SELECT COUNT(`fldMenuLabel`) AS 'noOfRecords' FROM `tbl_customer_admin_menu` WHERE `fldMenuKey`=" . $POST["menuKey"])) {
        if (mysqli_fetch_assoc($res)["noOfRecords"] == 1) {
            $ins = "UPDATE `tbl_customer_admin_menu`
                SET
                    `fldMenuLabel` = '" . $menuLabel . "',
                    `fldMenuIcon` = '" . $menuIcon . "',
                    `fldMenuFile` = '" . $menuFile . "',
...
function getAdministratorMenuList($parentMenuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_customer_admin_menu` WHERE `fldParentMenuKey`=" . $parentMenuKey . " AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
...
function getAdministratorMenuDetails($menuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_customer_admin_menu` WHERE `fldMenuKey`=" . $menuKey;
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function addNewAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_customer_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `customer_id`='".$customer_id."' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

              $ins = "INSERT INTO `tbl_customer_admin_roles` SET
...
function updateAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_customer_admin_roles` WHERE `fldRoleKey`=" . $roleKey . " AND `customer_id`='".$customer_id."' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 1) {

                $checkName = "SELECT * FROM `tbl_customer_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleKey`!=" . $roleKey . " AND `customer_id`='".$customer_id."' AND  `fldRoleStatus`!='deleted'";
                ...
                $ins = "UPDATE `tbl_customer_admin_roles` SET
                            `fldRoleName`='" . $roleName . "',
                            `fldRoleAccesses`='" . $roleAccesses . "',
...
function getAdministratorRoleDetails($key = null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_customer_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleKey`=" . $key . "";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorRoles()
{
    ...
    $returnData = [];
    $customer_id = $_SESSION["logedCustomerAdminInfo"]["customer_id"];
    $sql = "SELECT * FROM `tbl_customer_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `customer_id`=".$customer_id.""; 
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorUsers()
{
    ...
    $customer_id = $_SESSION["logedCustomerAdminInfo"]["customer_id"];
    $returnData = [];
   $sql = "SELECT * FROM `tbl_customer_admin_details`,`tbl_customer_admin_roles` WHERE `tbl_customer_admin_details`.`fldAdminRole`=`tbl_customer_admin_roles`.`fldRoleKey` AND `tbl_customer_admin_details`.`customer_id`='".$customer_id."' AND `tbl_customer_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_customer_admin_roles`.`fldRoleStatus`!='deleted'";
...
function getAdministratorUserDetails($key = null)
{
    ...
    $sql = "SELECT `tbl_customer_admin_details`.*, `tbl_customer_admin_roles`.`fldRoleName` FROM `tbl_customer_admin_details`,`tbl_customer_admin_roles` WHERE `tbl_customer_admin_details`.`fldAdminRole`=`tbl_customer_admin_roles`.`fldRoleKey` AND `tbl_customer_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_customer_admin_roles`.`fldRoleStatus`!='deleted' AND `fldAdminKey`=" . $key;
...
function updateAdministratorUserDetails($POST)
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_customer_admin_details` WHERE `fldAdminKey`='" . $adminKey . "'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $ins = "UPDATE `tbl_customer_admin_details` 
                            SET
...
function addNewAdministratorUser($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_customer_admin_details` WHERE `fldAdminEmail`='" . $adminEmail . "' AND `fldAdminStatus`!='deleted'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

                $ins = "INSERT INTO `tbl_customer_admin_details` 
                            SET
                                `fldAdminName`='" . $adminName . "',
                                `fldAdminEmail`='" . $adminEmail . "',
                                `fldAdminPhone`='" . $adminPhone . "',
...
function getAdministratorAccessesNames($roleAccesses = "0")
{
    ...
    if ($roleAccesses == "0") {
        ...
        $sql = "SELECT `fldMenuLabel` FROM `tbl_customer_admin_menu` WHERE `fldMenuKey` IN (" . $roleAccesses . ") AND `fldMenuStatus`='active'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $returnData["status"] = "success";
                $returnData["message"] = "success";
