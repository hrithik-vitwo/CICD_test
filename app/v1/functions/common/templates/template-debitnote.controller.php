...
class TemplateDebitNoteTaxController
{
    ...
    public function printDebitNotes($dr_note_id = 0)
    {
        ...
        if ($debitor_type == 'customer') {
            ...
            $vendorDoc_no =  $iv['data']['vendorDocumentNo'];
            $iv_date = explode(" ", $iv['data']['created_at'], 1);

            // console($iv_date);
            $source_address_sql = queryGet("SELECT * FROM `erp_vendor_bussiness_places` WHERE `vendor_id`= '" . $oneList['party_id'] . "' ")['data'];
                    ...
                    <table class="classic-view table-bordered tableBorder">
                            ...
                            <tr>
                                ...
                                <td colspan="6" class="border-right-0 border-left-0">
                                    <div style="text-align: left; padding: 5px;">
                                        <p class="font-bold"> <?= trim($companyData['company_name']) ?></p>
                                        <p><?= trim($companyData['location_building_no']) ?></p>
                                        <p>Flat No.<?= trim($companyData['location_flat_no']) ?>, <?= trim($companyData['location_street_name']) ?></p>
                                        <p><?= trim($companyData['location']) ?>, <?= trim($companyData['location_city']) ?>, <?= trim($companyData['location_district']) ?> <?= trim($companyData['location_pin_code']) ?></p>
                                ...
                                <td rowspan="2" colspan="7">
                                    <div class="d-flex">
                                        <div class="cust-details">
                                            ...
                                            <?php if ($debitor_type == 'customer') { ?>
                                                <p class="font-bold">Contact Person: <?= $customerData['customer_authorised_person_name'] ?></p>
                                                <p class="font-bold">Mobile No: <?= $customerData['customer_authorised_person_phone'] ?></p>
                                            <?php
                                            } else { ?>
                                                <p class="font-bold">Contact Person: <?= $customerData['vendor_authorised_person_name'] ?></p>
                                                <p class="font-bold">Mobile No: <?= $customerData['vendor_authorised_person_phone'] ?></p>
                                            <?php
                                            } ?>
                                        </div>
                                        <div class="add-details text-left">
                                            <?php if ($debitor_type == 'customer') { ?>
                                                <p><?= $businessTaxID ?> : <?= $customerData['customer_gstin'] ?></p>
                                                <p><?= $taxNumber ?> : <?= $customerData['customer_pan'] ?></p>
                                            ...
                                            <?php
                                                ...
                                                <p><?= $businessTaxID ?> : <?= $customerData['vendor_gstin'] ?></p>
                                                <p><?= $taxNumber ?> : <?= $customerData['vendor_pan'] ?></p>
                                                <p>State Name : <?= $state_name; ?></p>
                                                <?php if ($country_fields->state_code) { ?>
                            ...
                            ?>
                                <tr>
                                    ...
                                    ?>
                                        ...
                                        <?php foreach ($taxComponents as $tax) {
                                            // $itemRate = $item['item_rate']; 


                                            // $itemGstAmt = $itemRate * ($taxPercentage / 100);


                                            $gstType = $tax['gstType'];
                                            $itemGstAmount = $tax['taxAmount'];


                                            if (!isset($totalGSTs[$gstType])) {
                                                $totalGSTs[$gstType] = 0;
                                            }
                                            $totalGSTs[$gstType] += $itemGstAmount;


                                            $itemGstAmt = $tax['taxAmount'];

                                            $itemGstPer = $item['item_tax'] / (100 / $tax['taxPercentage']); ?>
                                            ...
                                            <td class="px-2" colspan="2">
                                                <p class="text-right"><?= decimalValuePreview($itemGstAmt) ?></p>
                                            </td>
                                    <?php
                                        }
                                    } ?>
                            ...
                            <tr>
                                ...
                                <td colspan="2" class="text-right font-bold px-2">
                                    ...
                                    <?php if ($countryCode == '103') {
                                        ...
                                        foreach ($totalGSTs as $gstType => $totalAmount) { ?>
                                            <p><span class="pr-1"></span><?= decimalValuePreview($totalAmount) ?></p>
                                        <?php }
                                    }
                                    if ($oneList['adjustment']) {
                        ...
                        <tbody>
                            <tr>
                                <td colspan="7" class="px-2">
                                    ...
                                    <p class="font-bold">
                                        <?= $companyCurrencyName . " " . number_to_words_indian_rupees($oneList['total']); ?>
                                        ONLY
                                    </p>
                                ...
                                <td colspan="5" class="px-2">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <!-- <?php if ($companyCurrencyName != $customerCurrencyName) { ?>
                                            <small class="text-small text-muted"><?= decimalValuePreview($oneList['total'] * $currencyConversionRate) ?></small>
                                        <?php } ?> -->
                                    </div>
                                </td>
                            </tr>
                        ...
                        <tfoot>
                            ...
                            <tr>
                                <td colspan="3">
                                    <p>Prepared By:
                                        <?= getCreatedByUser($oneList['created_by']); ?>
                                    </p>
                                </td>
                                <td colspan="4">
                                    <p>Checked By: </p>
                                ...
                                <td colspan="5">
                                    ...
                                    <p class="text-center sign-img">
                                        <img width="120" src="<?= COMP_STORAGE_URL ?>/profile/<?= $companyData['signature'] ?>"
                                            alt="">
                                    </p>
