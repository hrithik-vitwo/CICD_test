...
function administratorAuth()
{
    ...
    if (!isset($_SESSION["logedVendorAdminInfo"]["adminId"]) || !isset($_SESSION["logedVendorAdminInfo"]["adminRole"])) {
        redirect(VENDOR_URL . "login.php");
    } else {
        $adminRole = $_SESSION["logedVendorAdminInfo"]["adminRole"];
        if ($adminRole != 1) {
            $currentPage = basename($_SERVER['PHP_SELF']);
            if (!in_array($currentPage, $commonAccessPages)) {
                $sqlAccesses = "SELECT `fldRoleAccesses` FROM `tbl_vendor_admin_roles` WHERE `fldRoleStatus`='active' AND `fldRoleKey`=" . $adminRole;
                $accesses = "";
                ...
                $sqlCheckAccessableFile = "SELECT `fldMenuFile` FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey` IN (" . $accesses . ") AND `fldMenuStatus`='active' AND `fldMenuFile`='".$currentPage."'";
...
function getAdministratorMenuSubMenu()
{
    ...
    $adminRole = $_SESSION["logedVendorAdminInfo"]["adminRole"];
    if($adminRole==1){
        $sqlAccesses = "SELECT `fldMenuKey` FROM `tbl_vendor_admin_menu` WHERE `fldParentMenuKey`!=0 AND `fldMenuStatus`='active'";
        if ($qryAccesses = mysqli_query($dbCon, $sqlAccesses)) {
    ...
    $parentMenuKeysArr=[];
    $sqlParentMenuKeys = "SELECT `fldParentMenuKey` FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' GROUP BY `fldParentMenuKey`;";
    ...
    $sqlMenuList="SELECT * FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey` IN (".$parentMenuKeys.") AND `fldMenuStatus`='active' ORDER BY `fldMenuOrderBy` ASC";
    if($qryMenuList=mysqli_query($dbCon,$sqlMenuList)){
        if (mysqli_num_rows($qryMenuList)>0) {
            ...
            while($rowMenuList=mysqli_fetch_assoc($qryMenuList)){
                ...
                $sqlSubMenuList="SELECT * FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey` IN (".$accesses.") AND `fldMenuStatus`='active' AND `fldParentMenuKey`=".$parentMenuKey." ORDER BY `fldMenuOrderBy` ASC";
...
function loginAdministratorUser($POST){
    global $dbCon;
    $returnData=[];
    $sql="SELECT `tbl_vendor_admin_details`.*,".ERP_VENDOR_DETAILS.".vendor_code FROM `tbl_vendor_admin_details`,".ERP_VENDOR_DETAILS." WHERE ".ERP_VENDOR_DETAILS.".`vendor_id`= `tbl_vendor_admin_details`.`fldAdminVendorId` AND ".ERP_VENDOR_DETAILS.".`vendor_code`='".$POST["vendor_code"]."' AND `fldAdminStatus`='active'";
    if($result=mysqli_query($dbCon,$sql)){
        if(mysqli_num_rows($result)>0){
            $row=mysqli_fetch_assoc($result);
            if($POST["pass"]==$row["fldAdminPassword"]){
                $_SESSION["logedVendorAdminInfo"]["adminId"]=$row["fldAdminKey"];
                $_SESSION["logedVendorAdminInfo"]["adminName"]=$row["fldAdminName"];
                $_SESSION["logedVendorAdminInfo"]["adminEmail"]=$row["fldAdminEmail"];
                $_SESSION["logedVendorAdminInfo"]["adminPhone"]=$row["fldAdminPhone"];
                $_SESSION["logedVendorAdminInfo"]["adminRole"]=$row["fldAdminRole"];
                $_SESSION["logedVendorAdminInfo"]["fldAdminCompanyId"]=$row["fldAdminCompanyId"];
                $_SESSION["logedVendorAdminInfo"]["fldAdminVendorId"]=$row["fldAdminVendorId"];
                $_SESSION["logedVendorAdminInfo"]["adminType"]='vendor';
                $returnData["status"]="success";
...
function addAdministratorMenu($POST)
{
    ...
    $ins = "INSERT INTO `tbl_vendor_admin_menu`
                SET
...
function updateAdministratorMenu($POST)
{
    ...
    if ($res = mysqli_query($dbCon, "SELECT COUNT(`fldMenuLabel`) AS 'noOfRecords' FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey`=" . $POST["menuKey"])) {
        if (mysqli_fetch_assoc($res)["noOfRecords"] == 1) {
            $ins = "UPDATE `tbl_vendor_admin_menu`
                SET
                    `fldMenuLabel` = '" . $menuLabel . "',
                    `fldMenuIcon` = '" . $menuIcon . "',
                    `fldMenuFile` = '" . $menuFile . "',
...
function getAdministratorMenuList($parentMenuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_vendor_admin_menu` WHERE `fldParentMenuKey`=" . $parentMenuKey . " AND `fldMenuStatus`!='deleted' ORDER BY `fldMenuOrderBy` ASC";
    if ($res = mysqli_query($dbCon, $sql)) {
...
function getAdministratorMenuDetails($menuKey = 0)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey`=" . $menuKey;
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function addNewAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $fldRoleCompanyId = $_SESSION["logedVendorAdminInfo"]["fldAdminCompanyId"];
        $fldAdminVendorId = $_SESSION["logedVendorAdminInfo"]["fldAdminVendorId"];

        $sql = "SELECT * FROM `tbl_vendor_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleCompanyId`='".$fldRoleCompanyId."' AND `fldAdminVendorId`='".$fldAdminVendorId."' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

              $ins = "INSERT INTO `tbl_vendor_admin_roles` SET
                            `fldRoleName`='" . $roleName . "',
                            `fldRoleAccesses`='" . $roleAccesses . "',
                            `fldRoleDescription`='" . $roleDescription . "',
                            `fldRoleCompanyId`='" . $fldRoleCompanyId . "',
                            `fldAdminVendorId`='" . $fldAdminVendorId . "'";
...
function updateAdministratorRole($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_vendor_admin_roles` WHERE `fldRoleKey`=" . $roleKey . " AND `fldRoleCompanyId`='".$fldRoleCompanyId."' AND `fldAdminVendorId`='".$fldAdminVendorId."' AND `fldRoleStatus`!='deleted'";

        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 1) {

                $checkName = "SELECT * FROM `tbl_vendor_admin_roles` WHERE `fldRoleName`='" . $roleName . "' AND `fldRoleKey`!=" . $roleKey . " AND `fldRoleCompanyId`='".$fldRoleCompanyId."' AND `fldAdminVendorId`='".$fldAdminVendorId."' AND  `fldRoleStatus`!='deleted'";
                ...
                $ins = "UPDATE `tbl_vendor_admin_roles` SET
                            `fldRoleName`='" . $roleName . "',
                            `fldRoleAccesses`='" . $roleAccesses . "',
...
function getAdministratorRoleDetails($key = null)
{
    global $dbCon;
    $returnData = [];
    $sql = "SELECT * FROM `tbl_vendor_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleKey`=" . $key . "";
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
function getAllAdministratorRoles()
{
    ...
    $fldRoleCompanyId = $_SESSION["logedVendorAdminInfo"]["fldAdminCompanyId"];
    $fldAdminVendorId = $_SESSION["logedVendorAdminInfo"]["fldAdminVendorId"];
    $sql = "SELECT * FROM `tbl_vendor_admin_roles` WHERE `fldRoleStatus`!='deleted' AND `fldRoleCompanyId`=".$fldRoleCompanyId." AND `fldAdminVendorId`=".$fldAdminVendorId.""; 
    if ($res = mysqli_query($dbCon, $sql)) {
        if (mysqli_num_rows($res) > 0) {
            $returnData['status'] = "success";
            $returnData['message'] = "Data found";
...
{
    global $dbCon;
    $fldAdminCompanyId = $_SESSION["logedVendorAdminInfo"]["fldAdminCompanyId"];
    $fldAdminVendorId = $_SESSION["logedVendorAdminInfo"]["fldAdminVendorId"];
    $fldAdminBranchId = $_SESSION["logedVendorAdminInfo"]["fldAdminBranchId"];
    $returnData = [];
   $sql = "SELECT * FROM `tbl_vendor_admin_details`,`tbl_vendor_admin_roles` WHERE `tbl_vendor_admin_details`.`fldAdminRole`=`tbl_vendor_admin_roles`.`fldRoleKey` AND `tbl_vendor_admin_details`.`fldAdminCompanyId`='".$fldAdminCompanyId."' AND `tbl_vendor_admin_details`.`fldAdminVendorId`='".$fldAdminVendorId."' AND `tbl_vendor_admin_details`.`fldAdminBranchId`='".$fldAdminBranchId."' AND `tbl_vendor_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_vendor_admin_roles`.`fldRoleStatus`!='deleted'";
...
function getAdministratorUserDetails($key = null)
{
    ...
    $sql = "SELECT `tbl_vendor_admin_details`.*, `tbl_vendor_admin_roles`.`fldRoleName` FROM `tbl_vendor_admin_details`,`tbl_vendor_admin_roles` WHERE `tbl_vendor_admin_details`.`fldAdminRole`=`tbl_vendor_admin_roles`.`fldRoleKey` AND `tbl_vendor_admin_details`.`fldAdminStatus`!='deleted' AND `tbl_vendor_admin_roles`.`fldRoleStatus`!='deleted' AND `fldAdminKey`=" . $key;
...
function updateAdministratorUserDetails($POST)
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_vendor_admin_details` WHERE `fldAdminKey`='" . $adminKey . "'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $ins = "UPDATE `tbl_vendor_admin_details` 
                            SET
...
function addNewAdministratorUser($POST = [])
{
    ...
    if ($isValidate["status"] == "success") {
        ...
        $sql = "SELECT * FROM `tbl_vendor_admin_details` WHERE `fldAdminEmail`='" . $adminEmail . "' AND `fldAdminStatus`!='deleted'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) == 0) {

                $ins = "INSERT INTO `tbl_vendor_admin_details` 
                            SET
                                `fldAdminName`='" . $adminName . "',
                                `fldAdminEmail`='" . $adminEmail . "',
                                `fldAdminPhone`='" . $adminPhone . "',
...
function getAdministratorAccessesNames($roleAccesses = "0")
{
    ...
    if ($roleAccesses == "0") {
        ...
        $sql = "SELECT `fldMenuLabel` FROM `tbl_vendor_admin_menu` WHERE `fldMenuKey` IN (" . $roleAccesses . ") AND `fldMenuStatus`='active'";
        if ($res = mysqli_query($dbCon, $sql)) {
            if (mysqli_num_rows($res) > 0) {
                $returnData["status"] = "success";
                $returnData["message"] = "success";
