...
class TrailBalance
{
        ...
        if ($oppYerMonth != $startYerMonth) {
            ...
            $sql = 'SELECT log.gl_code,log.gl_label,SUM(log.debit_amount) AS debit_amount,SUM(log.credit_amount) AS credit_amount,opening.open_bal AS open_balance FROM
                ...
                FROM
                    ...
                    WHERE
                        journal.journal_status = "active" AND journal.company_id = ' . $this->company_id . ' AND journal.branch_id = ' . $this->branch_id . ' AND journal.location_id = ' . $this->location_id . '
                    )
                ...
                ((SELECT coa.gl_code,SUM(balance.closing_val) AS total_amount FROM erp_opening_closing_balance AS balance INNER JOIN `' . $erp_acc_coa . '` AS coa ON balance.gl=coa.id WHERE balance.company_id=' . $this->company_id . ' AND DATE_FORMAT(date,"%m")=DATE_FORMAT(DATE_SUB("' . $startDate . '",INTERVAL 1 MONTH),"%m") GROUP BY coa.gl_code)
                ...
                ((SELECT
                    ...
                    WHERE
                        ...
                        journal.company_id = ' . $this->company_id . ' AND
                        journal.branch_id = ' . $this->branch_id . ' AND
                        journal.location_id = ' . $this->location_id . ')
                    ...
                    WHERE
                        ...
                        journal.company_id = ' . $this->company_id . ' AND
                        journal.branch_id = ' . $this->branch_id . ' AND
                        journal.location_id = ' . $this->location_id . ')) AS table1 INNER JOIN `' . $erp_acc_coa . '` AS coa ON table1.glId = coa.id
            ...
            $sql = 'SELECT log.gl_code,log.gl_label,SUM(log.debit_amount) AS debit_amount,SUM(log.credit_amount) AS credit_amount,opening.open_bal AS open_balance FROM
                ...
                ((SELECT coa.gl_code,SUM(balance.closing_val) AS total_amount FROM erp_opening_closing_balance AS balance INNER JOIN `' . $erp_acc_coa . '` AS coa ON balance.gl=coa.id WHERE balance.company_id=' . $this->company_id . ' AND DATE_FORMAT(date,"%m")=DATE_FORMAT("' . $startDate . '","%m") GROUP BY coa.gl_code)
                UNION
            ...
            $sql = 'SELECT table1.gl_code,table1.gl_label,SUM(table1.total_amount)AS open_bal FROM
            ((SELECT coa.gl_code,coa.gl_label,SUM(balance.closing_val) AS total_amount FROM erp_opening_closing_balance AS balance INNER JOIN `' . $erp_acc_coa . '` AS coa ON balance.gl=coa.id WHERE balance.company_id=1 AND DATE_FORMAT(date,"%m")=DATE_FORMAT(DATE_SUB("' . $endDate . '",INTERVAL 1 MONTH),"%m") GROUP BY coa.gl_code,coa.gl_label)
            UNION
            (SELECT coa.gl_code AS gl_code,coa.gl_label,SUM(table1.Amount) AS total_amount FROM
            ((SELECT
                    journal.postingDate AS postingDate,
                    debit.glId AS glId,
                    debit.debit_amount AS Amount,
                ...
                WHERE
                    ...
                    journal.location_id=' . $this->location_id . ')) AS table1 INNER JOIN `' . $erp_acc_coa . '` AS coa ON table1.glId = coa.id
            WHERE
            table1.postingDate BETWEEN DATE_SUB("' . $endDate . '",INTERVAL (DATE_FORMAT("' . $endDate . '","%d")-1) DAY) AND DATE_SUB("' . $endDate . '",INTERVAL 1 DAY)
            GROUP BY
            coa.gl_code,coa.gl_label
            ORDER BY
            coa.gl_code,coa.gl_label)) AS table1
            GROUP BY table1.gl_code,table1.gl_label;
            
            To get transactional data
            
            SELECT log.gl_code,log.gl_label,SUM(debit_amount) AS debit_amt,SUM(credit_amount) AS credit_amt FROM 
            (SELECT
                coa.gl_code,
                coa.gl_label,
                SUM(
            ...
            ORDER BY
                coa.gl_code,
                coa.gl_label) AS log GROUP BY log.gl_code,log.gl_label';
        }
        // echo $sql;
    ...
    function getTrailBalanceData($start_date = null, $end_date = null)
    {
        ...
        $openingsForFirstDayOfMonth = 'SELECT t1.id, t1.p_id, t1.gl_code, t1.gl_label, t1.status, IFNULL(t2.opening_val, 0) AS opening_val
        FROM (
          SELECT id, p_id, gl_code, gl_label, status
          FROM `' . $erp_acc_coa . '`
          WHERE glStType = "account" AND status = "active"
          ORDER BY gl_code
        ) t1
        LEFT JOIN (
          SELECT gl, SUM(opening_val) AS opening_val
          FROM erp_opening_closing_balance
          WHERE company_id = ' . $this->company_id . ' AND DATE_FORMAT(date, "%Y-%m") = DATE_FORMAT("' . $start_date . '", "%Y-%m")
          GROUP BY gl
        ) t2 ON t1.id = t2.gl';

        $openingObj = queryGet($openingsForFirstDayOfMonth, true);
        $debitCreditForOpening = [];
        if (strtotime($start_date) > strtotime($startDate1stDay)) {
            $openingEndDate = date("Y-d-m", strtotime('-1 days', strtotime($start_date)));
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                ...
                $trailBalanceDataObj = $trailBalanceObj->getTrailBalanceData($start_date, $end_date);

                // console($trailBalanceDataObj);
                            ...
                            <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab">
                                ...
                                <table class="table defaultDataTable table-hover text-nowrap">
                                    ...
                                    <tbody>
                                        ...
                                        $grandTotalOpening = 0;
                                        $grandTotalClosing = 0;
                                        foreach ($trailBalanceDataObj["data"] as $row) {
                                            $grandTotalDebit += $row["debit_amount"];
                                            $grandTotalCredit += $row["credit_amount"];
                                        ...
                                        ?>
                                            <tr>
                                                <td><?= $row["gl_code"] ?></td>
                                                <td><?= $row["gl_label"] ?></td>
                                                <td class="text-right"><?= number_format($row["opening_val"], 2) ?></td>
                                                <td class="text-right"><?= number_format($row["debit_amount"], 2) ?></td>
                                                <td class="text-right"><?= number_format($row["credit_amount"], 2) ?></td>
                                                <td class="text-right"><?= number_format($row["closing_val"], 2) ?></td>
                                            </tr>
                                        <?php
                                        }
                                        ?>
                                        <tr>
                                            <td></td>
                                            <td class="font-weight-bold">Total</td>
                                            <td class="text-right font-weight-bold"><?= number_format($grandTotalOpening, 2) ?></td>
                                            <td class="text-right font-weight-bold"><?= number_format($grandTotalDebit, 2) ?></td>
                                            <td class="text-right font-weight-bold"><?= number_format($grandTotalCredit, 2) ?></td>
                                            <td class="text-right font-weight-bold"><?= number_format($grandTotalClosing, 2) ?></td>
                                        </tr>
                                    </tbody>
                                </table>
                                <?php
