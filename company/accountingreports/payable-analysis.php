                            ...
                            <div class="card card-tabs mb-0" style="border-radius: 20px;">
                                ...
                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                    <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                        ...
                                        $sql_list = "SELECT vendorId,vendorName,DATEDIFF(dueDate,CURDATE())AS due_days,COUNT(*) AS count_,SUM(dueAmt) AS total_due_amount FROM erp_grninvoice AS iv WHERE companyId=$company_id AND postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND dueAmt!=0 AND dueDate>CURDATE() GROUP BY vendorId,vendorName,due_days ORDER BY due_days,vendorName,total_due_amount desc;";
                                            ...
                                            <table id="dataTable" class="table table-hover transactional-book-table" style="width: 100%; position: relative;">
                                                ...
                                                <tbody>
                                                    ...
                                                    foreach ($array['making'] as $key => $data) {
                                                        ...
                                                        $overdue_sql = queryGet("SELECT SUM(dueAmt) AS total_due_amount FROM erp_grninvoice WHERE companyId = $branch_id AND vendorId=$vendor_id AND postingDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND dueAmt!=0 AND dueDate < CURRENT_DATE;");
                                                        //console($overdue_sql);
                            ...
                            <div class="card card-tabs mb-0" style="border-radius: 20px;">
                                ...
                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                    <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                        ...
                                        $sql_list = "SELECT branchId,locationId,vendorCode,vendorName,vendorDocumentDate AS invoice_date,vendorDocumentNo AS invoice_num,postingDate AS posting_date ,grnIvCode AS posting_doc_num,grnSubTotal AS base_amount,grnTotalCgst AS cgst,grnTotalSgst AS sgst,grnTotalIgst AS igst,grnTotalAmount AS invoice_amount,dueAmt,dueDate,grnCreatedAt AS created_at,grnCreatedBy AS created_by,grnUpdatedAt AS updated_at,grnUpdatedBy AS updated_by FROM erp_grninvoice WHERE companyId=$company_id AND dueAmt!=0 AND vendorDocumentDate BETWEEN '" . $f_date . "' AND '" . $to_date . "' ORDER BY invoice_date desc;";
