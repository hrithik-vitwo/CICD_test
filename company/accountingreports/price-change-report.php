                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    <div class="label-select">
                      <h3 class="card-title mb-0">Price Change Report</h3>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT branch.branch_name,loc.othersLocation_name,items.itemCode,items.itemName,bom.preparedDate,bom.cogm,bom.cogs,bom.msp,bom.createdAt,bom.createdBy,bom.updatedAt,bom.updatedBy FROM erp_bom AS bom INNER JOIN erp_inventory_items AS items ON bom.itemId=items.itemId LEFT JOIN erp_branch_otherslocation AS loc ON bom.locationId=loc.othersLocation_id LEFT JOIN erp_branches AS branch ON bom.branchId=branch.branch_id WHERE bom.companyId=$company_id ORDER BY bom.preparedDate desc;";
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo formatDateORDateTime($data['preparedDate']); ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo getCreatedByUser($data['createdBy']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo formatDateORDateTime($data['createdAt']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo getCreatedByUser($data['updatedBy']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td><?php echo formatDateORDateTime($data['updatedAt']);  ?></td>
                              <?php } ?>
                            </tr>
                          <?php
                          }
