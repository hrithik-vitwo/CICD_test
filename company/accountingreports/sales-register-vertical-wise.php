                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  ...
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="label-select">
                      <h3 class="card-title mb-0">Sales Register Verical Wise</h3>
                    </div>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT DATE_FORMAT(invoices.invoice_date,'%M, %y') AS month,branch.branch_name AS branch_name,loc.othersLocation_name AS loc_name,f_area.functionalities_name AS functional_area_name,SUM(invoices.sub_total_amt - invoices.totalDiscount) AS total_base_amount,SUM(invoices.all_total_amt) AS total_amount,SUM(invoices.due_amount) AS due_amount FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_company_functionalities AS f_area ON invoices.profit_center=f_area.functionalities_id LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id LEFT JOIN erp_branches AS branch ON invoices.branch_id=branch.branch_id WHERE invoices.company_id=$company_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.status='active' GROUP BY month,branch_name,loc_name,functional_area_name;";
                        ...
                        <tbody class="">
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td> <?php echo $data['functional_area_name']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
                                <td class="text-right"><?php echo $data['total_base_amount']; ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox_concised_view)) { ?>
...
<script>
  ...
  am4core.ready(function() {
    ...
    for (let obj of chartData.data) {

      const outerObj = finalData.map(obj => {
        return obj.category
      })
      outerIndex = outerObj.indexOf(obj.functional_area_name)

      if (outerIndex !== -1) {
        finalData[outerIndex].value += Number(obj.total_amount);
      } else {
        finalData.push({
          "category": obj.functional_area_name,
          "value": Number(obj.total_amount),
