              ...
              <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                ...
                <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                  ...
                  <div class="label-select">
                    <h3 class="card-title mb-0">Cash Report</h3>
                  </div>
                    ...
                    <table id="dataTable" class="table table-hover transactional-book-table" style="width: 100%; position: relative;">
                      ...
                      <tbody class="">
                        ...
                        foreach ($datas as $key => $data) {
                          ...
                          UNION
                          (SELECT balance.subgl AS acc_code,bank_cash.bank_name AS acc_name,SUM(balance.closing_val) AS total_amount FROM erp_opening_closing_balance AS balance INNER JOIN erp_acc_bank_cash_accounts AS bank_cash ON balance.subgl=bank_cash.acc_code WHERE balance.company_id=$company_id AND DATE_FORMAT(bank_cash.created_at,'%Y-%m') <= DATE_FORMAT('" . $f_date . "'/*start_date*/, '%Y-%m') GROUP BY balance.subgl,bank_cash.bank_name)
                          UNION
                          (SELECT balance.subgl AS acc_code,bank_cash.bank_name AS acc_name,SUM(balance.closing_val) AS total_amount FROM erp_opening_closing_balance AS balance INNER JOIN erp_acc_bank_cash_accounts AS bank_cash ON balance.subgl=bank_cash.acc_code WHERE balance.company_id=$company_id AND DATE_FORMAT(balance.date,'%Y-%m')=DATE_FORMAT(DATE_SUB('" . $f_date . "'/*start_date*/, INTERVAL 1 MONTH),'%Y-%m')GROUP BY balance.subgl,bank_cash.bank_name)) AS temp_table GROUP BY temp_table.acc_code,temp_table.acc_name) AS temp_table2 INNER JOIN erp_acc_bank_cash_accounts AS accounts ON temp_table2.acc_code=accounts.acc_code WHERE accounts.type_of_account='cash' AND accounts.company_id=$company_id  AND temp_table2.acc_code='" . $acc_code . "';";

                          $opening_balance_queryset = queryGet($opening_balance_sql);
                          //console($opening_balance_queryset);



                          // $openingBalance = $accountOpeningList[$data["acc_code"]] ?? $opening_balance_queryset["data"]["opening_bal"];
                          // if ($data["type"] == "Collection") {
                          //   $accountOpeningList[$data["acc_code"]] = $openingBalance + $data["amount"];
                          // } else {
                          //   $accountOpeningList[$data["acc_code"]] = $openingBalance - $data["amount"];
                          // }
