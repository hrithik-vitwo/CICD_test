                ...
                <ul class="nav nav-tabs" style="border: none;" id="custom-tabs-two-tab" role="tablist">
                  ...
                  <li class="pt-2 px-3 d-flex justify-content-between align-items-center" style="width:100%">
                    ...
                    <div class="label-select">
                      <h3 class="card-title mb-0">Sales Register SO Wise</h3>
                    </div>
                        ...
                        <tbody>
                          ...
                          ?>
                            <tr>
                              ...
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?php echo ($data['taxable_amount']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
                                <td class="text-right"><?php echo ($data['totalPrice']);  ?></td>
                              <?php }
                              $i++;
                              if (in_array($i, $settingsCheckbox)) { ?>
              ...
              <div class="card card-tabs mb-0" style="border-radius: 20px;">
                ...
                <div class="tab-content" id="custom-tabs-two-tabContent">
                  <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                    ...
                    $sql_list = "SELECT branch.branch_name AS branch_name,loc.othersLocation_name AS loc_name,invoices.so_number AS so_number,invoices.invoice_no AS invoice_num,invoices.invoice_date AS invoice_date,customer.customer_code AS customer_code,customer.trade_name AS customer_name,(invoices.sub_total_amt - invoices.totalDiscount) AS base_amount,invoices.igst AS igst,invoices.sgst AS sgst,invoices.cgst AS cgst,invoices.all_total_amt AS total_amount,invoices.due_amount AS receivable_amount,DATE_ADD(invoices.invoice_date, INTERVAL invoices.credit_period DAY) AS due_date,status.label AS invoice_status,invoices.created_at AS created_at,invoices.created_by AS created_by,invoices.updated_at AS updated_at, invoices.updated_by AS updated_by FROM erp_branch_sales_order_invoices AS invoices LEFT JOIN erp_customer AS customer ON invoices.customer_id=customer.customer_id LEFT JOIN erp_status_master AS status ON invoices.invoiceStatus=status.code LEFT JOIN erp_branch_otherslocation AS loc ON invoices.location_id=loc.othersLocation_id LEFT JOIN erp_branches AS branch ON invoices.branch_id=branch.branch_id WHERE invoices.company_id=$company_id AND invoices.invoice_date BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND invoices.so_number!='' AND invoices.status='active' ORDER BY so_number desc,invoice_date desc;";

                    $queryset = queryGet($sql_list, true);
                    // console($queryset);
                    $num_list = $queryset['numRows'];

                    $settingsTable = getTableSettings(TBL_BRANCH_ADMIN_TABLESETTINGS, "ERP_REPORT_CONCISED_VIEW_" . $pageName, $_SESSION["logedCompanyAdminInfo"]["adminId"]);
                    $settingsCh = ($settingsTable['data'][0]['settingsCheckbox']);
