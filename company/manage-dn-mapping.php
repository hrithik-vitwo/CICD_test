...
if (isset($_POST["submitInvNumber"])) {
  ...
  foreach ($_POST["branch"] as $key => $branch) {
    ...
    $check = queryGet("SELECT * FROM `erp_branch_func_dn_varient_map` WHERE `company_id`='$company_id' AND `branch_id`='$branch_id' AND `functional_area_id`='$functional_id' AND `location_id`='$location_id' AND `status`='1'", false);

    if ($check["numRows"] > 0) {
      //Update
      $update = queryUpdate("UPDATE `erp_branch_func_dn_varient_map` SET 
      ...
      //insert
      $insert = queryInsert("INSERT INTO `erp_branch_func_dn_varient_map` SET 
  ...
  <div class="content-header">
    ...
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#" class="text-dark"><i class="fa fa-list po-list-icon"></i>Manage Debit Note Mapping</a></li>
        ...
        <table class="table defaultDataTable table-hover">
          <thead>
            <tr class="alert-light">
              ...
              <th class="borderNone">Debit Note Varient</th>
            </tr>
          </thead>
          <tbody>
            <?php
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <select class="form-control prefixDropDown" name="prefix[]" required>
                      <?php
                      $query = queryGet("SELECT * FROM `erp_dn_varient` WHERE `company_id`='$company_id' AND `status`='active'", true);
                      $datas = $query["data"];
                      foreach ($datas as $data) {
                        $branch_id = $branch_data["branch_id"];
                        $functional_id = $location_data["functionalities_id"];
                        $location_id = $branch_data["othersLocation_id"];
                        $selected = queryGet("SELECT * FROM `erp_branch_func_dn_varient_map` WHERE `company_id`='$company_id' AND `branch_id`='$branch_id' AND `functional_area_id`='$functional_id' AND `location_id`='$location_id' AND `status`='1'", false);
