...
if($_GET['chart']=='dailyPeriodicSales'){
    $payload["dailyPeriodicSales"] = queryGet("SELECT invoice_date,SUM(sub_total_amt - totalDiscount) AS total_revenue FROM erp_branch_sales_order_invoices WHERE company_id=$company_id AND invoice_date BETWEEN '".$fromDate."' AND '".$toDate."' AND status='active' GROUP BY invoice_date", true);

} else if($_GET['chart']=='monthlyPeriodicSales'){
    // $payload["monthlyPeriodicSales"] = queryGet("SELECT date_format(invoice_date,'%M') AS month,SUM(sub_total_amt - totalDiscount) AS total_revenue FROM erp_branch_sales_order_invoices WHERE company_id=$company_id AND invoice_date BETWEEN '".$fromDate."' AND '".$toDate."' AND status='active' GROUP BY month", true);

} else if($_GET['chart']=='yearlyPeriodicSales'){
    // $payload["yearlyPeriodicSales"] = queryGet("SELECT SUM(sub_total_amt - totalDiscount) AS total_revenue FROM erp_branch_sales_order_invoices WHERE company_id=$company_id AND invoice_date BETWEEN '".$fromDate."' AND '".$toDate."' AND status='active'", true);
    ...
    $payload["stateWiseSalesProfitCenter"] = queryGet("SELECT table1.profit_center,table1.branch_id,branch.state AS state,table1.total_amount FROM (SELECT so_pc.profit_center AS profit_center,invoices.branch_id AS branch_id, SUM(invoices.sub_total_amt - totalDiscount) AS total_amount FROM (SELECT so.so_number as so_number,pc.functionalities_name as profit_center FROM erp_branch_sales_order AS so LEFT JOIN erp_company_functionalities as pc ON so.profit_center=pc.functionalities_id WHERE so.company_id=$company_id) AS so_pc INNER JOIN erp_branch_sales_order_invoices AS invoices ON so_pc.so_number=invoices.so_number WHERE invoices.invoice_date BETWEEN '".$fromDate."' AND '".$toDate."' GROUP BY profit_center,branch_id) AS table1 LEFT JOIN erp_branches AS branch ON table1.branch_id = branch.branch_id ORDER BY profit_center,total_amount DESC", true);
    ...
    $payload["vendorWisePayables"] = queryGet("SELECT vendorName,DATEDIFF(dueDate,CURDATE())AS due_days,COUNT(*) AS count_,SUM(dueAmt) AS total_due_amount FROM erp_grninvoice AS iv WHERE companyId=$company_id AND postingDate BETWEEN '".$fromDate."' AND '".$toDate."' AND dueAmt!=0 AND dueDate>CURDATE() GROUP BY vendorName,due_days ORDER BY due_days", true);
    ...
    $payload["salesVsCollection"] = [    
        "revenue" => queryGet("SELECT invoice_date AS date_, SUM(sub_total_amt - totalDiscount) AS total_revenue , SUM(due_amount) AS receivable FROM erp_branch_sales_order_invoices WHERE company_id=$company_id AND invoice_date BETWEEN '".$fromDate."' AND '".$toDate."' GROUP BY date_ ORDER BY date_;", true),
