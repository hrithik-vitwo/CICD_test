                                ...
                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                    <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                        ...
                                        $cond = '';
                                          ...
                                          $sql_list = "SELECT vendorId,vendorName,DATEDIFF(dueDate,CURDATE())AS due_days,COUNT(*) AS count_,SUM(dueAmt) AS total_due_amount FROM erp_grninvoice AS iv WHERE companyId=$company_id AND postingDate BETWEEN '2022-04-01' AND '2023-03-31' AND dueAmt!=0 AND dueDate>CURDATE() GROUP BY vendorId,vendorName,due_days ORDER BY due_days,vendorName,total_due_amount desc;";
                                            ...
                                            <table id="dataTable" class="table table-hover transactional-book-table" style="width: 100%; position: relative;">
                                                ...
                                                <tbody>
                                                    ...
                                                    foreach ($array['making'] as $key => $data) {
                                                       ...
                                                       $overdue_sql = queryGet("SELECT SUM(dueAmt) AS total_due_amount FROM erp_grninvoice WHERE companyId = $branch_id AND vendorId=$vendor_id AND postingDate BETWEEN '2022-04-01' AND '2023-03-31' AND dueAmt!=0 AND dueDate < CURRENT_DATE;");
                                                    //console($overdue_sql);
