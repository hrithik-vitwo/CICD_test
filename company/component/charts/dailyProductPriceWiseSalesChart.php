...
<script>
    ...
    function dailyProductPriceWiseSalesChart(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Daily Price Wise Sales for  ${$(".dailyProdP").find(":selected").text()}`);
    ...
    function get_dailyProductPriceWiseSalesChart(quickDrop, search, itemCode) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>company/ajaxs/reports/ajax-charts.php?chart=dailyProductPriceWiseSales&search=${search}&quickDrop=${quickDrop}&itemCode=${itemCode}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
