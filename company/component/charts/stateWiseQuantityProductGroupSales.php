...
$searchValue = "stateWiseQuantityProductGroupSales"; // Add this from ajax "?chart=" value
...
<div class="col-md-6 col-sm-6 d-flex">
    <div class="card flex-fill stateWiseQuantityProductGroupSales">
        <div class="card-header">
            <div class="head-title">
                <button class="btn btn-primary pin-btn border" id="stateWiseQuantityProductGroupSales"><?= chartExistorNot('stateWiseQuantityProductGroupSales.php') ? 'Pinned' : 'Pin'; ?></button>
...
<script>
    ...
    // ===============================================================================================
    var quickDrop = $(".stateWiseQuantityProductGroupSales").find("select.quickDrop").val();

    function get_stateWiseQuantityProductGroupSales(quickDrop, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>company/ajaxs/reports/ajax-charts.php?chart=stateWiseQuantityProductGroupSales&search=${search}&quickDrop=${quickDrop}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    get_stateWiseQuantityProductGroupSales(quickDrop, null);

    $('.applyFilter<?= $searchValue; ?>').click(function() {
        var quickDrop = $(".stateWiseQuantityProductGroupSales").find("select.quickDrop").val();

        get_stateWiseQuantityProductGroupSales(quickDrop, 'searching');
    });
</script>