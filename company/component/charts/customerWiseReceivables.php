...
<script>
    ...
    function get_customerWiseReceivables(quickDrop, search) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>company/ajaxs/reports/ajax-charts.php?chart=customerWiseReceivables&search=${search}&quickDrop=${quickDrop}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
    ...
    $('.applyFilter<?= $searchValue; ?>').click(function() {
        ...
        get_customerWiseReceivables(quickDrop, 'searching');
    });
</script>