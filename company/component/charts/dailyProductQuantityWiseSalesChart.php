...
<script>
    ...
    function dailyProductQuantityWiseSalesChart(chartData, chartTitle) {
        ...
        $(`.${chartTitle}`).text(`Daily Quantity Wise Sales for ${$(".dailyProdQ").find(":selected").text()}`);
    ...
    function get_dailyProductQuantityWiseSalesChart(quickDrop, search, itemCode) {
        // AJAX CALL 
        $.ajax({
            url: `<?= BASE_URL ?>company/ajaxs/reports/ajax-charts.php?chart=dailyProductQuantityWiseSales&search=${search}&quickDrop=${quickDrop}&itemCode=${itemCode}`,
            async: false,
            beforeSend: function() {
                $(".load-wrapp").show();
