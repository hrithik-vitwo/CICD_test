...
function generateGlDrilldownLink($id, $extraParams = "")
{
    //echo $id;
    // $sql = queryGet("SELECT parentGlId AS gl, CASE WHEN goodsType = 9 THEN 'asset-report.php' WHEN goodsType = 7 THEN 'servicepurchase-report.php'  WHEN goodsType = 5 THEN 'sarvicesale-report.php' ELSE 'manage-inventory.php' END AS link
    // FROM erp_inventory_items GROUP BY parentGlId,goodsType
    // UNION
    // SELECT parentGlId AS gl, 'accountingreports/receivable-analysis.php?bs&detailed-view' AS 'link' FROM erp_customer GROUP BY parentGlId
    // UNION
    // SELECT parentGlId AS gl, 'accountingreports/payable-analysis.php?bs&detailed-view' AS 'link' FROM erp_vendor_details GROUP BY parentGlId
    // UNION
    // SELECT parent_gl AS gl, 'banking-visualize-chart.php?bs' AS 'link' FROM erp_acc_bank_cash_accounts GROUP BY parent_gl", true);
    ...
    // $link = $href . $extraParams;



    // // return "target='_blank' href = '".BASE_URL."branch/location/accountingreports/payable-analysis.php?concised-view".$extraParams."'";
    // return "target='_blank' href = '" . BASE_URL . "branch/location/" . $link . "'";
    return "";
}
?>
                ...
                <div class="col-12 mt-2 p-0">
                    <div class="print-padding-none row p-0 m-0">
                        ...
                        <div class="printable-blncsheet">
                            ...
                            $glSummaryData = $glSummaryObj->getSummary(null, $balanceSheetReportDate);
                            // console($glSummaryData);
                            //For Profit before tax

                            $glSummaryObj1 = new GlSummary("company", false);
                            ...
                            $glSummaryForOpeningBalObj = new GlSummary("company");
                                    ...
                                    ?>
                                        <div class="card mb-2 bg-light text-dark">
                                            <div class="card-header p-0 d-flex bg-light text-dark accordionGlRowHover">
                                                ...
                                                <?php } else {
                                                    $extraParams = "&toDate=" . $balanceSheetReportDate;
                                                    // echo $oneData["id"];
                                                ?>
                                                    <p class="mr-2 text-sm font-weight-bold <?= $itemIsGroup ? "text-primary" : "text-dark" ?>"><a <?= generateGlDrilldownLink($oneData["id"], $extraParams) ?>><?= ($closingBalance < 0) ? "(" . number_format(abs($closingBalance), 2) . ")" : number_format($closingBalance, 2) ?></a></p>
                                                <?php } ?>
                                            </div>
