...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    WHERE company_id=" . $company_id . " AND branch_id = " . $branch_id . " AND location_id = " . $location_id . " AND type='vendor' AND reconciliationType='miscellaneous' " . $cond . " ORDER BY id desc limit " . $start . "," . $end . "";
    $iv_sql = queryGet($sql_list, true);

    if ($iv_sql['status'] == "success") {
        sendApiResponse([
            "status" => "success",
            "message" => $iv_sql["numRows"] . " Record Found",
            "data" =>  $iv_sql["data"]
