...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_list = "SELECT * FROM `erp_branch_sales_order_invoices`   WHERE `customer_id`='" . $customer_id . "'  AND `company_id`='" . $company_id . "' AND `branch_id` = '$branch_id' AND `location_id` = '$location_id' " . $cond . " ORDER BY `so_invoice_id` desc limit " . $start . "," . $end . " ";
    ...
    if ($iv_sql['status'] == "success") {
        ...
        foreach ($iv_data as $data) {
            ...
            $grnIvItemSql = "SELECT * FROM `erp_branch_sales_order_invoice_items` as iv_item, `" . ERP_INVENTORY_ITEMS . "` as item WHERE iv_item.so_invoice_item_id=item.itemId AND iv_item.so_invoice_id=$ivid";
