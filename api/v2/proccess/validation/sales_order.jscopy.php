    ...
    function customerDetailsInfo(customerId) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				let stateCodeSpan =
					stateCodeSpanElement.length > 0
						? stateCodeSpanElement.text().trim()
						: null;
				...
				if (customerGstinCode !== '') {
					console.log(
						'customerGstinCode is not empty 🏁🏁🏁',
						customerGstinCode
					);
					...
					$.ajax({
						...
						success: function (response2) {
							...
							if (data2.status == 'success') {
								...
								let complianceInvoiceType =
									data2.data.complianceInvoiceType;
								let placeOfSupply = data2.data.placeOfSupply;
								let invoiceNoFormate =
									data2.data.invoiceNoFormate;
      ...
      if (itemIdArry != '') {
        ...
        $.each(itemIdArryTo, function(index, value) {
          if (value.inventory_item_id > 0) {
            $.ajax({
              type: "GET",
              url: `ajaxs/so/ajax-items-list-direct-new-so.php`,
              data: {
                act: "listItem",
