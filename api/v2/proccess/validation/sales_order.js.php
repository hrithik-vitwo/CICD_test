...
$quotation = isset($quotation) && $quotation ? true : false;
$proforma = isset($proformaValidationJs) && $proformaValidationJs ? true : false;
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        setTimeout(() => {
          ...
          $(".rupee-symbol_dynamic").text("<?= $currencyName ?> ");
        }, 1000);
      },
    });
  }
...
<script>
  ...
  $("#profitCenterDropDown").on("change", function () {
    let functionalArea = (($(this).val()) > 0) ? ($(this).val()) : 1;
...
<script>
  $(document).ready(function () {
          ...
          if (value.inventory_item_id > 0) {
            $.ajax({
              ...
              success: function (response) {
                ...
                // $(".multiDeliveryDate").each(function() {
    ...
    function discount_varients(rowNo, customer_id, item_id, days) {
      //console.log(days);
      console.log("discount_varients is called");
      let itemVal = helperAmount($(`#itemBaseAmountInp_${rowNo}`).val());
      // alert(itemVal);
          ...
          $.each(obj, function (index, item) {
            ...
            if (item.discount_type == 'value') {
              ...
              var div = `<div class="discount-card">
                            <input type="radio" class="discount_radio" name="discount" id="discount_radio` + rand + `" data-attr="` + rand + `" value="` + decimalAmount(item.discount_value) + `">
                            <p><b>Rs.` + decimalAmount(item.discount_value) + ` off </b> </p>
                            <p>`
              if (item.minimum_value != 0 && item.minimum_value != null) {
                div += ` on minimum purchase value of<b>Rs.` + decimalAmount(item.minimum_value) + `</b> `
              }
              ...
              if (item.minimum_qty != 0 && item.minimum_qty != null) {
                div += ` minimum quantity of <b>` + decimalQuantity(item.minimum_qty) + `</b> `
              }
              ...
              div += `</div>
                          ...
                          <input type="hidden" id="percentage_val_${rand}" name="percentage_val" value="` + decimalQuantity(item.discount_value) + `">
                          <input type="hidden" id="discount_type_${rand}" name="discount_type" value="` + item.discount_type + `">
                          <input type="hidden" id="max_val_${rand}" name="max_val" value="` + decimalAmount(item.discount_max_value) + `">
                          <input type="hidden" id="min_val_${rand}" name="min_val" value="` + decimalAmount(item.minimum_value) + `">
                          <input type="hidden" id="min_qty_${rand}" name="min_qty" value="` + decimalQuantity(item.minimum_qty) + `">
                          <input type="hidden" id=base_price_${rand}" name="base_price" value="` + decimalAmount(itemVal) + `">
                        `;
            } else {
              var div = `
                          <div class="discount-card">
                            <input type="radio" class="discount_radio" name="discount" id="discount_radio" data-attr = "` + rand + `" value="` + decimalQuantity(item.discount_percentage) + `">
                            <p><b>` + decimalQuantity(item.discount_percentage) + ` % off </b>`;

              if (item.discount_max_value != 0 && item.discount_max_value != null) {
                div += `upto Rs.` + decimalAmount(item.discount_max_value);
              }

              if (item.minimum_value != 0 && item.minimum_value != null) {
                div += ` on minimum purchase value <b>Rs.` + decimalAmount(item.minimum_value) + `</b>`;
              }
              if (item.minimum_value != null && item.minimum_qty != null) {
                div += `` + item.condition;
              }
              ...
              div += `${selectedDiscountDivNote}
                          ...
                          <input type="hidden" id="discount_variant_id_${rand}" name="discount_variant_id" value="` + item.discount_variant_id + `">
                          <input type="hidden" id="percentage_val_${rand}" name="percentage_val" value="` + decimalQuantity(item.discount_percentage) + `">
                          <input type="hidden" id="discount_type_${rand}" name="discount_type" value="` + item.discount_type + `">
                          <input type="hidden" id="max_val_${rand}" name="max_val" value="` + decimalAmount(item.discount_max_value) + `">
                          <input type="hidden" id="min_val_${rand}" name="min_val" value="` + decimalAmount(item.minimum_value) + `">
                          <input type="hidden" id="min_qty_${rand}" name="min_qty" value="` + decimalQuantity(item.minimum_qty) + `">
      ...
      $(`.discountView_${rowNo}`).on('click', '.discount_radio', function () {
        ...
        var percentage_val = helperQuantity($(`#percentage_val_${random}`).val());
        ...
        let itemQty = helperAmount($(`#itemQty_${rowNo}`).val()) > 0 ? helperAmount($(`#itemQty_${rowNo}`).val()) : 0;
        ...
        if (discount_type == 'percentage') {
          ...
          $(`#itemTotalDiscount1_${rowNo}`).val(decimalQuantity(percentage_val));
          $(`#itemDiscount_${rowNo}`).val(decimalQuantity(percent));
          $(`#itemDiscountVarientId_${rowNo}`).val(discount_varient_id);
          ...
          $(`#itemTradeDiscountAmountSpan_${rowNo}`).text(decimalAmount(percentage_amount));
          var percentage = helperQuantity($(`#itemDiscount_${rowNo}`).val());
          $(`#itemTradeDiscountPercentageSpan_${rowNo}`).text(`${decimalQuantity(percentage)}`);
    ...
    function roundofftotal(total_value, sign, roudoff) {
      ...
      // console.log("final value for rounofftotal is :" + final_value);
    ...
    function tcsTotal(total_value, tcsAmount) {
      ...
      // console.log("final_value for TCSTotal is : " + final_value)
    ...
    function calculateOneItemAmounts(rowNo) {
      ...
      // console.log("convertedItemUnitPrice price by using decimalAmount : " + (convertedItemUnitPrice));
      // convertedItemUnitPrice = parseFloat(convertedItemUnitPrice)
      ...
      // convertedBasicPrice = parseFloat(convertedBasicPrice)
      ...
      // convertedTotalDiscount = parseFloat(convertedTotalDiscount)


      let convertedTotalCashDiscount = helperAmount(convertedBasicPrice * itemCashDiscount / 100);
      // convertedTotalCashDiscount = parseFloat(convertedTotalCashDiscount)
      ...
      // convertedPriceWithDiscount = parseFloat(convertedPriceWithDiscount)

      let taxableAmount = (helperAmount(grossAmount - cashDiscountAmount));
      // taxableAmount = parseFloat(taxableAmount)
      // alert(grossAmount - cashDiscountAmount)
      console.log("the taxable amount is this :" + parseFloat(taxableAmount))
      ...
      // convertedTotalTax = parseFloat(convertedTotalTax)

      let totalItemPrice = helperAmount(taxableAmount + totalTax);
      // totalItemPrice = parseFloat(totalItemPrice)
      //alert(totalItemPrice);
      let convertedTotalItemPrice = helperAmount(convertedPriceWithDiscount + convertedTotalTax);
      // convertedTotalItemPrice = parseFloat(convertedTotalItemPrice)
      ...
      console.log({
        ...
        itemTradeDiscountPercentageSpan,
        justaval: inputValueJS(300.253 - 100.248),
        originalItemUnitPrice,
        param,
        priceWithDiscount,
        rowNo,
      ...
      // alert((parseFloat(basicPrice) - parseFloat(itemTradeDiscountAmount)))
      let justaval = inputValueJS(300.253 - 100.248)
      console.log("the value is " + parseFloat(justaval))
    }
    ...
    function calculateGrandTotalAmount() {
      ...
      console.log('first calculateGrandTotalAmount has run here')
      let totalAmount = 0;
      ...
      if (compInvoiceType === "CBW" || compInvoiceType === "LUT" || compInvoiceType === "SEWOP" || compInvoiceType === "E") {
        ...
        $("#convertedGrandTotalDiscountAmount").text(decimalAmount(convertedItemDiscountAmountSpan));
        // alert(11);
        var r = helperAmount($("#round_value").val());
        if (r) {
          var s = $("#round_sign").val();
    ...
    function currency_conversion() {
      $(".convertedItemUnitPriceSpan").each(function () {
        console.log("called thr function")
        let elem = $(this);
        let rowNo = elem.attr("id").split("_")[1];

        // Retrieve and convert inputs to numbers
        let currRate = helperAmount($("#curr_rate").val());
        let originalPrice = helperAmount($(`#originalChangeItemUnitPriceInp_${rowNo}`).val());
        ...
        newVal = newVal > 0 ? newVal : helperAmount(elem.text()) || 0;
    ...
    $(document).on("keyup", ".itemQty", function () {
      ...
      $(`#itemTradeDiscountPercentageSpan_${rowNo}`).html(decimalQuantity(0));
      $(`#itemCashDiscountPercentageSpan_${rowNo}`).html(decimalQuantity(0));
      ...
      if ($(`#checkQty_${rowNo}`).length > 0) {
        ...
        let quotation = <?= json_encode($quotation) ?>;
        let proforma = <?= json_encode($proforma) ?>;

        if ((quotation == 'true' || proforma == 'true')) {
          if (itemVal <= splitQty) {
            $(`#itemSpecialDiscount_${rowNo}`).text(`Special Discount`);
            $(`#qtyMsg_${rowNo}`).hide();
          } else {
    ...
    $(document).on("keyup blur", ".originalChangeItemUnitPriceInp", function () {
      // alert("hiiii")
      let rowNo = ($(this).attr("id")).split("_")[1];
      let item_rate = $(`#originalChangeItemUnitPriceInp_${rowNo}`).val();
      let item_mrp = $(`#itemTargetPrice_${rowNo}`).val();

      if (parseFloat(item_mrp) < parseFloat(item_rate)) {
        $(`#rateMsg_${rowNo}`).show();
        $(`#originalChangeItemUnitPriceInp_${rowNo}`).val("");
      } else {
        $(`#rateMsg_${rowNo}`).hide();
      }
    ...
    function cashDiscountFunc(rowNo, keyValue) {
      ...
      let itemGrossAmount = helperAmount($(`#itemGrossAmountSpan_${rowNo}`).text()) || 0;
      let discountAmt = helperAmount(itemGrossAmount * keyValue) / 100;
      ...
      $(`#itemCashDiscountPercentageInp_${rowNo}`).val(`${decimalQuantity(keyValue)}`);
      $(`#itemCashDiscountPercentageSpan_${rowNo}`).text(`${decimalQuantity(keyValue)}`);
    ...
    $(document).on("keyup", ".itemTotalCashDiscount1", function () {
      let rowNo = ($(this).attr("id")).split("_")[1];
      let keyValue = helperAmount($(this).val());
    ...
    $(document).on("blur", ".itemTotalDiscount1", function () {
      ...
      let discountPercentage = helperQuantity(itemDiscountAmt * 100 / totalAmt);
  ...
  $('#customerDropDown').select2({
    ...
    if (!$results.find('a').length) {
      $results.append(`<div class="btn-row"><a type="button" class="btn btn-primary add-btn addCust" data-toggle="modal" data-target="#addNewCustomerModal">Add New</a></div>`);
    }
  });
...
<script>
  ...
  getOtherAdress(customerId);
    ...
    var validityDate = $('#dateInputvalid').val();

    var isInvalid = postingDate && validityDate && postingDate > validityDate;

    if (isInvalid) {
      $('#dateError').text('Posting Date must not be greater than Validity Date.');
  ...
  $('#postingDate').on('change keyup', function () {
    console.log("Posting date changed");
    validateDates();
  });

  $('#dateInputvalid').on('change keyup', validateDates);

  $('#soDate').on('change keyup', function() {
    var postingDateS0 = $(this).val();
    $('#deliveryDate').attr('min', postingDateS0);
    $('#deliveryDate').val(postingDateS0);
});
