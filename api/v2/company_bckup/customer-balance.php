...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach($data_set as $key => $data)
    {
        ...
        $get_gl_query = queryGet('SELECT `customer_code`,`parentGlId` FROM `erp_customer` WHERE `company_id`=' . $company_id . ' AND `company_branch_id`='.$branch_id.' AND `location_id`='.$location_id.' AND `customer_code`="'.$subglcode.'" AND `customer_status`= "active"');
