...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($data_set as $key => $data) {
        ...
        $get_gl_query = queryGet('SELECT `id` FROM `' . $table . '` WHERE `company_id`=' . $company_id . ' AND `status`= "active" AND `glStType` = "account" AND `gl_code`="' . $glcode . '"');

        $data["gl"] = $get_gl_query["data"]["id"];
        $data["get_gl_query"] = $get_gl_query;
