...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($data_set as $key => $data) {
        ...
        $get_gl_query = queryGet('SELECT `vendor_code`,`parentGlId` FROM `erp_vendor_details` WHERE `company_id`=' . $company_id . ' AND `company_branch_id`=' . $branch_id . ' AND `location_id`=' . $location_id . ' AND `vendor_code`="' . $subglcode . '" AND `vendor_status`= "active"');
