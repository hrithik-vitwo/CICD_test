...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    //find customer gl 
    $gl= queryGet("SELECT `cash_gl` FROM `erp_acc_gl_mapping` WHERE `company_id` = $company_id");
    //  console($gl['data']);
  ...
  if ($dayOfMonth === '01') {
    ...
    {
        $check_query = queryGet('SELECT `acc_code`,`type_of_account`,`bank_name` FROM `erp_acc_bank_cash_accounts` WHERE `company_id`=' . $company_id . ' AND `status`= "active"  AND `type_of_account` = "cash" ORDER BY `acc_code` ASC',true);
        ...
        else
        {
            sendApiResponse([
                ...
                "parentCashGl" => $cash_opening_balance
