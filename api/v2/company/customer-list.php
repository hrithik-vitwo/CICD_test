...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    //find customer gl 
    $gl= queryGet("SELECT `customer_gl` FROM `erp_acc_gl_mapping` WHERE `company_id` = $company_id");
    // console($gl['data']['customer_gl']);
    $gl_id = $gl['data']['customer_gl'];
  ...
  if ($dayOfMonth === '01') {
    ...
    {
        $check_query = queryGet('SELECT `customer_code`,`customer_gstin`,`trade_name` FROM `erp_customer` WHERE `company_id`=' . $company_id . ' AND `company_branch_id`='.$branch_id.' AND `location_id`='.$location_id.' AND `customer_status`= "active" ORDER BY `customer_code` ASC',true);
