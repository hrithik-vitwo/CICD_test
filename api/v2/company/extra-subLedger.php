...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach($data_set as $key => $data)
    {
        ...
        $get_gl_query = queryGet('SELECT `sl_code`,`parentGlId` FROM `erp_extra_sub_ledger` WHERE `company_id`=' . $company_id . ' AND `sl_code` LIKE "%'.$subglcode.'" AND `status`= "active"');
