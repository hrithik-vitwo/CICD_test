...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    //find customer gl 
    $gl= queryGet("SELECT `vendor_gl` FROM `erp_acc_gl_mapping` WHERE `company_id` = $company_id");
    // console($gl['data']['customer_gl']);
    $gl_id = $gl['data']['vendor_gl'];

     // Check if the day of the month is 1
     $opening_balance = 0;

     if ($dayOfMonth === '01') {
        $opening_query = queryGet("SELECT SUM(opening_val) AS opening FROM erp_opening_closing_balance WHERE company_id=$company_id AND branch_id=$branch_id AND location_id=$location_id AND DATE_FORMAT(date,'%Y-%m')=DATE_FORMAT('" . $today . "','%Y-%m') AND subgl=$gl_id");
