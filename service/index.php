<?php
require_once("../app/v1/connection-service.php");
...
if (isset($_POST['assign_task'])) {
    ...
    $comment = transfar_func($_POST);
    swalToast($comment["status"], $comment["message"]);
}
$user_id = $_SESSION["logedAdminInfo"]["adminId"];
?>
                            ...
                            <div class="card card-tabs mb-0" style="border-radius: 20px;">
                                ...
                                <div class="tab-content" id="custom-tabs-two-tabContent">
                                    <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                        ...
                                        $sql_list = "SELECT * FROM `erp_bug_list`  WHERE `created_at` BETWEEN '" . $f_date . "' AND '" . $to_date . "' AND `assign_to` = $user_id ";
                                        $queryset = queryGet($sql_list, true);
                                        //console($queryset);
                                        $num_list = $queryset['numRows'];


                                        $settingsTable = getTableSettings(TBL_BRANCH_ADMIN_TABLESETTINGS, "ERP_REPORT_DETAILED_VIEW_" . $pageName, $_SESSION["logedAdminInfo"]["adminId"]);
                                        $settingsCh = ($settingsTable['data'][0]['settingsCheckbox']);
                                        $settingsCheckbox = unserialize($settingsCh);
                                        //console($settingsCheckbox);
                                        // console($_SESSION["logedAdminInfo"]["adminId"]);
                                            ...
                                            <table id="dataTable" class="table table-hover transactional-book-table" style="width: 100%; position: relative;">
                                                ...
                                                <tbody>
                                                    ...
                                                    foreach ($datas as $data) {
                                                        ...
                                                        $overdue_date = date('Y-m-d H:i:s', strtotime("+72 hours", strtotime($date)));
                                                        $today = date("Y-m-d H:i:s");
                                                        // Convert to timestamps
                                                        $overdue_timestamp = strtotime($overdue_date);
                                                        $now_timestamp = strtotime($today);

                                                        // Calculate the absolute difference in hours
                                                        $diff_seconds = abs($now_timestamp - $overdue_timestamp);
                                                    ...
                                                    ?>
                                                        <tr>
                                                            ...
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo ($data['bug_code']);  ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo $data['module_name'] ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo $data['sub_module_name']; ?></td>
                                                            <?php }

                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"> <?php echo $data['page_name']; ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"> <?php echo $data['page_url']; ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"> <?php echo $data['bug_description']; ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                            ...
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo $data['created_user'];  ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo formatDateORDateTime($data['created_at']);  ?></td>
                                                            <?php }
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo $data['created_user'];  ?></td>
                                                            ...
                                                            $i++;
                                                            if (in_array($i, $settingsCheckbox)) { ?>
                                                                <td class="text-center"><?php echo formatDateORDateTime($data['updated_at']);  ?></td>
                                                            <?php } ?>
                                                            <td class="text-center">
                                                                <?php
                                                                if ($data['assign_to'] != NULL) {
                                                                    echo '<i class="fa fa-user-secret" aria-hidden="true"></i> ' . assign_username_func($data['assign_to'])['data']['fldAdminName'] . '<br>';
                                                                ?>
                                                                    <a type="button" data-toggle="modal" data-target="#assign<?= $data['id'] ?>"> Edit </a>
                                                                <?php
                                                                } else {
                                                                ?>
                                                                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#assign<?= $data['id'] ?>">
                                                                        Assign
                                                                    </button>
                                                                <?php
                                                                }
                                                                ...
                                                                <div class="modal fade" id="assign<?= $data['id'] ?>" role="dialog" aria-labelledby="myModalLabel" data-backdrop="true" style="display: none;" aria-hidden="true">
                                                                    <div class="modal-dialog">
                                                                        <div class="modal-content">
                                                                            ...
                                                                            <div class="modal-body" style="height: 334px;">
                                                                                <form action="" method="POST">
                                                                                    <input type="hidden" name="assign_task">
                                                                                    <input type="hidden" name="bug_id" value="<?= $data['id'] ?>">
                                                                                    <input type="hidden" name="user_id" value="<?= $_SESSION["logedAdminInfo"]["adminId"]; ?>">
                                                                                    ...
                                                                                    <div class="row">
                                                                                        <div class="col-lg-4 col-md-4 col-sm-12">
                                                                                            <div class="form-input">
                                                                                                ...
                                                                                                <select name="assign_to" class="form-control" required>
                                                                                                    <option value="">Select One</option>
                                                                                                    <?php
                                                                                                    $bguser_sql = queryGet("SELECT * FROM `erp_bug_user_details` WHERE fldAdminStatus='active' AND `working_status`='Y'AND user_type='Performer'", true);
                                                                                                    $bguser_data = $bguser_sql['data'];
                                                                                                    foreach ($bguser_data as $bgdata) {
                                                                                                        if ($bgdata['fldAdminKey'] == $data['assign_to']) {
                                                                                                            continue;
                                                                                                        }
                                                                                                    ?>

                                                                                                        <option value="<?= $bgdata['fldAdminKey'] ?>" <?php if ($bgdata['fldAdminKey'] ==  $data['assign_to']) {
                                                                                                                                                            echo "selected";
                                                                                                                                                        } ?>><?= $bgdata['fldAdminName'] ?></option>
                                                                                                    <?php
                                                                                                    }
                                                                                                    ?>
                                                                                                </select>
                                                                                        ...
                                                                                        <div class="col-lg-4 col-md-4 col-sm-12">
                                                                                            <div class="form-input">
                                                                                                ...
                                                                                                <select name="reviewer" class="form-control" required>
                                                                                                    <option value="">Select One</option>
                                                                                                    <?php
                                                                                                    $bguser_sql = queryGet("SELECT * FROM `erp_bug_user_details` WHERE fldAdminStatus='active' AND  user_type='Performer' OR user_type='Approver'", true);
                                                                                                    $bguser_data = $bguser_sql['data'];
                                                                                                    foreach ($bguser_data as $bgdata) {
                                                                                                    ?>
                                                                                        ...
                                                                                        <div class="col-lg-4 col-md-4 col-sm-12">
                                                                                            <div class="form-input">
                                                                                                ...
                                                                                                <input type="number" name="durationtime" id="" class="form-control" value="<?= $data['duration']; ?>" required>
                                                                                            </div>
                                                                                        </div>
                                                            ...
                                                            <td class="text-center"> <input type="hidden" value="<?= $data['status'] ?>" class="old-status oldStatus_<?= $data['id'] ?>">
                                                                <div class="status-dr" data-id="<?= $data['id'] ?>"> <?= $data['status'] ?> </div>
                                                                <?php if ($data['status'] == 'assigned') { ?>
                                                                    <select name="status" class="form-control1 status-dropdown" data-id="<?= $data['id'] ?>">
                                                                        <option>change status</option>
                                                                        <option value="wip">WIP</option>
                                                                        <option value="solved">Solved</option>
                                                                    </select>
                                                                <?php  } else if ($data['status'] == 'wip') { ?>
                                                                    <select name="status" class="form-control1 status-dropdown" data-id="<?= $data['id'] ?>">
                                                                        <option>change status</option>
                                                                        <option value="solved">Solved</option>
                                                                    </select>
                                                                <?php  } else if ($data['status'] == 'solved') { ?>
                                                                    <select name="status" class="form-control1 status-dropdown" data-id="<?= $data['id'] ?>">
                                                                        <option>change status</option>
                                                                        <option value="reopen">Reopen</option>
                                                                    </select>
                                                                <?php  } else if ($data['status'] == 'reopen') { ?>
                                                                    <select name="status" class="form-control1 status-dropdown" data-id="<?= $data['id'] ?>">
                                                                        <option>change status</option>
                                                                        <option value="wip">WIP</option>
                                                                        <option value="solved">Solved</option>
                                                                ...
                                                                <?php  } else { ?>
                                                                    <select name="status" class="form-control1 status-dropdown" data-id="<?= $data['id'] ?>">
                                                                        <option value="reopen" <?= $data['status'] == 'reopen' ? 'selected' : '' ?>>Reopen</option>
                                                                        <option value="solved" <?= $data['status'] == 'solved' ? 'selected' : '' ?>>Solved</option>
                                                                    </select>
                                                                <?php } ?>

                                                            </td>
                                                            <td class="text-center overdue" data-id="<?= $data['id'] ?>"> <?php if ($today > $overdue_date && $data['status'] != 'solved') { ?>
                                                                 <span style="color: red;">  overdue by <?= $diff_hours ?> hours</span>
                                                                <?php } else { ?>

                                                                <?php } ?> </td>
                                                            <td class="text-center">
                                                                <a style="cursor:pointer" data-toggle="modal" data-target="#fluidModalRightSuccessDemo<?= $data['id'] ?>" class="btn btn-sm waves-effect waves-light" onclick="getChatHistory(<?= $data['id'] ?>)"><i class="fa fa-eye po-list-icon"></i></a>
                                                                <div class="modal fade right customer-modal" id="fluidModalRightSuccessDemo<?= $data['id'] ?>" role="dialog" aria-labelledby="myModalLabel" data-backdrop="true" style="display: none;" aria-hidden="true">
                                                                            ...
                                                                            <div class="modal-body">
                                                                                <div class="tab-content" id="myTabContent">
                                                                                    <div class="tab-pane fade show active" id="home<?= $data['id'] ?>" role="tabpanel" aria-labelledby="home-tab">
                                                                                        <img src="<?= $finalUrl?>"class="img-bug-screenshot">
                                                                                    </div>
                                                                                                        ...
                                                                                                        <div class="chat">
                                                                                                            <div class="chat-container" id="chat-con<?= $data['id'] ?>">
                                                                                                                <div class="user-bar">
                                                                                                                    ...
                                                                                                                    <div class="name">
                                                                                                                        <span><?= isset($data['created_user']) ? $data['created_user'] : 'Client' ?></span>
                                                                                                                    </div>
                        ...
                        <div class="card card-tabs reports-card">
                            ...
                            <div class="tab-content" id="custom-tabs-two-tabContent">
                                <div class="tab-pane fade show active" id="listTabPan" role="tabpanel" aria-labelledby="listTab" style="background: #fff; border-radius: 20px;">
                                    ...
                                    $open_sql = queryGet("SELECT COUNT(`id`) as  total_open FROM `erp_bug_list` WHERE `status`='open' AND `assign_to` = $user_id ");
                                    $assigned_sql = queryGet("SELECT COUNT(`id`) as total_assigned FROM `erp_bug_list` WHERE `status`='assigned' AND `assign_to` = $user_id ");
                                    $todo_sql = queryGet("SELECT COUNT(`id`) as total_todo FROM `erp_bug_list` WHERE `status`='todo' AND `assign_to` = $user_id ");
                                    $solved_sql = queryGet("SELECT COUNT(`id`) as total_solved FROM `erp_bug_list` WHERE `status`='solved' AND `assign_to` = $user_id ");
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12 d-flex">
                                                <div class="card flex-fill reports-card">
                                                    <div class="card-body">
                                                        <div id="chartDivSalesRegisterVerticalWise" class="chartContainer"></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<script>
    ...
    am4core.ready(function() {
        ...
        var chart = am4core.create("chartDivSalesRegisterVerticalWise", am4charts.XYChart);
        ...
        let finalData = [{
            ...
            {
                "category": "Assigned",
                "value": Number(chartData[0].assigned),
            },
            {
                "category": "Todo",
                "value": Number(chartData[0].todo),
            },
            {
                "category": "Solved",
        ...
        labelTemplate.dy = -50; // moves it a bit down;
...
<script>
    function displayFileName(id) {
        ...
        if (input.files.length > 0) {
            ...
            var allowedTypes = ['application/pdf', 'image/jpeg', 'image/png', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'];
    ...
    function sendMessage(e, $bug_Id) {
        ...
        document.getElementById('attachFileCard' + bug_id).style.display = 'none';
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                response.data.forEach(function(messageData) {
                    ...
                    var fileObj;
                    var attachmentObj = '../uploads/bugimages/' + image_check;
                    // alert(attachmentObj);

                    if (attachmentObj.endsWith("jpg") || attachmentObj.endsWith("png")) {
                        fileObj = `<img src="${attachmentObj}" alt="Image File" class="attach_img" height="200" width="500">`;
                    } else if (attachmentObj.endsWith("pdf")) {
                        fileObj = `<iframe src="${attachmentObj}" class="pdf"></iframe>`;
                    } else if (attachmentObj.endsWith("xlsx")) {
                        fileObj = `<iframe src="" class="xlsx"></iframe>`;
                    }
                    let type = created_by.endsWith("Performer");
                    console.log(type);
    ...
    $(document).ready(function() {
        $('.status-dropdown').change(function() {
            var status = $(this).val();
            var id = $(this).data('id');
            var old = $(`.oldStatus_` + id).val();
            // var overdue = <?php echo $diff_hours ?>;
            // var today = <?php echo $today ?>;
            // var overdue_date = <?php echo $overdue_date ?>
            //  alert(old);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (responsestatus.toLowerCase() == 'success') {
                            // alert(1);
                            // $(`.oldStatus_` + id).val(status);
                            // $(".status-dr[data-id='" + id + "']").text(obj.changes_status);

                            // if (obj.changes_status == 'solved') {
                            //     //alert(1);
                            //     // $(".overdue[data-id='" + id + "']").text('');
                            //     $(".status-dropdown[data-id='" + id + "']").html(`
                            //             <option >change status</option>
                            //             <option value="reopen">Reopen</option>`);
                            // } else if (obj.changes_status == 'reopen') {

                            // //    $(".overdue[data-id='" + id + "']").text('overdue by ' + overdue + ' hours');
                            //     $(".status-dropdown[data-id='" + id + "']").html(`
                            //             <option >change status</option>
                            //             <option value="wip">WIP</option>
                            //             <option value="solved">Solved</option>`);
                            // }else if(obj.changes_status == 'wip'){
                            //     // $(".overdue[data-id='" + id + "']").text('overdue by ' + overdue + ' hours');
                            //      $(".status-dropdown[data-id='" + id + "']").html(`
                            //             <option >change status</option>
                            //             <option value="solved">Solved</option>`);
